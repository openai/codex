...
def artifact_for_target(spec: TargetSpec, manifest_path: Path) -> RgArtifact:
    ...
    try:
        platform_info = manifest["platforms"][spec.rg_platform]
    except KeyError as exc:
        raise RuntimeError(
            f"ripgrep manifest {manifest_path} is missing platform {spec.rg_platform!r}"
        ) from exc

    providers = platform_info.get("providers")
    if not providers:
        raise RuntimeError(
            f"ripgrep manifest {manifest_path} has no providers for {spec.rg_platform!r}"
        )

    hash_name = platform_info.get("hash")
    if hash_name != "sha256":
        raise RuntimeError(
            f"Unsupported ripgrep hash {hash_name!r} for {spec.rg_platform!r}; expected sha256"
        )
