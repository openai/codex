...
func main() {
	...
	tempDir, err := os.MkdirTemp("", "codex-schema-")
	if err != nil {
		fatal(err)
	}
	defer func() {
	...
	if err := exportSchemas(repoRoot, tempDir); err != nil {
		fatal(err)
	}
	if err := generateProtocolTypes(tempDir, repoRoot); err != nil {
		fatal(err)
	}

	if err := generateRPCStubs(tempDir, repoRoot); err != nil {
		fatal(err)
	}
}

func exportSchemas(repoRoot, outDir string) error {
	cmd := exec.Command("cargo", "run", "-p", "codex-app-server-protocol", "--bin", "export", "--", "--out", outDir)
	cmd.Dir = filepath.Join(repoRoot, "codex-rs")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	return cmd.Run()
}

func generateProtocolTypes(schemaDir, repoRoot string) error {
	...
	outDir := filepath.Join(repoRoot, "sdk", "go", "protocol")
	...
	if err := writeFallbackTypes(outDir, fallbacks, generated); err != nil {
		return err
	}

	if err := writeProtocolAliases(outDir, generated, fallbacks); err != nil {
		return err
	}
...
func generateRPCStubs(schemaDir, repoRoot string) error {
	...
	outDir := filepath.Join(repoRoot, "sdk", "go", "rpc")
	if err := os.MkdirAll(outDir, 0o755); err != nil {
		return err
	}

	if err := writeFile(outDir, "client_requests_gen.go", renderClientRequests(clientRPC)); err != nil {
		return err
	}

	if err := writeFile(outDir, "server_requests_gen.go", renderServerRequests(serverRPC)); err != nil {
		return err
	}

	if err := writeFile(outDir, "notifications_gen.go", renderNotifications(notifications)); err != nil {
		return err
	}
...
func writeFallbackTypes(outDir string, titles []string, generated map[string]struct{}) error {
...
func writeProtocolAliases(outDir string, generated map[string]struct{}, fallbacks []string) error {
...
func renderClientRequests(methods []rpcMethod) []byte {
	var b strings.Builder
	b.WriteString(generatedHeader)
	...
	b.WriteString("func BuildClientRequest(method string, params any, id RequestID) JSONRPCRequest {\n")
	b.WriteString("\tvar raw json.RawMessage\n\tif params != nil {\n\t\tdata, _ := json.Marshal(params)\n\t\traw = data\n\t}\n\treturn JSONRPCRequest{ID: id, Method: method, Params: raw}\n}\n")

	return []byte(b.String())
}

func renderServerRequests(methods []rpcMethod) []byte {
	var b strings.Builder
	b.WriteString(generatedHeader)
	b.WriteString("package rpc\n\n")
	b.WriteString("import (\n\t\"context\"\n\t\"encoding/json\"\n\t\"fmt\"\n\n\t\"github.com/openai/codex/sdk/go/protocol\"\n)\n\n")
...
func renderNotifications(notifications []rpcNotification) []byte {
	var b strings.Builder
	b.WriteString(generatedHeader)
	b.WriteString("package rpc\n\n")
	b.WriteString("import (\n\t\"encoding/json\"\n\n\t\"github.com/openai/codex/sdk/go/protocol\"\n)\n\n")
...
func normalizeGeneratedHeader(src []byte) []byte {
	if len(src) == 0 {
		return src
	}
	...
	return []byte(generatedHeader + rest)
}
...
func sanitizeSchemaFile(path string) (string, func(), error) {
	...
	tmp, err := os.CreateTemp(filepath.Dir(path), "sanitized-*.json")
	if err != nil {
		return "", nil, err
	}
...
func repoRoot() (string, error) {
	...
	// Expected to run from sdk/go; walk up until we find "codex-rs".
	current := cwd
	for {
	...
	return "", errors.New("unable to locate repo root containing codex-rs")
}
