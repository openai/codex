...
func TestNormalizeGeneratedHeader(t *testing.T) {
	...
	normalized := normalizeGeneratedHeader(src)
	if !strings.HasPrefix(string(normalized), generatedHeader) {
	...
	if got := normalizeGeneratedHeader(unchanged); string(got) != string(unchanged) {
...
func TestWriteFallbackTypesAndAliases(t *testing.T) {
	...
	if err := writeFallbackTypes(dir, fallbacks, generated); err != nil {
	...
	if err := writeProtocolAliases(dir, generated, fallbacks); err != nil {
...
func TestRenderHelpers(t *testing.T) {
	...
	client := string(renderClientRequests(methods))
	...
	server := string(renderServerRequests(methods))
	if !strings.Contains(server, "unsupported server request") {
		t.Fatalf("expected server dispatch")
	}

	notes := string(renderNotifications([]rpcNotification{{Method: "turn/started", ParamsType: "TurnStartedNotification"}}))
...
func TestRepoRoot(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "codex-rs")); err != nil {
		t.Fatalf("expected codex-rs in repo root: %v", err)
	}
}
...
func TestGenerateProtocolTypes(t *testing.T) {
	...
	if err := generateProtocolTypes(schemaDir, root); err != nil {
		t.Fatalf("generateProtocolTypes error: %v", err)
	}
	if !exists(filepath.Join(root, "sdk", "go", "protocol", "types_gen.go")) {
...
func TestGenerateRPCStubs(t *testing.T) {
	...
	if err := generateRPCStubs(schemaDir, root); err != nil {
		t.Fatalf("generateRPCStubs error: %v", err)
	}
	if !exists(filepath.Join(root, "sdk", "go", "rpc", "client_requests_gen.go")) {
		t.Fatalf("expected client_requests_gen.go output")
	}
	if !exists(filepath.Join(root, "sdk", "go", "rpc", "notifications_gen.go")) {
