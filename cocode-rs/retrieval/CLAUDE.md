...
**cocode-retrieval** - Code retrieval system providing intelligent code search for cocode-rs. Combines BM25 full-text search, vector semantic search, and AST-aware symbol extraction.

**IMPORTANT:** This crate is part of the cocode-rs workspace. Read `../CLAUDE.md` (or `codex/CLAUDE.md`) for workspace-wide conventions before making changes.
...
```bash
# From cocode-rs/ directory (required)
cargo build -p cocode-retrieval                    # Standard build
cargo build -p cocode-retrieval --features local   # With local embeddings + reranking
cargo test -p cocode-retrieval                     # Run tests
cargo check -p cocode-retrieval                    # Quick check

# Run CLI/TUI for testing
cargo run -p cocode-retrieval --bin retrieval -- --help
cargo run -p cocode-retrieval --bin retrieval -- /path/to/project              # TUI mode (default)
cargo run -p cocode-retrieval --bin retrieval -- /path/to/project --no-tui build
cargo run -p cocode-retrieval --bin retrieval -- /path/to/project --no-tui search "query"
```
...
```bash
# Unit tests
cargo test -p cocode-retrieval

# Integration tests
cargo test -p cocode-retrieval --test cli_test
cargo test -p cocode-retrieval --test indexing_test
cargo test -p cocode-retrieval --test vector_search_test

# With local features
cargo test -p cocode-retrieval --features local
```
