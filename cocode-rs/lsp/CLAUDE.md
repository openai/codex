...
`cocode-lsp` is an AI-friendly LSP client library that abstracts the Language Server Protocol to enable **symbol name-based queries** instead of exact line/column positions. Part of the cocode-rs workspace.
...
```bash
# From cocode-rs/ directory (REQUIRED - never run from lsp/)
cd cocode-rs

# Build
cargo build -p cocode-lsp

# Test
cargo test -p cocode-lsp

# Check (fast iteration)
cargo check -p cocode-lsp
