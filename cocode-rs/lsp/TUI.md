...
A simple TUI for testing `cocode-lsp` functionality and inspecting internal LSP operations.

## Quick Start

```bash
# Build with TUI feature
cargo build -p cocode-lsp --features tui

# Run with a workspace directory
cargo run -p cocode-lsp --features tui -- /path/to/rust/project

# Or specify an initial file
cargo run -p cocode-lsp --features tui -- /path/to/project -f src/lib.rs
```
...
1. Start TUI: `cargo run -p cocode-lsp --features tui -- .`
...
2. Check logs (output to stderr):
   ```bash
   cargo run -p cocode-lsp --features tui -- . 2>lsp.log
   ```
...
The TUI is implemented as an optional feature of the `cocode-lsp` crate:
...
# Run tests
cargo test -p cocode-lsp

# Check without building
cargo check -p cocode-lsp --features tui
```
