...
When conversation context approaches the model's context window limit, the system automatically summarizes older messages to free up space.
...
impl AgentLoop {
    ...
    async fn compact(&mut self) -> Result<(), LoopError> {
        ...
        // 3. Summarize older messages (uses same model)
...
1. **Preserve recent**: Keep last N turns (typically 2-4)
2. **Preserve system**: Never remove system prompt
3. **Summarize middle**: Compress older assistant/user exchanges
4. **Tool results**: Summarize lengthy tool outputs

```
Before:
[System] [User1] [Asst1] [Tool1] [User2] [Asst2] [Tool2] [User3] [Asst3]
                 └────────────────────┬───────────────────┘
                              Summarize these

After:
[System] [Summary] [User3] [Asst3]
```
...
Micro-compaction removes low-value tool results without full context summarization:
...
Session memory preserves context about recently accessed files across compaction, allowing the agent to maintain awareness of the codebase even after context is summarized.

### Implementation

```rust
/// Session memory for preserving file context
pub struct SessionMemory {
    /// Cached file contents
    pub files: HashMap<PathBuf, CachedFile>,
    /// Token budget for session memory (default: 50k)
...
impl SessionMemory {
    ...
    /// Build restoration content after compaction
    pub fn build_restoration_content(&self) -> String {
        let mut content = String::new();
        content.push_str("## Session Memory (Recently Read Files)\n\n");

        // Sort by access_count descending
        let mut files: Vec<_> = self.files.iter().collect();
        files.sort_by(|a, b| b.1.access_count.cmp(&a.1.access_count));

        for (path, cached) in files.iter().take(10) {
...
Session memory is automatically restored after context compaction:
...
impl AgentLoop {
    async fn compact(&mut self) -> Result<(), LoopError> {
        ...
        // 2. Restore session memory
        let memory_content = self.session_memory.build_restoration_content();
        self.context.add_system_attachment(memory_content);
