...
│                    VS Code Extension                        │
│  ┌──────────────┐  ┌──────────────┐  ┌─────────────────┐  │
│  │  Commands    │  │  Status Bar  │  │  Blueprint UI   │  │
│  │  (/blueprint)│  │  (Shift+Tab) │  │  (WebView)      │  │
│  └──────┬───────┘  └──────┬───────┘  └────────┬────────┘  │
│         │                  │                    │            │
│         └──────────────────┴────────────────────┘            │
│                            │                                 │
...
│  ┌────────────────────────────────────────────────────────┐ │
│  │  RPC Methods (24 total, 8 blueprint-specific)          │ │
│  │  - blueprint.create/get/update/approve/reject/export   │ │
│  │  - blueprint.setMode, blueprint.addResearch            │ │
│  └────────────────────────────────────────────────────────┘ │
│                            │                                 │
│                            ▼                                 │
│  ┌────────────────────────────────────────────────────────┐ │
...
**Operations**:
- `save_markdown(blueprint)` → PathBuf
- `save_json(blueprint)` → PathBuf
- `load_json(id)` → BlueprintBlock
- `list_blueprints()` → Vec<String>
...
**Methods**:
- `create_blueprint(goal, title)` → String (bp-id)
- `get_blueprint(id)` → BlueprintBlock
- `update_blueprint(id, update_fn)` → String
- `approve_blueprint(id, approver, role)` → Result<()>
- `reject_blueprint(id, reason)` → Result<()>
- `export_blueprint(id)` → (md_path, json_path)
...
3. **CompetitionRunner**: Orchestration
   - `run_competition(blueprint)` → CompetitionResult
...
2. **Immutable After Approval**:
   - `state == Approved` → Cannot modify blueprint
...
**Scenarios**:
- Full blueprint lifecycle (create → approve → execute)
