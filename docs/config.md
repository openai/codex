...
Persist the default state for automatic checkpoints across sessions. When set to
`true`, new Codex sessions begin with `/checkpoint auto` already enabled. Using
`/checkpoint auto on` or `/checkpoint auto off` from the TUI updates this value.
...
Store the default state for automatic commits. When `true`, Codex will commit
after each response unless you disable it in the session. Running `/commit auto`
commands toggles this value automatically.
...
When enabled, Codex automatically runs `/compact` once the remaining portion of
the model's context window drops below `auto_compact_threshold_percent`. This is
equivalent to running `/compact auto` at the start of every session.
...
Sets the percentage of the effective context window that should trigger
automatic compaction when `auto_compact` is enabled. The value must be between 1
and 99. Defaults to `20` when not specified. Using `/compact auto <percent>` in
the TUI updates this value automatically.

```toml
auto_compact_threshold_percent = 25
```
