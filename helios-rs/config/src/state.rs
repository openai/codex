...
use helios_app_server_protocol::ConfigLayer;
...
impl ConfigLayerEntry {
    ...
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_helios_folder } => Some(dot_helios_folder.clone()),
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_helios_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_helios_folder: current_project_dot_helios_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_helios_folder {
                ...
                if previous == current_project_dot_helios_folder
                    || !current_project_dot_helios_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_helios_folder = Some(current_project_dot_helios_folder);
        }
    }
