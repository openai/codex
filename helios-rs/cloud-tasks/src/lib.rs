...
use anyhow::anyhow;
use chrono::Utc;
use helios_cloud_tasks_client::TaskStatus;
...
struct ApplyJob {
    task_id: helios_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn helios_cloud_tasks_client::CloudBackend>,
    base_url: String,
}

async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    let use_mock = matches!(
        std::env::var("HELIOS_CLOUD_TASKS_MODE").ok().as_deref(),
        Some("mock") | Some("MOCK")
    );
    let base_url = std::env::var("HELIOS_CLOUD_TASKS_BASE_URL")
    ...
    if use_mock {
        return Ok(BackendContext {
            backend: Arc::new(helios_cloud_tasks_client::MockClient),
            base_url,
        });
    }
...
impl GitInfoProvider for RealGitInfo {
    async fn default_branch_name(&self, path: &std::path::Path) -> Option<String> {
        helios_core::git_info::default_branch_name(path).await
    }

    async fn current_branch_name(&self, path: &std::path::Path) -> Option<String> {
        helios_core::git_info::current_branch_name(path).await
    }
}
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("helios_cloud_tasks_exec").await?;
    ...
    let created = helios_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        &git_ref,
...
fn parse_task_id(raw: &str) -> anyhow::Result<helios_cloud_tasks_client::TaskId> {
    ...
    Ok(helios_cloud_tasks_client::TaskId(id.to_string()))
}
...
async fn collect_attempt_diffs(
    backend: &dyn helios_cloud_tasks_client::CloudBackend,
    task_id: &helios_cloud_tasks_client::TaskId,
) -> anyhow::Result<Vec<AttemptDiffData>> {
    let text =
        helios_cloud_tasks_client::CloudBackend::get_task_text(backend, task_id.clone()).await?;
    let mut attempts = Vec::new();
    if let Some(diff) =
        helios_cloud_tasks_client::CloudBackend::get_task_diff(backend, task_id.clone()).await?
    {
    ...
    if let Some(turn_id) = text.turn_id {
        let siblings = helios_cloud_tasks_client::CloudBackend::list_sibling_attempts(
            backend,
            task_id.clone(),
            turn_id,
        )
...
fn summary_line(summary: &helios_cloud_tasks_client::DiffSummary, colorize: bool) -> String {
...
fn format_task_status_lines(
    task: &helios_cloud_tasks_client::TaskSummary,
...
fn format_task_list_lines(
    tasks: &[helios_cloud_tasks_client::TaskSummary],
    base_url: &str,
    now: chrono::DateTime<Utc>,
    colorize: bool,
) -> Vec<String> {
...
async fn run_status_command(args: crate::cli::StatusCommand) -> anyhow::Result<()> {
    let ctx = init_backend("helios_cloud_tasks_status").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let summary =
        helios_cloud_tasks_client::CloudBackend::get_task_summary(&*ctx.backend, task_id).await?;
    let now = Utc::now();
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    let ctx = init_backend("helios_cloud_tasks_list").await?;
    let env_filter = if let Some(env) = args.environment {
        Some(resolve_environment_id(&ctx, &env).await?)
    } else {
        None
    };
    let page = helios_cloud_tasks_client::CloudBackend::list_tasks(
        &*ctx.backend,
        env_filter.as_deref(),
        Some(args.limit),
...
async fn run_diff_command(args: crate::cli::DiffCommand) -> anyhow::Result<()> {
    let ctx = init_backend("helios_cloud_tasks_diff").await?;
    let task_id = parse_task_id(&args.task_id)?;
...
async fn run_apply_command(args: crate::cli::ApplyCommand) -> anyhow::Result<()> {
    let ctx = init_backend("helios_cloud_tasks_apply").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let attempts = collect_attempt_diffs(&*ctx.backend, &task_id).await?;
    let selected = select_attempt(&attempts, args.attempt)?;
    let outcome = helios_cloud_tasks_client::CloudBackend::apply_task(
        &*ctx.backend,
        task_id,
    ...
    if !matches!(
        outcome.status,
        helios_cloud_tasks_client::ApplyStatus::Success
    ) {
        std::process::exit(1);
    }
    Ok(())
...
fn level_from_status(status: helios_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        helios_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        helios_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        helios_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = helios_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = helios_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
pub async fn run_main(cli: Cli, _helios_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext { backend, .. } = init_backend("helios_cloud_tasks_tui").await?;
    let backend = backend;
    ...
    let force_internal = matches!(
        std::env::var("HELIOS_CLOUD_TASKS_FORCE_INTERNAL")
            .ok()
            .as_deref(),
    ...
    append_error_log(format!(
        ...
        force_internal,
        helios_core::default_client::get_helios_user_agent()
    ));
    ...
    // Fetch environment list in parallel so the header can show friendly names quickly.
    {
        ...
        tokio::spawn(async move {
            let base_url = util::normalize_base_url(
                &std::env::var("HELIOS_CLOUD_TASKS_BASE_URL")
                    .unwrap_or_else(|_| "https://chatgpt.com/backend-api".to_string()),
            );
            let headers = util::build_chatgpt_headers().await;
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx.send(Instant::now() + helios_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match helios_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, helios_cloud_tasks_client::ApplyStatus::Success) {
                    ...
                    Some(Ok(Event::Key(key))) if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) => {
                        ...
                        if is_ctrl_o && app.new_task.is_some() {
                            ...
                            if should_fetch {
                                    let tx = tx.clone();
                                    tokio::spawn(async move {
            let base_url = crate::util::normalize_base_url(&std::env::var("HELIOS_CLOUD_TASKS_BASE_URL").unwrap_or_else(|_| "https://chatgpt.com/backend-api".to_string()));
            let headers = crate::util::build_chatgpt_headers().await;
                                        let res = crate::env_detect::list_environments(&base_url, &headers).await;
                                        let _ = tx.send(app::AppEvent::EnvironmentsLoaded(res));
                                    });
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                        ...
                                        // Ignore input while submitting
                                    } else if let helios_tui::ComposerAction::Submitted(text) = page.composer.input(key) {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    let result = helios_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, false, best_of_n).await;
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match helios_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match helios_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = helios_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match helios_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
