...
use helios_login::AuthManager;

pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = helios_core::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("helios_cloud_tasks_tui");
    let ua = helios_core::default_client::get_helios_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("helios-cli")),
    );
