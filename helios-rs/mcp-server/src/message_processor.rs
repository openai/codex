...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_helios_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            thread_manager,
            running_requests_id_to_helios_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_helios_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_helios_tool_call_param(),
                create_tool_for_helios_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            ...
            "helios-reply" => {
                self.handle_tool_call_helios_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_codex(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg
                    .into_config(self.helios_linux_sandbox_exe.clone())
                    .await
                {
                    Ok(cfg) => cfg,
                    Err(e) => {
        ...
        task::spawn(async move {
            ...
            crate::helios_tool_runner::run_helios_tool_session(
                ...
                outgoing,
                thread_manager,
                running_requests_id_to_helios_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_helios_session_reply(
        &self,
        ...
        // parse arguments
        let helios_tool_call_reply_param: CodexToolCallReplyParam = match arguments {
            ...
            None => {
                tracing::error!(
                    "Missing arguments for helios-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for helios-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match helios_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let prompt = helios_tool_call_reply_param.prompt.clone();
        tokio::spawn({
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_helios_uuid.lock().await;
        ...
        if let Err(e) = helios_arc
            .submit_with_id(Submission {
                ...
                op: helios_protocol::protocol::Op::Interrupt,
            })
            .await
        {
        ...
        self.running_requests_id_to_helios_uuid
            .lock()
            .await
            .remove(&request_id);
    }
