...
use helios_mcp_server::CodexToolCallParam;
use helios_mcp_server::ExecApprovalElicitRequestParams;
use helios_mcp_server::ExecApprovalResponse;
use helios_mcp_server::PatchApprovalElicitRequestParams;
use helios_mcp_server::PatchApprovalResponse;
use helios_protocol::protocol::FileChange;
use helios_protocol::protocol::ReviewDecision;
use helios_shell_command::parse_command;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // as an elicitation.
    let helios_request_id = mcp_process
        .send_helios_tool_call(CodexToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            helios_request_id.to_string(),
            params.helios_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    let helios_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(helios_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(helios_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    helios_mcp_tool_call_id: String,
    ...
    let helios_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        helios_elicitation: "exec-approval".to_string(),
        helios_mcp_tool_call_id,
        helios_event_id,
        helios_command: command,
        helios_cwd: workdir.to_path_buf(),
        helios_call_id: "call1234".to_string(),
        helios_parsed_cmd,
    })?;
    Ok(params_json)
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_helios_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = helios_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn helios_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    let mut mcp_process = McpProcess::new(helios_home.path()).await?;
    ...
    .await??;
    assert_eq!(helios_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(helios_response.id, RequestId::Number(helios_request_id));
    assert_eq!(
        helios_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
