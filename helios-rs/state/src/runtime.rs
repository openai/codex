...
impl StateRuntime {
    ...
    pub async fn init(
        ...
        tokio::fs::create_dir_all(&helios_home).await?;
        remove_legacy_state_files(&helios_home).await;
        let state_path = state_db_path(helios_home.as_path());
...
pub fn state_db_path(helios_home: &Path) -> PathBuf {
    helios_home.join(state_db_filename())
}

async fn remove_legacy_state_files(helios_home: &Path) {
    let current_name = state_db_filename();
    let mut entries = match tokio::fs::read_dir(helios_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read helios_home for state db cleanup {}: {err}",
                helios_home.display()
            );
            return;
        }
...
mod tests {
    ...
    fn unique_temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "helios-state-runtime-test-{nanos}-{}",
            Uuid::new_v4()
        ))
    }
    ...
    async fn init_removes_legacy_state_db_files() {
        let helios_home = unique_temp_dir();
        tokio::fs::create_dir_all(&helios_home)
            .await
            .expect("create helios_home");

        let current_name = state_db_filename();
        let previous_version = STATE_DB_VERSION.saturating_sub(1);
        let unversioned_name = format!("{STATE_DB_FILENAME}.sqlite");
        for suffix in ["", "-wal", "-shm", "-journal"] {
            let path = helios_home.join(format!("{unversioned_name}{suffix}"));
            tokio::fs::write(path, b"legacy")
                .await
                .expect("write legacy");
            let old_version_path = helios_home.join(format!(
        ...
        let unrelated_path = helios_home.join("state.sqlite_backup");
        tokio::fs::write(&unrelated_path, b"keep")
            .await
            .expect("write unrelated");
        let numeric_path = helios_home.join("123");
        tokio::fs::write(&numeric_path, b"keep")
            .await
            .expect("write numeric");

        let _runtime = StateRuntime::init(helios_home.clone(), "test-provider".to_string(), None)
            .await
            .expect("initialize runtime");

        for suffix in ["", "-wal", "-shm", "-journal"] {
            let legacy_path = helios_home.join(format!("{unversioned_name}{suffix}"));
            assert_eq!(
        ...
        assert_eq!(
            tokio::fs::try_exists(helios_home.join(current_name))
                .await
        ...
        let _ = tokio::fs::remove_dir_all(helios_home).await;
    }

    #[tokio::test]
    async fn backfill_state_persists_progress_and_completion() {
        let helios_home = unique_temp_dir();
        let runtime = StateRuntime::init(helios_home.clone(), "test-provider".to_string(), None)
            .await
            .expect("initialize runtime");
...
WHERE id = 1
    ...
    async fn stage1_claim_skips_when_up_to_date() {
        ...
        let metadata = test_thread_metadata(&helios_home, thread_id, helios_home.join("a"));
        runtime
    ...
    async fn stage1_running_stale_can_be_stolen_but_fresh_running_is_skipped() {
        ...
        let owner_a = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let owner_b = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let cwd = helios_home.join("workspace");
        runtime
            .upsert_thread(&test_thread_metadata(&helios_home, thread_id, cwd))
            .await
            .expect("upsert thread");
    ...
    async fn claim_stage1_jobs_filters_by_age_idle_and_current_thread() {
        ...
        let mut current =
            test_thread_metadata(&helios_home, current_thread_id, helios_home.join("current"));
        ...
        let mut fresh =
            test_thread_metadata(&helios_home, fresh_thread_id, helios_home.join("fresh"));
        ...
        let mut just_under_idle = test_thread_metadata(
            ...
            helios_home.join("just-under-idle"),
        );
        ...
        let mut eligible_idle = test_thread_metadata(
            ...
            helios_home.join("eligible-idle"),
        );
        ...
        let mut old = test_thread_metadata(&helios_home, old_thread_id, helios_home.join("old"));
    ...
    async fn claim_stage1_jobs_prefilters_threads_with_up_to_date_memory() {
        ...
        let mut stale =
            test_thread_metadata(&helios_home, stale_thread_id, helios_home.join("stale"));
    ...
    async fn claim_stage1_jobs_enforces_global_running_cap() {
        ...
        for idx in 0..total_candidates {
            ...
            let mut metadata = test_thread_metadata(
                &helios_home,
                thread_id,
                helios_home.join(format!("thread-{idx}")),
            );
...
WHERE kind = 'memory_stage1'
  ...
  AND lease_until > ?
    ...
    async fn list_stage1_outputs_for_global_returns_latest_outputs() {
        ...
        assert_eq!(outputs[0].cwd, helios_home.join("workspace-b"));
        ...
        assert_eq!(outputs[1].cwd, helios_home.join("workspace-a"));
    ...
    async fn list_stage1_outputs_for_global_skips_empty_payloads() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &helios_home,
                thread_id_non_empty,
                helios_home.join("workspace-non-empty"),
            ))
            .await
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &helios_home,
                thread_id_empty,
                helios_home.join("workspace-empty"),
            ))
            .await
...
VALUES (?, ?, ?, ?, ?)
        ...
        assert_eq!(outputs[0].cwd, helios_home.join("workspace-non-empty"));
    ...
    fn test_thread_metadata(
        ...
        ThreadMetadata {
            id: thread_id,
            rollout_path: helios_home.join(format!("rollout-{thread_id}.jsonl")),
            created_at: now,
            updated_at: now,
