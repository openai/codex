...
struct Args {
    ...
    helios_home: Option<PathBuf>,

    /// Direct path to the SQLite database. Overrides --helios-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let helios_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(helios_home, "logs-client".to_string(), None).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let helios_home = args.helios_home.clone().unwrap_or_else(default_helios_home);
    Ok(helios_state::state_db_path(helios_home.as_path()))
}

fn default_helios_home() -> PathBuf {
