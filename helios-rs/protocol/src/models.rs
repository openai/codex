...
use helios_utils_image::load_and_resize_to_fit;
...
use helios_git::GhostCommit;
use helios_utils_image::error::ImageProcessingError;
...
// (Moved event mapping logic into helios-core to avoid coupling protocol to UI-facing events.)
    ...
    fn includes_request_rule_instructions_for_on_request() {
        ...
        exec_policy
            .add_prefix_rule(
                ...
                helios_execpolicy::Decision::Allow,
            )
            .expect("add rule");
        let instructions = DeveloperInstructions::from_permissions_with_network(
            SandboxMode::WorkspaceWrite,
