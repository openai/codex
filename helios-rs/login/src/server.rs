...
use helios_core::token_data::parse_chatgpt_jwt_claims;
use rand::RngCore;
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("helios_cli_simplified_flow".to_string(), "true".to_string()),
...
pub(crate) async fn persist_tokens_async(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        save_auth(&helios_home, &auth, auth_credentials_store_mode)
    })
    .await
...
fn is_missing_helios_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_helios_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_helios_entitlement_error(error_code, error_description) {
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_helios_entitlement_error(code, error_description) {
            (
