...
use helios_login::run_device_code_login;
use serde_json::json;
use std::sync::Arc;
...
async fn device_code_login_integration_succeeds() -> anyhow::Result<()> {
    ...
    let opts = server_opts(&helios_home, issuer, AuthCredentialsStoreMode::File);

    run_device_code_login(opts)
        .await
        .expect("device code login integration should succeed");

    let auth = load_auth_dot_json(helios_home.path(), AuthCredentialsStoreMode::File)
...
async fn device_code_login_rejects_workspace_mismatch() -> anyhow::Result<()> {
    ...
    let issuer = mock_server.uri();
    let mut opts = server_opts(&helios_home, issuer, AuthCredentialsStoreMode::File);
