...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=HELIOS_BWRAP_SOURCE_DIR");
...
///
/// Priority:
/// 1. `HELIOS_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `helios-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("HELIOS_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "HELIOS_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set HELIOS_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under helios-rs/vendor.",
        vendor_dir.display()
    ))
}
