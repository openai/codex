...
use helios_backend_client as backend;
use helios_backend_client::CodeTaskDetailsResponseExt;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("HELIOS_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
    ...
    impl<'a> Apply<'a> {
        ...
        pub(crate) async fn run(
            ...
            let r = helios_git::apply_git_patch(&req)
