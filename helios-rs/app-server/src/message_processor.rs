...
use crate::helios_message_processor::CodexMessageProcessor;
use crate::helios_message_processor::CodexMessageProcessorArgs;
...
use helios_app_server_protocol::ChatgptAuthTokensRefreshParams;
use helios_app_server_protocol::ChatgptAuthTokensRefreshReason;
use helios_app_server_protocol::ChatgptAuthTokensRefreshResponse;
use helios_app_server_protocol::ClientInfo;
use helios_app_server_protocol::ClientRequest;
use helios_app_server_protocol::ConfigBatchWriteParams;
use helios_app_server_protocol::ConfigReadParams;
use helios_app_server_protocol::ConfigValueWriteParams;
use helios_app_server_protocol::ConfigWarningNotification;
use helios_app_server_protocol::ExperimentalApi;
use helios_app_server_protocol::InitializeResponse;
use helios_app_server_protocol::JSONRPCError;
use helios_app_server_protocol::JSONRPCErrorError;
use helios_app_server_protocol::JSONRPCNotification;
use helios_app_server_protocol::JSONRPCRequest;
use helios_app_server_protocol::JSONRPCResponse;
use helios_app_server_protocol::ServerNotification;
use helios_app_server_protocol::ServerRequestPayload;
use helios_app_server_protocol::experimental_required_message;
use helios_core::AuthManager;
use helios_core::ThreadManager;
use helios_core::auth::ExternalAuthRefreshContext;
use helios_core::auth::ExternalAuthRefreshReason;
use helios_core::auth::ExternalAuthRefresher;
use helios_core::auth::ExternalAuthTokens;
use helios_core::config::Config;
use helios_core::config_loader::CloudRequirementsLoader;
use helios_core::config_loader::LoaderOverrides;
use helios_core::default_client::SetOriginatorError;
use helios_core::default_client::USER_AGENT_SUFFIX;
use helios_core::default_client::get_helios_user_agent;
use helios_core::default_client::set_default_client_residency_requirement;
use helios_core::default_client::set_default_originator;
use helios_feedback::CodexFeedback;
...
pub(crate) struct MessageProcessor {
    outgoing: Arc<OutgoingMessageSender>,
    helios_message_processor: CodexMessageProcessor,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let helios_message_processor = CodexMessageProcessor::new(CodexMessageProcessorArgs {
            auth_manager,
            thread_manager,
            outgoing: outgoing.clone(),
            helios_linux_sandbox_exe,
    ...
    pub(crate) async fn process_request(
        ...
        let helios_request = match serde_json::from_value::<ClientRequest>(request_json) {
            Ok(helios_request) => helios_request,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INVALID_REQUEST_ERROR_CODE,
                    message: format!("Invalid request: {err}"),
        ...
        match helios_request {
            ...
            ClientRequest::Initialize { request_id, params } => {
                ...
                if session.initialized {
                    ...
                    if let Err(error) = set_default_originator(name.clone()) {
                        match error {
                            ...
                            SetOriginatorError::AlreadyInitialized => {
                                ...
                                // TODO(owen): Once we remove support for HELIOS_INTERNAL_ORIGINATOR_OVERRIDE,
                    ...
                    let user_agent = get_helios_user_agent();
        ...
        if let Some(reason) = helios_request.experimental_reason()
    ...
    pub(crate) fn thread_created_receiver(&self) -> broadcast::Receiver<ThreadId> {
        self.helios_message_processor.thread_created_receiver()
    }
