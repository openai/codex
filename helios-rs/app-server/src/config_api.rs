...
use helios_app_server_protocol::ConfigRequirements;
use helios_app_server_protocol::ConfigRequirementsReadResponse;
...
use helios_app_server_protocol::NetworkRequirements;
use helios_app_server_protocol::SandboxMode;
use helios_core::config::ConfigService;
use helios_core::config::ConfigServiceError;
...
use helios_core::config_loader::ResidencyRequirement as CoreResidencyRequirement;
use helios_core::config_loader::SandboxModeRequirement as CoreSandboxModeRequirement;
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(helios_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> helios_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => helios_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: helios_core::config_loader::NetworkRequirementsToml,
) -> NetworkRequirements {
    NetworkRequirements {
...
mod tests {
    use super::*;
    use helios_core::config_loader::NetworkRequirementsToml as CoreNetworkRequirementsToml;
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        let requirements = ConfigRequirementsToml {
            ...
            allowed_web_search_modes: Some(vec![
                helios_core::config_loader::WebSearchModeRequirement::Cached,
            ]),
            mcp_servers: None,
            rules: None,
        ...
        assert_eq!(
            ...
            Some(vec![
                helios_app_server_protocol::AskForApproval::Never,
                helios_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(helios_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
