...
use helios_app_server_protocol::TextPosition as AppTextPosition;
use helios_app_server_protocol::TextRange as AppTextRange;
use helios_core::ExecPolicyError;
use helios_core::check_execpolicy_for_warnings;
use helios_core::config_loader::ConfigLoadError;
use helios_core::config_loader::TextRange as CoreTextRange;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
        ...
        .get_layers(ConfigLayerStackOrdering::LowestPrecedenceFirst, true)
    {
        ...
        if let ConfigLayerSource::Project { dot_helios_folder } = &layer.name {
            disabled_folders.push((
                dot_helios_folder.as_path().display().to_string(),
                layer
                    .disabled_reason
                    .as_ref()
...
pub async fn run_main_with_transport(
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) = helios_core::personality_migration::maybe_migrate_personality(
                        &config.helios_home,
                        &config_toml,
                    )
                    .await
    ...
    let otel = helios_core::otel_init::build_provider(
        &config,
