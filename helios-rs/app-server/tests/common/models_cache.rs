use chrono::DateTime;
use chrono::Utc;
use helios_core::test_support::all_model_presets;
use helios_protocol::openai_models::ConfigShellToolType;
use helios_protocol::openai_models::ModelInfo;
use helios_protocol::openai_models::ModelPreset;
use helios_protocol::openai_models::ModelVisibility;
...
pub fn write_models_cache(helios_home: &Path) -> std::io::Result<()> {
    ...
    write_models_cache_with_models(helios_home, models)
}
...
pub fn write_models_cache_with_models(
    ...
    let cache_path = helios_home.join("models_cache.json");
    // DateTime<Utc> serializes to RFC3339 format by default with serde
    let fetched_at: DateTime<Utc> = Utc::now();
    let client_version = helios_core::models_manager::client_version_to_whole();
    let cache = json!({
