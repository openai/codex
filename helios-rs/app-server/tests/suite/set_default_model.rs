...
use helios_app_server_protocol::SetDefaultModelParams;
use helios_app_server_protocol::SetDefaultModelResponse;
...
async fn set_default_model_persists_overrides() -> Result<()> {
    ...
    create_config_toml(helios_home.path())?;
    ...
    let config_path = helios_home.path().join("config.toml");
...
fn create_config_toml(helios_home: &Path) -> std::io::Result<()> {
    let config_toml = helios_home.join("config.toml");
    std::fs::write(
        config_toml,
        r#"
model = "gpt-5.1-helios-max"
