...
use helios_app_server_protocol::GetAuthStatusParams;
use helios_app_server_protocol::GetAuthStatusResponse;
...
use helios_app_server_protocol::LogoutChatGptResponse;
use helios_app_server_protocol::RequestId;
use helios_core::auth::AuthCredentialsStoreMode;
use helios_login::login_with_api_key;
...
async fn logout_chatgpt_removes_auth() -> Result<()> {
    ...
    assert!(helios_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(helios_home.path(), &[("OPENAI_API_KEY", None)]).await?;
    ...
    assert!(
        !helios_home.path().join("auth.json").exists(),
...
fn create_config_toml_forced_login(helios_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = helios_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn login_chatgpt_rejected_when_forced_api() -> Result<()> {
    let helios_home = TempDir::new()?;
    create_config_toml_forced_login(helios_home.path(), "api")?;
...
async fn login_chatgpt_includes_forced_workspace_query_param() -> Result<()> {
    let helios_home = TempDir::new()?;
    create_config_toml_forced_workspace(helios_home.path(), "ws-forced")?;
