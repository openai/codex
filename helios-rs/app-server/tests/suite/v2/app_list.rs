...
use helios_app_server_protocol::AppBranding;
use helios_app_server_protocol::AppInfo;
use helios_app_server_protocol::AppListUpdatedNotification;
use helios_app_server_protocol::AppMetadata;
use helios_app_server_protocol::AppReview;
use helios_app_server_protocol::AppScreenshot;
use helios_app_server_protocol::AppsListParams;
use helios_app_server_protocol::AppsListResponse;
...
async fn list_apps_uses_thread_feature_flag_when_thread_id_is_provided() -> Result<()> {
    ...
    let helios_home = TempDir::new()?;
    write_connectors_config(helios_home.path(), &server_url)?;
    write_chatgpt_auth(
        helios_home.path(),
...
fn write_connectors_config(helios_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
