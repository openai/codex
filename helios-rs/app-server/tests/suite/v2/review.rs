...
use helios_app_server_protocol::ReviewDelivery;
use helios_app_server_protocol::ReviewStartParams;
use helios_app_server_protocol::ReviewStartResponse;
use helios_app_server_protocol::ReviewTarget;
...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    let helios_home = TempDir::new()?;
    create_config_toml_with_approval_policy(helios_home.path(), &server.uri(), "untrusted")?;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": helios_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
fn create_config_toml(helios_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    create_config_toml_with_approval_policy(helios_home, server_uri, "never")
}

fn create_config_toml_with_approval_policy(
    helios_home: &std::path::Path,
    server_uri: &str,
    approval_policy: &str,
) -> std::io::Result<()> {
