...
use helios_app_server_protocol::ByteRange;
use helios_app_server_protocol::ClientInfo;
use helios_app_server_protocol::CommandExecutionApprovalDecision;
use helios_app_server_protocol::CommandExecutionRequestApprovalResponse;
use helios_app_server_protocol::CommandExecutionStatus;
use helios_app_server_protocol::FileChangeApprovalDecision;
use helios_app_server_protocol::FileChangeOutputDeltaNotification;
use helios_app_server_protocol::FileChangeRequestApprovalResponse;
use helios_app_server_protocol::ItemCompletedNotification;
use helios_app_server_protocol::ItemStartedNotification;
use helios_app_server_protocol::JSONRPCNotification;
use helios_app_server_protocol::JSONRPCResponse;
use helios_app_server_protocol::PatchApplyStatus;
use helios_app_server_protocol::PatchChangeKind;
use helios_app_server_protocol::RequestId;
use helios_app_server_protocol::ServerRequest;
use helios_app_server_protocol::TextElement;
use helios_app_server_protocol::ThreadItem;
use helios_app_server_protocol::ThreadStartParams;
use helios_app_server_protocol::ThreadStartResponse;
use helios_app_server_protocol::TurnCompletedNotification;
use helios_app_server_protocol::TurnStartParams;
use helios_app_server_protocol::TurnStartResponse;
use helios_app_server_protocol::TurnStartedNotification;
use helios_app_server_protocol::TurnStatus;
use helios_app_server_protocol::UserInput as V2UserInput;
use helios_core::config::ConfigToml;
use helios_core::features::FEATURES;
use helios_core::features::Feature;
use helios_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
...
const TEST_ORIGINATOR: &str = "helios_vscode";
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    assert_eq!(
        started.turn.status,
        helios_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-helios-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_uses_migrated_pragmatic_personality_without_override_v2() -> Result<()> {
    ...
    let persisted_toml: ConfigToml = toml::from_str(&std::fs::read_to_string(
        helios_home.path().join("config.toml"),
    )?)?;
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    ...
    let image_path = helios_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let helios_home = tmp.path().to_path_buf();
    ...
    let mut mcp = McpProcess::new(helios_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(helios_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(helios_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(&helios_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(helios_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(helios_app_server_protocol::SandboxPolicy::WorkspaceWrite {
                writable_roots: vec![first_cwd.try_into()?],
                read_only_access: helios_app_server_protocol::ReadOnlyAccess::FullAccess,
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![helios_app_server_protocol::FileUpdateChange {
