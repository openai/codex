...
fn create_config_toml(helios_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(helios_home.path(), CreateConfigTomlParams::default())?;
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(helios_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<helios_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(helios_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![helios_app_server_protocol::UserInput::Text {
