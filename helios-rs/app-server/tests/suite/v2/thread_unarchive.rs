...
async fn thread_unarchive_moves_rollout_back_into_sessions_directory() -> Result<()> {
    ...
    let found_rollout_path = find_thread_path_by_id_str(helios_home.path(), &thread.id)
        .await?
    ...
    let archived_path = find_archived_thread_path_by_id_str(helios_home.path(), &thread.id)
        .await?
