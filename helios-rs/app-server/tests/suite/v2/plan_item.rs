...
use helios_app_server_protocol::PlanDeltaNotification;
...
async fn start_plan_mode_turn(mcp: &mut McpProcess) -> Result<helios_app_server_protocol::Turn> {
    let thread_req = mcp
...
fn create_config_toml(helios_home: &Path, server_uri: &str) -> std::io::Result<()> {
