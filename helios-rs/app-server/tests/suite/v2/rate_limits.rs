...
use helios_app_server_protocol::LoginApiKeyParams;
use helios_app_server_protocol::RateLimitSnapshot;
use helios_app_server_protocol::RateLimitWindow;
use helios_app_server_protocol::RequestId;
use helios_core::auth::AuthCredentialsStoreMode;
use helios_protocol::account::PlanType as AccountPlanType;
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(helios_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "helios_other",
                "metered_feature": "helios_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
...
fn write_chatgpt_base_url(helios_home: &Path, base_url: &str) -> std::io::Result<()> {
