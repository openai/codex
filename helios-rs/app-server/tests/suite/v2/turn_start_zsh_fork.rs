...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&helios_home, &workspace).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(helios_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(helios_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
...
"#,
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(helios_app_server_protocol::AskForApproval::OnRequest),
            sandbox_policy: Some(if cfg!(target_os = "linux") {
                ...
                helios_app_server_protocol::SandboxPolicy::DangerFullAccess
            } else {
                helios_app_server_protocol::SandboxPolicy::ReadOnly {
                    access: helios_app_server_protocol::ReadOnlyAccess::FullAccess,
                }
            }),
            model: Some("mock-model".to_string()),
            effort: Some(helios_protocol::openai_models::ReasoningEffort::Medium),
...
async fn create_zsh_test_mcp_process(helios_home: &Path, zdotdir: &Path) -> Result<McpProcess> {
    let zdotdir = zdotdir.to_string_lossy().into_owned();
    McpProcess::new_with_env(helios_home, &[("ZDOTDIR", Some(zdotdir.as_str()))]).await
}
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = helios_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("helios-rs/exec-server/tests/suite/zsh");
