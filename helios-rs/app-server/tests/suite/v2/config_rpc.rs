...
use helios_app_server_protocol::ConfigEdit;
...
use helios_app_server_protocol::ToolsV2;
...
fn write_config(helios_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let helios_home_path = helios_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(helios_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let helios_home = TempDir::new()?;
    write_config(&helios_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_helios_folder: project_config
        }
    );
...
"#,
    ...
    let mut mcp = McpProcess::new_with_env(
        helios_home.path(),
        &[(
            "HELIOS_APP_SERVER_MANAGED_CONFIG_PATH",
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let helios_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", helios_home)?;
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(helios_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let helios_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(helios_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[helios_app_server_protocol::ConfigLayer],
