...
async fn websocket_transport_routes_per_connection_handshake_and_responses() -> Result<()> {
    ...
    let mut process = spawn_websocket_server(helios_home.path(), bind_addr).await?;
...
async fn spawn_websocket_server(helios_home: &Path, bind_addr: SocketAddr) -> Result<Child> {
    let program = helios_utils_cargo_bin::cargo_bin("helios-app-server")
