...
use chrono::DateTime;
use chrono::Utc;
use helios_app_server_protocol::GitInfo as ApiGitInfo;
...
use helios_app_server_protocol::SessionSource;
use helios_app_server_protocol::ThreadListResponse;
use helios_app_server_protocol::ThreadSortKey;
use helios_app_server_protocol::ThreadSourceKind;
use helios_app_server_protocol::ThreadStartParams;
use helios_app_server_protocol::ThreadStartResponse;
use helios_app_server_protocol::ThreadStatus;
use helios_app_server_protocol::TurnStartParams;
use helios_app_server_protocol::TurnStartResponse;
use helios_app_server_protocol::UserInput;
use helios_core::ARCHIVED_SESSIONS_SUBDIR;
use helios_protocol::ThreadId;
use helios_protocol::protocol::GitInfo as CoreGitInfo;
use helios_protocol::protocol::RolloutItem;
use helios_protocol::protocol::RolloutLine;
use helios_protocol::protocol::SessionSource as CoreSessionSource;
...
async fn init_mcp(helios_home: &Path) -> Result<McpProcess> {
...
async fn list_threads_with_sort(
    ...
    let request_id = mcp
        .send_thread_list_request(helios_app_server_protocol::ThreadListParams {
            cursor,
            limit,
            sort_key,
...
async fn thread_list_basic_empty() -> Result<()> {
    ...
    let mut mcp = init_mcp(helios_home.path()).await?;
...
async fn thread_list_reports_system_error_idle_flag_after_failed_turn() -> Result<()> {
    ...
    let helios_home = TempDir::new()?;
    create_runtime_config(helios_home.path(), &server.uri())?;
    let mut mcp = init_mcp(helios_home.path()).await?;
...
fn create_minimal_config(helios_home: &std::path::Path) -> std::io::Result<()> {
...
fn create_runtime_config(helios_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = helios_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
async fn thread_list_respects_cwd_filter() -> Result<()> {
    ...
    let target_cwd = helios_home.path().join("target-cwd");
    fs::create_dir_all(&target_cwd)?;
    set_rollout_cwd(
        rollout_path(helios_home.path(), "2025-01-02T10-00-00", &filtered_id).as_path(),
        &target_cwd,
    )?;
...
async fn thread_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(helios_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(helios_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(helios_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_paginates_with_cursor() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(helios_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(helios_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(helios_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
...
async fn thread_list_updated_at_uses_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(helios_home.path(), "2025-02-01T10-00-00", &thread_id).as_path(),
...
async fn thread_list_archived_filter() -> Result<()> {
    ...
    let archived_dir = helios_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    fs::create_dir_all(&archived_dir)?;
    let archived_source = rollout_path(helios_home.path(), "2025-03-01T09-00-00", &archived_id);
