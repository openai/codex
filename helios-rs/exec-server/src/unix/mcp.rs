...
use anyhow::Result;
use helios_core::MCP_SANDBOX_STATE_CAPABILITY;
use helios_core::MCP_SANDBOX_STATE_METHOD;
use helios_core::SandboxState;
use helios_execpolicy::Policy;
use helios_protocol::protocol::SandboxPolicy;
use helios_shell_escalation::EscalationPolicyFactory;
use helios_shell_escalation::run_escalate_server;
...
const HELIOS_BASH_PATH_ENV_VAR: &str = "HELIOS_BASH_PATH";

const SANDBOX_STATE_CAPABILITY_VERSION: &str = "1.0.0";

pub(crate) fn get_bash_path() -> Result<PathBuf> {
    std::env::var(HELIOS_BASH_PATH_ENV_VAR)
        .map(PathBuf::from)
        .context(format!("{HELIOS_BASH_PATH_ENV_VAR} must be set"))
}
...
impl From<helios_shell_escalation::ExecResult> for ExecResult {
    fn from(result: helios_shell_escalation::ExecResult) -> Self {
        Self {
...
impl From<ExecParams> for helios_shell_escalation::ExecParams {
...
impl EscalationPolicyFactory for McpEscalationPolicyFactory {
    ...
    fn create_policy(
        &self,
        policy: Arc<RwLock<Policy>>,
        stopwatch: helios_shell_escalation::Stopwatch,
    ) -> Self::Policy {
...
impl ExecTool {
    ...
    async fn shell(
        ...
        let effective_timeout = Duration::from_millis(
            params
                .timeout_ms
                .unwrap_or(helios_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS),
        );
        let sandbox_state =
            self.sandbox_state
                .read()
