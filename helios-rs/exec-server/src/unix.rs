...
//! from us, and we give its fd number in the HELIOS_ESCALATE_SOCKET environment variable.
//!
...
use helios_shell_command::is_dangerous_command::command_might_be_dangerous;
use helios_shell_escalation as shell_escalation;
...
const HELIOS_EXECVE_WRAPPER_EXE_NAME: &str = "helios-execve-wrapper";
...
pub async fn main_mcp_server() -> anyhow::Result<()> {
    ...
    let execve_wrapper = match cli.execve_wrapper {
        ...
        None => {
            ...
            cwd.parent()
                .map(|p| p.join(HELIOS_EXECVE_WRAPPER_EXE_NAME))
                .ok_or_else(|| {
...
async fn load_exec_policy() -> anyhow::Result<Policy> {
    let helios_home = find_helios_home().context("failed to resolve helios_home for execpolicy")?;

    // TODO(mbolin): At a minimum, `cwd` should be configurable via
    // `codex/sandbox-state/update` or some other custom MCP call.
    let cwd = None;
    let cli_overrides = Vec::new();
    let overrides = helios_core::config_loader::LoaderOverrides::default();
    let config_layer_stack = helios_core::config_loader::load_config_layers_state(
        &helios_home,
        cwd,
        &cli_overrides,
        overrides,
        helios_core::config_loader::CloudRequirementsLoader::default(),
    )
    .await?;

    helios_core::load_exec_policy(&config_layer_stack)
        .await
