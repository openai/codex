    ...
    P: AsRef<Path>,
{
    ...
    create_transport_with_shell_path(helios_home, dotslash_cache, bash).await
}
...
where
    ...
    R: AsRef<Path>,
{
    let mcp_executable = helios_utils_cargo_bin::cargo_bin("helios-exec-mcp-server")?;
    let execve_wrapper = helios_utils_cargo_bin::cargo_bin("helios-execve-wrapper")?;
    ...
    let transport = TokioChildProcess::new(Command::new(&mcp_executable).configure(|cmd| {
        ...
        cmd.env("HELIOS_HOME", helios_home.as_ref());
...
pub async fn write_default_execpolicy<P>(policy: &str, helios_home: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    let policy_dir = helios_home.as_ref().join("rules");
