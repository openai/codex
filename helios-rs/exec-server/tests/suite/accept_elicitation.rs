...
use anyhow::Result;
use anyhow::ensure;
use helios_exec_server::ExecResult;
...
async fn accept_elicitation_for_prompt_rule() -> Result<()> {
    ...
    // $HELIOS_HOME with an execpolicy that prompts for `git init` commands.
...
"#,
    ...
    let transport = create_transport(helios_home.as_ref(), dotslash_cache).await?;
...
"#,
    ...
    let transport =
        create_transport_with_shell_path(helios_home.as_ref(), dotslash_cache, &zsh_path).await?;
    run_accept_elicitation_for_prompt_rule_with_transport(transport).await
}
...
async fn run_accept_elicitation_for_prompt_rule_with_transport(
    ...
    let helios_linux_sandbox_exe = if cfg!(target_os = "linux") {
        let helios_linux_sandbox_exe = linux_sandbox_exe_folder.path().join("helios-linux-sandbox");
        let helios_cli = ensure_helios_cli()?;
        symlink(&helios_cli, &helios_linux_sandbox_exe)?;
        Some(helios_linux_sandbox_exe)
    } else {
        None
    };
    let response =
        notify_readable_sandbox(&project_root_path, helios_linux_sandbox_exe, &service).await?;
...
async fn resolve_test_zsh_path(dotslash_cache: &std::path::Path) -> Result<PathBuf> {
    let dotslash_zsh = helios_utils_cargo_bin::find_resource!("tests/suite/zsh")?;
...
fn ensure_helios_cli() -> Result<PathBuf> {
    let helios_cli = helios_utils_cargo_bin::cargo_bin("codex")?;

    let metadata = helios_cli.metadata().with_context(|| {
        format!(
        ...
        metadata.is_file(),
        "expected codex binary at {} to be a file; run `cargo build -p helios-cli --bin codex` before this test",
        helios_cli.display()
    );
    ...
    let mode = metadata.permissions().mode();
        ...
        mode & 0o111 != 0,
        "codex binary at {} is not executable (mode {mode:o}); run `cargo build -p helios-cli --bin codex` before this test",
        helios_cli.display()
    );
