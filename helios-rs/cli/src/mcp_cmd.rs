...
use helios_core::config::edit::ConfigEditsBuilder;
use helios_core::config::find_helios_home;
use helios_core::config::load_global_mcp_servers;
use helios_core::config::types::McpServerConfig;
use helios_core::config::types::McpServerTransportConfig;
use helios_core::mcp::auth::McpOAuthLoginSupport;
use helios_core::mcp::auth::compute_auth_statuses;
use helios_core::mcp::auth::oauth_login_support;
use helios_protocol::protocol::McpAuthStatus;
use helios_rmcp_client::delete_oauth_tokens;
use helios_rmcp_client::perform_oauth_login;
use helios_utils_cli::CliConfigOverrides;
use helios_utils_cli::format_env_display::format_env_display;
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let helios_home = find_helios_home().context("failed to resolve HELIOS_HOME")?;
    let mut servers = load_global_mcp_servers(&helios_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", helios_home.display()))?;
    ...
    ConfigEditsBuilder::new(&helios_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", helios_home.display()))?;
