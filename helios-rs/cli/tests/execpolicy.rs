...
fn execpolicy_check_matches_expected_json() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let policy_path = helios_home.path().join("rules").join("policy.rules");
    fs::create_dir_all(
        policy_path
            .parent()
...
"#,
    ...
    let output = Command::new(helios_utils_cargo_bin::cargo_bin("codex")?)
        .env("HELIOS_HOME", helios_home.path())
        .args([
            "execpolicy",
            "check",
            "--rules",
