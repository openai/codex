...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = helios_command(helios_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = helios_command(helios_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = helios_command(helios_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!helios_home.path().join(".credentials.json").exists());
    assert!(!helios_home.path().join(".env").exists());
