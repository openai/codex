...
fn build_file_watcher(helios_home: PathBuf, skills_manager: Arc<SkillsManager>) -> Arc<FileWatcher> {
    ...
    let file_watcher = match FileWatcher::new(helios_home) {
...
pub struct ThreadManager {
    ...
    _test_helios_home_guard: Option<TempCodexHomeGuard>,
}
...
impl ThreadManager {
    pub fn new(
        ...
        let skills_manager = Arc::new(SkillsManager::new(helios_home.clone()));
        let file_watcher = build_file_watcher(helios_home.clone(), Arc::clone(&skills_manager));
        Self {
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let helios_home = std::env::temp_dir().join(format!(
            "helios-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&helios_home)
            .unwrap_or_else(|err| panic!("temp codex home dir create failed: {err}"));
        let mut manager =
            Self::with_models_provider_and_home_for_tests(auth, provider, helios_home.clone());
        manager._test_helios_home_guard = Some(TempCodexHomeGuard { path: helios_home });
        manager
    }
