...
use helios_state::DB_METRIC_COMPARE_ERROR;
pub use helios_state::LogEntry;
use helios_state::STATE_DB_VERSION;
...
pub type StateDbHandle = Arc<helios_state::StateRuntime>;
...
pub(crate) async fn init_if_enabled(
    ...
    let runtime = match helios_state::StateRuntime::init(
    ...
    if backfill_state.status != helios_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &Config, otel: Option<&OtelManager>) -> Option<StateDbHandle> {
    let state_path = helios_state::state_db_path(config.helios_home.as_path());
    if !config.features.enabled(Feature::Sqlite)
    ...
    .await
    .ok()?;
    require_backfill_complete(runtime, config.helios_home.as_path()).await
}

/// Open the state runtime when the SQLite file exists, without feature gating.
///
/// This is used for parity checks during the SQLite migration phase.
pub async fn open_if_present(helios_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = helios_state::state_db_path(helios_home);
    ...
    .await
    .ok()?;
    require_backfill_complete(runtime, helios_home).await
}
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == helios_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<helios_state::Anchor> {
    let cursor = cursor?;
    ...
    .with_nanosecond(0)?;
    Some(helios_state::Anchor { ts, id })
}
...
pub async fn list_thread_ids_db(
    context: Option<&helios_state::StateRuntime>,
    helios_home: &Path,
    page_size: usize,
    ...
    if ctx.helios_home() != helios_home {
        warn!(
            "state db helios_home mismatch: expected {}, got {}",
            ctx.helios_home().display(),
            helios_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => helios_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => helios_state::SortKey::UpdatedAt,
            },
...
pub fn record_discrepancy(stage: &str, reason: &str) {
    ...
    if let Some(metric) = helios_otel::metrics::global() {
