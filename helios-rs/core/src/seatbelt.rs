...
use helios_network_proxy::PROXY_URL_ENV_KEYS;
use helios_network_proxy::has_proxy_url_env_vars;
use helios_network_proxy::proxy_url_env_value;
...
pub async fn spawn_command_under_seatbelt(
    ...
    let arg0 = None;
    env.insert(HELIOS_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
...
mod tests {
    ...
    fn seatbelt_args_allow_explicit_wrapper_unix_socket_path() {
        let cwd = std::env::temp_dir();
        let socket_path = std::env::temp_dir().join("helios-zsh-wrapper-test.sock");
        let args = create_seatbelt_command_args(
            vec!["echo".to_string(), "ok".to_string()],
            &SandboxPolicy::new_read_only_policy(),
            cwd.as_path(),
    ...
    #[test]
    fn create_seatbelt_args_with_read_only_git_and_helios_subpaths() {
...
"#,
        ...
        let mut expected_args = vec![
            ...
            format!(
                ...
                dot_helios_canonical.to_string_lossy()
            ),
            format!(
                "-DWRITABLE_ROOT_1={}",
        ...
        // Seatbelt policy.
        let config_toml = dot_helios_canonical.join("config.toml");
...
"#,
    ...
    fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
        ...
        let dot_helios_canonical = vulnerable_root_canonical.join(".codex");
