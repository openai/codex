...
pub async fn append_thread_name(
    ...
    append_session_index_entry(helios_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(helios_home);
...
pub async fn find_thread_path_by_name_str(
    ...
    let Some(thread_id) = find_thread_id_by_name(helios_home, name).await? else {
        return Ok(None);
    };
    super::list::find_thread_path_by_id_str(helios_home, &thread_id.to_string()).await
}

fn session_index_path(helios_home: &Path) -> PathBuf {
    helios_home.join(SESSION_INDEX_FILE)
}
