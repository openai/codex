...
impl From<helios_state::Anchor> for Cursor {
    fn from(anchor: helios_state::Anchor) -> Self {
...
pub(crate) async fn get_threads(
    ...
    let root = helios_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    let state_db_ctx = state_db::open_if_present(helios_home, "").await;
    ...
    let mut root = helios_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(helios_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(helios_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
