...
pub(crate) fn map_session_init_error(err: &anyhow::Error, helios_home: &Path) -> CodexErr {
    if let Some(mapped) = err
        .chain()
        .filter_map(|cause| cause.downcast_ref::<std::io::Error>())
        .find_map(|io_err| map_rollout_io_error(io_err, helios_home))
    {
        return mapped;
    }
...
fn map_rollout_io_error(io_err: &std::io::Error, helios_home: &Path) -> Option<CodexErr> {
    let sessions_dir = helios_home.join(SESSIONS_SUBDIR);
