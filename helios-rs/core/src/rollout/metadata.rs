...
use helios_state::BackfillState;
use helios_state::BackfillStats;
use helios_state::BackfillStatus;
use helios_state::DB_ERROR_METRIC;
use helios_state::DB_METRIC_BACKFILL;
use helios_state::DB_METRIC_BACKFILL_DURATION_MS;
use helios_state::ExtractionOutcome;
use helios_state::ThreadMetadataBuilder;
use helios_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(
    runtime: &helios_state::StateRuntime,
    config: &Config,
    otel: Option<&OtelManager>,
) {
    ...
    let sessions_root = config.helios_home.join(rollout::SESSIONS_SUBDIR);
    let archived_root = config.helios_home.join(rollout::ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.helios_home.as_path(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(helios_home: &Path, path: &Path) -> String {
    path.strip_prefix(helios_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
...
mod tests {
    ...
    use chrono::Utc;
    use helios_protocol::ThreadId;
    use helios_protocol::protocol::CompactedItem;
    use helios_protocol::protocol::RolloutItem;
    ...
    async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
        let dir = tempdir().expect("tempdir");
        let helios_home = dir.path().to_path_buf();
        ...
        config.helios_home = helios_home.clone();
    ...
    fn write_rollout_in_sessions(
        ...
        let sessions_dir = helios_home.join("sessions");
