use chrono::DateTime;
use chrono::Utc;
use helios_api::AuthProvider as ApiAuthProvider;
use helios_api::TransportError;
use helios_api::error::ApiError;
use helios_api::rate_limits::parse_promo_message;
use helios_api::rate_limits::parse_rate_limit_for_limit;
use http::HeaderMap;
...
const ACTIVE_LIMIT_HEADER: &str = "x-helios-active-limit";
...
mod tests {
    ...
    fn map_api_error_maps_usage_limit_limit_name_header() {
        ...
        headers.insert(
            ACTIVE_LIMIT_HEADER,
            http::HeaderValue::from_static("helios_other"),
        );
        headers.insert(
            "x-helios-other-limit-name",
            http::HeaderValue::from_static("helios_other"),
        );
        let body = serde_json::json!({
            "error": {
