...
pub const HELIOS_THREAD_ID_ENV_VAR: &str = "HELIOS_THREAD_ID";
...
///
/// `HELIOS_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
    ...
    I: IntoIterator<Item = (String, String)>,
{
    ...
    if let Some(thread_id) = thread_id {
        env_map.insert(HELIOS_THREAD_ID_ENV_VAR.to_string(), thread_id.to_string());
    }
...
mod tests {
    ...
    fn test_core_inherit_defaults_keep_sensitive_vars() {
        ...
        expected.insert(HELIOS_THREAD_ID_ENV_VAR.to_string(), thread_id.to_string());
