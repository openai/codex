...
pub fn apply_blocking(
    ...
    let config_path = helios_home.join(CONFIG_TOML_FILE);
...
pub async fn apply(
    ...
    let helios_home = helios_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&helios_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(helios_home: &Path) -> Self {
        Self {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.helios_home, self.profile.as_deref(), &self.edits)
    }
...
"#;
    ...
    fn set_skill_config_removes_entry_when_enabled() {
        ...
        let helios_home = tmp.path();
        std::fs::write(
            helios_home.join(CONFIG_TOML_FILE),
            r#"[[skills.config]]
path = "/tmp/skills/demo/SKILL.md"
enabled = false
"#,
...
"#,
        ...
        let raw = std::fs::read_to_string(helios_home.join(CONFIG_TOML_FILE)).expect("read config");
    ...
    fn blocking_set_model_writes_through_symlink_chain() {
        ...
        let link_path = helios_home.join("config-link.toml");
        let config_path = helios_home.join(CONFIG_TOML_FILE);
...
"#;
    ...
    fn blocking_set_model_replaces_symlink_on_cycle() {
        let tmp = tempdir().expect("tmpdir");
        let helios_home = tmp.path();
        let link_a = helios_home.join("a.toml");
        let link_b = helios_home.join("b.toml");
        let config_path = helios_home.join(CONFIG_TOML_FILE);
...
"#;
        std::fs::write(helios_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
"#;
    ...
    #[test]
    fn blocking_set_hide_gpt_5_1_helios_max_migration_prompt_preserves_table() {
        let tmp = tempdir().expect("tmpdir");
        let helios_home = tmp.path();
        std::fs::write(
            helios_home.join(CONFIG_TOML_FILE),
...
"#,
        ...
        apply_blocking(
            ...
            None,
            &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
                "hide_gpt-5.1-helios-max_migration_prompt".to_string(),
                true,
            )],
        )
        .expect("persist");
        ...
        let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-helios-max_migration_prompt" = true
"#;
        assert_eq!(contents, expected);
    }
...
"#,
        ...
        apply_blocking(helios_home, None, &[ConfigEdit::ReplaceMcpServers(servers)])
            .expect("persist");
...
"#;
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !helios_home.join(CONFIG_TOML_FILE).exists(),
