...
/// `helios_feedback::CodexFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// helios_core::feedback_tags!(model = "gpt-5", cached = true);
/// helios_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
