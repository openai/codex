...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: helios_protocol::config_types::WindowsSandboxLevel,
...
pub async fn process_exec_tool_call(
            ...
            if enforce_managed_network {
                get_platform_sandbox(
                    windows_sandbox_level
                        != helios_protocol::config_types::WindowsSandboxLevel::Disabled,
                )
    ...
    let exec_req = manager
        .transform(crate::sandboxing::SandboxTransformRequest {
            ...
            helios_linux_sandbox_exe: helios_linux_sandbox_exe.as_ref(),
...
fn record_windows_sandbox_spawn_failure(
    ...
    windows_sandbox_level: helios_protocol::config_types::WindowsSandboxLevel,
    err: &str,
) {
    ...
    let level = if matches!(
        windows_sandbox_level,
        helios_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_helios_home;
    use helios_protocol::config_types::WindowsSandboxLevel;
    use helios_windows_sandbox::run_windows_sandbox_capture;
    use helios_windows_sandbox::run_windows_sandbox_capture_elevated;
    ...
    let helios_home = find_helios_home().map_err(|err| {
        CodexErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve helios_home: {err}"
        )))
    })?;
...
mod tests {
    ...
    fn sandbox_detection_ignores_network_policy_text_in_non_sandbox_mode() {
        let output = make_exec_output(
            ...
            "",
            "",
            r#"HELIOS_NETWORK_POLICY_DECISION {"decision":"ask","reason":"not_allowed","source":"decider","protocol":"http","host":"google.com","port":80}"#,
        );
        assert!(!is_likely_sandbox_denied(SandboxType::None, &output));
    }
    ...
    fn sandbox_detection_ignores_network_policy_text_with_zero_exit_code() {
        let output = make_exec_output(
            ...
            "",
            "",
            r#"HELIOS_NETWORK_POLICY_DECISION {"decision":"ask","source":"decider","protocol":"http","host":"google.com","port":80}"#,
        );
