...
use helios_execpolicy::AmendError;
use helios_execpolicy::Decision;
use helios_execpolicy::Error as ExecPolicyRuleError;
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: helios_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(helios_home);
...
fn exec_policy_message_for_display(source: &helios_execpolicy::Error) -> String {
...
fn default_policy_path(helios_home: &Path) -> PathBuf {
    helios_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
mod tests {
    ...
    fn config_stack_for_dot_helios_folder(dot_helios_folder: &Path) -> ConfigLayerStack {
        let dot_helios_folder = AbsolutePathBuf::from_absolute_path(dot_helios_folder)
            .expect("absolute dot_helios_folder");
    ...
    async fn returns_empty_policy_when_no_policy_files_exist() {
        ...
        let config_stack = config_stack_for_dot_helios_folder(temp_dir.path());
    ...
    async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
        ...
        let project_dot_helios_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
        let layers = vec![ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_helios_folder: project_dot_helios_folder,
            },
            TomlValue::Table(Default::default()),
            "marked untrusted",
        )];
...
"#;
    ...
    async fn append_execpolicy_amendment_updates_policy_and_file() {
        let helios_home = tempdir().expect("create temp dir");
        let prefix = vec!["echo".to_string(), "hello".to_string()];
        let manager = ExecPolicyManager::default();

        manager
            .append_amendment_and_update(helios_home.path(), &ExecPolicyAmendment::from(prefix))
            .await
        ...
        let contents = fs::read_to_string(default_policy_path(helios_home.path()))
...
"#
    ...
    async fn append_execpolicy_amendment_rejects_empty_prefix() {
        ...
        let result = manager
            .append_amendment_and_update(helios_home.path(), &ExecPolicyAmendment::from(vec![]))
            .await;
