...
//!
//! The [`McpConnectionManager`] owns one [`helios_rmcp_client::RmcpClient`] per
...
use helios_protocol::approvals::ElicitationRequestEvent;
...
use helios_protocol::protocol::McpStartupCompleteEvent;
use helios_protocol::protocol::McpStartupFailure;
use helios_protocol::protocol::McpStartupStatus;
use helios_protocol::protocol::McpStartupUpdateEvent;
...
const HELIOS_APPS_TOOLS_CACHE_SCHEMA_VERSION: u8 = 1;
const HELIOS_APPS_TOOLS_CACHE_DIR: &str = "cache/helios_apps_tools";
...
pub(crate) fn helios_apps_tools_cache_key(
...
impl CodexAppsToolsCacheContext {
    fn cache_path(&self) -> PathBuf {
        ...
        self.helios_home
            .join(HELIOS_APPS_TOOLS_CACHE_DIR)
...
struct ManagedClient {
    ...
    helios_apps_tools_cache_context: Option<CodexAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.helios_apps_tools_cache_context.as_ref()
            && let CachedCodexAppsToolsLoad::Hit(tools) =
                load_cached_helios_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.helios_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    fn new(
        ...
        let startup_snapshot = load_startup_cached_helios_apps_tools_snapshot(
            &server_name,
            helios_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = async {
                ...
                match start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        helios_apps_tools_cache_context,
                    },
                )
...
pub struct SandboxState {
    ...
    pub helios_linux_sandbox_exe: Option<PathBuf>,
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        helios_home: PathBuf,
        helios_apps_tools_cache_key: CodexAppsToolsCacheKey,
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let helios_apps_tools_cache_context = if server_name == HELIOS_APPS_MCP_SERVER_NAME {
                Some(CodexAppsToolsCacheContext {
                    helios_home: helios_home.clone(),
                    user_key: helios_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
    ...
    pub async fn hard_refresh_helios_apps_tools_cache(&self) -> Result<()> {
        let managed_client = self
            .clients
            .get(HELIOS_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{HELIOS_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
            .context("failed to get client")?;
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{HELIOS_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_helios_apps_tools_if_needed(
            HELIOS_APPS_MCP_SERVER_NAME,
            managed_client.helios_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
            MCP_TOOLS_LIST_DURATION_METRIC,
...
pub(crate) fn filter_helios_apps_mcp_tools_only(
...
pub(crate) fn filter_non_helios_apps_mcp_tools_only(
    mcp_tools: &HashMap<String, ToolInfo>,
) -> HashMap<String, ToolInfo> {
    mcp_tools
        .iter()
...
fn normalize_helios_apps_tool_title(
    server_name: &str,
    connector_name: Option<&str>,
    value: &str,
) -> String {
    if server_name != HELIOS_APPS_MCP_SERVER_NAME {
...
fn write_cached_helios_apps_tools_if_needed(
    server_name: &str,
    ...
    if let Some(cache_context) = cache_context {
        ...
        write_cached_helios_apps_tools(cache_context, tools);
        emit_duration(
...
fn load_startup_cached_helios_apps_tools_snapshot(
    server_name: &str,
    cache_context: Option<&CodexAppsToolsCacheContext>,
) -> Option<Vec<ToolInfo>> {
    if server_name != HELIOS_APPS_MCP_SERVER_NAME {
    ...
    match load_cached_helios_apps_tools(cache_context) {
...
#[cfg(test)]
fn read_cached_helios_apps_tools(
...
fn load_cached_helios_apps_tools(
    cache_context: &CodexAppsToolsCacheContext,
    ...
    if cache.schema_version != HELIOS_APPS_TOOLS_CACHE_SCHEMA_VERSION {
        return CachedCodexAppsToolsLoad::Invalid;
    }
    CachedCodexAppsToolsLoad::Hit(filter_disallowed_helios_apps_tools(cache.tools))
}

fn write_cached_helios_apps_tools(cache_context: &CodexAppsToolsCacheContext, tools: &[ToolInfo]) {
    let cache_path = cache_context.cache_path();
    ...
    let tools = filter_disallowed_helios_apps_tools(tools.to_vec());
    let Ok(bytes) = serde_json::to_vec_pretty(&CodexAppsToolsDiskCache {
        schema_version: HELIOS_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        tools,
    }) else {
        return;
    };
...
fn filter_disallowed_helios_apps_tools(tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
    tools
        .into_iter()
        .filter(|tool| {
            tool.connector_id
...
fn emit_duration(metric: &str, duration: Duration, tags: &[(&str, &str)]) {
    if let Some(metrics) = helios_otel::metrics::global() {
...
async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .map(|tool| {
            ...
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title =
                    normalize_helios_apps_tool_title(server_name, connector_name.as_deref(), title);
        ...
        .collect();
    if server_name == HELIOS_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_helios_apps_tools(tools));
    }
    Ok(tools)
}
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = HELIOS_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
...
mod tests {
    ...
    fn create_helios_apps_tools_cache_context(
        helios_home: PathBuf,
    ...
    #[test]
    fn helios_apps_tools_cache_is_overwritten_by_last_write() {
        let helios_home = tempdir().expect("tempdir");
        let cache_context = create_helios_apps_tools_cache_context(
            helios_home.path().to_path_buf(),
            Some("account-one"),
            Some("user-one"),
        );
        let tools_gateway_1 = vec![create_test_tool(HELIOS_APPS_MCP_SERVER_NAME, "one")];
        let tools_gateway_2 = vec![create_test_tool(HELIOS_APPS_MCP_SERVER_NAME, "two")];

        write_cached_helios_apps_tools(&cache_context, &tools_gateway_1);
        let cached_gateway_1 = read_cached_helios_apps_tools(&cache_context)
            .expect("cache entry exists for first write");
        assert_eq!(cached_gateway_1[0].tool_name, "one");

        write_cached_helios_apps_tools(&cache_context, &tools_gateway_2);
        let cached_gateway_2 = read_cached_helios_apps_tools(&cache_context)
            .expect("cache entry exists for second write");
        assert_eq!(cached_gateway_2[0].tool_name, "two");
    }

    #[test]
    fn helios_apps_tools_cache_is_scoped_per_user() {
        let helios_home = tempdir().expect("tempdir");
        let cache_context_user_1 = create_helios_apps_tools_cache_context(
            helios_home.path().to_path_buf(),
            Some("account-one"),
            Some("user-one"),
        );
        let cache_context_user_2 = create_helios_apps_tools_cache_context(
            helios_home.path().to_path_buf(),
            Some("account-two"),
            Some("user-two"),
        );
        let tools_user_1 = vec![create_test_tool(HELIOS_APPS_MCP_SERVER_NAME, "one")];
        let tools_user_2 = vec![create_test_tool(HELIOS_APPS_MCP_SERVER_NAME, "two")];

        write_cached_helios_apps_tools(&cache_context_user_1, &tools_user_1);
        write_cached_helios_apps_tools(&cache_context_user_2, &tools_user_2);

        let read_user_1 =
            read_cached_helios_apps_tools(&cache_context_user_1).expect("cache entry for user one");
        let read_user_2 =
            read_cached_helios_apps_tools(&cache_context_user_2).expect("cache entry for user two");
    ...
    #[test]
    fn helios_apps_tools_cache_filters_disallowed_connectors() {
        let helios_home = tempdir().expect("tempdir");
        let cache_context = create_helios_apps_tools_cache_context(
            helios_home.path().to_path_buf(),
            Some("account-one"),
        ...
        write_cached_helios_apps_tools(&cache_context, &tools);
        let cached =
            read_cached_helios_apps_tools(&cache_context).expect("cache entry exists for user");
    ...
    #[test]
    fn helios_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
        let helios_home = tempdir().expect("tempdir");
        let cache_context = create_helios_apps_tools_cache_context(
            helios_home.path().to_path_buf(),
            Some("account-one"),
        ...
        let bytes = serde_json::to_vec_pretty(&serde_json::json!({
            "schema_version": HELIOS_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
            "tools": [create_test_tool(HELIOS_APPS_MCP_SERVER_NAME, "one")],
        }))
        .expect("serialize");
        std::fs::write(cache_path, bytes).expect("write");

        assert!(read_cached_helios_apps_tools(&cache_context).is_none());
    }

    #[test]
    fn helios_apps_tools_cache_is_ignored_when_json_is_invalid() {
        let helios_home = tempdir().expect("tempdir");
        let cache_context = create_helios_apps_tools_cache_context(
            helios_home.path().to_path_buf(),
            Some("account-one"),
    ...
    #[test]
    fn startup_cached_helios_apps_tools_loads_from_disk_cache() {
        let helios_home = tempdir().expect("tempdir");
        let cache_context = create_helios_apps_tools_cache_context(
            helios_home.path().to_path_buf(),
            Some("account-one"),
        ...
        write_cached_helios_apps_tools(&cache_context, &cached_tools);
        ...
        assert_eq!(startup_tools[0].server_name, HELIOS_APPS_MCP_SERVER_NAME);
    ...
    async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
        ...
        manager.clients.insert(
            HELIOS_APPS_MCP_SERVER_NAME.to_string(),
            AsyncManagedClient {
        ...
        let tool = tools
            .get("mcp__helios_apps__calendar_create_event")
            .expect("tool from startup cache");
        assert_eq!(tool.server_name, HELIOS_APPS_MCP_SERVER_NAME);
    ...
    #[test]
    fn elicitation_capability_enabled_only_for_helios_apps() {
        let helios_apps_capability = elicitation_capability_for_server(HELIOS_APPS_MCP_SERVER_NAME);
        assert!(matches!(
            helios_apps_capability,
            Some(ElicitationCapability {
                form: Some(FormElicitationCapability {
                    schema_validation: None
                }),
