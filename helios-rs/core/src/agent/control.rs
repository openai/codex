...
impl AgentControl {
    ...
    pub(crate) async fn resume_agent_from_rollout(
        ...
        let notification_source = session_source.clone();
        let rollout_path =
            find_thread_path_by_id_str(config.helios_home.as_path(), &thread_id.to_string())
                .await?
...
mod tests {
    ...
    use helios_protocol::protocol::TurnAbortedEvent;
    ...
    async fn test_config_with_cli_overrides(
        ...
        let config = ConfigBuilder::default()
            .helios_home(home.path().to_path_buf())
