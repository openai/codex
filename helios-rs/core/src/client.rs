...
//! per-turn state such as the `x-helios-turn-state` token used for sticky routing.
//!
...
use helios_api::CompactClient as ApiCompactClient;
use helios_api::CompactionInput as ApiCompactionInput;
use helios_api::MemoriesClient as ApiMemoriesClient;
use helios_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use helios_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use helios_api::RawMemory as ApiRawMemory;
use helios_api::RequestTelemetry;
use helios_api::ReqwestTransport;
use helios_api::ResponseAppendWsRequest;
use helios_api::ResponseCreateWsRequest;
use helios_api::ResponsesApiRequest;
use helios_api::ResponsesClient as ApiResponsesClient;
use helios_api::ResponsesOptions as ApiResponsesOptions;
use helios_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use helios_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use helios_api::SseTelemetry;
use helios_api::TransportError;
use helios_api::WebsocketTelemetry;
use helios_api::build_conversation_headers;
use helios_api::common::Reasoning;
use helios_api::common::ResponsesWsRequest;
use helios_api::create_text_param_for_request;
use helios_api::error::ApiError;
...
use crate::flags::HELIOS_RS_SSE_FIXTURE;
...
struct CurrentClientSetup {
    ...
    api_provider: helios_api::Provider,
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-helios-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-helios-turn-state` request header for
...
impl ModelClientSession {
    ...
    async fn stream_responses_api(
        ...
        if let Some(path) = &*HELIOS_RS_SSE_FIXTURE {
            warn!(path, "Streaming from fixture");
            let stream = helios_api::stream_from_fixture(
                path,
...
fn build_ws_client_metadata(turn_metadata_header: Option<&str>) -> Option<HashMap<String, String>> {
    ...
    client_metadata.insert(X_HELIOS_TURN_METADATA_HEADER.to_string(), turn_metadata);
...
/// - `x-helios-turn-metadata`: optional per-turn metadata for observability.
fn build_responses_headers(
    ...
    {
        headers.insert("x-helios-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_HELIOS_TURN_STATE_HEADER, header_value);
    }
    if let Some(header_value) = turn_metadata_header {
        headers.insert(X_HELIOS_TURN_METADATA_HEADER, header_value.clone());
    }
    headers
}
