...
pub use helios_app_server_protocol::AppBranding;
pub use helios_app_server_protocol::AppInfo;
pub use helios_app_server_protocol::AppMetadata;
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let mcp_servers = with_helios_apps_mcp(HashMap::new(), true, auth.as_ref(), config);
    ...
    if force_refetch
        && let Err(err) = mcp_connection_manager
            .hard_refresh_helios_apps_tools_cache()
            .await
    {
        warn!(
            "failed to force-refresh tools for MCP server '{HELIOS_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
        );
    }

    let mut tools = mcp_connection_manager.list_all_tools().await;
    let mut should_reload_tools = false;
    let helios_apps_ready = if let Some(cfg) = mcp_servers.get(HELIOS_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(HELIOS_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(HELIOS_APPS_MCP_SERVER_NAME, timeout)
                .await;
...
pub(crate) fn helios_app_tool_is_enabled(
    config: &Config,
    tool_info: &crate::mcp_connection_manager::ToolInfo,
) -> bool {
    if tool_info.server_name != HELIOS_APPS_MCP_SERVER_NAME {
        return true;
    }
...
pub(crate) fn filter_helios_apps_tools_by_policy(
    mut mcp_tools: HashMap<String, crate::mcp_connection_manager::ToolInfo>,
    config: &Config,
) -> HashMap<String, crate::mcp_connection_manager::ToolInfo> {
    mcp_tools.retain(|_, tool_info| helios_app_tool_is_enabled(config, tool_info));
    mcp_tools
}
