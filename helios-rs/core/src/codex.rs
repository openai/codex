...
use helios_hooks::HookResult;
use helios_hooks::Hooks;
use helios_hooks::HooksConfig;
...
use helios_protocol::dynamic_tools::DynamicToolSpec;
use helios_protocol::items::PlanItem;
...
use helios_protocol::protocol::HasLegacyEvent;
use helios_protocol::protocol::ItemCompletedEvent;
use helios_protocol::protocol::ItemStartedEvent;
use helios_protocol::protocol::RawResponseItemEvent;
use helios_protocol::protocol::ReviewRequest;
...
use helios_protocol::protocol::TurnContextItem;
use helios_protocol::protocol::TurnContextNetworkItem;
use helios_protocol::protocol::TurnStartedEvent;
...
use crate::mcp::HELIOS_APPS_MCP_SERVER_NAME;
...
use helios_otel::TelemetryAuthMode;
...
use helios_protocol::models::DeveloperInstructions;
...
const SUBMISSION_CHANNEL_CAPACITY_ENV_VAR: &str = "HELIOS_SUBMISSION_CHANNEL_CAPACITY";
...
impl Codex {
    ...
    pub(crate) async fn spawn(
            ...
            && config.features.enabled(Feature::Sqlite)
        {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "helios_spawn")
                        .await
                }
                None => None,
            }
        ...
        let session_configuration = SessionConfiguration {
            ...
            helios_home: config.helios_home.clone(),
            thread_name: None,
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.helios_home)
        })?;
...
pub(crate) struct TurnContext {
    ...
    pub(crate) helios_linux_sandbox_exe: Option<PathBuf>,
...
impl TurnContext {
    ...
    pub(crate) async fn with_model(&self, model: String, models_manager: &ModelsManager) -> Self {
        ...
        Self {
            ...
            helios_linux_sandbox_exe: self.helios_linux_sandbox_exe.clone(),
...
impl SessionConfiguration {
    pub(crate) fn helios_home(&self) -> &PathBuf {
        &self.helios_home
    }
...
impl Session {
    /// Builds the `x-helios-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        network_policy_decider: Option<Arc<dyn helios_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn helios_network_proxy::BlockedRequestObserver>>,
    ...
    pub(crate) async fn helios_home(&self) -> PathBuf {
        let state = self.state.lock().await;
        state.session_configuration.helios_home().clone()
    }
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            helios_linux_sandbox_exe: per_turn_config.helios_linux_sandbox_exe.clone(),
            tool_call_gate: Arc::new(ReadinessFlag::new()),
    ...
    async fn new(
        ...
        let zsh_exec_bridge =
            ZshExecBridge::new(config.zsh_path.clone(), config.helios_home.clone());
        zsh_exec_bridge
        ...
        let sandbox_state = SandboxState {
            sandbox_policy: session_configuration.sandbox_policy.get().clone(),
            helios_linux_sandbox_exe: config.helios_linux_sandbox_exe.clone(),
        ...
        let (mcp_connection_manager, cancel_token) = McpConnectionManager::new(
            ...
            sandbox_state,
            config.helios_home.clone(),
            helios_apps_tools_cache_key(auth),
        )
        .await;
        {
    ...
    pub(crate) async fn update_settings(
        ...
        match state.session_configuration.apply(&updates) {
            Ok(updated) => {
                ...
                let helios_home = updated.helios_home.clone();
                state.session_configuration = updated;
                drop(state);

                self.maybe_refresh_shell_snapshot_for_cwd(&previous_cwd, &next_cwd, &helios_home);
    ...
    pub(crate) async fn new_turn_with_sub_id(
        ...
        let (session_configuration, sandbox_policy_changed, previous_cwd, helios_home) = {
            let mut state = self.state.lock().await;
            match state.session_configuration.clone().apply(&updates) {
                Ok(next) => {
                    ...
                    let helios_home = next.helios_home.clone();
                    state.session_configuration = next.clone();
                    (next, sandbox_policy_changed, previous_cwd, helios_home)
                }
                Err(err) => {
                    drop(state);
                    self.send_event_raw(Event {
                        ...
                        msg: EventMsg::Error(ErrorEvent {
                            ...
                            helios_error_info: Some(CodexErrorInfo::BadRequest),
                        }),
                    })
                    .await;
                    return Err(err);
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&helios_home, amendment)
            .await?;
    ...
    pub(crate) async fn build_initial_context(
        ...
        if let Some(memory_prompt) =
            build_memory_tool_developer_instructions(&turn_context.config.helios_home).await
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        helios_error: CodexErr,
    ) {
        let additional_details = helios_error.to_string();
        let helios_error_info = CodexErrorInfo::ResponseStreamDisconnected {
            http_status_code: helios_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            helios_error_info: Some(helios_error_info),
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let mcp_servers = with_helios_apps_mcp(
            mcp_servers,
        ...
        let sandbox_state = SandboxState {
            ...
            helios_linux_sandbox_exe: turn_context.helios_linux_sandbox_exe.clone(),
        ...
        let (refreshed_manager, cancel_token) = McpConnectionManager::new(
            ...
            sandbox_state,
            config.helios_home.clone(),
            helios_apps_tools_cache_key(auth.as_ref()),
        )
        .await;
        {
            let mut cancel_guard = self.services.mcp_startup_cancellation_token.lock().await;
            ...
            Op::RealtimeConversationStart(params) => {
                    ...
                    handle_realtime_conversation_start(&sess, sub.id.clone(), params).await
                {
                    sess.send_event_raw(Event {
                        ...
                        msg: EventMsg::Error(ErrorEvent {
                            message: err.to_string(),
                            helios_error_info: Some(CodexErrorInfo::Other),
                        }),
                    })
                    .await;
                }
...
mod handlers {
    ...
    use helios_protocol::custom_prompts::CustomPrompt;
    use helios_protocol::protocol::CodexErrorInfo;
    use helios_protocol::protocol::ErrorEvent;
    use helios_protocol::protocol::Event;
    use helios_protocol::protocol::EventMsg;
    use helios_protocol::protocol::ListCustomPromptsResponseEvent;
    use helios_protocol::protocol::ListRemoteSkillsResponseEvent;
    use helios_protocol::protocol::ListSkillsResponseEvent;
    use helios_protocol::protocol::McpServerRefreshConfig;
    use helios_protocol::protocol::Op;
    use helios_protocol::protocol::RemoteSkillDownloadedEvent;
    use helios_protocol::protocol::RemoteSkillHazelnutScope;
    use helios_protocol::protocol::RemoteSkillProductSurface;
    use helios_protocol::protocol::RemoteSkillSummary;
    use helios_protocol::protocol::ReviewDecision;
    use helios_protocol::protocol::ReviewRequest;
    use helios_protocol::protocol::SkillsListEntry;
    use helios_protocol::protocol::ThreadNameUpdatedEvent;
    use helios_protocol::protocol::ThreadRolledBackEvent;
    use helios_protocol::protocol::TurnAbortReason;
    ...
    use helios_protocol::config_types::Settings;
    use helios_protocol::dynamic_tools::DynamicToolResponse;
    use helios_protocol::mcp::RequestId as ProtocolRequestId;
    use helios_protocol::user_input::UserInput;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: helios_protocol::approvals::ElicitationAction,
    ) {
        let action = match decision {
            helios_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            helios_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            helios_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| helios_protocol::message_history::HistoryEntry {
    ...
    pub async fn drop_memories(sess: &Arc<Session>, config: &Arc<Config>, sub_id: String) {
        ...
        let memory_root = crate::memories::memory_root(&config.helios_home);
        ...
        if num_turns == 0 {
            sess.send_event_raw(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    helios_error_info: Some(CodexErrorInfo::ThreadRollbackFailed),
                }),
            })
            .await;
            return;
    ...
    ///
    /// This appends the name to `HELIOS_HOME/sessions_index.jsonl` via `session_index::append_thread_name` for the
    ...
    pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
        ...
        let helios_home = sess.helios_home().await;
        if let Err(e) =
            session_index::append_thread_name(&helios_home, sess.conversation_id, &name).await
        {
            let event = Event {
                id: sub_id,
                msg: EventMsg::Error(ErrorEvent {
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        helios_linux_sandbox_exe: parent_turn_context.helios_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
...
fn filter_helios_apps_mcp_tools(
    ...
    mcp_tools
        .iter()
        .filter(|(_, tool)| {
            if tool.server_name != HELIOS_APPS_MCP_SERVER_NAME {
                return true;
            }
            let Some(connector_id) = helios_apps_connector_id(tool) else {
                return false;
            };
            allowed.contains(connector_id) && connectors::helios_app_tool_is_enabled(config, tool)
        })
        .map(|(name, tool)| (name.clone(), tool.clone()))
        .collect()
}

fn helios_apps_connector_id(tool: &crate::mcp_connection_manager::ToolInfo) -> Option<&str> {
...
async fn built_tools(
    ...
    let app_tools = connectors.as_ref().map(|connectors| {
        filter_helios_apps_mcp_tools(&mcp_tools, connectors, &turn_context.config)
    });
    ...
    if let Some(connectors) = connectors.as_ref() {
        ...
        let mut selected_mcp_tools = filter_non_helios_apps_mcp_tools_only(&mcp_tools);
        ...
        selected_mcp_tools.extend(filter_helios_apps_mcp_tools_only(
            &mcp_tools,
...
fn agent_message_text(item: &helios_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            helios_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: helios_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(helios_protocol::items::AgentMessageItem {
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(helios_async_utils::CancelErr::Cancelled) => break Err(CodexErr::TurnAborted),
        };
...
mod tests {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use helios_app_server_protocol::AppInfo;
    use helios_otel::TelemetryAuthMode;
    use helios_protocol::models::BaseInstructions;
    use helios_protocol::models::ContentItem;
    use helios_protocol::models::ResponseInputItem;
    ...
    use helios_protocol::mcp::CallToolResult as McpCallToolResult;
    ...
    async fn reload_user_config_layer_updates_effective_apps_config() {
        let (session, _turn_context) = make_session_and_context().await;
        let helios_home = session.helios_home().await;
        std::fs::create_dir_all(&helios_home).expect("create codex home");
        let config_toml_path = helios_home.join(CONFIG_TOML_FILE);
        std::fs::write(
            &config_toml_path,
    ...
    fn non_app_mcp_tools_remain_visible_without_search_selection() {
        let mcp_tools = HashMap::from([
            (
                "mcp__helios_apps__calendar_create_event".to_string(),
                make_mcp_tool(
                    HELIOS_APPS_MCP_SERVER_NAME,
        ...
        let mut selected_mcp_tools = mcp_tools
            .iter()
            .filter(|(_, tool)| tool.server_name != HELIOS_APPS_MCP_SERVER_NAME)
            .map(|(name, tool)| (name.clone(), tool.clone()))
            .collect::<HashMap<_, _>>();
        ...
        let apps_mcp_tools = filter_helios_apps_mcp_tools_only(&mcp_tools, &connectors);
    ...
    #[test]
    fn search_tool_selection_keeps_helios_apps_tools_without_mentions() {
    ...
    #[test]
    fn apps_mentions_add_helios_apps_tools_to_search_selected_set() {
    ...
    fn extract_mcp_tool_selection_from_rollout_reads_search_tool_output() {
        let rollout_items = vec![
            ...
            function_call_output_rollout_item(
                ...
                &json!({
                    "active_selected_tools": [
                        "mcp__helios_apps__calendar_create_event",
                        "mcp__helios_apps__calendar_list_events",
                    ],
                })
                .to_string(),
            ),
        ...
        assert_eq!(
            ...
            Some(vec![
                "mcp__helios_apps__calendar_create_event".to_string(),
                "mcp__helios_apps__calendar_list_events".to_string(),
            ])
        );
    }
    ...
    fn extract_mcp_tool_selection_from_rollout_latest_valid_payload_wins() {
        let rollout_items = vec![
            ...
            function_call_output_rollout_item(
                "search-1",
                &json!({
                    "active_selected_tools": ["mcp__helios_apps__calendar_create_event"],
                })
                .to_string(),
            ),
            ...
            function_call_output_rollout_item(
                "search-2",
                &json!({
                    "active_selected_tools": ["mcp__helios_apps__calendar_delete_event"],
                })
                .to_string(),
            ),
        ];
        ...
        assert_eq!(
            selected,
            Some(vec!["mcp__helios_apps__calendar_delete_event".to_string(),])
        );
    }
    ...
    fn extract_mcp_tool_selection_from_rollout_ignores_non_search_and_malformed_payloads() {
        let rollout_items = vec![
            ...
            function_call_output_rollout_item(
                "shell-1",
                &json!({
                    "active_selected_tools": ["mcp__helios_apps__should_be_ignored"],
                })
                .to_string(),
            ),
            function_call_rollout_item(SEARCH_TOOL_BM25_TOOL_NAME, "search-1"),
            ...
            function_call_output_rollout_item(
                "unknown-search-call",
                &json!({
                    "active_selected_tools": ["mcp__helios_apps__also_ignored"],
                })
                .to_string(),
            ),
            function_call_output_rollout_item(
                "search-1",
                &json!({
                    "active_selected_tools": ["mcp__helios_apps__calendar_list_events"],
                })
                .to_string(),
            ),
        ];
        ...
        assert_eq!(
            selected,
            Some(vec!["mcp__helios_apps__calendar_list_events".to_string(),])
        );
    }
    ...
    async fn record_initial_history_resumed_hydrates_previous_model_from_lifecycle_turn_with_missing_turn_context_id()
     {
        ...
        let rollout_items = vec![
            RolloutItem::EventMsg(EventMsg::TurnStarted(
                helios_protocol::protocol::TurnStartedEvent {
            ...
            RolloutItem::EventMsg(EventMsg::UserMessage(
                helios_protocol::protocol::UserMessageEvent {
            ...
            RolloutItem::EventMsg(EventMsg::TurnComplete(
                helios_protocol::protocol::TurnCompleteEvent {
                    turn_id,
    ...
    async fn record_initial_history_resumed_rollback_skips_only_user_turns() {
        ...
        let rollout_items = vec![
            ...
            RolloutItem::EventMsg(EventMsg::ThreadRolledBack(
                helios_protocol::protocol::ThreadRolledBackEvent { num_turns: 1 },
            )),
        ];
    ...
    async fn set_rate_limits_retains_previous_credits() {
        let helios_home = tempfile::tempdir().expect("create temp dir");
        let config = build_test_config(helios_home.path()).await;
        let config = Arc::new(config);
        ...
        let initial = RateLimitSnapshot {
            ...
            plan_type: Some(helios_protocol::account::PlanType::Plus),
        };
        state.set_rate_limits(initial.clone());

        let update = RateLimitSnapshot {
            limit_id: Some("helios_other".to_string()),
            limit_name: Some("helios_other".to_string()),
            primary: Some(RateLimitWindow {
                used_percent: 40.0,
    ...
    async fn set_rate_limits_updates_plan_type_when_present() {
        ...
        let update = RateLimitSnapshot {
            ...
            secondary: None,
            credits: None,
            plan_type: Some(helios_protocol::account::PlanType::Pro),
        };
        state.set_rate_limits(update.clone());
        ...
        loop {
            ...
            match evt.msg {
                EventMsg::Error(payload)
                    if payload.helios_error_info == Some(CodexErrorInfo::ThreadRollbackFailed) =>
                {
                    return payload;
                }
                _ => continue,
    ...
    async fn build_test_config(helios_home: &Path) -> Config {
        ConfigBuilder::default()
            .helios_home(helios_home.to_path_buf())
            .build()
            .await
    ...
    async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
        let helios_home = tempfile::tempdir().expect("create temp dir");
        let mut config = build_test_config(helios_home.path()).await;
    ...
    pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let skills_manager = Arc::new(SkillsManager::new(config.helios_home.clone()));
        let network_approval = Arc::new(NetworkApprovalService::default());
