...
use helios_protocol::models::VIEW_IMAGE_TOOL_NAME;
use helios_protocol::openai_models::ApplyPatchToolType;
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams) -> Self {
        ...
        let shell_type = if !features.enabled(Feature::ShellTool) {
            ...
            if helios_utils_pty::conpty_supported() {
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        ...
        description: "Runs JavaScript in a persistent Node kernel with top-level await. This is a freeform tool: send raw JavaScript source text, optionally with a first-line pragma like `// helios-js-repl: timeout_ms=15000`; do not send JSON/quotes/markdown fences."
            .to_string(),
...
mod tests {
    ...
    #[test]
    fn test_full_toolset_specs_for_gpt5_helios_unified_exec_web_search() {
    ...
    fn js_repl_freeform_grammar_blocks_common_non_js_prefixes() {
        ...
        assert!(format.definition.contains("helios-js-repl:"));
        assert!(!format.definition.contains("(?!"));
    }
    ...
    #[test]
    fn test_build_specs_gpt5_helios_default() {
        let mut features = Features::with_defaults();
        features.enable(Feature::CollaborationModes);
    ...
    #[test]
    fn test_build_specs_gpt51_helios_default() {
        let mut features = Features::with_defaults();
        features.enable(Feature::CollaborationModes);
        assert_default_model_tools(
            "gpt-5.1-codex",
    ...
    #[test]
    fn test_build_specs_gpt5_helios_unified_exec_web_search() {
        let mut features = Features::with_defaults();
        features.enable(Feature::UnifiedExec);
        features.enable(Feature::CollaborationModes);
        assert_model_tools(
    ...
    #[test]
    fn test_build_specs_gpt51_helios_unified_exec_web_search() {
        let mut features = Features::with_defaults();
        features.enable(Feature::UnifiedExec);
        features.enable(Feature::CollaborationModes);
        assert_model_tools(
    ...
    #[test]
    fn test_gpt_5_1_helios_max_defaults() {
        let mut features = Features::with_defaults();
        features.enable(Feature::CollaborationModes);
        assert_default_model_tools(
            "gpt-5.1-helios-max",
    ...
    #[test]
    fn test_helios_5_1_mini_defaults() {
        let mut features = Features::with_defaults();
        features.enable(Feature::CollaborationModes);
        assert_default_model_tools(
            "gpt-5.1-helios-mini",
    ...
    #[test]
    fn test_gpt_5_1_helios_max_unified_exec_web_search() {
        let mut features = Features::with_defaults();
        features.enable(Feature::UnifiedExec);
        features.enable(Feature::CollaborationModes);
        assert_model_tools(
    ...
    #[test]
    fn search_tool_description_includes_only_helios_apps_connector_names() {
        let config = test_config();
        let model_info =
            ModelsManager::construct_model_info_offline_for_tests("gpt-5-codex", &config);
        let mut features = Features::with_defaults();
        ...
        let (tools, _) = build_specs(
            ...
            Some(HashMap::from([
                (
                    "mcp__helios_apps__calendar_create_event".to_string(),
                    ToolInfo {
                        server_name: crate::mcp::HELIOS_APPS_MCP_SERVER_NAME.to_string(),
