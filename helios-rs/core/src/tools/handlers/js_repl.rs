...
fn parse_freeform_args(input: &str) -> Result<JsReplArgs, FunctionCallError> {
    if input.trim().is_empty() {
        return Err(FunctionCallError::RespondToModel(
            "js_repl expects raw JavaScript tool input (non-empty). Provide JS source text, optionally with first-line `// helios-js-repl: ...`."
                .to_string(),
        ));
    }
...
fn reject_json_or_quoted_source(code: &str) -> Result<(), FunctionCallError> {
    ...
    if trimmed.starts_with("```") {
        return Err(FunctionCallError::RespondToModel(
            "js_repl expects raw JavaScript source, not markdown code fences. Resend plain JS only (optional first line `// helios-js-repl: ...`)."
                .to_string(),
        ));
    }
    ...
    match value {
        JsonValue::Object(_) | JsonValue::String(_) => Err(FunctionCallError::RespondToModel(
            "js_repl is a freeform tool and expects raw JavaScript source. Resend plain JS only (optional first line `// helios-js-repl: ...`); do not send JSON (`{\"code\":...}`), quoted code, or markdown fences."
                .to_string(),
        )),
        _ => Ok(()),
    }
...
mod tests {
    ...
    fn parse_freeform_args_with_pragma() {
        let input = "// helios-js-repl: timeout_ms=15000\nconsole.log('ok');";
    ...
    fn parse_freeform_args_rejects_unknown_key() {
        let err = parse_freeform_args("// helios-js-repl: nope=1\nconsole.log('ok');")
    ...
    fn parse_freeform_args_rejects_reset_key() {
        let err = parse_freeform_args("// helios-js-repl: reset=true\nconsole.log('ok');")
            .expect_err("expected error");
        assert_eq!(
            err.to_string(),
