...
impl ToolHandler for SearchToolBm25Handler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        let mcp_tools = filter_helios_apps_mcp_tools(mcp_tools, &connectors);
        let mcp_tools = connectors::filter_helios_apps_tools_by_policy(mcp_tools, &turn.config);
...
mod tests {
    ...
    #[test]
    fn filter_helios_apps_mcp_tools_keeps_enabled_apps_only() {
        let mcp_tools = HashMap::from([
            ...
            make_tool(
                "mcp__helios_apps__drive_search",
                HELIOS_APPS_MCP_SERVER_NAME,
                "drive_search",
                Some("drive"),
            ),
        ...
        let mut filtered: Vec<String> = filter_helios_apps_mcp_tools(mcp_tools, &connectors)
            .into_keys()
            .collect();
        filtered.sort();

        assert_eq!(filtered, vec!["mcp__helios_apps__drive_search".to_string()]);
    }

    #[test]
    fn filter_helios_apps_mcp_tools_drops_apps_without_connector_id() {
        let mcp_tools = HashMap::from([
            make_tool(
                "mcp__helios_apps__unknown",
                HELIOS_APPS_MCP_SERVER_NAME,
                "unknown",
                None,
            ),
        ...
        let mut filtered: Vec<String> =
            filter_helios_apps_mcp_tools(mcp_tools, &[make_connector("calendar", true)])
                .into_keys()
                .collect();
        filtered.sort();
