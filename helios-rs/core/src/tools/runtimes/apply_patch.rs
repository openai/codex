...
//! `codex --helios-run-as-apply-patch`, and runs under the current
...
impl ApplyPatchRuntime {
    ...
    fn build_command_spec(req: &ApplyPatchRequest) -> Result<CommandSpec, ToolError> {
        use std::env;
        let exe = if let Some(path) = &req.helios_exe {
            path.clone()
        } else {
            env::current_exe()
        ...
        Ok(CommandSpec {
            program,
            args: vec![
                HELIOS_CORE_APPLY_PATCH_ARG1.to_string(),
