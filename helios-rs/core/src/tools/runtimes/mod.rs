...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    let captures = keys
        ...
        .map(|(idx, key)| {
            format!(
                "__HELIOS_SNAPSHOT_OVERRIDE_SET_{idx}=\"${{{key}+x}}\"\n__HELIOS_SNAPSHOT_OVERRIDE_{idx}=\"${{{key}-}}\""
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    let restores = keys
        ...
        .map(|(idx, key)| {
            format!(
                "if [ -n \"${{__HELIOS_SNAPSHOT_OVERRIDE_SET_{idx}}}\" ]; then export {key}=\"${{__HELIOS_SNAPSHOT_OVERRIDE_{idx}}}\"; else unset {key}; fi"
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
...
mod tests {
    ...
    fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
        ...
        std::fs::write(
            &snapshot_path,
            "# Snapshot file\nexport HELIOS_TEST_UNSET_OVERRIDE='snapshot-value'\n",
        )
        .expect("write snapshot");
        let session_shell = shell_with_snapshot(
            ShellType::Bash,
        ...
        let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${HELIOS_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$HELIOS_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
        let explicit_env_overrides = HashMap::from([(
            "HELIOS_TEST_UNSET_OVERRIDE".to_string(),
        ...
        let output = Command::new(&rewritten[0])
            .args(&rewritten[1..])
            .env_remove("HELIOS_TEST_UNSET_OVERRIDE")
            .output()
            .expect("run rewritten command");
        assert!(output.status.success(), "command failed: {output:?}");
