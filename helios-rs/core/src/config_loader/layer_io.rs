...
use helios_config::config_error_from_toml;
use helios_config::io_error_from_config_error;
...
#[cfg(unix)]
const HELIOS_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/codex/managed_config.toml";
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(helios_home)),
    )?;
...
pub(super) fn managed_config_default_path(helios_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = helios_home;
        PathBuf::from(HELIOS_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        helios_home.join("managed_config.toml")
    }
}
