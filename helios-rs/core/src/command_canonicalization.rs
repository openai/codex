...
const CANONICAL_BASH_SCRIPT_PREFIX: &str = "__helios_shell_script__";
const CANONICAL_POWERSHELL_SCRIPT_PREFIX: &str = "__helios_powershell_script__";
...
mod tests {
    ...
    fn canonicalizes_word_only_shell_scripts_to_inner_command() {
        let command_a = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "cargo test -p helios-core".to_string(),
        ];
        let command_b = vec![
            "bash".to_string(),
            "-lc".to_string(),
    ...
    fn canonicalizes_heredoc_scripts_to_stable_script_key() {
        ...
        assert_eq!(
            canonicalize_command_for_approval(&command_a),
            vec![
                "__helios_shell_script__".to_string(),
                "-lc".to_string(),
                script.to_string(),
            ]
        );
    ...
    fn canonicalizes_powershell_wrappers_to_stable_script_key() {
        ...
        assert_eq!(
            canonicalize_command_for_approval(&command_a),
            vec![
                "__helios_powershell_script__".to_string(),
                script.to_string(),
            ]
        );
        assert_eq!(
