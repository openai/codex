...
fn memory_root_uses_shared_global_path() {
    ...
    let helios_home = dir.path().join("codex");
    assert_eq!(memory_root(&helios_home), helios_home.join("memories"));
}
...
keywords: helios-state, migrations
---
### Task 1: migration-test
task: add-migration-test
task_group: helios-state
...
- Confirmed no ordering regressions."
    ...
    assert!(raw_memories.contains("task_group: helios-state"));
...
mod phase2 {
    ...
    use helios_state::Phase2JobClaimOutcome;
    use helios_state::Stage1Output;
    use helios_state::ThreadMetadataBuilder;
    use std::path::PathBuf;
    use std::sync::Arc;
    use tempfile::TempDir;
    ...
    struct DispatchHarness {
        _helios_home: TempDir,
        config: Arc<Config>,
        session: Arc<Session>,
        manager: ThreadManager,
        state_db: Arc<helios_state::StateRuntime>,
    }

    impl DispatchHarness {
        async fn new() -> Self {
            let helios_home = tempfile::tempdir().expect("create temp codex home");
            let mut config = test_config();
            config.helios_home = helios_home.path().to_path_buf();
            config.cwd = config.helios_home.clone();
            let config = Arc::new(config);

            let state_db = helios_state::StateRuntime::init(
        ...
        async fn seed_stage1_output(&self, source_updated_at: i64) {
            ...
            let ownership_token = match claim {
                helios_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
        ...
        match config_snapshot.sandbox_policy {
            SandboxPolicy::WorkspaceWrite { writable_roots, .. } => {
                assert!(
                    writable_roots
                        .iter()
                        .any(|root| root.as_path() == harness.config.helios_home.as_path()),
                    "consolidation subagent should have helios_home as writable root"
                );
            }
    ...
    async fn dispatch_with_empty_stage1_outputs_rebuilds_local_artifacts() {
        let harness = DispatchHarness::new().await;
        let root = memory_root(&harness.config.helios_home);
