...
use helios_secrets::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<helios_state::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            ...
            helios_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    session: &Arc<Session>,
    claimed_candidates: Vec<helios_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(in crate::memories) async fn run(
        session: &Session,
        claim: helios_state::Stage1JobClaim,
        stage_one_context: &RequestContext,
    ) -> JobResult {
