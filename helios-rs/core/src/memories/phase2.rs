...
pub(super) async fn run(session: &Arc<Session>, config: Arc<Config>) {
    ...
    let root = memory_root(&config.helios_home);
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            helios_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
...
mod agent {
    ...
    pub(super) fn get_config(config: Arc<Config>) -> Option<Config> {
        ...
        match AbsolutePathBuf::from_absolute_path(agent_config.helios_home.clone()) {
            Ok(helios_home) => writable_roots.push(helios_home),
            Err(err) => warn!(
                "memory phase-2 consolidation could not add helios_home writable root {}: {err}",
                agent_config.helios_home.display()
            ),
        }
        // The consolidation agent only needs local helios_home write access and no network.
    ...
    pub(super) fn handle(
        ...
        new_watermark: i64,
        thread_id: ThreadId,
        phase_two_e2e_timer: Option<helios_otel::Timer>,
    ) {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[helios_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
