...
pub enum CodexErr {
    ...
    #[error("helios-linux-sandbox was required but not provided")]
...
impl CodexErr {
    ...
    pub fn to_helios_protocol_error(&self) -> CodexErrorInfo {
        match self {
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            helios_error_info: Some(self.to_helios_protocol_error()),
        }
    }
...
mod tests {
    ...
    use chrono::TimeZone;
    use chrono::Utc;
    use helios_protocol::protocol::RateLimitWindow;
    ...
    #[test]
    fn usage_limit_reached_error_hides_upsell_for_non_helios_limit_name() {
        let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
        let resets_at = base + ChronoDuration::hours(1);
        with_now_override(base, move || {
            let expected_time = format_retry_timestamp(&resets_at);
            ...
            let expected = format!(
                "You've hit your usage limit for helios_other. Switch to another model now, or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
