...
use helios_protocol::request_user_input::RequestUserInputQuestionOption;
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&helios_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&helios_home)
