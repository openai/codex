...
pub(crate) const HELIOS_APPS_MCP_SERVER_NAME: &str = "helios_apps";
const HELIOS_CONNECTORS_TOKEN_ENV_VAR: &str = "HELIOS_CONNECTORS_TOKEN";
...
fn helios_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(HELIOS_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(HELIOS_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(HELIOS_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn helios_apps_mcp_bearer_token(auth: Option<&CodexAuth>) -> Option<String> {
...
fn helios_apps_mcp_http_headers(auth: Option<&CodexAuth>) -> Option<HashMap<String, String>> {
    let mut headers = HashMap::new();
    if let Some(token) = helios_apps_mcp_bearer_token(auth) {
...
fn selected_config_helios_apps_mcp_gateway(config: &Config) -> CodexAppsMcpGateway {
...
fn normalize_helios_apps_base_url(base_url: &str) -> String {
...
fn helios_apps_mcp_url_for_gateway(base_url: &str, gateway: CodexAppsMcpGateway) -> String {
    if gateway == CodexAppsMcpGateway::MCPGateway {
        return format!("{OPENAI_CONNECTORS_MCP_BASE_URL}{OPENAI_CONNECTORS_MCP_PATH}");
    }

    let base_url = normalize_helios_apps_base_url(base_url);
...
pub(crate) fn helios_apps_mcp_url(config: &Config) -> String {
    helios_apps_mcp_url_for_gateway(
        &config.chatgpt_base_url,
        selected_config_helios_apps_mcp_gateway(config),
    )
}

fn helios_apps_mcp_server_config(config: &Config, auth: Option<&CodexAuth>) -> McpServerConfig {
    let bearer_token_env_var = helios_apps_mcp_bearer_token_env_var();
    let http_headers = if bearer_token_env_var.is_some() {
        None
    } else {
        helios_apps_mcp_http_headers(auth)
    };
    let url = helios_apps_mcp_url(config);
...
pub(crate) fn with_helios_apps_mcp(
    ...
    if connectors_enabled {
        servers.insert(
            ...
            helios_apps_mcp_server_config(config, auth),
        );
    } else {
        servers.remove(HELIOS_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
mod tests {
    ...
    #[test]
    fn helios_apps_mcp_url_for_default_gateway_keeps_existing_paths() {
        assert_eq!(
            helios_apps_mcp_url_for_gateway(
    ...
    #[test]
    fn helios_apps_mcp_url_for_gateway_uses_openai_connectors_gateway() {
    ...
    #[test]
    fn helios_apps_mcp_url_uses_default_gateway_when_feature_is_disabled() {
        let mut config = crate::config::test_config();
        config.chatgpt_base_url = "https://chatgpt.com".to_string();

        assert_eq!(
            helios_apps_mcp_url(&config),
            "https://chatgpt.com/backend-api/wham/apps"
        );
    }

    #[test]
    fn helios_apps_mcp_url_uses_openai_connectors_gateway_when_feature_is_enabled() {
        let mut config = crate::config::test_config();
        config.chatgpt_base_url = "https://chatgpt.com".to_string();
    ...
    #[test]
    fn helios_apps_server_config_switches_gateway_with_flags() {
        let mut config = crate::config::test_config();
        config.chatgpt_base_url = "https://chatgpt.com".to_string();

        let mut servers = with_helios_apps_mcp(HashMap::new(), false, None, &config);
        assert!(!servers.contains_key(HELIOS_APPS_MCP_SERVER_NAME));

        config.features.enable(Feature::Apps);

        servers = with_helios_apps_mcp(servers, true, None, &config);
        let server = servers
