...
use helios_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(helios_home: &Path) -> bool {
    helios_windows_sandbox::sandbox_setup_is_complete(helios_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_helios_home: &Path) -> bool {
    false
}

#[cfg(target_os = "windows")]
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = helios_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = helios_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if helios_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            helios_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
...
pub fn run_elevated_setup(
    ...
    helios_windows_sandbox::run_elevated_setup(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
pub fn run_legacy_setup_preflight(
    ...
    helios_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        helios_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    helios_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let helios_home = request.helios_home;
    let active_profile = request.active_profile;
    let setup_helios_home = helios_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_helios_home.as_path()) {
                    run_elevated_setup(
                        &policy,
                        policy_cwd.as_path(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_helios_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(helios_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let Some(metrics) = helios_otel::metrics::global() else {
        return;
    };
