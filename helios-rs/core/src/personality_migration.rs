...
pub async fn maybe_migrate_personality(
    ...
    let marker_path = helios_home.join(PERSONALITY_MIGRATION_FILENAME);
    ...
    if !has_recorded_sessions(helios_home, model_provider_id.as_str()).await? {
        create_marker(&marker_path).await?;
...
async fn has_recorded_sessions(helios_home: &Path, default_provider: &str) -> io::Result<bool> {
    let allowed_sources: &[SessionSource] = &[];

    if let Some(state_db_ctx) = state_db::open_if_present(helios_home, default_provider).await
    ...
    let sessions = get_threads_in_root(
        helios_home.join(SESSIONS_SUBDIR),
        1,
        None,
        ThreadSortKey::CreatedAt,
        ThreadListConfig {
    ...
    let archived_sessions = get_threads_in_root(
        helios_home.join(ARCHIVED_SESSIONS_SUBDIR),
        1,
        None,
        ThreadSortKey::CreatedAt,
        ThreadListConfig {
