...
use crate::spawn::HELIOS_SANDBOX_ENV_VAR;
use helios_client::CodexHttpClient;
pub use helios_client::CodexRequestBuilder;
...
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-helios-residency";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(HELIOS_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(HELIOS_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "helios_atlas" || originator_value == "helios_chatgpt_desktop"
}

pub fn get_helios_user_agent() -> String {
...
fn is_sandboxed() -> bool {
    std::env::var(HELIOS_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    fn is_first_party_chat_originator_matches_known_values() {
        assert_eq!(is_first_party_chat_originator("helios_atlas"), true);
        assert_eq!(
            is_first_party_chat_originator("helios_chatgpt_desktop"),
            true
        );
        assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
        assert_eq!(is_first_party_chat_originator("helios_vscode"), false);
    }
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "helios_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "helios_cli_rs/0.0.0 (bad_suffix)"
        );
    }
