...
use helios_core::exec::ExecToolCallOutput;
use helios_core::exec::SandboxType;
use helios_core::exec::process_exec_tool_call;
use helios_core::sandboxing::SandboxPermissions;
use helios_core::spawn::HELIOS_SANDBOX_ENV_VAR;
...
use helios_core::get_platform_sandbox;

fn skip_test() -> bool {
    if std::env::var(HELIOS_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{HELIOS_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
