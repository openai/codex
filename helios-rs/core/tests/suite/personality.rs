use helios_core::config::types::Personality;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    ...
    let model_info = helios_core::test_support::construct_model_info_offline("gpt-5.1", &config);
    assert_eq!(
...
async fn base_instructions_override_disables_personality_template() {
    ...
    let model_info =
        helios_core::test_support::construct_model_info_offline("gpt-5.2-codex", &config);
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_codex()
        .with_model("exp-helios-personality")
        .with_config(|config| {
            config.features.enable(Feature::Personality);
        });
    let test = builder.build(&server).await?;
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "helios-remote-default-personality";
    ...
    let mut builder = test_codex()
        .with_auth(helios_core::CodexAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config.features.enable(Feature::Personality);
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "helios-remote-personality";
