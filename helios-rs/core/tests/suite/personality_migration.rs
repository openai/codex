...
use helios_core::SESSIONS_SUBDIR;
use helios_core::config::ConfigToml;
use helios_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use helios_core::personality_migration::PersonalityMigrationStatus;
use helios_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(helios_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(helios_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(helios_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(helios_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = helios_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(helios_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = helios_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
