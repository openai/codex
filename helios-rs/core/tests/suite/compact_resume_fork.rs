...
use helios_core::compact::SUMMARIZATION_PROMPT;
use helios_core::config::Config;
use helios_core::spawn::HELIOS_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
fn network_disabled() -> bool {
    std::env::var(HELIOS_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn resume_conversation(
    ...
    let auth_manager = helios_core::test_support::auth_manager_from_auth(
        helios_core::CodexAuth::from_api_key("dummy"),
    );
    manager
