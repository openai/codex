...
use helios_protocol::protocol::SessionMeta;
use helios_protocol::protocol::SessionMetaLine;
...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db_path = helios_state::state_db_path(test.config.helios_home.as_path());
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_pre_build_hook(move |helios_home| {
            let rollout_path = helios_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = helios_state::state_db_path(test.config.helios_home.as_path());
    let rollout_path = test.config.helios_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let subscriber = tracing_subscriber::registry().with(helios_state::log_db::start(db.clone()));
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = helios_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
