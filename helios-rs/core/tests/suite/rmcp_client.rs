...
use helios_utils_cargo_bin::cargo_bin;
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: helios_protocol::openai_models::ReasoningEffort::Medium,
...
/// This test writes to a fallback credentials file in HELIOS_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire HELIOS_HOME through the code.
#[serial(helios_home)]
#[test]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("HELIOS_HOME", temp_home.path().as_os_str());
    ...
    let fixture = test_codex()
        .with_config(move |config| {
            // This test seeds OAuth tokens in HELIOS_HOME/.credentials.json and
