...
async fn model_selects_expected_tools() {
    ...
    let gpt51_helios_max_tools = collect_tool_identifiers_for_model("gpt-5.1-helios-max").await;
    assert_eq!(
        gpt51_helios_max_tools,
        ...
        "gpt-5.1-helios-max should expose the apply_patch tool",
    );

    let gpt5_helios_tools = collect_tool_identifiers_for_model("gpt-5-codex").await;
    assert_eq!(
        gpt5_helios_tools,
        expected_default_tools(
            "shell_command",
    ...
    let gpt51_helios_tools = collect_tool_identifiers_for_model("gpt-5.1-codex").await;
    assert_eq!(
        gpt51_helios_tools,
        expected_default_tools(
            "shell_command",
