...
const REMOTE_MODEL_SLUG: &str = "helios-test";
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let manager = helios_core::test_support::models_manager_with_provider(
        helios_home.path().to_path_buf(),
        helios_core::test_support::auth_manager_from_auth(auth),
        provider,
    );

    manager.list_models(RefreshStrategy::OnlineIfUncached).await;

    let model_info = manager.get_model_info("gpt-5.3-helios-test", &config).await;

    assert_eq!(model_info.slug, "gpt-5.3-helios-test");
...
async fn remote_models_long_model_slug_is_sent_with_high_reasoning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-helios-test";
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "helios-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "helios-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        1,
...
async fn remote_models_preserve_builtin_presets() -> Result<()> {
    ...
    assert!(
        available
            .iter()
            .any(|model| model.model == "gpt-5.1-helios-max"),
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let remote_model = test_remote_model("helios-auto-balanced", ModelVisibility::Hide, 0);
    let models_mock = mount_models_once(
        &server,
        ModelsResponse {
            models: vec![remote_model],
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "helios-auto-balanced")
