...
async fn helios_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    helios_returns_json_result("gpt-5.1".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn helios_returns_json_result_for_gpt5_codex() -> anyhow::Result<()> {
    helios_returns_json_result("gpt-5.1-codex".to_string()).await
}

async fn helios_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("helios_output_schema".into()))
