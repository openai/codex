...
use chrono::Utc;
use helios_core::EventPersistenceMode;
use helios_core::RolloutRecorder;
use helios_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(helios_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = helios_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(helios_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(helios_home, "sessions", id)
}

async fn upsert_thread_metadata(helios_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(helios_home.to_path_buf(), "test-provider".to_string(), None)
        .await
        .unwrap();
    ...
    builder.cwd = helios_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_helios_home_directory() {
    let repo = TempDir::new().unwrap();
    let helios_home = repo.path().join(".codex");
    std::fs::create_dir_all(&helios_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".codex/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&helios_home, id);

    let found = find_thread_path_by_id_str(&helios_home, &id.to_string())
        .await
        .unwrap();
