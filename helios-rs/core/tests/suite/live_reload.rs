...
fn enable_trusted_project(config: &mut helios_core::config::Config) {
...
async fn live_skills_reload_refreshes_skill_cache_after_skill_change() -> Result<()> {
    ...
    let skill_path = dunce::canonicalize(test.helios_home_path().join("skills/demo/SKILL.md"))?;
    ...
    write_skill(test.helios_home_path(), "demo", "demo skill", skill_v2);
