...
use helios_core::NewThread;
use helios_core::Prompt;
use helios_core::ResponseEvent;
use helios_core::ResponsesWebsocketVersion;
use helios_core::ThreadManager;
use helios_core::WireApi;
use helios_core::auth::AuthCredentialsStoreMode;
use helios_core::built_in_model_providers;
use helios_core::default_client::originator;
use helios_core::error::CodexErr;
use helios_core::features::Feature;
...
use helios_protocol::config_types::ReasoningSummary;
use helios_protocol::config_types::Settings;
use helios_protocol::config_types::Verbosity;
use helios_protocol::models::ContentItem;
use helios_protocol::models::FunctionCallOutputPayload;
use helios_protocol::models::LocalShellAction;
use helios_protocol::models::LocalShellExecAction;
use helios_protocol::models::LocalShellStatus;
use helios_protocol::models::MessagePhase;
use helios_protocol::models::ReasoningItemContent;
...
use helios_protocol::openai_models::ReasoningEffort;
...
use core_test_support::test_helios::TestCodex;
...
/// Writes an `auth.json` into the provided `helios_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        helios_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = helios_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![helios_protocol::models::ContentItem::InputText {
    ...
    let prior_system = helios_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![helios_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = helios_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let helios_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codex()
        .with_home(helios_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, helios_home, session_path.clone())
        .await
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_codex()
        .with_auth(create_dummy_helios_auth())
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    // Init session
    let helios_home = TempDir::new().unwrap();
    ...
    let auth_manager =
        match CodexAuth::from_auth_storage(helios_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => helios_core::test_support::auth_manager_from_auth(auth),
            Ok(None) => panic!("No CodexAuth found in helios_home"),
...
async fn skills_append_to_instructions() {
    ...
    let helios_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = helios_home.path().join("skills/demo");
    ...
    let helios_home_path = helios_home.path().to_path_buf();
    let mut builder = test_codex()
        .with_home(helios_home.clone())
        .with_auth(CodexAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = helios_home_path;
        });
    let codex = builder
        .build(&server)
        .await
    ...
    let _helios_home_guard = helios_home;
}
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let model = helios_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        helios_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let conversation_id = ThreadId::new();
    let auth_manager =
        helios_core::test_support::auth_manager_from_auth(CodexAuth::from_api_key("Test API Key"));
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-helios-primary-used-percent", "12.5")
        .insert_header("x-helios-secondary-used-percent", "40.0")
        .insert_header("x-helios-primary-window-minutes", "10")
        .insert_header("x-helios-secondary-window-minutes", "60")
        .insert_header("x-helios-primary-reset-at", "1704069000")
        .insert_header("x-helios-secondary-reset-at", "1704074400")
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5.1-helios-max in tests → 95% usable context window
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-helios-primary-used-percent", "100.0")
        .insert_header("x-helios-secondary-used-percent", "87.5")
        .insert_header("x-helios-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-helios-primary-window-minutes", "15")
        .insert_header("x-helios-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_codex();
    let helios_fixture = builder.build(&server).await?;
    let codex = helios_fixture.codex.clone();
...
fn create_dummy_helios_auth() -> CodexAuth {
