...
const CALENDAR_CREATE_TOOL: &str = "mcp__helios_apps__calendar_create_event";
const CALENDAR_LIST_TOOL: &str = "mcp__helios_apps__calendar_list_events";
...
async fn search_tool_selection_persists_across_turns() -> Result<()> {
    ...
    for tool in search_result_tools(&search_output_payload) {
        assert_eq!(
            ...
            Some("helios_apps"),
            "search results should only include helios_apps tools: {search_output_payload:?}"
        );
    }
...
async fn search_tool_selection_unions_results_within_turn() -> Result<()> {
    ...
    for tool in search_result_tools(&second_search_payload) {
        assert_eq!(
            tool.get("server").and_then(Value::as_str),
            Some("helios_apps"),
            "search results should only include helios_apps tools: {second_search_payload:?}"
        );
    }
...
async fn search_tool_selection_drops_when_fork_excludes_search_turn() -> Result<()> {
    ...
    assert!(
        !forked_tools
            .iter()
            .any(|name| name.starts_with("mcp__helios_apps__")),
