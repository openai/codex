...
use helios_protocol::protocol::ExecCommandBeginEvent;
use helios_protocol::protocol::ExecCommandEndEvent;
...
use core_test_support::test_helios::TestCodexHarness;
...
const SNAPSHOT_MARKER_VAR: &str = "HELIOS_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(helios_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = helios_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let helios_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&helios_home).await?;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.helios_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestCodexHarness::with_builder(builder).await?;
    let helios_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(helios_home));
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let helios_home = home.path().to_path_buf();
