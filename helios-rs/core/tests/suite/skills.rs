...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let skill_path = test.helios_home_path().join("skills/demo/SKILL.md");
    ...
    core_test_support::wait_for_event(test.codex.as_ref(), |event| {
        matches!(event, helios_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;
...
async fn skill_load_errors_surface_in_session_configured() -> Result<()> {
    ...
    let response =
        core_test_support::wait_for_event_match(test.codex.as_ref(), |event| match event {
            helios_protocol::protocol::EventMsg::ListSkillsResponse(response) => {
                Some(response.clone())
            }
            _ => None,
        })
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    let system_skill_path = system_skill_md_path(test.helios_home_path(), SYSTEM_SKILL_NAME);
    assert!(
        system_skill_path.exists(),
    ...
    assert_eq!(skill.scope, helios_protocol::protocol::SkillScope::System);
