...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let Some(helios_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
...
async fn plan_mode_emits_plan_item_from_proposed_plan_block() -> anyhow::Result<()> {
    ...
    codex
        .submit(Op::UserTurn {
            ...
            approval_policy: helios_protocol::protocol::AskForApproval::Never,
            sandbox_policy: helios_protocol::protocol::SandboxPolicy::DangerFullAccess,
            model: session_configured.model.clone(),
            effort: None,
