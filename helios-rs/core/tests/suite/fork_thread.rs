...
use helios_core::parse_turn_item;
...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    let fork1_path = helios_fork1.rollout_path().expect("rollout path");
    ...
    let fork2_path = helios_fork2.rollout_path().expect("rollout path");
