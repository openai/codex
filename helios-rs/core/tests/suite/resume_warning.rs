...
use helios_protocol::protocol::ResumedHistory;
...
fn resume_history(
    config: &helios_core::config::Config,
    previous_model: &str,
...
async fn emits_warning_when_resumed_model_differs() {
    ...
    let thread_manager = helios_core::test_support::thread_manager_with_models_provider(
        CodexAuth::from_api_key("test"),
        config.model_provider.clone(),
    );
    let auth_manager =
        helios_core::test_support::auth_manager_from_auth(CodexAuth::from_api_key("test"));
