...
use anyhow::Result;
use helios_core::config::Constrained;
...
use helios_protocol::protocol::ApplyPatchApprovalRequestEvent;
use helios_protocol::protocol::AskForApproval;
use helios_protocol::protocol::EventMsg;
use helios_protocol::protocol::ExecApprovalRequestEvent;
use helios_protocol::protocol::ExecPolicyAmendment;
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/helios-fallback-rule-test.txt && echo hello > /tmp/helios-fallback-rule-test.txt";
