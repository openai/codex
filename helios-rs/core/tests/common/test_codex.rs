...
impl TestCodexBuilder {
    ...
    async fn build_from_config(
        ...
        let thread_manager = helios_core::test_support::thread_manager_with_models_provider_and_home(
            auth.clone(),
        ...
        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = helios_core::test_support::auth_manager_from_auth(auth);
                thread_manager
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = helios_utils_cargo_bin::cargo_bin("codex") {
            config.helios_linux_sandbox_exe = Some(path);
        }
...
impl TestCodex {
    ...
    pub fn helios_home_path(&self) -> &Path {
