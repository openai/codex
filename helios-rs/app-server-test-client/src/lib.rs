...
use clap::Parser;
use clap::Subcommand;
use helios_app_server_protocol::AddConversationListenerParams;
use helios_app_server_protocol::AddConversationSubscriptionResponse;
...
use helios_app_server_protocol::CommandExecutionRequestApprovalParams;
...
use helios_app_server_protocol::DynamicToolSpec;
use helios_app_server_protocol::FileChangeApprovalDecision;
use helios_app_server_protocol::FileChangeRequestApprovalParams;
use helios_app_server_protocol::FileChangeRequestApprovalResponse;
use helios_app_server_protocol::GetAccountRateLimitsResponse;
use helios_app_server_protocol::InitializeCapabilities;
use helios_app_server_protocol::InitializeParams;
use helios_app_server_protocol::InitializeResponse;
use helios_app_server_protocol::InputItem;
...
use helios_app_server_protocol::LoginChatGptCompleteNotification;
use helios_app_server_protocol::LoginChatGptResponse;
use helios_app_server_protocol::ModelListParams;
use helios_app_server_protocol::ModelListResponse;
use helios_app_server_protocol::NewConversationParams;
use helios_app_server_protocol::NewConversationResponse;
use helios_app_server_protocol::ReadOnlyAccess;
use helios_app_server_protocol::RequestId;
use helios_app_server_protocol::SandboxPolicy;
use helios_app_server_protocol::SendUserMessageParams;
use helios_app_server_protocol::SendUserMessageResponse;
...
use helios_app_server_protocol::ThreadListParams;
use helios_app_server_protocol::ThreadListResponse;
use helios_app_server_protocol::ThreadResumeParams;
use helios_app_server_protocol::ThreadResumeResponse;
...
struct Cli {
    ...
    #[arg(long, env = "HELIOS_BIN", global = true)]
    helios_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--helios-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "HELIOS_APP_SERVER_URL", global = true)]
    url: Option<String>,
...
enum CliCommand {
    ...
    ///
    /// Logs are written to:
    ///   `/tmp/helios-app-server-test-client/`
    Serve {
...
pub fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let helios_bin = helios_bin.unwrap_or_else(|| PathBuf::from("codex"));
            serve(&helios_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(helios_bin, url)?;
...
fn resolve_endpoint(helios_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if helios_bin.is_some() && url.is_some() {
        bail!("--helios-bin and --url are mutually exclusive");
    }
    if let Some(helios_bin) = helios_bin {
        return Ok(Endpoint::SpawnCodex(helios_bin));
    }
    if let Some(url) = url {
...
fn serve(helios_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/helios-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,helios_=trace {}",
        shell_quote(&helios_bin.display().to_string())
    );
    ...
    let child = Command::new("nohup")
        ...
        .spawn()
        .with_context(|| format!("failed to start `{}` app-server", helios_bin.display()))?;
...
pub fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnCodex(helios_bin.to_path_buf());
    send_message_v2_endpoint(&endpoint, config_overrides, user_message, dynamic_tools)
}
...
impl CodexClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnCodex(helios_bin) => Self::spawn_stdio(helios_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(helios_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let helios_bin_display = helios_bin.display();
        let mut cmd = Command::new(helios_bin);
        if let Some(helios_bin_parent) = helios_bin.parent() {
            let mut path = OsString::from(helios_bin_parent.as_os_str());
        ...
        let mut helios_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{helios_bin_display}` app-server"))?;

        let stdin = helios_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = helios_app_server
            .stdout
            .take()
    ...
    fn connect_websocket(url: &str) -> Result<Self> {
        ...
        let (socket, _response) = connect(parsed.as_str()).with_context(|| {
            format!(
                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `helios-app-server-test-client serve --listen {url}`"
            )
        })?;
        Ok(Self {
