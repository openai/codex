...
pub fn sandbox_dir(helios_home: &Path) -> PathBuf {
    helios_home.join(".sandbox")
}

pub fn sandbox_secrets_dir(helios_home: &Path) -> PathBuf {
    helios_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(helios_home: &Path) -> PathBuf {
    sandbox_dir(helios_home).join("setup_marker.json")
}

pub fn sandbox_users_path(helios_home: &Path) -> PathBuf {
    sandbox_secrets_dir(helios_home).join("sandbox_users.json")
}
...
fn run_setup_refresh_inner(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| helios_home.to_path_buf());
    log_note(
        &format!(
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("helios-windows-sandbox-setup.exe");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    PathBuf::from("helios-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(helios_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(helios_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(helios_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(helios_home);
    ...
    run_setup_exe(&payload, needs_elevation, helios_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, helios_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, helios_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to HELIOS_HOME or anything under HELIOS_HOME/.sandbox.
    // These locations contain sandbox control/state and must remain tamper-resistant.
    let helios_home_key = canonical_path_key(helios_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(helios_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(helios_home));
