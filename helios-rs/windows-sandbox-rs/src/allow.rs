...
mod tests {
    ...
    #[test]
    fn denies_helios_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let helios_dir = command_cwd.join(".codex");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&helios_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&helios_dir).unwrap(),
