mod windows_impl {
    ...
    /// Locates `helios-command-runner.exe` next to the current binary.
    fn find_runner_exe() -> PathBuf {
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                let candidate = dir.join("helios-command-runner.exe");
                if candidate.exists() {
                    return candidate;
                }
            }
        }
        PathBuf::from("helios-command-runner.exe")
    }

    /// Generates a unique named-pipe path used to communicate with the runner process.
    fn pipe_name(suffix: &str) -> String {
        let mut rng = SmallRng::from_entropy();
        format!(r"\\.\pipe\helios-runner-{:x}-{}", rng.gen::<u128>(), suffix)
    }
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = helios_home.join(".sandbox");
        ensure_helios_home_exists(&sandbox_base)?;

        let logs_base_dir: Option<&Path> = Some(sandbox_base.as_path());
        log_start(&command, logs_base_dir);
        let sandbox_creds =
            require_logon_sandbox_creds(&policy, sandbox_policy_cwd, cwd, &env_map, helios_home)?;
        ...
        let caps = load_or_create_cap_sids(helios_home)?;
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => (
                ...
                vec![
                    ...
                    crate::cap::workspace_cap_sid_for_cwd(helios_home, cwd)?,
                ],
            ),
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let runner_cmdline = runner_exe
            .to_str()
            .map(|s| s.to_string())
            .unwrap_or_else(|| "helios-command-runner.exe".to_string());
        ...
        let payload = RunnerPayload {
            ...
            helios_home: sandbox_base.clone(),
            real_helios_home: helios_home.to_path_buf(),
