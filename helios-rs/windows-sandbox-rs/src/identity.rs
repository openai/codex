...
pub fn sandbox_setup_is_complete(helios_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(helios_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(helios_home), Ok(Some(users)) if users.version_matches())
}

fn load_marker(helios_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(helios_home);
...
fn load_users(helios_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(helios_home);
...
fn select_identity(policy: &SandboxPolicy, helios_home: &Path) -> Result<Option<SandboxIdentity>> {
    let _marker = match load_marker(helios_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(helios_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(helios_home);
    ...
    // NOTE: Do not add HELIOS_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match load_marker(helios_home)? {
        Some(marker) if marker.version_matches() => {
            _existing_marker = Some(marker.clone());
            let selected = select_identity(policy, helios_home)?;
            if selected.is_none() {
                setup_reason =
    ...
    if identity.is_none() {
        ...
        identity = select_identity(policy, helios_home)?;
    }
    // Always refresh ACLs (non-elevated) for current roots via the setup binary.
    crate::setup::run_setup_refresh(policy, policy_cwd, command_cwd, env_map, helios_home)?;
