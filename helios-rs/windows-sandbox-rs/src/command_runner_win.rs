...
use anyhow::Context;
use anyhow::Result;
use helios_windows_sandbox::allow_null_device;
use helios_windows_sandbox::convert_string_sid_to_sid;
use helios_windows_sandbox::create_process_as_user;
use helios_windows_sandbox::create_readonly_token_with_caps_from;
use helios_windows_sandbox::create_workspace_write_token_with_caps_from;
use helios_windows_sandbox::get_current_token_for_restriction;
use helios_windows_sandbox::hide_current_user_profile_dir;
use helios_windows_sandbox::log_note;
use helios_windows_sandbox::parse_policy;
use helios_windows_sandbox::to_wide;
use helios_windows_sandbox::SandboxPolicy;
...
struct RunnerRequest {
    ...
    helios_home: PathBuf,
    // Real user's HELIOS_HOME for shared data (caps, config).
    real_helios_home: PathBuf,
    cap_sids: Vec<String>,
    command: Vec<String>,
    cwd: PathBuf,
...
pub fn main() -> Result<()> {
    ...
    let log_dir = Some(req.helios_home.as_path());
    hide_current_user_profile_dir(req.helios_home.as_path());
    log_note(
        &format!(
            "runner start cwd={} cmd={:?} real_helios_home={}",
            req.cwd.display(),
            req.command,
            req.real_helios_home.display()
        ),
        Some(&req.helios_home),
    );
