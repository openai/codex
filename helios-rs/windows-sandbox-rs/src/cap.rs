...
pub fn cap_sid_file(helios_home: &Path) -> PathBuf {
    helios_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(helios_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(helios_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(helios_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(helios_home);
    let mut caps = load_or_create_cap_sids(helios_home)?;
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let helios_home = temp.path().join("helios-home");
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&helios_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&helios_home, alt_spelling.as_path()).expect("second sid");

        assert_eq!(first_sid, second_sid);

        let caps = load_or_create_cap_sids(&helios_home).expect("load caps");
