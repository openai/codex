...
use base64::Engine;
use helios_windows_sandbox::canonicalize_path;
use helios_windows_sandbox::convert_string_sid_to_sid;
use helios_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use helios_windows_sandbox::ensure_allow_write_aces;
use helios_windows_sandbox::extract_setup_failure;
use helios_windows_sandbox::hide_newly_created_users;
use helios_windows_sandbox::is_command_cwd_root;
use helios_windows_sandbox::load_or_create_cap_sids;
use helios_windows_sandbox::log_note;
use helios_windows_sandbox::path_mask_allows;
use helios_windows_sandbox::protect_workspace_agents_dir;
use helios_windows_sandbox::protect_workspace_helios_dir;
...
use helios_windows_sandbox::workspace_cap_sid_for_cwd;
use helios_windows_sandbox::write_setup_error_report;
use helios_windows_sandbox::SetupErrorCode;
use helios_windows_sandbox::SetupErrorReport;
use helios_windows_sandbox::SetupFailure;
use helios_windows_sandbox::LOG_FILE_NAME;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(helios_home) = std::env::var("HELIOS_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&helios_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.helios_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.helios_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.helios_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.helios_home, &payload.command_cwd)?;
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.helios_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.helios_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.helios_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.helios_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.helios_home).join("sandbox_users.json");
    ...
    match unsafe { protect_workspace_helios_dir(&payload.command_cwd, workspace_psid) } {
        Ok(true) => {
