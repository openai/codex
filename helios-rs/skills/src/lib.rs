...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".helios-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills.
///
/// This is typically located at `HELIOS_HOME/skills/.system`.
pub fn system_cache_root_dir(helios_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(helios_home)
        .and_then(|helios_home| system_cache_root_dir_abs(&helios_home))
...
fn system_cache_root_dir_abs(helios_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    helios_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `HELIOS_HOME/skills/.system`.
///
...
/// install is skipped.
pub fn install_system_skills(helios_home: &Path) -> Result<(), SystemSkillsError> {
    let helios_home = AbsolutePathBuf::try_from(helios_home)
        .map_err(|source| SystemSkillsError::io("normalize codex home dir", source))?;
    let skills_root_dir = helios_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&helios_home)
