use anyhow::Context;
use anyhow::Result;
use helios_app_server_protocol::read_schema_fixture_tree;
use helios_app_server_protocol::write_schema_fixtures;
...
fn schema_root() -> Result<std::path::PathBuf> {
    ...
    let typescript_index = helios_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_bundle =
        helios_utils_cargo_bin::find_resource!("schema/json/helios_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/helios_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
