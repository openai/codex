...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("helios_app_server_protocol.schemas.json"),
        &bundle,
    )?;
...
mod tests {
    ...
    fn generated_ts_optional_nullable_fields_only_in_params() -> Result<()> {
        ...
        let output_dir = std::env::temp_dir().join(format!("helios_ts_types_{}", Uuid::now_v7()));
    ...
    fn generate_ts_with_experimental_api_retains_experimental_entries() -> Result<()> {
        let output_dir =
            std::env::temp_dir().join(format!("helios_ts_types_experimental_{}", Uuid::now_v7()));
        fs::create_dir(&output_dir)?;
    ...
    fn stable_schema_filter_removes_mock_thread_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("helios_schema_{}", Uuid::now_v7()));
        fs::create_dir(&output_dir)?;
    ...
    fn experimental_type_fields_ts_filter_handles_interface_shape() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("helios_ts_filter_{}", Uuid::now_v7()));
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json =
            fs::read_to_string(output_dir.join("helios_app_server_protocol.schemas.json"))?;
