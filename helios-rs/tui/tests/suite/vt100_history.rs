...
struct TestScenario {
    term: helios_tui::custom_terminal::Terminal<VT100Backend>,
}

impl TestScenario {
    fn new(width: u16, height: u16, viewport: Rect) -> Self {
        let backend = VT100Backend::new(width, height);
        let mut term = helios_tui::custom_terminal::Terminal::with_options(backend)
    ...
    fn run_insert(&mut self, lines: Vec<Line<'static>>) {
        helios_tui::insert_history::insert_history_lines(&mut self.term, lines)
