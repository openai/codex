...
async fn test_config(temp_home: &TempDir) -> Config {
    ConfigBuilder::default()
        .helios_home(temp_home.path().to_path_buf())
        .build()
        .await
        .expect("load config")
}
...
fn token_info_for(model_slug: &str, config: &Config, usage: &TokenUsage) -> TokenUsageInfo {
    let context_window =
        helios_core::test_support::construct_model_info_offline(model_slug, config).context_window;
    TokenUsageInfo {
...
async fn status_snapshot_includes_reasoning_details() {
    ...
    config.model = Some("gpt-5.1-helios-max".to_string());
    ...
    let model_slug = helios_core::test_support::get_model_offline(config.model.as_deref());
