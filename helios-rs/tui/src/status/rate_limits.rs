...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `codex` or `helios_other`).
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let combine_non_helios_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_helios_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_helios_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_helios_single_limit_renders_combined_row() {
        let now = Local::now();
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "helios-other".to_string(),
            captured_at: now,
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "helios-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_helios_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "helios-other".to_string(),
            captured_at: now,
