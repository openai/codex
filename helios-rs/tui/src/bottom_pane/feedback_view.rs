...
const HELIOS_FEEDBACK_INTERNAL_URL: &str = "http://go/helios-feedback-internal";
...
pub(crate) struct FeedbackNoteView {
    ...
    snapshot: helios_feedback::CodexLogSnapshot,
            ...
            Ok(()) => {
                ...
                let mut lines = vec![Line::from(match issue_url.as_ref() {
                    Some(_) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        format!("{prefix} Please report this in #helios-feedback:")
                    }
                ...
                match issue_url {
                    Some(url) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        lines.extend([
                            ...
                            Line::from(vec![
                                "    ".into(),
                                format!("https://go/helios-feedback/{thread_id}").bold(),
                            ]),
                        ]);
                    }
                    Some(url) => {
...
fn slack_feedback_url(_thread_id: &str) -> String {
    HELIOS_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "helios-logs.log".into()]).into(),
    ];
...
mod tests {
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = helios_feedback::CodexFeedback::new().snapshot(None);
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/helios-feedback-internal".to_string();
