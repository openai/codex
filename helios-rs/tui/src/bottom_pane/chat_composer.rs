...
//!
//! For the detailed burst state machine, see `helios-rs/tui/src/bottom_pane/paste_burst.rs`.
...
use helios_protocol::custom_prompts::PROMPTS_CMD_PREFIX;
use helios_protocol::models::local_image_label_text;
use helios_protocol::user_input::ByteRange;
...
use helios_chatgpt::connectors::AppInfo;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
        ...
        if let Some(skills) = self.skills.as_ref() {
            for skill in skills {
                ...
                mentions.push(MentionItem {
                    ...
                    category_tag: (skill.scope == helios_protocol::protocol::SkillScope::Repo)
                        .then(|| "[Repo]".to_string()),
                });
            }
        }
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let slug = helios_core::connectors::connector_mention_slug(connector);
...
程序员 与 Unicode 同行";
    ...
    fn pasting_filepath_attaches_image() {
        ...
        let tmp_path: PathBuf = tmp.path().join("helios_tui_test_paste_image.png");
