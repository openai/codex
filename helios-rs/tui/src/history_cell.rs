...
use helios_core::web_search::web_search_detail;
use helios_otel::RuntimeMetricsSummary;
use helios_protocol::account::PlanType;
use helios_protocol::mcp::Resource;
use helios_protocol::mcp::ResourceTemplate;
...
use helios_protocol::plan_tool::PlanItemArg;
...
use helios_protocol::protocol::McpInvocation;
use helios_protocol::protocol::SessionConfiguredEvent;
use helios_protocol::request_user_input::RequestUserInputAnswer;
use helios_protocol::request_user_input::RequestUserInputQuestion;
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{HELIOS_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: helios_protocol::protocol::ReviewDecision,
) -> Box<dyn HistoryCell> {
...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<helios_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<helios_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(helios_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`helios-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, helios_protocol::mcp::Tool>,
