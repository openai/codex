...
//! under `{HELIOS_HOME}/themes/`.  It provides:
//!
...
fn theme_picker_subtitle(helios_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = helios_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    helios_home: Option<&Path>,
    terminal_width: Option<u16>,
) -> SelectionViewParams {
    // Snapshot the current theme so we can restore on cancel.
    let original_theme = highlight::current_syntax_theme();

    let entries = highlight::list_available_themes(helios_home);
    let helios_home_owned = helios_home.map(Path::to_path_buf);
    ...
    let preview_home = helios_home_owned.clone();
...
mod tests {
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_helios_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let helios_home = home.join(".codex");

        let subtitle = theme_picker_subtitle(Some(&helios_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let helios_home = home.join(long_segment).join(".codex");

        let subtitle = theme_picker_subtitle(Some(&helios_home), Some(140));
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&helios_home), Some(94));
