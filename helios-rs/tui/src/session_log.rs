...
pub(crate) fn maybe_init(config: &Config) {
    let enabled = std::env::var("HELIOS_TUI_RECORD_SESSION")
    ...
    let path = if let Ok(path) = std::env::var("HELIOS_TUI_SESSION_LOG_PATH") {
        PathBuf::from(path)
    } else {
        let mut p = match helios_core::config::log_dir(config) {
            Ok(dir) => dir,
...
pub(crate) fn log_inbound_app_event(event: &AppEvent) {
    ...
    match event {
        AppEvent::CodexEvent(ev) => {
            write_record("to_tui", "helios_event", ev);
        }
        AppEvent::NewSession => {
            let value = json!({
                "ts": now_ts(),
