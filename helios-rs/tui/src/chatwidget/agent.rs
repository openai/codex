...
pub(crate) fn spawn_agent(
    ...
    let (helios_op_tx, mut helios_op_rx) = unbounded_channel::<Op>();
    ...
    tokio::spawn(async move {
        ...
        let ev = helios_protocol::protocol::Event {
            // The `id` does not matter for rendering, so we can use a fake value.
            id: "".to_string(),
            msg: helios_protocol::protocol::EventMsg::SessionConfigured(session_configured),
        };
        app_event_tx_clone.send(AppEvent::CodexEvent(ev));

        let thread_clone = thread.clone();
        tokio::spawn(async move {
            while let Some(op) = helios_op_rx.recv().await {
