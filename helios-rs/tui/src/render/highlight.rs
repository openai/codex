...
//! | `HELIOS_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static HELIOS_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// fork reloads complete). The first call persists `name` and `helios_home` in
...
pub(crate) fn set_theme_override(
    ...
    let mut warning = validate_theme_name(name.as_deref(), helios_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let helios_home_set_ok = HELIOS_HOME.set(helios_home.clone()).is_ok();
    ...
    if !override_set_ok || !helios_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, helios_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = helios_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$HELIOS_HOME/themes/{name}.tmTheme"));
...
fn custom_theme_path(name: &str, helios_home: &Path) -> PathBuf {
    helios_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{helios_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, helios_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, helios_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, helios_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {HELIOS_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = helios_home
...
fn build_default_theme() -> Theme {
    ...
    let helios_home = HELIOS_HOME
        .get()
        .and_then(|helios_home| helios_home.as_deref());
    resolve_theme_with_override(name, helios_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = HELIOS_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, helios_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{HELIOS_HOME}/themes/`.
...
/// found in `{helios_home}/themes/`.
pub(crate) fn list_available_themes(helios_home: Option<&Path>) -> Vec<ThemeEntry> {
