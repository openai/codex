...
use chrono::DateTime;
use chrono::Utc;
use helios_core::Cursor;
use helios_core::INTERACTIVE_SESSION_SOURCES;
use helios_core::RolloutRecorder;
use helios_core::ThreadItem;
use helios_core::ThreadSortKey;
use helios_core::ThreadsPage;
use helios_core::config::Config;
use helios_core::find_thread_names_by_ids;
use helios_core::path_utils;
...
async fn run_session_picker(
    ...
    let helios_home = config.helios_home.as_path();
...
impl PickerState {
    ...
    async fn update_thread_names(&mut self) {
        ...
        let names = find_thread_names_by_ids(&self.helios_home, &missing_ids)
            .await
            .unwrap_or_default();
