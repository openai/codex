...
use helios_core::config::ConstraintResult;
use helios_core::config::types::Notifications;
use helios_core::config::types::WindowsSandboxModeToml;
use helios_core::config_loader::ConfigLayerStackOrdering;
use helios_core::features::FEATURES;
use helios_core::features::Feature;
use helios_core::find_thread_name_by_id;
use helios_core::git_info::current_branch_name;
use helios_core::git_info::get_git_repo_root;
use helios_core::git_info::local_git_branches;
use helios_core::models_manager::manager::ModelsManager;
use helios_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use helios_core::skills::model::SkillMetadata;
use helios_core::terminal::TerminalName;
use helios_core::terminal::terminal_info;
...
use helios_protocol::items::AgentMessageItem;
use helios_protocol::models::MessagePhase;
use helios_protocol::models::local_image_label_text;
use helios_protocol::parse_command::ParsedCommand;
use helios_protocol::protocol::AgentMessageDeltaEvent;
use helios_protocol::protocol::AgentMessageEvent;
use helios_protocol::protocol::AgentReasoningDeltaEvent;
use helios_protocol::protocol::AgentReasoningEvent;
use helios_protocol::protocol::AgentReasoningRawContentDeltaEvent;
use helios_protocol::protocol::AgentReasoningRawContentEvent;
use helios_protocol::protocol::ApplyPatchApprovalRequestEvent;
use helios_protocol::protocol::BackgroundEventEvent;
use helios_protocol::protocol::CodexErrorInfo;
use helios_protocol::protocol::CreditsSnapshot;
use helios_protocol::protocol::DeprecationNoticeEvent;
...
use helios_protocol::protocol::ExecCommandOutputDeltaEvent;
use helios_protocol::protocol::ExecCommandSource;
use helios_protocol::protocol::ExitedReviewModeEvent;
use helios_protocol::protocol::ListCustomPromptsResponseEvent;
use helios_protocol::protocol::ListSkillsResponseEvent;
use helios_protocol::protocol::McpListToolsResponseEvent;
...
use helios_protocol::protocol::McpToolCallBeginEvent;
use helios_protocol::protocol::McpToolCallEndEvent;
use helios_protocol::protocol::Op;
use helios_protocol::protocol::PatchApplyBeginEvent;
use helios_protocol::protocol::RateLimitSnapshot;
use helios_protocol::protocol::ReviewRequest;
use helios_protocol::protocol::ReviewTarget;
use helios_protocol::protocol::SkillMetadata as ProtocolSkillMetadata;
use helios_protocol::protocol::StreamErrorEvent;
use helios_protocol::protocol::TerminalInteractionEvent;
...
use helios_protocol::protocol::TurnCompleteEvent;
use helios_protocol::protocol::TurnDiffEvent;
use helios_protocol::protocol::UndoCompletedEvent;
use helios_protocol::protocol::UndoStartedEvent;
use helios_protocol::protocol::UserMessageEvent;
use helios_protocol::protocol::ViewImageToolCallEvent;
use helios_protocol::protocol::WarningEvent;
use helios_protocol::protocol::WebSearchBeginEvent;
use helios_protocol::protocol::WebSearchEndEvent;
...
use helios_utils_approval_presets::ApprovalPreset;
...
const NUDGE_MODEL_SLUG: &str = "gpt-5.1-helios-mini";
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: helios_feedback::CodexFeedback,
...
/// and by sending requests back to helios-core.
///
...
pub(crate) struct ChatWidget {
    ...
    helios_op_tx: UnboundedSender<Op>,
    ...
    /// Tracks whether helios-core currently considers an agent turn to be in progress.
    ///
    ...
    feedback: helios_feedback::CodexFeedback,
    ...
    session_network_proxy: Option<helios_protocol::protocol::SessionNetworkProxyRuntime>,
    // Shared latch so we only warn once about invalid status-line item IDs.
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: helios_protocol::protocol::SessionConfiguredEvent) {
        self.bottom_pane
        ...
        // Ask helios-core to enumerate custom prompts for this session.
    ...
    fn emit_forked_thread_event(&self, forked_from_id: ThreadId) {
        ...
        let helios_home = self.config.helios_home.clone();
        tokio::spawn(async move {
            ...
            match find_thread_name_by_id(&helios_home, &forked_from_id).await {
    ...
    fn on_thread_name_updated(&mut self, event: helios_protocol::protocol::ThreadNameUpdatedEvent) {
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_helios_limit = limit_id.eq_ignore_ascii_case("codex");
            let warnings = if is_helios_limit {
            ...
            let high_usage = is_helios_limit
                && (snapshot
                    .secondary
                    .as_ref()
    ...
    fn on_patch_apply_end(&mut self, event: helios_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: helios_protocol::protocol::GetHistoryEntryResponseEvent,
    ) {
        let helios_protocol::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: helios_protocol::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        let helios_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), thread_manager);
        ...
        widget.bottom_pane.set_windows_degraded_sandbox_active(
            helios_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                && matches!(
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<helios_core::CodexThread>,
        session_configured: helios_protocol::protocol::SessionConfiguredEvent,
    ) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Logout => {
                if let Err(e) = helios_core::auth::logout(
                    &self.config.helios_home,
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                let Some(name) = helios_core::util::normalize_thread_name(&prepared_args) else {
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                let Some(name) = helios_core::util::normalize_thread_name(&name) else {
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = helios_core::connectors::connector_mention_slug(&app);
        ...
        self.helios_op_tx.send(op).unwrap_or_else(|e| {
    ...
    pub(crate) fn handle_helios_event(&mut self, event: Event) {
        let Event { id, msg } = event;
        self.dispatch_event_msg(Some(id), msg, None);
    }

    pub(crate) fn handle_helios_event_replay(&mut self, event: Event) {
        let Event { msg, .. } = event;
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_helios_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::Error(ErrorEvent {
                ...
                if let Some(info) = helios_error_info
            ...
            EventMsg::ItemCompleted(event) => {
                let item = event.item;
                if let helios_protocol::items::TurnItem::Plan(plan_item) = &item {
                    self.on_plan_item_completed(plan_item.text.clone());
                }
                if let helios_protocol::items::TurnItem::AgentMessage(item) = item {
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        ...
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| helios_core::review_prompts::user_facing_hint(&review.target));
    ...
    fn open_theme_picker(&mut self) {
        let helios_home = helios_core::config::find_helios_home().ok();
    ...
    fn status_line_project_root(&self) -> Option<PathBuf> {
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_helios_folder } => {
                    dot_helios_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    fn status_line_value_for_item(&self, item: &StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::CodexVersion => Some(HELIOS_CLI_VERSION.to_string()),
    ...
    fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
        ...
        tokio::spawn(async move {
            ...
            let should_schedule_force_refetch =
                !force_refetch && !accessible_result.helios_apps_ready;
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("helios-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "helios-auto-fast" => 0,
            "helios-auto-balanced" => 1,
            "helios-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("gpt-5.1-codex")
            || preset.model.starts_with("gpt-5.1-helios-max")
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        let show_elevate_sandbox_hint = helios_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
        ...
        for preset in presets.into_iter() {
            ...
            let actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    {
                        let preset_clone = preset.clone();
                        if helios_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && helios_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.helios_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match helios_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.helios_home.as_path(),
            cwd.as_path(),
            &env_map,
            self.config.permissions.sandbox_policy.get(),
            Some(self.config.helios_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
            ...
            && (previous_model != next_model || previous_effort != next_effort)
        {
            ...
            if !next_model.starts_with("helios-auto-") {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd = helios_core::util::resume_command(Some(name), thread_id)
    ...
    pub(crate) fn submit_op(&mut self, op: Op) -> bool {
        ...
        if let Err(e) = self.helios_op_tx.send(op) {
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = helios_core::git_info::recent_commits(cwd, 100).await;
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatWidget,
    entries: Vec<helios_core::git_info::CommitLogEntry>,
) {
