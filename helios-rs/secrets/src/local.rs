...
impl LocalSecretsBackend {
    pub fn new(helios_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            helios_home,
            keyring_store,
        }
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.helios_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(helios_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let helios_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(helios_home.path());
        keyring.set_error(
            &account,
