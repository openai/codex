...
impl SecretsManager {
    pub fn new(helios_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(helios_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub(crate) fn compute_keyring_account(helios_home: &Path) -> String {
    let canonical = helios_home
        .canonicalize()
        .unwrap_or_else(|_| helios_home.to_path_buf())
        .to_string_lossy()
        .into_owned();
    let mut hasher = Sha256::new();
    hasher.update(canonical.as_bytes());
...
mod tests {
    ...
    fn manager_round_trips_local_backend() -> Result<()> {
        let helios_home = tempfile::tempdir().expect("tempdir");
