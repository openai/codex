...
use helios_app_server_protocol::ClientNotification;
...
impl AppServerClient {
    pub fn spawn(
        ...
        let mut child = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{helios_bin}` app-server"))?;
    ...
    pub fn initialize(&mut self) -> Result<()> {
        ...
        let request = ClientRequest::Initialize {
            ...
            params: helios_app_server_protocol::InitializeParams {
        ...
        let _parsed: helios_app_server_protocol::InitializeResponse =
...
fn handle_server_request(
    ...
    let Ok(server_request) = helios_app_server_protocol::ServerRequest::try_from(request) else {
        return Ok(());
    };

    match server_request {
        helios_app_server_protocol::ServerRequest::CommandExecutionRequestApproval {
            request_id,
            ..
        } => {
            let response = helios_app_server_protocol::CommandExecutionRequestApprovalResponse {
                decision: CommandExecutionApprovalDecision::Decline,
            };
            send_jsonrpc_response(stdin, request_id, response)
        }
        helios_app_server_protocol::ServerRequest::FileChangeRequestApproval {
            request_id, ..
        } => {
            let response = helios_app_server_protocol::FileChangeRequestApprovalResponse {
