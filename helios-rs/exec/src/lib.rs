...
use helios_core::auth::enforce_login_restrictions;
use helios_core::check_execpolicy_for_warnings;
...
use helios_core::config::load_config_as_toml_with_cli_overrides;
use helios_core::config::resolve_oss_provider;
use helios_core::config_loader::ConfigLoadError;
use helios_core::config_loader::format_config_error_with_source;
use helios_core::format_exec_policy_error_with_source;
use helios_core::git_info::get_git_repo_root;
use helios_core::models_manager::manager::RefreshStrategy;
use helios_protocol::approvals::ElicitationAction;
...
use helios_core::find_thread_path_by_id_str;
use helios_core::find_thread_path_by_name_str;
...
struct ThreadEventEnvelope {
    thread_id: helios_protocol::ThreadId,
    thread: Arc<helios_core::CodexThread>,
    event: Event,
}

pub async fn run_main(cli: Cli, helios_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("helios_exec".to_string()) {
    ...
    #[allow(clippy::print_stderr)]
    let helios_home = match find_helios_home() {
        Ok(helios_home) => helios_home,
        Err(err) => {
    ...
    let cloud_requirements =
        cloud_requirements_loader(cloud_auth_manager, chatgpt_base_url, helios_home.clone());
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        helios_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, false)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
    ...
    let (initial_operation, prompt_summary) = match (command, prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = helios_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    while let Some(envelope) = rx.recv().await {
        ...
        if let EventMsg::McpStartupUpdate(update) = &event.msg
            ...
            && let helios_protocol::protocol::McpStartupStatus::Failed { error } = &update.status
        {
            error_seen = true;
            eprintln!(
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        match helios_core::RolloutRecorder::find_latest_thread_path(
            config,
            1,
            None,
            helios_core::ThreadSortKey::UpdatedAt,
            &[],
        ...
        if Uuid::parse_str(id_str).is_ok() {
            let path = find_thread_path_by_id_str(&config.helios_home, id_str).await?;
            Ok(path)
        } else {
            let path = find_thread_path_by_name_str(&config.helios_home, id_str).await?;
            Ok(path)
        }
    } else {
        Ok(None)
