...
use helios_core::OLLAMA_OSS_PROVIDER_ID;
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(helios_core::spawn::HELIOS_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
                "{} is set; skipping test_fetch_models_happy_path",
                helios_core::spawn::HELIOS_SANDBOX_NETWORK_DISABLED_ENV_VAR
            );
            return;
        }
