...
//!
//! If the keyring is not available or fails, we fall back to HELIOS_HOME/.credentials.json which is consistent with other coding CLI agents.
...
use helios_keyring_store::DefaultKeyringStore;
use helios_keyring_store::KeyringStore;
use rmcp::transport::auth::AuthorizationManager;
use tokio::sync::Mutex;

use helios_utils_home_dir::find_helios_home;
...
mod tests {
    ...
    use helios_keyring_store::tests::MockKeyringStore;
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create HELIOS_HOME temp dir");
            unsafe {
                std::env::set_var("HELIOS_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempCodexHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("HELIOS_HOME");
            }
        }
    }
