...
Note: This fork ships a binary named `codex-custom`. Anywhere these docs mention "Codex CLI" or show commands with `codex`, substitute `codex-custom` when using this repository's build.
...
## delegate_enforce_read_only

Controls how the experimental sub‑agent (`delegate_task` tool) executes shell commands when `allow_shell=true` is passed to it. By default, the sub‑agent always uses a strict read‑only sandbox, regardless of the session’s `sandbox_mode`.

```toml
# default: true
delegate_enforce_read_only = true
```

Set to `false` to let the sub‑agent inherit the session’s sandbox policy (e.g., `workspace-write`). This can improve fidelity when the sub‑agent needs to list or read files relative to the workspace in the same way as the main agent. It never bypasses approvals; the sub‑agent does not show approval prompts.
