...
impl ModelProviderInfo {
    ...
    /// Construct a full URL for an arbitrary endpoint under the provider's base URL.
    /// For example, endpoint "files" resolves to "{base}/files{?query}".
    pub(crate) fn get_url_for_endpoint(&self, auth: &Option<CodexAuth>, endpoint: &str) -> String {
        let default_base_url = if matches!(
            auth,
            Some(CodexAuth {
                mode: AuthMode::ChatGPT,
    ...
    /// Create a POST RequestBuilder for a specific endpoint (e.g., "files").
    pub async fn create_request_builder_for_endpoint<'a>(
        &'a self,
        client: &'a reqwest::Client,
        auth: &Option<CodexAuth>,
        endpoint: &str,
        ...
        let url = self.get_url_for_endpoint(&effective_auth, endpoint);
        let mut builder = client.post(url);
        if let Some(auth) = effective_auth.as_ref() {
            builder = builder.bearer_auth(auth.get_token().await?);
        }
