...
use crate::config_types::ImageDetail;
...
pub struct Config {
    ...
    /// Whether to expose the experimental delegate tool (sub-agent).
    pub include_delegate_tool: bool,

    /// The value for the `originator` header included with Responses API requests.
    pub internal_originator: Option<String>,

    /// Default `detail` to apply to input_image parts when not explicitly set.
    /// If `None` or `Some(Auto|None)`, the field is omitted.
    pub default_image_detail: Option<ImageDetail>,

    /// If true, attempt to upload local/base64 images to the Files API and
    /// reference them by `file_id` instead of embedding as data URLs.
    pub use_files_api_for_images: bool,

    /// Optional minimum byte size to trigger upload when using the Files API.
    /// If unset, all local/base64 images are uploaded.
    pub image_upload_min_bytes: Option<usize>,

    /// If true, sub-agent (delegate_task) shell execution is forced to read-only
    /// regardless of the session's sandbox policy. Defaults to true.
    pub delegate_enforce_read_only: bool,
}
...
pub struct ConfigToml {
    ...
    /// Default vision detail for input images ("low", "high", "auto", or "none").
    pub image_detail: Option<ImageDetail>,

    /// Promote local/base64 images to Files API uploads and send `file_id`.
    pub use_files_api_for_images: Option<bool>,

    /// Minimum size (in bytes) to upload; smaller images continue as data URLs.
    pub image_upload_min_bytes: Option<usize>,

    /// When true (default), delegate_task forces read-only sandbox for shell.
    pub delegate_enforce_read_only: Option<bool>,
}
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let config = Self {
            ...
            default_image_detail: cfg.image_detail,
            use_files_api_for_images: cfg.use_files_api_for_images.unwrap_or(false),
            image_upload_min_bytes: cfg.image_upload_min_bytes,
            delegate_enforce_read_only: cfg.delegate_enforce_read_only.unwrap_or(true),
        };
        Ok(config)
    }
