...
impl Prompt {
    ...
    pub(crate) fn get_formatted_input(&self) -> Vec<ResponseItem> {
        ...
        // Defensive deduplication: ensure we never send duplicate reasoning
        // items (same `id`) in a single request. The Responses API rejects
        // payloads where the same item id appears more than once.
        let mut seen_reasoning_ids: HashSet<String> = HashSet::new();
        input_with_instructions
            .into_iter()
            .filter(|item| match item {
                ResponseItem::Reasoning { id, .. } => seen_reasoning_ids.insert(id.clone()),
                _ => true,
            })
            .collect()
    }
