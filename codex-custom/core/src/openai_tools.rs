...
/// Create the `delegate_task` function tool (Responses API tool schema).
///
/// Arguments:
/// - instructions: string – system prompt for the sub-agent
/// - input: string – task/query for the sub-agent
/// - allow_shell: boolean – if true, sub-agent may use a shell tool (MVP: off by default)
/// - timeout_ms: number – optional shell timeout (ms)
pub(crate) fn create_delegate_tool() -> OpenAiTool {
    let mut props = BTreeMap::new();
    props.insert(
        "instructions".to_string(),
        JsonSchema::String {
            description: Some("System instructions for the sub-agent".to_string()),
        },
    );
    props.insert(
        "input".to_string(),
        JsonSchema::String {
            description: Some("User input/task for the sub-agent".to_string()),
        },
    );
    props.insert(
        "allow_shell".to_string(),
        JsonSchema::Boolean {
            description: Some("If true, allow a read-only shell for the sub-agent".to_string()),
        },
    );
    props.insert(
        "timeout_ms".to_string(),
        JsonSchema::Number {
            description: Some("Optional timeout (ms) for shell commands".to_string()),
        },
    );
    ...
    OpenAiTool::Function(ResponsesApiTool {
        ...
        description:
            "Run a one-shot sub-agent with its own instructions and return a concise report"
                .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties: props,
            required: Some(vec!["instructions".to_string(), "input".to_string()]),
            additional_properties: Some(false),
        },
    })
}
...
pub(crate) fn get_openai_tools(
    ...
    if config.delegate_tool {
        tools.push(create_delegate_tool());
    }
