...
mod tests {
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd codex-custom && rg --files"),
            vec![
                ParsedCommand::Unknown {
                    cmd: vec_str(&["cd", "codex-custom"]),
                },
                ParsedCommand::Search {
                    cmd: vec_str(&["rg", "--files"]),
                    query: None,
    ...
    fn pnpm_exec_vitest_is_unknown() {
        // From commands_combined: cd codex-custom-cli && pnpm exec vitest run tests/... --threads=false --passWithNoTests
        let inner = "cd codex-custom-cli && pnpm exec vitest run tests/file-tag-utils.test.ts --threads=false --passWithNoTests";
        assert_parsed(
            &shlex_split_safe(inner),
            vec![
                ParsedCommand::Unknown {
                    cmd: shlex_split_safe("cd codex-custom-cli"),
                },
                ParsedCommand::Unknown {
                    cmd: shlex_split_safe(
