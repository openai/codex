...
use crate::delegate_tool::handle_delegate_task;
...
pub(crate) struct Session {
    ...
    /// When true, delegate_task shell execution is forced to read-only sandbox
    /// regardless of the session sandbox policy. Defaults to true for safety.
    delegate_enforce_read_only: bool,
}

impl Session {
    pub(crate) fn environment_context(&self) -> crate::client_common::EnvironmentContext {
        crate::client_common::EnvironmentContext {
            cwd: self.cwd.clone(),
    ...
    pub(crate) fn sandbox_policy_ref(&self) -> &SandboxPolicy {
        &self.sandbox_policy
    }

    pub(crate) fn delegate_enforce_read_only(&self) -> bool {
        self.delegate_enforce_read_only
    }
    ...
    /// Expose a crate-visible helper to stream a raw prompt through the
    /// configured model client. Used by internal tools that run ad-hoc
    /// sub-requests without mutating conversation history.
    pub(crate) async fn model_stream(
        &self,
        prompt: &crate::client_common::Prompt,
    ) -> crate::error::Result<crate::client_common::ResponseStream> {
        self.client.clone().stream(prompt).await
    }

    /// Execute a command under a read-only sandbox and return its output.
    pub(crate) async fn exec_read_only(
        &self,
        params: crate::exec::ExecParams,
    ) -> crate::error::Result<crate::exec::ExecToolCallOutput> {
        use crate::exec::process_exec_tool_call;
        use crate::protocol::SandboxPolicy;
        let sandbox_type = crate::get_platform_sandbox().unwrap_or(crate::exec::SandboxType::None);
    ...
    /// Execute a command under the provided sandbox policy and return its output.
    pub(crate) async fn exec_with_policy(
        &self,
        params: crate::exec::ExecParams,
        policy: &crate::protocol::SandboxPolicy,
    ) -> crate::error::Result<crate::exec::ExecToolCallOutput> {
        use crate::exec::process_exec_tool_call;
        let sandbox_type = crate::get_platform_sandbox().unwrap_or(crate::exec::SandboxType::None);
        process_exec_tool_call(
    ...
    /// Build the default environment for subprocess execution according to the
    /// configured `shell_environment_policy`.
    pub(crate) fn default_exec_env(&self) -> std::collections::HashMap<String, String> {
        crate::exec_env::create_env(&self.shell_environment_policy)
    }
}
        ...
        match sub.op {
            ...
            Op::ConfigureSession {
                ...
                sess = Some(Arc::new(Session {
                    ...
                    delegate_enforce_read_only: config.delegate_enforce_read_only,
                }));
            ...
            Op::Compact { instructions } => {
                ...
                // If the user passed explicit focus instructions, append them to the
                // summarization base instructions in a clearly delimited section.
                let compact_instructions = match instructions
                    .as_ref()
                    .map(|s| s.trim())
                    .filter(|s| !s.is_empty())
                {
                    Some(focus) => format!(
                        "{base}\n\nAdditional focus for this summary (user-provided):\n{focus}\n",
                        base = SUMMARIZATION_PROMPT
                    ),
                    None => SUMMARIZATION_PROMPT.to_string(),
                };

                // Always run summarization as a dedicated task so the
                // summarization instructions are applied consistently even if
                // another task is currently running. `set_task` will abort any
                // existing task before starting this one.
                let items = vec![InputItem::Text {
                    text: "Start Summarization".to_string(),
                }];
                let task = AgentTask::compact(sess.clone(), sub.id, items, compact_instructions);
                sess.set_task(task);
            }
            Op::Shutdown => {
...
async fn run_task(sess: Arc<Session>, sub_id: String, input: Vec<InputItem>) {
    ...
    loop {
        ...
        // Record any pending user inputs (typed while the model was running)
        // into the conversation history so they appear in the transcript and
        // are included when we build the turn input from history below.
        //
        // IMPORTANT: Do not also append these to the `extra` vector passed to
        // `turn_input_with_history`, otherwise they will be duplicated in the
        // request payload, which can trigger 400 errors from the provider.
        sess.record_conversation_items(&pending_input).await;

        // Construct the input that we will send to the model. When using the
        // Responses or Chat APIs, the model needs the full conversation
        // history on each turn. Because we just recorded `pending_input` into
        // history, we do not need to (and must not) pass it again here.
        let turn_input: Vec<ResponseItem> = sess.turn_input_with_history(Vec::new());
...
async fn handle_function_call(
    ...
    match name.as_str() {
        ...
        "delegate_task" => handle_delegate_task(sess, sub_id, call_id, arguments).await,
        _ => {
