...
/// - `path`: Path to the matched entry (relative to the search directory).
/// - `is_dir`: Whether this match refers to a directory.
...
pub fn run(
    ...
    let matches: Vec<FileMatch> = raw_matches
        ...
        .map(|(score, path)| {
            ...
            let is_dir = search_directory.join(&path).is_dir();
            FileMatch {
                score,
                path,
                is_dir,
