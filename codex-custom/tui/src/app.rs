            ...
            match event {
                ...
                AppEvent::KeyEvent(key_event) => {
                    match key_event {
                        ...
                        // Attempt to attach an image from the system clipboard on Ctrl+V.
                        // Many terminals handle paste themselves and will not deliver this
                        // key event. When delivered, this provides a quick way to paste
                        // images as [image_N] placeholders.
                        KeyEvent {
                            code: KeyCode::Char('v'),
                            modifiers,
                            kind: KeyEventKind::Press,
                            ...
                            if let AppState::Chat { widget } = &mut self.app_state {
                                if widget.attach_image_from_clipboard() {
                                    self.app_event_tx.send(AppEvent::RequestRedraw);
                                }
                            }
                        }
                ...
                AppEvent::BranchToPastInput {
                    resume_path,
                    prefill_text,
                } => {
                    if let AppState::Chat { widget } = &mut self.app_state {
                        widget.resume_from_rollout(resume_path);
                        widget.set_composer_text(prefill_text);
                        self.app_event_tx.send(AppEvent::RequestRedraw);
                    }
                }
                ...
                AppEvent::DispatchCommand { command, args } => match command {
                    ...
                    SlashCommand::PasteImage => {
                        if let AppState::Chat { widget } = &mut self.app_state {
                            let attached = widget.attach_image_from_clipboard();
                            if attached {
                                self.app_event_tx.send(AppEvent::RequestRedraw);
                            } else {
                                // No image found: show a small note in history for clarity.
                                use ratatui::text::Line as RLine;
                                self.app_event_tx
                                    .send(AppEvent::InsertHistory(vec![RLine::from(
                    ...
                    SlashCommand::Compact => {
                        if let AppState::Chat { widget } = &mut self.app_state {
                            // Add a banner that echoes any user-provided focus text.
                            widget.add_compact_note(args.clone());
                            widget.clear_token_usage();
                            self.app_event_tx
                                .send(AppEvent::CodexOp(Op::Compact { instructions: args }));
                        }
                    }
                    SlashCommand::Quit => {
                        break;
                    ...
                    SlashCommand::Prompts => {
                        if let AppState::Chat { widget } = &mut self.app_state {
                            // Show the prompts info cell and open the manager popup.
                            widget.add_prompts_output();
                            widget.open_prompts_popup();
                        }
                    }
                ...
                AppEvent::ResumeSelected(path) => {
                    if let AppState::Chat { widget } = &mut self.app_state {
                        widget.resume_from_rollout(path);
                    }
                }
            }
        }
