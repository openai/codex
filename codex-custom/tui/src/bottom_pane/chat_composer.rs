...
use crate::bottom_pane::AttachedImage;
use crate::bottom_pane::ImageSource;
...
pub(crate) struct ChatComposer {
    ...
    has_focus: bool,
    has_queued: bool,
    // Image attachments captured via paste; tracked by placeholder token -> source
    attached_images: Vec<(String, ImageSource)>,
...
impl ChatComposer {
    pub fn new(
        ...
        // Prefer showing Shift+Enter for newline only when the terminal
        // supports keyboard enhancement flags (so Shift on Enter is
        // distinguishable). Otherwise, hint Ctrl+J which always works.
    ...
    /// Read an image from the system clipboard (if present) and attach it to
    /// the composer as a `[image_N]` placeholder backed by a `data:image/png`
    /// URL. Returns true on success.
    pub fn attach_image_from_clipboard(&mut self) -> bool {
        match Self::read_clipboard_image_data_url() {
            Ok(Some(data_url)) => {
                let placeholder = format!("[image_{}]", self.next_image_index);
                self.next_image_index += 1;
                self.textarea.insert_str(&placeholder);
                self.attached_images
                    .push((placeholder, ImageSource::DataUrl(data_url)));
                self.sync_command_popup();
                self.sync_file_search_popup();
                true
            }
            Ok(None) => {
                // No image available on clipboard.
                false
            }
            Err(_) => false,
        }
    ...
    /// Helper: read PNG data URL from clipboard if available.
    #[allow(clippy::expect_used)]
    fn read_clipboard_image_data_url() -> anyhow::Result<Option<String>> {
        ...
        // arboard returns raw 32‑bpp pixel data. Convert to an ImageBuffer and
        // encode as PNG. arboard does not expose stride/format here, so assume
        // tightly‑packed RGBA8 which matches common platforms.
        let width = img.width as u32;
        let height = img.height as u32;
        let rgba = img.bytes.into_owned();

        // Wrap as ImageBuffer and encode to PNG bytes.
        let buffer = image::ImageBuffer::<image::Rgba<u8>, _>::from_raw(width, height, rgba)
            .ok_or_else(|| anyhow::anyhow!("invalid clipboard image buffer"))?;
        let dynimg = image::DynamicImage::ImageRgba8(buffer);
    ...
    pub fn handle_paste(&mut self, pasted: String) -> bool {
        // If the paste looks like an image reference (file path/URL/data URL),
        // attach it and insert a compact placeholder instead of the full text.
        if let Some(src) = Self::parse_image_reference(&pasted) {
            let placeholder = format!("[image_{}]", self.next_image_index);
            self.next_image_index += 1;
            self.textarea.insert_str(&placeholder);
            self.attached_images.push((placeholder, src));
            self.sync_command_popup();
            self.sync_file_search_popup();
            return true;
        }
    ...
    fn handle_key_event_with_slash_popup(&mut self, key_event: KeyEvent) -> (InputResult, bool) {
        ...
        match key_event {
            ...
            KeyEvent {
                ...
                if let Some(cmd) = popup.selected_command() {
                    // Extract optional args after the slash-command on the first line.
                    let first_line = self
                        .textarea
                        .text()
                        .lines()
                        ...
                        .unwrap_or("")
                        .to_string();
                    let args = Self::extract_command_args(&first_line, cmd.command());
    ...
    /// Extract the argument text that follows a slash command on the first line.
    /// Example: "/compact   focus on tests" => Some("focus on tests")
    fn extract_command_args(first_line: &str, command: &str) -> Option<String> {
        let s = first_line.trim_start();
        let after_slash = match s.strip_prefix('/') {
            Some(r) => r,
            None => return None,
        };
        let rest = match after_slash.strip_prefix(command) {
            Some(r) => r,
            None => return None,
        };
    ...
    fn handle_key_event_with_file_popup(&mut self, key_event: KeyEvent) -> (InputResult, bool) {
        ...
        match key_event {
            ...
            | KeyEvent {
                ...
                if let Some((sel_path, is_dir)) =
                    popup.selected_match().map(|m| (m.path.clone(), m.is_dir))
                {
                    // Drop popup borrow before using self mutably again.
                    if is_dir {
                        self.insert_selected_directory(&sel_path);
                    } else {
    ...
    /// Replace the active `@token` with a directory selection.
    ///
    /// Differs from `insert_selected_path` by preserving the leading '@' and
    /// omitting the trailing space so the token remains active, allowing the
    /// user to continue narrowing the search within the selected directory.
    fn insert_selected_directory(&mut self, path: &str) {
        let cursor_offset = self.textarea.cursor();
        let text = self.textarea.text();
        ...
        // Replace the slice `[start_idx, end_idx)` with the chosen directory
        // path, preserving the leading '@' to keep the token active.
        let mut new_text =
            String::with_capacity(text.len() - (end_idx - start_idx) + path.len() + 1);
        new_text.push_str(&text[..start_idx]);
        new_text.push('@');
        ...
        self.textarea.set_text(&new_text);
        let new_cursor = start_idx.saturating_add(1).saturating_add(path.len());
        self.textarea.set_cursor(new_cursor);
    }
    ...
    fn handle_input_basic(&mut self, input: KeyEvent) -> (InputResult, bool) {
        ...
        // Also drop any image attachment whose placeholder was deleted.
        self.attached_images
            .retain(|(ph, _)| text_after.contains(ph));

        (InputResult::None, true)
    }

    /// Replace the entire text in the textarea and place the cursor at the
    /// end of the text so the user can immediately continue typing.
    ...
    /// Check whether `s` is an image reference the composer should attach.
    /// Supported forms:
    /// - Absolute/relative file path with a common image extension
    /// - file:/// URL
    /// - http(s):// URL with a common image extension
    /// - data:image/...;base64,... URL
    fn parse_image_reference(s: &str) -> Option<ImageSource> {
        let trimmed = s.trim().trim_matches(['"', '\'']);

        // data URL
        if trimmed.starts_with("data:image/") && trimmed.contains(";base64,") {
            return Some(ImageSource::DataUrl(trimmed.to_string()));
        }
        ...
        if let Some(rest) = trimmed.strip_prefix("file://") {
            ...
            if Self::is_supported_image_path(&path) {
                return Some(ImageSource::Local(path));
            }
        }

        // http(s) URL – accept only if extension looks like an image
        if trimmed.starts_with("http://") || trimmed.starts_with("https://") {
            if Self::has_image_extension(trimmed) {
    ...
    fn has_image_extension(url_or_path: &str) -> bool {
    ...
    fn is_supported_image_path(path: &std::path::Path) -> bool {
        ...
        {
            Some(ext) => matches!(ext.as_str(), "png" | "jpg" | "jpeg" | "webp" | "gif"),
            None => false,
        }
    }

    /// Consume image attachments referenced in `text` in the order their
    /// placeholders appear. Unreferenced attachments (if any) are dropped.
    pub(crate) fn take_image_attachments_in_text_order(
        &mut self,
        text: &str,
    ) -> Vec<AttachedImage> {
        let mut ordered: Vec<AttachedImage> = Vec::new();
        let mut i = 0usize;
        while let Some(start) = text[i..].find("[image_") {
            let global_start = i + start;
            // find closing bracket
            if let Some(close_rel) = text[global_start..].find(']') {
                let end = global_start + close_rel + 1;
                let placeholder = &text[global_start..end];
                if let Some((idx, (_, src))) = self
                    .attached_images
                    .iter()
                    .enumerate()
                    .find(|(_, (ph, _))| ph == placeholder)
                {
                    let src = src.clone();
                    ordered.push(AttachedImage { source: src });
                    // remove from list to prevent duplicates
                    self.attached_images.remove(idx);
                }
                i = end; // continue after this token
            } else {
                break;
            }
        }
        // If everything was consumed, reset the counter; otherwise, keep the sequence
    ...
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        match &self.active_popup {
            ...
            ActivePopup::None => {
                ...
                // Add quick shortcut hint for past inputs.
                hint.push(Span::from("   "));
                hint.push("2x Esc".set_style(key_hint_style));
                hint.push(Span::from(" past inputs"));
                ...
                // Subtle queued tag when a message is queued to auto-send.
                if self.has_queued {
                    hint.push(Span::from("   "));
                    hint.push(
                        Span::from("queued").style(
                            Style::default()
                                .fg(Color::Gray)
                ...
                // Show current workflow (approval + sandbox) and hint to cycle it.
                hint.push(Span::from("   "));
                hint.push("⏵⏵".set_style(key_hint_style));
                hint.push(Span::from(" "));
                hint.push(
                    Span::from(self.workflow_label())
                        .style(Style::default().add_modifier(Modifier::DIM)),
                );
                hint.push(Span::from(" ("));
                hint.push("Shift+Tab".set_style(key_hint_style));
                hint.push(Span::from(" to cycle)"));
...
mod tests {
    ...
    fn handle_paste_small_inserts_text() {
        ...
        let mut composer = ChatComposer::new(
            ...
            codex_core::protocol::SandboxPolicy::new_workspace_write_policy(),
        );
    ...
    fn slash_init_dispatches_command_and_does_not_submit_literal_text() {
        ...
        match rx.try_recv() {
            Ok(AppEvent::DispatchCommand { command: cmd, args }) => {
                assert_eq!(cmd.command(), "init");
                assert!(args.is_none(), "/init should not carry args");
            }
