...
- For unattended runs, prefer bounded auto-continue instead of unbounded loops.
- Use `--auto-continue` together with at least one stop bound: `--auto-continue-max-turns <n>` and/or `--auto-continue-max-duration <seconds>`.
- Keep the follow-up prompt explicit when needed with `--auto-continue-prompt <text>`; default is `"continue"`.
- Treat `--auto-continue` as execution control only; AGENTS guidance may influence behavior, but does not replace runtime stop bounds.
