...
describe("BaseAdapter", () => {
  ...
  describe("createClient", () => {
    ...
    test("includes additional headers from auth provider", async () => {
      ...
      const authProvider = new MockAuthProvider(false, "Bearer test-key", additionalHeaders);
      const adapter = new TestAdapter(testConfig, authProvider);
      
      const client = await adapter.createClient();
      
      expect((client as any).config.defaultHeaders).toMatchObject(additionalHeaders);
    });
  ...
  describe("validateConfiguration", () => {
    test("default implementation does nothing", async () => {
      ...
      // Should not throw
      await expect((adapter as any).validateConfiguration()).resolves.toBeUndefined();
    });
  });
});