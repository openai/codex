...
import type { ToolCallResult } from "./tool-executor";
...
import { McpManager } from "../mcp-manager"; // ADD McpManager import
...
export class AgentLoop {
  ...
  // MCP SDK client instance and connection promise
  private mcpManager: McpManager; // ADD McpManager property
  ...
  public terminate(): void {
    ...
    this.cancel();
    this.mcpManager?.dispose(); // Dispose MCP Manager
  }
  ...
  constructor({
    ...
    this.mcpManager.initialize().catch((err) => {
      log(`[AgentLoop] Failed to initialize McpManager: ${err}`);
      // Decide if this should be a fatal error or just log
    });
      ...
      while (turnInput.length > 0) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            // Generate system prompt with MCP tools if available
            const systemPrefix = generatePrefix(this.mcpManager);

            const mergedInstructions = [systemPrefix, this.instructions]
              .filter(Boolean)
              .join("\n");
            if (isLoggingEnabled()) {
              log(
            ...
            stream = await this.oai.responses.create({
              ...
              tools: [
                ...
                // Add all MCP tools as functions
                ...this.mcpManager.getAvailableTools().map((tool) => {
                  // Warn if schema is incomplete
                  if (
                    !tool.parameters ||
                    ...
                    log(
                      `[AgentLoop] Warning: MCP tool "${tool.name}" is missing a complete JSON Schema (properties or required fields). The LLM may not prompt for missing parameters.`,
                    );
                  }
                  return {
  ...
  private async processEventsWithoutStreaming(
    ...
    for (const item of output) {
      if (item.type === "function_call") {
        ...
        // (Removed: MCP registry-based tool call post-processing)
      }
...
// Function to generate the system prefix with MCP tools
function generatePrefix(mcpManager?: McpManager) {
  const mcpToolsDetails =
    mcpManager
      ?.getAvailableTools?.()
      ?.map?.((tool) => {
        return `${tool.name} - ${
  ...
  return `You are operating as and within the Codex CLI, a terminal-based agentic coding assistant built by OpenAI. It wraps OpenAI models to enable natural language interaction with a local codebase. You are expected to be precise, safe, and helpful.
