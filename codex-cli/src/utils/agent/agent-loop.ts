...
const shellToolDescription = `Runs a shell command, and returns its output.

In addition, for the purposes of this task, you can call this function with \`apply_patch\` command as input.  \`apply_patch\` effectively allows you to execute a diff/patch against a file, but the format of the diff specification is unique to this task, so pay careful attention to these instructions. To use the \`apply_patch\` command, you should pass a message of the following structure as "input":
