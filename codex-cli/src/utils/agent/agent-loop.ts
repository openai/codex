...
import { getAvailableTools } from "./tool-integration.js";
import { loadPrompts, adaptPromptForProvider } from "../../prompts/loader.js";
import {
  OPENAI_TIMEOUT_MS,
  OPENAI_ORGANIZATION,
  OPENAI_PROJECT,
...
// Base shell tool is defined in getAvailableTools
      ...
      while (turnInput.length > 0) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            // Load prompts and adapt for the current provider
            const prompts = loadPrompts({
              cwd: process.cwd(),
              provider: this.provider,
              model: this.model,
              supportsBrowserUse: true, // Default to true to enable browser actions
              userInstructions: this.instructions || "",
            });

            // Get the system prompt adapted for the current provider
            const systemPrompt = adaptPromptForProvider(
            ...
            log(
              `instructions (length ${systemPrompt.length}): ${systemPrompt.substring(0, 100)}...`,
            );
            ...
            stream = await responseCall({
              ...
              tools: getAvailableTools(this.config || {}, true),
...
// Using system prompt from the prompt loader instead of hard-coded prefix
