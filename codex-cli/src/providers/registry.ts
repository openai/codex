...
const authFactories: Record<AuthType, AuthProviderFactory> = {
  [AuthType.API_KEY]: (config) =>
    new ApiKeyAuthProvider(getApiKey(config.id), config.name),
...
const adapterFactories = {
  standard: (config: ProviderConfig, auth: AuthProvider) =>
    new StandardAdapter(config, auth),
  azure: (config: ProviderConfig, auth: AuthProvider) =>
    new AzureAdapter(config, auth),
  vertex: (config: ProviderConfig, auth: AuthProvider) =>
    new VertexAdapter(config, auth),
};
...
export async function createProviderAdapter(
  ...
  const authProvider =
    authProviderOrPromise instanceof Promise
      ? await authProviderOrPromise
      : authProviderOrPromise;
