...
import { handleSecurityMode } from "./security/cli-integration";
...
const cli = meow(
  ...
  Security options
    --security-mode            Launch in security mode as Adversys Cyber Agent
    --offensive                Enable adversary-emulation mode (Cyber Kill Chain)
    --predator                 Enable Predator Mode (fully autonomous adversary)
    --target <url/ip>          Target for security testing (with --security-mode)
    --session <name>           Session name for security testing (with --security-mode)
    --ipc-socket <path>        Path to IPC socket for shell-agent integration (security mode only)
    --no-install               Disable auto-installation of missing security tools
...
`,
  {
    ...
    flags: {
      ...
      securityMode: {
        type: "boolean",
        description: "Launch in security mode as Adversys Cyber Agent",
      },
      offensive: {
        type: "boolean",
        description: "Enable adversary-emulation mode (Cyber Kill Chain)",
      },
      predator: {
        type: "boolean",
        description: "Enable Predator Mode (fully autonomous adversary)",
      },
      playbook: {
        type: "string",
        description: "Path to a YAML/JSON playbook file (only with --security-mode --predator)",
      },
      dryRun: {
        type: "boolean",
        aliases: ["dry-run"],
        description: "Validate and list playbook steps without executing HTTP calls",
      },
      target: {
        type: "string",
        description: "Target for security testing (with --security-mode)",
      },
      session: {
        type: "string",
        description: "Session name for security testing (with --security-mode)",
      },
      ipcSocket: {
        type: "string",
        description: "Path to IPC socket for shell-agent integration",
      },
      generatePlaybook: {
        type: "boolean",
        aliases: ["generate-playbook"],
        description: "Interactive LLM-driven playbook generation (Predator mode only)",
      },
      explain: {
        type: "boolean",
        description: "With 'playbook lint', show a natural-language summary of each step",
      },
      // Auto-install control
      noInstall: {
        type: "boolean",
        description: "Disable auto-installation of missing security tools",
        aliases: ["no-install"],
      },
    },
  },
);
// DEBUG: log raw CLI invocation
console.log("[DEBUG][Main] Received args:", process.argv.slice(2));
console.log("[DEBUG][Main] cli.input:", cli.input);
console.log("[DEBUG][Main] cli.flags:", cli.flags);
...
// Handle 'shell-agent' subcommand before security mode
if (cli.input[0] === "shell-agent") {
    const sock = cli.flags.ipcSocket as string;
    if (!sock) {
      console.error('Usage: codex shell-agent --ipc-socket <path>');
      process.exit(1);
    }
    const { runShellAgent } = await import('./security/shell-agent');
  await runShellAgent(sock);
  // shell-agent process will remain alive
}
// Handle 'playbook lint' subcommand
if (cli.input[0] === 'playbook' && cli.input[1] === 'lint') {
  const file = cli.input[2];
  if (!file) {
    console.error('Usage: codex playbook lint <file>');
    process.exit(1);
  }
  try {
    const { lintPlaybook } = await import('./security/playbook/lint');
    await lintPlaybook(file, Boolean(cli.flags.explain));
    process.exit(0);
  } catch (err: any) {
    console.error(`Error linting playbook: ${err.message}`);
    process.exit(1);
  }
}
...
if (await handleSecurityMode(cli, prompt, config)) {
  process.exit(0);
}
