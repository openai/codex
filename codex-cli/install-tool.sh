...
# 3. Install whatweb (fallback to gem if brew formula missing)
if ! command -v whatweb &>/dev/null; then
  echo "Attempting to install whatweb via Homebrew…"
  if hb install whatweb; then
    echo "whatweb installed via Homebrew."
  else
    echo "Homebrew formula for whatweb not found; installing via RubyGems…"
