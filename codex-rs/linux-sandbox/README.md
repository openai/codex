...
- In managed proxy mode, the helper creates an isolated network namespace before
  wiring an internal TCP->UDS->TCP routing bridge so tool traffic reaches only
  configured proxy endpoints.
- For full-disk-write policies in managed proxy mode, the helper skips the
  outer bubblewrap re-exec and enters the inner stage directly.
