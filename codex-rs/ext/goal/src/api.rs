...
impl GoalService {
    ...
    pub(crate) async fn replace_goal_for_turn_start(
        &self,
        state_db: &codex_state::StateRuntime,
        thread_id: ThreadId,
        objective: &str,
        ...
        let previous_state_goal = state_db
            .thread_goals()
            .get_thread_goal(thread_id)
            .await
            .map_err(|err| {
        ...
        let goal = match state_db
            ...
            Err(err) => {
                if let Some(runtime) = runtime.as_ref() {
                    runtime.restore_goal_after_failed_mutation(previous_state_goal.as_ref());
                }
                return Err(err);
            }
        };
        fill_empty_thread_preview_if_possible(state_db, thread_id, &goal).await;
        if let Some(runtime) = runtime.as_ref() {
            runtime.apply_initial_goal_set(&goal, previous_goal.as_ref());
        }
        Ok(GoalSetOutcome {
            goal: protocol_goal_from_state(goal.clone()),
            state_goal: goal,
