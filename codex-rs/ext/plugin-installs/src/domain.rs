...
pub struct RequestPluginInstallsMeta<'a> {
    ...
    #[serde(flatten)]
    legacy_single_entry: Option<RequestPluginInstallLegacyMeta<'a>>,
}

#[derive(Debug, Serialize, PartialEq, Eq)]
struct RequestPluginInstallLegacyMeta<'a> {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    app_connector_ids: Option<&'a [String]>,
}
...
fn build_request_plugin_installs_meta<'a>(
    ...
    RequestPluginInstallsMeta {
        ...
        entries,
        categories,
        legacy_single_entry: match resolved_entries {
            [entry] => Some(build_request_plugin_install_legacy_meta(entry)),
            _ => None,
        },
    }
}

fn build_request_plugin_install_legacy_meta(
    entry: &RequestPluginInstallResolvedPickerEntry,
) -> RequestPluginInstallLegacyMeta<'_> {
    let tool = &entry.tool;
    ...
    RequestPluginInstallLegacyMeta {
        tool_type: tool.tool_type(),
        suggest_reason: discoverable_tool_description(tool).unwrap_or_else(|| tool.name()),
        tool_id: tool.id(),
        tool_name: tool.name(),
