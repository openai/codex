...
impl HttpClientFactory {
    ...
    fn cached_proxy_route(&self, request_url: &str) -> Option<OutboundProxyRoute> {
        ...
        cached_system_proxy_decision(&request_url)
            .map(|decision| route_from_system_decision(&ProcessEnv, &origin, decision))
    }
...
fn resolve_proxy_route(
    ...
    route_from_system_decision(env, &origin, resolve_system_proxy(&request_url, &origin))
}
...
fn route_from_system_decision(
    ...
    match decision {
        ...
        SystemProxyDecision::Unavailable { .. } => resolve_env_proxy_route(env, origin),
    }
}

fn resolve_env_proxy_route(env: &dyn EnvSource, origin: &RequestOrigin) -> OutboundProxyRoute {
