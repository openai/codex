...
async fn run_command_under_sandbox(
    ...
    let env = create_env(&config.effective_permissions.shell_environment_policy, None);
    ...
    if let SandboxType::Windows = sandbox_type {
        #[cfg(target_os = "windows")]
        {
            ...
            let policy_str =
                serde_json::to_string(config.effective_permissions.sandbox_policy.get())?;
    ...
    let network_proxy = match config.effective_permissions.network.as_ref() {
        Some(spec) => Some(
            spec.start_proxy()
                .await
