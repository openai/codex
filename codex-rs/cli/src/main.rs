...
#[cfg(target_os = "macos")]
type SandboxArgs = codex_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type SandboxArgs = codex_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type SandboxArgs = codex_cli::WindowsCommand;
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            if let Some(os) = sandbox_cli
                .command
                .first()
                .filter(|arg| matches!(arg.as_str(), "linux" | "macos" | "windows"))
            {
                anyhow::bail!(
                    "`codex sandbox` no longer takes an OS subcommand. Remove `{os}` and run `codex sandbox [COMMAND]...`."
                );
            }
            #[cfg(target_os = "macos")]
