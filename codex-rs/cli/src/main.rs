...
async fn run_exec_server_command(
    ...
    if let Some(base_url) = cmd.remote {
        ...
        let config =
            load_exec_server_config(root_config_overrides, config_profile, cmd.strict_config)
                .await?;
        let auth_provider =
    ...
    if cmd.strict_config {
        let _ = load_exec_server_config(root_config_overrides, config_profile, cmd.strict_config)
            .await?;
    }
    let listen_url = cmd
        .listen
