...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            let profile_v2 = sandbox_cli
                .config_profile_v2
                .as_ref()
                .or(interactive.config_profile_v2.as_ref());
            let loader_overrides = loader_overrides_for_profile(profile_v2)?;
            prepend_config_flags(
...
mod tests {
    ...
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        ...
        assert_eq!(
            profile_v2_for_args(&["codex", "--profile", "work", "sandbox"])
                .expect("sandbox supports profile-v2")
                .as_deref(),
            Some("work")
        );
    }
    ...
    fn sandbox_parses_profile_v2() {
        ...
        assert_eq!(command.config_profile_v2.as_deref(), Some("work"));
        assert_eq!(command.command, vec!["echo"]);
    }
