//! Builds the `/status` response card rendered in the transcript.
//!
//! This module assembles a status snapshot from config, auth, token usage, and
//! rate limit data, then renders it as a bordered [`HistoryCell`]. It owns the
//! presentation details: label ordering, width-sensitive formatting, and which
//! fields appear for a given account type.
//!
//! The card does not fetch data, mutate config, or evaluate models. It assumes
//! all inputs are already in-memory snapshots and focuses on turning those
//! snapshots into stable, readable lines within the available width.
...
/// Context-window usage details for the model used in the current session.
#[derive(Debug, Clone)]
struct StatusContextWindowData {
    /// Percentage of tokens still available in the model context window.
    percent_remaining: i64,
    /// Tokens currently consumed within the model context window.
    tokens_in_context: i64,
    /// Total token capacity for the model context window.
    window: i64,
}

/// Aggregate token usage breakdown presented in the status card.
#[derive(Debug, Clone)]
pub(crate) struct StatusTokenUsageData {
    /// Blended total across cached and uncached usage.
    total: i64,
    /// Input tokens sent to the model (excluding cached segments).
    input: i64,
    /// Output tokens generated by the model.
    output: i64,
    /// Optional context window snapshot tied to the active model.
    context_window: Option<StatusContextWindowData>,
}

/// Render-ready status data captured for the `/status` history cell.
#[derive(Debug)]
struct StatusHistoryCell {
    /// Display name for the current model, already normalized for presentation.
    model_name: String,
    /// Additional model detail tags derived from config entries.
    model_details: Vec<String>,
    /// Working directory used for the current session.
    directory: PathBuf,
    /// Approval mode string pulled from config summary entries.
    approval: String,
    /// Human-readable sandbox policy summary.
    sandbox: String,
    /// Summary line describing active Agents.md configuration.
    agents_summary: String,
    /// Optional model provider label, omitted when using default OpenAI.
    model_provider: Option<String>,
    /// Optional account display data tied to the authenticated user.
    account: Option<StatusAccountDisplay>,
    /// Optional session identifier displayed in the card.
    session_id: Option<String>,
    /// Optional session identifier for the fork source.
    forked_from: Option<String>,
    /// Token usage breakdown for the current session.
    token_usage: StatusTokenUsageData,
    /// Rate limit data snapshot for the current account and model.
    rate_limits: StatusRateLimitData,
}

/// Builds the composite `/status` output (command echo plus status card).
...
impl StatusHistoryCell {
    /// Captures a presentation snapshot of config, account, token, and limit data.
    #[allow(clippy::too_many_arguments)]
    fn new(
        config: &Config,
        auth_manager: &AuthManager,
    ...
    /// Formats the token usage summary line for the main card body.
    ...
    /// Formats the context window summary line when available.
    ...
    /// Builds the rate-limit section lines for the current snapshot.
    fn rate_limit_lines(
        &self,
    ...
    /// Expands each rate limit row into one or more display lines.
    ...
    /// Collects label text in display order for width alignment.
...
impl HistoryCell for StatusHistoryCell {
    /// Renders the status card within the provided width.
    ///
    /// The layout is built in phases: header and usage note, configurable fields,
    /// token usage (when applicable), and rate limit lines. The content is then
    /// width-trimmed and wrapped in a border sized to the widest visible line.
...
/// Formats a model provider string, omitting defaults when using OpenAI.
...
/// Sanitizes provider base URLs by stripping credentials and fragments.
