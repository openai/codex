...
//! [`BottomPaneView`]s (popups/modals), and footer-only UI such as status banners, queued messages,
//! and transcript hints. Modal views temporarily replace the composer as the active input surface,
//! but the composer remains alive so draft input returns unchanged when the modal is dismissed.
//!
...
///
/// The pane also owns footer-level status UI and queued-message banners so these elements remain
/// consistent regardless of which view is active.
...
impl BottomPane {
    ...
    /// Update transcript scrolled/selection state shown in the composer footer.
    ///
    /// This is a lightweight bridge from the transcript view so the footer can show navigation
    /// hints, selection ranges, and copy confirmation while the composer still owns input.
