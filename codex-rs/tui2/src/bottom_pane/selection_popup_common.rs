...
//! on the widest visible name. Selected rows are restyled with a cyan bold
//! highlight so the same rendering logic applies to both active and inactive
//! entries. Height calculations mirror the wrapping rules used at render time
//! so callers can size popups without reimplementing layout logic.
//!
...
///
/// When `wrap_indent` is not set, rows with descriptions align their wrapped
/// content under the shared description column.
...
/// indices are bolded, and the description is dimmed and aligned to `desc_col`.
