...
pub struct Cli {
    ...
    // Internal controls set by the top-level `codexel resume` subcommand.
    // These are not exposed as user flags on the base `codexel` command.
    #[clap(skip)]
    ...
    /// top-level `codexel resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
