    ...
    pub(crate) fn add_connectors_output(&mut self) {
        if !self.connectors_enabled() {
            self.add_info_message(
                ...
                Some("Enable the connectors feature to use /apps.".to_string()),
            );
            return;
        }
    ...
    fn open_connectors_popup(&mut self, connectors: &[connectors::ConnectorInfo]) {
        ...
        for connector in connectors {
            ...
            let mut item = SelectionItem {
                name: connector_label.clone(),
                description: Some(description),
                search_value: Some(search_value),
                ..Default::default()
            };
            if connector.is_accessible {
                let insert_text = format!("{connector_label} connector");
                let actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
                    tx.send(AppEvent::InsertComposerText(insert_text.clone()));
                })];
                item.actions = actions;
                item.dismiss_on_select = true;
            } else if let Some(install_url) = connector.install_url.clone() {
                ...
                item.actions = vec![Box::new(|tx| {
                    tx.send(AppEvent::InsertHistoryCell(Box::new(
                        history_cell::new_info_event("Install link unavailable.".to_string(), None),
                    )));
                })];
                item.dismiss_on_select = true;
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Select an app to insert it or view install instructions.".to_string()),
            footer_hint: Some(standard_popup_hint_line()),
            items,
            is_searchable: true,
