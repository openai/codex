...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    ...
    // Keep the elevated sandbox user's SID in the restricted token when possible because
    // it helps with owner-scoped IPC access, but retry without that extra restriction on
    // hosts where the nested CreateProcessAsUserW call fails with ERROR_NO_SUCH_LOGON_SESSION.
