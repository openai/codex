...
/// sandboxed command; split policies that opt into platform defaults carry that
/// explicitly with the override.
...
pub fn resolve_windows_restricted_token_filesystem_overrides(
    ...
    if !windows_sandbox_active || windows_sandbox_level == WindowsSandboxLevel::Elevated {
        return Ok(None);
    }
    ...
    let additional_deny_read_paths =
        crate::resolve_windows_deny_read_paths(&file_system_sandbox_policy, sandbox_policy_cwd)?;
    ...
    let legacy_projection = codex_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        permission_profile,
...
pub fn resolve_windows_elevated_filesystem_overrides(
    ...
    if !windows_sandbox_active || !use_windows_elevated_backend {
        return Ok(None);
    }
