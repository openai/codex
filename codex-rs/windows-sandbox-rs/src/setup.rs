...
fn run_setup_refresh_inner(
    ...
    let exe = find_setup_exe(request.codex_home, Some(&sbx_dir));
...
fn find_setup_exe(codex_home: &Path, log_dir: Option<&Path>) -> PathBuf {
    resolve_helper_for_launch(HelperExecutable::SetupHelper, codex_home, log_dir)
}
...
fn run_setup_exe(
    ...
    let exe = find_setup_exe(codex_home, Some(&sandbox_dir(codex_home)));
