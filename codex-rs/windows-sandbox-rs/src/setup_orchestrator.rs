...
use crate::helper_materialization::is_windows_apps_path;
...
fn gather_helper_read_roots(codex_home: &Path) -> Vec<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = current_exe_parent_read_root_from_path(&exe)
    {
        roots.push(dir);
    }
...
fn current_exe_parent_read_root_from_path(exe: &Path) -> Option<PathBuf> {
    let dir = exe.parent()?;
    (!is_windows_apps_path(dir)).then(|| dir.to_path_buf())
}
...
mod tests {
    ...
    #[test]
    fn gather_helper_read_roots_skips_windows_apps_parent() {
        let codex_home = Path::new(r"C:\Users\example\.codex");
        let exe_path = Path::new(
            r"C:\Program Files\WindowsApps\OpenAI.Codex_1.0.0.0_x64__8wekyb3d8bbwe\codex.exe",
        );

        assert_eq!(None, current_exe_parent_read_root_from_path(exe_path),);
        assert_eq!(vec![helper_bin_dir(codex_home)], {
            let mut roots = Vec::new();
            roots.extend(current_exe_parent_read_root_from_path(exe_path));
            roots.push(helper_bin_dir(codex_home));
            roots
        });
    }
