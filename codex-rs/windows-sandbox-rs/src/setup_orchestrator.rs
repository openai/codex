...
use crate::helper_materialization::current_exe_helper_read_root;
...
fn run_setup_refresh_inner(
    ...
    let exe = resolve_setup_exe(codex_home);
...
fn gather_helper_read_roots(codex_home: &Path) -> Vec<PathBuf> {
    ...
    if let Some(dir) = current_exe_helper_read_root() {
        roots.push(dir);
    }
...
fn resolve_setup_exe(codex_home: &Path) -> PathBuf {
