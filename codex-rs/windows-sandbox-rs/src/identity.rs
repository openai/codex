...
pub fn require_logon_sandbox_creds(
    ...
    if identity.is_none() {
        ...
        if !allow_setup_bootstrap {
            return Err(anyhow!(
                "windows unelevated sandbox cannot auto-run elevated setup to enforce restricted network access; rerun Windows sandbox setup in elevated mode or enable network access"
            ));
        }
        run_elevated_setup(
