...
pub fn intersect_permission_profiles(
    ...
    let file_system = requested
        ...
        .map(|requested_file_system| {
            ...
            let requested_policy = FileSystemSandboxPolicy::from(&requested_file_system);
            let mut entries = Vec::new();
            if let Some(granted_file_system) = granted.file_system.as_ref() {
                for entry in &granted_file_system.entries {
                    if requested_policy.covers_entry_with_cwd(entry, cwd)
            ...
            for entry in &requested_file_system.entries {
                if granted_policy.covers_entry_with_cwd(entry, cwd) && !entries.contains(entry) {
                    entries.push(entry.clone());
                }
            }
            let mut capped_policy = FileSystemSandboxPolicy::restricted(entries.clone());
            for entry in requested_file_system.entries {
                if !entries.contains(&entry)
                    && capped_policy_exceeds_entry(&capped_policy, &entry, cwd)
                {
                    entries.push(entry);
                    capped_policy = FileSystemSandboxPolicy::restricted(entries.clone());
                }
            }
            FileSystemPermissions { entries }
        })
...
fn capped_policy_exceeds_entry(
    capped_policy: &FileSystemSandboxPolicy,
    entry: &FileSystemSandboxEntry,
    cwd: &Path,
) -> bool {
    match entry.access {
        FileSystemAccessMode::Write => false,
        FileSystemAccessMode::Read => capped_policy.covers_entry_with_cwd(
        ...
        FileSystemAccessMode::None => !capped_policy.covers_entry_with_cwd(entry, cwd),
    }
}
