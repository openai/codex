...
pub fn cleanup_legacy_managed_mitm_ca_private_key() -> Result<(), SandboxTransformError> {
    NetworkProxy::remove_legacy_mitm_ca_private_key()
        .map_err(|err| SandboxTransformError::ManagedMitmCaKeyCleanup(err.to_string()))
}
...
pub enum SandboxTransformError {
    ...
    ManagedMitmCaKeyCleanup(String),
    #[cfg(target_os = "linux")]
...
impl std::fmt::Display for SandboxTransformError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ManagedMitmCaKeyCleanup(err) => {
                write!(f, "failed to remove legacy managed MITM CA key: {err}")
            }
            #[cfg(target_os = "linux")]
...
impl std::error::Error for SandboxTransformError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            Self::ManagedMitmCaKeyCleanup(_) => None,
            #[cfg(target_os = "linux")]
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        cleanup_legacy_managed_mitm_ca_private_key()?;
