...
mod tests {
    ...
    /// 2. model_info.experimental_supported_tools contains "smart_edit"
    ...
    fn test_register_smart_edit_tool_feature_disabled() {
        ...
        let mut model_info =
            crate::models_manager::model_info::find_model_info_for_slug("test-model");
        model_info.experimental_supported_tools = vec!["smart_edit".to_string()];
        let mut features = Features::with_defaults();
        features.disable(Feature::SmartEdit);

        register_smart_edit_tool(&mut builder, &model_info, &features);
    ...
    fn test_register_smart_edit_tool_model_disabled() {
        ...
        let mut model_info =
            crate::models_manager::model_info::find_model_info_for_slug("test-model");
        model_info.experimental_supported_tools = vec![]; // no smart_edit
        let mut features = Features::with_defaults();
        ...
        assert!(
            !tools
                .iter()
                .any(|t| matches!(&t.spec, ToolSpec::Function(f) if f.name == "smart_edit")),
            "Should not register when model doesn't support it"
        );
    }
    ...
    fn test_register_smart_edit_tool_both_enabled() {
        ...
        assert!(
            tools
                .iter()
                .any(|t| matches!(&t.spec, ToolSpec::Function(f) if f.name == "smart_edit")),
            "Should register when both feature and model enabled"
        );
    }
}
