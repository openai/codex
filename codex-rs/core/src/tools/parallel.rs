...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call_with_source(
        ...
        let tool_call_timing =
            ToolCallTimingLog::capture(started, &session.thread_id, &turn.sub_id, &call, &source);
        let execution_started_at = tool_call_timing
            .as_ref()
        ...
        async move {
            let _tool_call_timing = tool_call_timing;
            tokio::select! {
...
impl Drop for ToolCallTimingLog {
    fn drop(&mut self) {
        if !tracing::enabled!(
            target: "codex_core::tools::parallel",
            tracing::Level::INFO
