...
#[tokio::test]
async fn multi_agent_v2_reaper_closes_stale_task_name_edge_before_releasing_path() {
    let (mut session, mut turn) = make_session_and_context().await;
    let mut config = (*turn.config).clone();
    config.agent_max_threads = Some(1);
    config
    ...
    SpawnAgentHandlerV2::default()
        ...
        .await
        .expect("spawn_agent should close the stale edge before reusing the task name");
    let replacement_id = session
        .services
        .agent_control
        .resolve_agent_reference(session.conversation_id, &turn.session_source, "worker")
        .await
        .expect("replacement worker path should resolve");
    assert_ne!(replacement_id, stale_agent_id);
    ...
    let open_children = state_db
        ...
        .await
        .expect("open children should load");
    assert_eq!(open_children, vec![replacement_id]);
    let closed_children = state_db
        .list_thread_spawn_children_with_status(
            root.thread_id,
            DirectionalThreadSpawnEdgeStatus::Closed,
        ...
        .await
        .expect("closed children should load");
    assert_eq!(closed_children, vec![stale_agent_id]);
