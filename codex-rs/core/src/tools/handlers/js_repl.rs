        ...
        for token in directive.split_whitespace() {
            let (key, value) = token.split_once('=').ok_or_else(|| {
                FunctionCallError::RespondToModel(format!(
                    "js_repl pragma expects space-separated key=value pairs (supported keys: timeout_ms, reset); got `{token}`"
                ))
            })?;
            match key {
                "timeout_ms" => {
                ...
                _ => {
                    return Err(FunctionCallError::RespondToModel(format!(
                        "js_repl pragma only supports timeout_ms and reset; got `{key}`"
                    )));
                }
            }
        }
...
mod tests {
    ...
    fn parse_freeform_args_rejects_unknown_key() {
        ...
        assert_eq!(
            err.to_string(),
            "js_repl pragma only supports timeout_ms and reset; got `nope`"
        );
    }
