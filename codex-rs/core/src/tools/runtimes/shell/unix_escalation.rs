...
impl CoreShellActionProvider {
    ...
    fn turn_escalation_permissions(&self) -> EscalationPermissions {
        EscalationPermissions::Permissions(EscalatedPermissions {
    ...
    fn effective_skill_escalation_permissions(
        &self,
        skill: &SkillMetadata,
    ) -> EscalationPermissions {
        Self::skill_escalation_permissions(skill)
            .unwrap_or_else(|| self.turn_escalation_permissions())
    }
    ...
    async fn determine_action(
        ...
        if let Some(approval) = approval {
            ...
            let permissions = approval
                .skill
                .as_ref()
                .map(|skill| self.effective_skill_escalation_permissions(skill));
        ...
        if let Some(skill) = self.find_skill(program).await {
            ...
            return self
                .process_decision(
                    ...
                    Some(self.effective_skill_escalation_permissions(&skill)),
                    decision_source,
                )
                .await;
        }
