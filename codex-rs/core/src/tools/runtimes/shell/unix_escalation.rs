...
pub(super) async fn try_run_zsh_fork(
    ...
    // Build the script from the original shell argv, not the sandbox-transformed
    // command. `attempt.env_for()` may wrap the command with `sandbox-exec` on
    // macOS, and passing those wrapper flags (`-p`, `-D...`) through zsh breaks
    // the zsh-fork path before subcommand approval runs.
...
fn extract_shell_script(command: &[String]) -> Result<String, ToolError> {
    match command {
        [_, flag, script, ..] if flag == "-c" || flag == "-lc" => Ok(script.clone()),
