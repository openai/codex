...
#[cfg(unix)]
pub(crate) fn prepend_path_entry(env: &mut HashMap<String, String>, path_entry: &str) -> String {
...
#[cfg(unix)]
pub(crate) fn zsh_fork_bin_dir_for_path(shell_zsh_path: &Path) -> Option<String> {
    shell_zsh_path
        .parent()
...
pub(crate) fn apply_zsh_fork_path_prepend(
    ...
    let Some(zsh_bin_dir) = zsh_fork_bin_dir_for_path(shell_zsh_path) else {
        return;
    };
    let updated_path = prepend_path_entry(env, &zsh_bin_dir);
