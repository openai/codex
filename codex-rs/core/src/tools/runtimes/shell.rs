...
use codex_shell_command::bash::{parse_shell_lc_plain_commands, parse_shell_lc_single_command_prefix};
#[cfg(unix)]
...
#[cfg(unix)]
use codex_shell_escalation::unix::escalate_server::{
    run_escalate_server,
...
impl Approvable<ShellRequest> for ShellRuntime {
    ...
    fn start_approval_async<'a>(
        ...
        let reason = ctx.retry_reason.clone().or_else(|| req.justification.clone());
...
impl ShellActionProvider for CoreShellActionProvider {
    async fn determine_action(
        ...
        let command = std::iter::once(file.to_string_lossy().to_string())
            ...
            .collect::<Vec<_>>();
        let (commands, used_complex_parsing) = if let Some(commands) =
...
impl ToolRuntime<ShellRequest, ExecToolCallOutput> for ShellRuntime {
    ...
    async fn run(
            ...
            && matches!(ctx.session.user_shell().shell_type, ShellType::Zsh)
        {
            ...
            .map_err(|_| ToolError::Rejected("missing shell command".to_string()))?;
            let env = attempt
            ...
            let exec_result = run_escalate_server(
                ...
                &sandbox_state,
                shell_exec_zsh_path(ctx.session.services.shell_zsh_path.as_ref().unwrap()),
                shell_execve_wrapper()?,
                exec_policy,
