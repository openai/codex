...
#[cfg(unix)]
#[test]
fn apply_path_prepend_records_runtime_path_prepend() {
    ...
    assert_eq!(runtime_path_prepends, vec!["/package/codex-path"]);
}
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    assert_eq!(
        ...
        vec!["/package/codex-resources/zsh/bin"]
    );
}
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_package_path_prepend() {
    ...
    let rewritten = maybe_wrap_shell_lc_with_snapshot_and_path_prepends(
        &command,
        &session_shell,
        &dir.path().abs(),
        &explicit_env_overrides,
