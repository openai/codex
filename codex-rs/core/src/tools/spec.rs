...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        ...
        description: "Runs raw JavaScript against the preinstalled Codex @oai/artifact-tool runtime for creating presentations, spreadsheets, or inline interactive visuals. This is plain JavaScript executed by a local Node-compatible runtime with top-level await, not TypeScript: do not use type annotations, `interface`, `type`, or `import type`. Author code the same way you would for `import { Presentation, Workbook, PresentationFile, SpreadsheetFile, FileBlob, ... } from \"@oai/artifact-tool\"`, but omit that import line because the package surface is already preloaded. Named exports are available directly on `globalThis`, and the full module is available as `globalThis.artifactTool` (also aliased as `globalThis.artifacts` and `globalThis.codexArtifacts`). Use `codex.emitVisual({ title, html, summary?, heightPx? })` (or the top-level `emitVisual(...)` alias) to attach a self-contained HTML mini app as a first-class visual artifact in the thread, and `codex.setDocument({ title?, markdown })` (or `setDocument(...)`) to seed an editable markdown workspace beside it. Node built-ins such as `node:fs/promises` may still be imported when needed for saving preview bytes. This is a freeform tool: send raw JavaScript source text, optionally with a first-line pragma like `// codex-artifacts: timeout_ms=15000` or `// codex-artifact-tool: timeout_ms=15000`; do not send JSON/quotes/markdown fences."
            .to_string(),
        format: FreeformToolFormat {
            r#type: "grammar".to_string(),
            syntax: "lark".to_string(),
