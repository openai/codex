...
use crate::tools::handlers::TimerCreateHandler;
use crate::tools::handlers::TimerDeleteHandler;
use crate::tools::handlers::TimerListHandler;
...
pub(crate) fn build_specs_with_discoverable_tools(
    ...
    for handler in plan.handlers {
        match handler.kind {
            ToolHandlerKind::TimerCreate => {
                builder.register_handler(handler.name, Arc::new(TimerCreateHandler));
            }
            ToolHandlerKind::TimerDelete => {
                builder.register_handler(handler.name, Arc::new(TimerDeleteHandler));
            }
            ToolHandlerKind::TimerList => {
                builder.register_handler(handler.name, Arc::new(TimerListHandler));
            }
