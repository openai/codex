...
pub(crate) fn build_tool_router(
    ...
    ToolRouter::from_parts(registry, model_visible_specs, step_context)
}
...
fn add_shell_tools(context: &CoreToolPlanContext<'_>, planned_tools: &mut PlannedTools) {
    let turn_context = context.turn_context;
    let features = turn_context.config.features.get();
    let environment_mode = context.step_context.tool_environment_mode();
    ...
    match shell_type_for_model_and_features(&turn_context.model_info, features) {
        ConfigShellToolType::UnifiedExec => {
            planned_tools.add(ExecCommandHandler::new(ExecCommandHandlerOptions {
                ...
                include_shell_parameter: unified_exec_should_include_shell_parameter(context),
            }));
...
fn unified_exec_should_include_shell_parameter(context: &CoreToolPlanContext<'_>) -> bool {
    !matches!(
        &context.turn_context.unified_exec_shell_mode,
