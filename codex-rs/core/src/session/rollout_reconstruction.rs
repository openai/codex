...
/// Controls whether reconstruction may seed the active context-window ids from
/// `SessionMeta.context_window`.
///
/// This only applies to the session-level initial-window fallback. Window ids
/// recorded on `CompactedItem`s are still replayed whenever present, because
/// they describe compaction boundaries that are part of the reconstructed
/// history.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum SessionMetaWindowRestore {
    /// Use the current thread's session metadata as an initial-window fallback.
    ///
    /// This is appropriate for resume and rollback replay, where the rollout
    /// items belong to the thread whose runtime state is being reconstructed.
    Restore,
    /// Ignore session metadata while reconstructing copied rollout history.
    ///
    /// This is appropriate for forks, where source rollout items may contain a
    /// parent thread's initial window id but the fork already has its own fresh
    /// initial window id. Compacted window metadata in the copied history still
