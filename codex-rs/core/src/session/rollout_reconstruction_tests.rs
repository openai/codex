...
async fn reconstruct_history_restores_initial_window_from_session_meta() {
    ...
    let rollout_items = vec![RolloutItem::SessionMeta(SessionMetaLine {
        meta: SessionMeta {
            ...
            id: thread_id,
            context_window: Some(initial_window_id.to_string()),
            ..SessionMeta::default()
        },
        git: None,
    })];
...
#[tokio::test]
async fn reconstruct_history_ignores_source_session_meta_context_window() {
    let (session, turn_context) = make_session_and_context().await;
    let source_thread_id = ThreadId::new();
    let source_initial_window_id = Uuid::now_v7();
    let rollout_items = vec![RolloutItem::SessionMeta(SessionMetaLine {
        meta: SessionMeta {
            session_id: source_thread_id.into(),
            id: source_thread_id,
            context_window: Some(source_initial_window_id.to_string()),
            ..SessionMeta::default()
        },
        git: None,
    })];
...
#[tokio::test]
async fn reconstruct_history_counts_legacy_compactions_over_session_meta_window_number() {
    let (session, turn_context) = make_session_and_context().await;
    let thread_id = session.thread_id();
    let initial_window_id = Uuid::now_v7();
    let rollout_items = vec![
