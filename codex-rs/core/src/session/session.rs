...
impl SessionConfiguration {
    ...
    pub(super) fn resolved_environment_selections(&self) -> Vec<TurnEnvironmentSelection> {
        let default_permission_profile = self.permission_profile();
        self.environment_selections()
            .iter()
            .cloned()
            .map(|mut environment| {
                if environment.permission_profile.is_none() {
                    environment.permission_profile = Some(default_permission_profile.clone());
                }
                environment
            })
            .collect()
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            turn_environments
                .update_selections(&session_configuration.resolved_environment_selections());
