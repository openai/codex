...
#[test]
fn rollout_storage_preserves_small_function_call_output() {
    let item = ResponseItem::FunctionCallOutput {
        call_id: "call-small".to_string(),
        output: FunctionCallOutputPayload::from_text("small output".to_string()),
    };

    let got = response_item_for_rollout_storage(&item, TruncationPolicy::Bytes(128));

    assert_eq!(got, item);
}

#[test]
fn rollout_storage_preserves_non_output_items() {
    assert_eq!(
        response_item_for_rollout_storage(&ResponseItem::Other, TruncationPolicy::Bytes(128)),
        ResponseItem::Other
    );
}

#[test]
fn rollout_storage_truncates_large_function_call_output() {
    let long_output = "large function output\n".repeat(1_000);
    let item = ResponseItem::FunctionCallOutput {
...
#[test]
fn rollout_storage_truncates_large_custom_tool_call_output() {
    let long_output = "large custom tool output\n".repeat(1_000);
    let item = ResponseItem::CustomToolCallOutput {
        call_id: "custom-large".to_string(),
    ...
    match got {
        ResponseItem::CustomToolCallOutput {
            ...
            assert_eq!(call_id, "custom-large");
            assert_eq!(name.as_deref(), Some("custom_tool"));
            let content = output.text_content().expect("text output");
            assert_ne!(content, long_output);
            assert!(content.len() < long_output.len());
            assert!(
