...
#[test]
fn record_items_preserves_pre_truncated_exec_output() {
    let mut history = ContextManager::new();
    let output = FunctionCallOutputPayload::from_text("exec output".repeat(100));
    let items = [
        ResponseItem::CustomToolCall {
            id: None,
            status: None,
    ...
    history.record_items(items.iter(), TruncationPolicy::Tokens(1));
...
#[test]
fn record_items_preserves_pre_truncated_wait_output() {
    let mut history = ContextManager::new();
    let output = FunctionCallOutputPayload::from_text("wait output".repeat(100));
    let items = [
        ResponseItem::FunctionCall {
            id: None,
            name: crate::tools::code_mode::WAIT_TOOL_NAME.to_string(),
            namespace: None,
