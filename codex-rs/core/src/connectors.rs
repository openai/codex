...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let connector_ids = tool_suggest_connector_ids(config, loaded_plugin_app_connector_ids).await;
...
async fn tool_suggest_connector_ids(
    ...
    let mut connector_ids = PluginsManager::new(config.codex_home.to_path_buf())
        ...
        .collect::<HashSet<_>>();
    connector_ids.extend(loaded_plugin_app_connector_ids.iter().cloned());
