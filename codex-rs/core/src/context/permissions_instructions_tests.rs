...
fn renders_sandbox_mode_text() {
    assert_eq!(
        ...
        "Filesystem sandboxing defines which files can be read or written. `sandbox_mode` is `workspace-write`: The sandbox permits reading files, and editing files in the configured writable roots. Editing files in other directories requires approval. Network access is restricted."
    );
...
fn writable_roots_text_limits_long_root_lists() {
    ...
    assert!(text.contains(&visible_root_labels[0]));
    assert!(text.contains(&visible_root_labels[MAX_WRITABLE_ROOTS_IN_PROMPT - 1]));
    assert!(!text.contains(&visible_root_labels[MAX_WRITABLE_ROOTS_IN_PROMPT]));
