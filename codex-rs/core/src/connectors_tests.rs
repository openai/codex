...
#[test]
fn app_tool_policy_matches_unique_tool_config_without_connector_metadata() {
    let apps_config = AppsConfigToml {
        default: None,
        apps: HashMap::from([(
            "asdk_app_699cfa310c948191905cdbbb589a1718".to_string(),
            AppConfig {
                enabled: true,
                destructive_enabled: None,
                open_world_enabled: None,
                ...
                tools: Some(AppToolsConfig {
                    tools: HashMap::from([(
                        "kepler_submit_query_to_file".to_string(),
                        AppToolConfig {
                            enabled: None,
                            approval_mode: Some(AppToolApproval::Approve),
                        },
    ...
    let policy = app_tool_policy_from_apps_config(
        Some(&apps_config),
        /*connector_id*/ None,
        "kepler_submit_query_to_file",
        Some("submit_query_to_file"),
        Some(&annotations(Some(true), Some(true))),
    );
...
#[test]
fn app_tool_policy_ignores_ambiguous_tool_config_without_connector_metadata() {
    ...
    let apps_config = AppsConfigToml {
        default: None,
        apps: HashMap::from([
            ("app_one".to_string(), app_config.clone()),
            ("app_two".to_string(), app_config),
        ]),
    };
    ...
    assert_eq!(policy, AppToolPolicy::default());
}
