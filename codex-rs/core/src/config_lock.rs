...
pub(crate) fn config_lock_metadata() -> ConfigLockToml {
    ConfigLockToml {
...
pub(crate) fn validate_config_lock_replay(
    ...
    let expected_metadata = match expected_lock.config_lock.as_ref() {
        Some(expected) => {
    ...
    let actual_metadata = match actual_lock.config_lock.as_ref() {
        Some(actual) => {
    ...
    if !options.allow_codex_version_mismatch
        && expected_metadata.codex_version != actual_metadata.codex_version
    {
        return Err(config_lock_error(format!(
            "config lock Codex version mismatch: lock was generated by {}, current version is {}; set debug.config_lock.allow_codex_version_mismatch=true to ignore this",
            expected_metadata.codex_version, actual_metadata.codex_version
        )));
    }
...
pub(crate) fn clear_config_lock_debug_controls(config: &mut ConfigToml) {
    if let Some(debug) = config.debug.as_mut() {
        debug.config_lock = None;
    }
    if config
        .debug
        .as_ref()
        .is_some_and(|debug| debug.config_lock.is_none())
    {
        config.debug = None;
    }
}
...
fn config_lock_for_comparison(config: &ConfigToml, options: ConfigLockReplayOptions) -> ConfigToml {
    let mut config = config.clone();
    clear_config_lock_debug_controls(&mut config);
    if options.allow_codex_version_mismatch
        && let Some(config_lock) = config.config_lock.as_mut()
    {
        config_lock.codex_version.clear();
    }
    config
}
