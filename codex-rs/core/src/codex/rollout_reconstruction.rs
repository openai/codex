...
impl Session {
    pub(super) async fn reconstruct_history_from_rollout(
        ...
        //
        // A `TurnContextItem` appears in rollout only for user turns that emit model-visible
        // context items. Mid-turn compaction can also re-establish the baseline by reinjecting
        // full initial context. That means replay only needs to answer:
