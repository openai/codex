    ...
    fn try_from(toml: ConfigRequirementsWithSources) -> Result<Self, Self::Error> {
        ...
        let web_search_mode = match allowed_web_search_modes {
            Some(Sourced {
                ...
                let initial_value: Option<WebSearchMode> = None;
                let requirement_source_for_error = requirement_source.clone();
                let constrained = Constrained::new(initial_value, move |candidate| {
                    let Some(mode) = candidate else {
                        return Ok(());
                    };
                    if accepted.contains(&(*mode).into()) {
                        Ok(())
                    } else {
                        Err(ConstraintError::InvalidValue {
...
mod tests {
    ...
    fn deserialize_allowed_web_search_modes() -> Result<()> {
        ...
        assert_eq!(requirements.web_search_mode.value(), None);
        assert!(
            requirements
                .web_search_mode
        ...
        assert!(requirements.web_search_mode.can_set(&None).is_ok());
