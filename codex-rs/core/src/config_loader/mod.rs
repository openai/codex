...
async fn load_project_layers(
    ...
    for dir in dirs {
        ...
        let dot_codex_normalized =
            normalize_path(dot_codex.path().as_path()).unwrap_or_else(|_| dot_codex.to_path_buf());
        if dot_codex.path() == &codex_home_abs || dot_codex_normalized == codex_home_normalized {
            continue;
        }
