...
#[tokio::test]
async fn cwd_local_config_applies_even_when_untrusted_or_unknown() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    tokio::fs::create_dir_all(nested.join(".codex")).await?;
...
#[tokio::test]
async fn invalid_cwd_local_config_errors_under_codez_policy() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    tokio::fs::create_dir_all(nested.join(".codex")).await?;
    ...
    for (name, trust_level) in cases {
        ...
        assert!(
            result.is_err(),
            "expected invalid cwd-local config to error under codez policy ({name})"
        );
    }
