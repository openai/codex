...
#[cfg(unix)]
use crate::tools::runtimes::maybe_wrap_shell_lc_with_snapshot_and_path_prepends;
...
fn prepare_user_shell_exec_command_with_path_prepend(
    ...
    shell_environment_set: &HashMap<String, String>,
    exec_env_map: &mut HashMap<String, String>,
    apply_path_prepend: impl FnOnce(&mut HashMap<String, String>, &mut Vec<String>),
) -> Vec<String> {
    let explicit_env_overrides = shell_environment_set.clone();
    let mut runtime_path_prepends = Vec::new();
    apply_path_prepend(exec_env_map, &mut runtime_path_prepends);
