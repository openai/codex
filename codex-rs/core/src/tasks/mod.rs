...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    fn supports_auto_continue(&self) -> bool {
        false
    }
...
impl Session {
    pub async fn spawn_task<T: SessionTask>(
        ...
        let auto_continue_eligible = task.supports_auto_continue();
    ...
    pub async fn abort_all_tasks(self: &Arc<Self>, reason: TurnAbortReason) {
        if reason == TurnAbortReason::Interrupted {
            self.disable_auto_continue(AutoContinueStopReason::Interrupted)
                .await;
        }
    ...
    pub async fn on_task_finished(
        ...
        match self
            .auto_continue_decision(last_agent_message.as_deref())
            .await
        {
            AutoContinueDecision::Continue { prompt } => {
                self.submit_auto_continue_prompt(prompt).await;
            }
            AutoContinueDecision::Stop(_reason) => {}
        }
    }
