...
mod tests {
    ...
    async fn skills_are_appended_to_project_doc() {
        ...
        let expected = format!(
            "base doc\n\n## Skills\nThese skills are discovered at startup from ~/.codex/skills and project-local ./.codex/skills; each entry shows name, description, and file path so you can open the source for full instructions. Content is not inlined to keep context lean.\n- pdf-processing: extract from pdfs (file: {expected_path_str})\n{usage_rules}"
        );
        assert_eq!(res, expected);
    }
