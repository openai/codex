...
fn openai_file_input_params_for_server(
    ...
    Some(OpenAIFileInputParams {
        names,
        store_in_oai_library: declared_openai_file_upload_store_in_oai_library(input_schema),
    })
}
...
fn declared_openai_file_upload_store_in_oai_library(
    ...
    // Durable Library storage must be visible as the exact
    // `store_in_oai_library: true` arg Guardian reviews.
