...
impl From<EnvironmentContext> for ResponseItem {
    fn from(ec: EnvironmentContext) -> Self {
        let formatted_ec = match ec.get_formatted_environment_context() {
            Some(formatted_ec) => formatted_ec,
            None => format!("{ENVIRONMENT_CONTEXT_START}{ec}{ENVIRONMENT_CONTEXT_END}"),
        };
        ResponseItem::Message {
            id: None,
            role: "user".to_string(),
            content: vec![ContentItem::InputText { text: formatted_ec }],
        }
    }
}
