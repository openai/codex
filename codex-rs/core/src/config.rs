...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let exp_instr_path = config_profile
            .experimental_instructions_file
            .as_ref()
            .or(cfg.experimental_instructions_file.as_ref());
        let file_base_instructions = Self::get_base_instructions(exp_instr_path, &resolved_cwd)?;
...
"#;
    ...
    #[test]
    fn profile_can_set_experimental_instructions_file() -> std::io::Result<()> {
        use std::fs;
        use tempfile::TempDir;

        // Create a temp file with custom base instructions content
        let tmp = TempDir::new().unwrap();
        let custom_path = tmp.path().join("custom_base.md");
        let marker = "profile-experimental-instructions-marker";
        fs::write(&custom_path, marker).unwrap();

        // Root-level config sets a different (nonexistent) path to ensure the
        // profile value takes precedence.
        let cfg_toml = toml::toml! {
            model = "o3"
            [profiles.p]
            model = "o3"
            experimental_instructions_file = "${custom}"
        };

        // Serialize with the absolute path substituted in.
        let mut cfg_str = toml::to_string(&cfg_toml).unwrap();
        cfg_str = cfg_str.replace("${custom}", &custom_path.to_string_lossy());
        let cfg: ConfigToml = toml::from_str(&cfg_str).unwrap();
        ...
        // We expect the base instructions override to be loaded from the profile path.
        assert_eq!(config.base_instructions, Some(marker.to_string()));

        // Also ensure that using a relative path in the profile resolves against cwd.
        let project = TempDir::new().unwrap();
        let rel_cfg_str = "model=\"o3\"\n[profiles.p]\nmodel=\"o3\"\nexperimental_instructions_file=\"custom_base.md\"\n".to_string();
        let cfg_rel: ConfigToml = toml::from_str(&rel_cfg_str).unwrap();
        let overrides = ConfigOverrides {
            config_profile: Some("p".to_string()),
            cwd: Some(project.path().to_path_buf()),
            ..Default::default()
        };
        // Place the file in the cwd
        let rel_path = project.path().join("custom_base.md");
        fs::write(&rel_path, marker).unwrap();
        let config_rel = Config::load_from_base_config_with_overrides(
            cfg_rel,
            overrides,
            codex_home.path().to_path_buf(),
        )?;
        assert_eq!(config_rel.base_instructions, Some(marker.to_string()));
