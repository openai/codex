    ...
    pub(crate) async fn ensure_execution_capacity_for_op(
        ...
        match op {
            Op::InterAgentCommunication { communication } => {
                self.ensure_execution_capacity_for_communication(thread_id, communication)
                    .await
            }
            Op::UserInput { .. } => self.ensure_execution_capacity_for_thread(thread_id).await,
            _ => Ok(()),
        }
    }

    pub(super) async fn ensure_execution_capacity_for_communication(
        &self,
        thread_id: ThreadId,
