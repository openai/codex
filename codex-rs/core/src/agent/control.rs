...
#[derive(Clone, Debug)]
pub(crate) enum AgentSubmission {
    Op(Box<Op>),
...
impl AgentControl {
    ...
    pub(crate) async fn send_input(
        ...
        let submission = AgentSubmission::op(initial_operation);
        self.ensure_execution_capacity_for_submission(agent_id, &submission)
            .await?;
        self.send_input_after_capacity_check(agent_id, &state, submission)
            .await
    }
    ...
    async fn send_input_after_capacity_check(
        ...
        let initial_operation = match submission {
            ...
            AgentSubmission::Op(initial_operation) => *initial_operation,
        };
    ...
    pub(crate) async fn send_inter_agent_communication(
        ...
        let state = self.upgrade()?;
        let submission = AgentSubmission::InterAgentCommunication {
            communication,
            context: agent_communication_context,
        };
        self.ensure_execution_capacity_for_submission(agent_id, &submission)
            .await?;
        self.send_input_after_capacity_check(agent_id, &state, submission)
