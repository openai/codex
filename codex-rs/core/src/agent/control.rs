...
impl AgentControl {
    ...
    /// Interrupt every thread in this agent tree, enqueueing the triggering thread last.
    pub(crate) async fn interrupt_all_threads(
        &self,
        triggering_thread_id: ThreadId,
    ) -> CodexResult<()> {
        let other_thread_ids = self
            .state
            .live_agents()
            .into_iter()
            .filter_map(|metadata| metadata.agent_id)
            .chain(std::iter::once(ThreadId::from(self.session_id)))
            .filter(|thread_id| *thread_id != triggering_thread_id);
        ...
        self.interrupt_agent(triggering_thread_id).await?;
        Ok(())
    }
