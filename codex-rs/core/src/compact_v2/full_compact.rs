...
fn create_model_client_with_provider(
    ...
    // Get model info for the provider's model
    // Use model_name from provider.ext if available, otherwise use existing client's model info
    let model_info = provider
        .ext
        .model_name
        .as_ref()
        .map(|name| find_model_info_for_slug(name))
        .unwrap_or_else(|| existing_client.get_model_info());
