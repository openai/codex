...
mod tests {
    ...
    #[tokio::test]
    async fn model_and_personality_change_only_appends_model_instructions() {
        let (mut session, previous_context) = make_session_and_context().await;
        session.features.enable(Feature::Personality);
        let previous_context = Arc::new(previous_context);
        let previous_model = previous_context.model_info.slug.clone();
        let next_model = if previous_model == "exp-codex-personality" {
            "gpt-5.2-codex".to_string()
        } else {
            "exp-codex-personality".to_string()
        };
        let next_context = session
            .new_turn_with_sub_id(
                "sub_id_2".to_string(),
            ...
            .await
            .expect("new turn with model and personality update should succeed");
        assert_ne!(next_context.model_info.slug, previous_model);
        assert!(
            Session::personality_message_for(&next_context.model_info, Personality::Pragmatic)
                .is_some(),
            "expected next model to provide a personality template for the test"
        );

        let update_items =
            session.build_settings_update_items(Some(&previous_context), next_context.as_ref());

        let expected_model_item = ResponseItem::Message {
            id: None,
            role: "developer".to_string(),
            content: vec![ContentItem::InputText {
                text: DeveloperInstructions::model_switch_message(
        ...
        assert!(
            update_items.contains(&expected_model_item),
            "expected model switch developer message when model changes"
        );
        assert!(
            !update_items.iter().any(|item| {
                matches!(
                    item,
                    ResponseItem::Message { role, content, .. }
                    if role == "developer"
                        && content.iter().any(|entry| {
                            matches!(
                                entry,
                                ContentItem::InputText { text } if text.contains("<personality_spec>")
                            )
                        })
                )
            }),
            "did not expect personality update developer message when model changed in same turn"
        );
    }
