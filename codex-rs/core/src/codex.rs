...
impl Session {
    ...
    pub async fn notify_request_permission_preset_response(
        ...
        let Some((turn_state, tx_response)) = ({
            let mut active = self.active_turn.lock().await;
            match active.as_mut() {
                Some(at) => {
                    let turn_state = at.turn_state.clone();
                    let entry = {
                        let mut ts = turn_state.lock().await;
                        ts.remove_pending_request_permission_preset(call_id)
                    };
                    entry.map(|tx_response| (turn_state, tx_response))
                }
                None => None,
            }
        }) else {
                ...
                .await
            {
                Some(preset) => {
                        ...
                        .await
                    {
                        ...
                        response = RequestPermissionPresetResponse {
                            ...
                            message: format!(
                                "requested permission preset could not be applied: {err}"
                            ),
                        };
                    } else {
                        let mut ts = turn_state.lock().await;
                ...
                None => {
                    response = RequestPermissionPresetResponse {
                        decision: RequestPermissionPresetDecision::Declined,
                        preset: response.preset,
                        message: "requested permission preset is no longer available".to_string(),
                    };
                }
            }
        }
