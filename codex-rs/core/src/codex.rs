...
impl Session {
    ...
    pub(crate) async fn auto_continue_decision(
        &self,
        last_agent_message: Option<&str>,
    ) -> AutoContinueDecision {
        let mut state = self.state.lock().await;
        state.auto_continue_mut().decide(last_agent_message)
    }

    pub(crate) async fn rearm_auto_continue_if_allowed(&self) -> bool {
        let mut state = self.state.lock().await;
        state.auto_continue_mut().rearm_if_allowed()
    }

    pub(crate) async fn disable_auto_continue(&self, reason: AutoContinueStopReason) {
        let mut state = self.state.lock().await;
        state.auto_continue_mut().disable(reason);
    }

    pub(crate) async fn submit_auto_continue_prompt(&self, prompt: String) {
        ...
        if let Err(err) = self.tx_sub.send(submission).await {
            warn!("failed to submit auto-continue follow-up turn: {err}");
        }
    }
...
mod handlers {
    ...
    pub async fn user_input_or_turn(
        ...
        if let Err(SteerInputError::NoActiveTurn(items)) = sess.steer_input(items, None).await {
            sess.rearm_auto_continue_if_allowed().await;
...
mod tests {
    ...
    async fn task_finish_persists_leftover_pending_input() {
        ...
        sess.on_task_finished(Arc::clone(&tc), false, None).await;
