...
pub(crate) const ZSH_EXEC_BRIDGE_WRAPPER_ORIGIN_ENV_VAR: &str =
    "CODEX_ZSH_EXEC_BRIDGE_WRAPPER_ORIGIN";
...
impl ZshExecBridge {
    ...
    async fn handle_wrapper_request(
        ...
        let (request_id, file, argv, cwd, origin) = match request {
            WrapperIpcRequest::ExecRequest {
                ...
                cwd,
                origin,
            } => (request_id, file, argv, cwd, origin),
        };

        let wrapper_request = WrapperExecRequest {
            file,
            argv,
            cwd,
            origin,
        ...
        if should_skip_host_approval_for_request(&wrapper_request) {
            write_json_line(
                &mut stream,
        ...
        let command_for_approval = if wrapper_request.argv.is_empty() {
            vec![wrapper_request.file.clone()]
        } else {
            wrapper_request.argv.clone()
        };
        ...
        let decision = session
            .request_command_approval(
                ...
                PathBuf::from(wrapper_request.cwd),
                approval_reason,
                None,
...
fn wrapper_exec_origin_from_env() -> Option<WrapperExecOrigin> {
    let raw = std::env::var(ZSH_EXEC_BRIDGE_WRAPPER_ORIGIN_ENV_VAR).ok()?;
    match raw.as_str() {
