...
Default to doing the work yourself. Use sub-agents only when the task has multiple independent, bounded workstreams that can run in parallel and materially shorten the critical path. Do not spawn sub-agents for simple tasks, small edits, routine code search, or work you can complete directly in a few tool calls.
