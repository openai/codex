...
#[tokio::test]
async fn permissions_profiles_network_mitm_allows_managed_network_requirement()
-> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    std::fs::write(cwd.path().join(".git"), "gitdir: nowhere")?;

    let network_requirements = crate::config_loader::NetworkConstraints {
        mitm: Some(true),
        ..Default::default()
    };
    let network_requirements_toml = crate::config_loader::NetworkRequirementsToml {
        mitm: Some(true),
        ..Default::default()
    };
    let config_layer_stack = crate::config_loader::ConfigLayerStack::new(
        ...
        crate::config_loader::ConfigRequirements {
            network: Some(crate::config_loader::Sourced::new(
        ...
        crate::config_loader::ConfigRequirementsToml {
            network: Some(network_requirements_toml),
            ..Default::default()
        },
    )
