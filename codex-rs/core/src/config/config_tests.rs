...
fn forced_chatgpt_workspace_id_accepts_string_and_list() -> std::io::Result<()> {
    ...
    let empty_list_cfg: ConfigToml = toml::from_str(r#"forced_chatgpt_workspace_id = []"#)
        .expect("empty workspace id list should parse");

    let empty_list_config = Config::load_from_base_config_with_overrides(
        empty_list_cfg,
        ConfigOverrides::default(),
        codex_home.abs(),
    )?;
    ...
    assert_eq!(
        empty_list_config.forced_chatgpt_workspace_id,
        Some(ForcedChatgptWorkspaceIds::Multiple(Vec::new()))
    );
