...
///
/// This keeps approval decisions stable across harmless whitespace changes in
/// parsed word-only shell scripts while preserving the executable and wrapper
/// arguments. They are part of the key because they take effect before the
/// script and can be attacker controlled.
pub(crate) fn canonicalize_command_for_approval(command: &[String]) -> Vec<String> {
    ...
    if extract_powershell_command(command).is_some() {
        let mut canonical_command = vec![CANONICAL_POWERSHELL_SCRIPT_PREFIX.to_string()];
        canonical_command.extend(command.iter().cloned());
        return canonical_command;
    }
