...
impl<'a> AgentsMdManager<'a> {
    ...
    pub(crate) async fn user_instructions_with_fs(
        ...
        let agents_md_docs = match self.executor_cwd(fs) {
            Ok(cwd) => self.read_agents_md(&cwd).await,
            Err(err) => Err(err),
        };
    ...
    pub async fn instruction_sources(&self, fs: &dyn ExecutorFileSystem) -> Vec<AbsolutePathBuf> {
        let mut paths = Self::load_global_instructions(Some(&self.config.codex_home))
            ...
            .unwrap_or_default();
        let cwd = match self.executor_cwd(fs) {
            Ok(cwd) => cwd,
            Err(err) => {
                tracing::warn!(error = %err, "failed to normalize cwd for instruction sources");
                return paths;
            }
        };
    ...
    fn executor_cwd<'fs>(
        &self,
        fs: &'fs dyn ExecutorFileSystem,
    ) -> io::Result<ExecutorPathRef<'fs>> {
        let mut dir = self.config.cwd.clone();
        if let Ok(canon) = normalize_path(&dir) {
            dir = AbsolutePathBuf::try_from(canon)?;
        }
        Ok(ExecutorPathRef::new(fs, dir))
    }
