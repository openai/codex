...
#[tokio::test]
async fn request_permissions_response_persist_rejects_unreloadable_profile_update() {
    let (session, _turn_context) = make_session_and_context().await;
    let codex_home = session.codex_home().await;
    std::fs::create_dir_all(&codex_home).expect("create codex home");
...
"#;
    ...
    std::fs::write(&config_path, original_config).expect("write user config");
    session.reload_user_config_layer().await;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let session = Arc::new(session);
    let codex_home_path =
        AbsolutePathBuf::try_from(codex_home.as_path()).expect("absolute codex home");
    let permissions = RequestPermissionProfile {
        file_system: Some(codex_protocol::models::FileSystemPermissions {
            read: None,
            write: Some(vec![codex_home_path]),
        }),
        ..RequestPermissionProfile::default()
    };
    let response = codex_protocol::request_permissions::RequestPermissionsResponse {
    ...
    assert_eq!(received_response, response);
    assert_eq!(
        std::fs::read_to_string(config_path).expect("read user config"),
        original_config
    );
    assert_eq!(
        session.granted_turn_permissions().await,
    ...
    assert_eq!(session.granted_session_permissions().await, None);
}
