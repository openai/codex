...
mod tests {
    ...
    #[tokio::test]
    async fn aggregated_stream_forwards_rate_limit_snapshots() {
        let snapshot = RateLimitSnapshotEvent {
            primary_used_percent: 15.5,
            protection_used_percent: 42.0,
            primary_to_protection_ratio_percent: Some(30.0),
            primary_window_minutes: Some(300),
            protection_window_minutes: Some(10_080),
        };

        let inner = stream::iter(vec![Ok(ResponseEvent::RateLimits(snapshot.clone()))]);
        let mut aggregated = AggregatedChatStream::new(inner, AggregateMode::AggregatedOnly);

        match aggregated.next().await {
            Some(Ok(ResponseEvent::RateLimits(observed))) => {
                assert_eq!(observed.primary_used_percent, snapshot.primary_used_percent);
                assert_eq!(
                    observed.protection_used_percent,
                    snapshot.protection_used_percent
                );
                assert_eq!(
                    observed.primary_to_protection_ratio_percent,
                    snapshot.primary_to_protection_ratio_percent
                );
                assert_eq!(
                    observed.primary_window_minutes,
                    snapshot.primary_window_minutes
                );
                assert_eq!(
                    observed.protection_window_minutes,
                    snapshot.protection_window_minutes
                );
            }
            other => panic!("expected rate limit snapshot, got {other:?}"),
        }

        assert!(aggregated.next().await.is_none(), "unexpected extra events");
    }
}
