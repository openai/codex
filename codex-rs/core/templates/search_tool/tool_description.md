...
- If the user names an Apps tool directly (for example `query_project`) and it is not already available, do not run that name as a shell command. First call `search_tool_bm25` with that tool name, then invoke the matching `mcp__codex_apps__...` tool.
