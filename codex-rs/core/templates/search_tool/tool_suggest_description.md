...
Do not use this tool for adjacent capabilities, broad recommendations, or tools that merely seem useful. The user's intent must clearly match one listed tool.
...
5. After the request flow completes:
   ...
   - if the user did not finish, continue without that tool, and don't request that tool again unless the user explicitly asks for it.
