...
async fn run_test_in_subprocess() -> Result<()> {
    ...
    // Reqwest disables its default environment proxy matcher in CGI processes. The route-aware
    // client resolves `HTTP_PROXY` explicitly, so this marker makes the test fail if either
    // Responses endpoint regresses to the default client.
