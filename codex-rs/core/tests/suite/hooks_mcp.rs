...
fn insert_rmcp_test_server(config: &mut Config, command: String) {
...
fn enable_hooks_and_rmcp_server(config: &mut Config, rmcp_test_server_bin: String) {
    ...
    insert_rmcp_test_server(config, rmcp_test_server_bin);
}
...
async fn pre_tool_use_blocks_mcp_tool_before_execution() -> Result<()> {
    ...
    let test = test_codex()
        ...
        .with_config(move |config| {
            enable_hooks_and_rmcp_server(config, rmcp_test_server_bin);
        })
        .build(&server)
        .await?;
