...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn normal_loop_context_window_error_stops_after_overflow_retry_budget_is_spent() {
    skip_if_no_network!();
    ...
    codex
        .submit(Op::UserInput {
            environments: None,
            items: vec![UserInput::Text {
                text: "overflow rescue budget".into(),
                text_elements: Vec::new(),
            }],
            final_output_json_schema: None,
            responsesapi_client_metadata: None,
    ...
    assert!(
        error_message.contains("Codex ran out of room in the model's context window."),
        "expected the terminal overflow error after retry budget exhaustion, got {error_message}"
    );
    let requests = request_log.requests();
    assert_eq!(
        requests.len(),
        3,
        "expected one overflow rescue attempt, then a terminal retry overflow"
    );
    assert!(
        requests[1].body_contains_text(SUMMARIZATION_PROMPT),
        "expected the single retry-budgeted rescue to issue automatic compaction"
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn normal_loop_context_window_error_preserves_image_user_turn_across_local_compaction() {
    skip_if_no_network!();

    let server = start_mock_server().await;
    let image_url = "data:image/png;base64,overflow-preserve-image";
    let user_message = "what is this image?";
    let request_log = mount_sse_sequence(
        &server,
        vec![
            sse_failed(
    ...
    codex
        ...
        .await
        .expect("submit overflowing image user turn");
    wait_for_event(&codex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    let recovered_request = &requests[2];
    assert_eq!(
        recovered_request
            ...
            .count(),
        1,
        "recovered sampling request should preserve the current user text exactly once"
    );
    assert_eq!(
        recovered_request
            ...
            .count(),
        1,
        "recovered sampling request should preserve the current user image exactly once"
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn normal_loop_context_window_error_preserves_large_user_turn_across_local_compaction() {
    skip_if_no_network!();

    let server = start_mock_server().await;
    let user_message = format!("{}tail", "oversized current user turn ".repeat(4_000));
    let request_log = mount_sse_sequence(
        &server,
        vec![
            sse_failed(
    ...
    codex
        ...
        .await
        .expect("submit overflowing large user turn");
    wait_for_event(&codex, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    assert_eq!(
        requests[2]
            ...
            .count(),
        1,
        "recovered sampling request should preserve the complete large user turn exactly once"
    );
}
