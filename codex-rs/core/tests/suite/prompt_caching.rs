...
async fn send_user_turn_with_no_changes_does_not_send_environment_context() -> anyhow::Result<()> {
    ...
    let default_approval_policy = config.effective_permissions.approval_policy.value();
    let default_sandbox_policy = config.effective_permissions.sandbox_policy.get();
