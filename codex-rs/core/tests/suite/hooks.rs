...
#[tokio::test]
async fn async_user_prompt_output_waits_for_fresh_turn_after_steer() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let third_chunks = vec![StreamingSseChunk {
        ...
        body: sse(vec![
            ev_response_created("resp-3"),
            ev_assistant_message("msg-3", "fresh turn handled"),
            ev_completed("resp-3"),
        ]),
    }];
    ...
    .await;
    test.submit_turn("following fresh prompt").await?;

    let requests = server.requests().await;
    assert_eq!(requests.len(), 3);
    assert!(
        request_message_input_texts(&requests[2], "developer")
            .contains(&"async context for initial prompt".to_string())
    );
