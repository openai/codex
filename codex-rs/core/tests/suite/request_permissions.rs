...
#[tokio::test(flavor = "current_thread")]
async fn guardian_can_approve_request_permissions_tool_call() -> Result<()> {
    skip_if_no_network!(Ok(()));
    skip_if_sandbox!(Ok(()));
    ...
    let outside_dir = tempfile::tempdir()?;
    let outside_write = outside_dir.path().join("guardian-sticky-shell-write.txt");
    let command = format!(
        "printf {:?} > {:?} && cat {:?}",
        "guardian-sticky-shell-ok", outside_write, outside_write
    );
    let requested_permissions = requested_directory_write_permissions(outside_dir.path());
    let guardian_assessment = json!({
        ...
        "outcome": "allow",
        "rationale": "The user asked to grant this temporary filesystem permission.",
    })
    .to_string();
    let responses = mount_sse_sequence(
        &server,
        vec![
            sse(vec![
                ev_response_created("resp-guardian-permissions-1"),
                request_permissions_tool_event(
                    "permissions-call",
                    "Allow writing outside the workspace",
                    &requested_permissions,
                )?,
                ev_completed("resp-guardian-permissions-1"),
            ]),
            sse(vec![
            ...
            sse(vec![
                ev_response_created("resp-guardian-permissions-2"),
                shell_command_event("shell-call", &command)?,
                ev_completed("resp-guardian-permissions-2"),
            ]),
            sse(vec![
                ev_response_created("resp-guardian-permissions-3"),
                ev_assistant_message("msg-guardian-permissions-1", "done"),
                ev_completed("resp-guardian-permissions-3"),
            ]),
        ],
    )
    .await;
    ...
    test.submit_turn_with_policies(
        "write outside the workspace after guardian approval",
        approval_policy,
        sandbox_policy,
    )
    .await?;
    ...
    assert!(
        approval.is_none(),
        "guardian-approved request_permissions should not require a later exec approval"
    );
    assert_eq!(
        responses.requests().len(),
        4,
        "guardian should run once between the request_permissions call and later shell call",
    );
    ...
    assert!(
        result.exit_code.is_none_or(|exit_code| exit_code == 0),
        "shell command should succeed after guardian approves request_permissions: {shell_output:#?}"
    );
    assert!(
        result.stdout.contains("guardian-sticky-shell-ok"),
        "shell command should read back written content: {shell_output:#?}"
    );
...
#[tokio::test(flavor = "current_thread")]
async fn request_permissions_hook_receives_guardian_review() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let requested_permissions = requested_directory_write_permissions(test.cwd.path());
    let guardian_assessment = json!({
        "risk_level": "low",
        "user_authorization": "high",
        "outcome": "allow",
        "rationale": "The user asked to grant this workspace permission.",
    })
    .to_string();
    let responses = mount_sse_sequence(
        &server,
        vec![
            sse(vec![
                ev_response_created("resp-hook-guardian-permissions-1"),
                request_permissions_tool_event(
                    "permissions-call",
                    "Allow writing inside the workspace",
                    &requested_permissions,
                )?,
                ev_completed("resp-hook-guardian-permissions-1"),
            ]),
            sse(vec![
                ev_response_created("resp-hook-guardian-review"),
                ev_assistant_message("msg-hook-guardian-review", &guardian_assessment),
                ev_completed("resp-hook-guardian-review"),
            ]),
            sse(vec![
                ev_response_created("resp-hook-guardian-permissions-2"),
                ev_assistant_message("msg-hook-guardian-permissions-1", "done"),
                ev_completed("resp-hook-guardian-permissions-2"),
            ]),
        ],
    )
    .await;
    ...
    test.submit_turn_with_policies(
        "ask for workspace write permission",
        approval_policy,
        sandbox_policy,
    )
    .await?;
    ...
    assert_eq!(
        responses.requests().len(),
        3,
        "guardian should run once before the hook resolves the permission request",
    );
    let hook_inputs = read_permission_request_hook_inputs(test.codex_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(hook_inputs[0]["tool_name"], "request_permissions");
