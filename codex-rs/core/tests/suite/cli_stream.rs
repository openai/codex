...
async fn exec_cli_applies_model_instructions() {
    ...
    let marker = "cli-model-instructions-marker";
    ...
    cmd.arg("exec")
        ...
        .arg("model_provider=\"mock\"")
        .arg("-c")
        .arg(format!("model_instructions=\"{custom_path_str}\""))
        .arg("-C")
        .arg(&repo_root)
        .arg("hello?\n");
    cmd.env("CODEX_HOME", home.path())
