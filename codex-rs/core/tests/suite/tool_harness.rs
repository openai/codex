...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shell_tool_executes_command_and_streams_output_freeform_gpt_5_1() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let call_id = "shell-tool-call-freeform-gpt-5.1";
    let command = vec!["/bin/echo", "tool harness"];
    let first_response = sse(vec![
        ev_response_created("resp-1"),
        ev_local_shell_call(call_id, "completed", command),
    ...
    assert!(
        serde_json::from_str::<Value>(&output_text).is_err(),
        "expected gpt-5.1 shell tool output to be plain text"
    );
