...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn user_turn_truncates_skill_description_only_in_model_context() -> Result<()> {
    // TODO(anp): Remove after skill-path helpers use target-native paths.
    skip_if_wine_exec!(Ok(()), "requires native cross-OS skill paths");
    skip_if_no_network!(Ok(()));
    ...
    let setup_description = description.clone();
    let skill_body = "skill body";
    let mut builder = test_codex().with_workspace_setup(move |cwd, fs| async move {
        write_repo_skill(cwd, fs, "demo", &setup_description, skill_body).await
    });
    let test = builder.build_with_remote_env(&server).await?;
    ...
    let request = mock.single_request();
    let user_texts = request.message_input_texts("user");
    let injected_skill = user_texts
        .iter()
        .find(|text| text.contains("<skill>\n<name>demo</name>"))
        .ok_or_else(|| anyhow::anyhow!("expected injected skill in {user_texts:?}"))?;
    assert!(injected_skill.contains(&format!("description: {expected_description}")));
    assert!(!injected_skill.contains(&format!("description: {description}")));
    assert!(injected_skill.contains(skill_body));

    let source = std::fs::read_to_string(&skill_path)?;
    assert!(source.contains(&format!("description: {description}")));
