...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shell_escalated_permissions_rejected_then_ok_freeform_gpt_5_1() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let command = ["/bin/echo", "shell ok"];
    let call_id_blocked = "shell-blocked-freeform-gpt-5.1";
    let call_id_success = "shell-success-freeform-gpt-5.1";
    ...
    assert!(
        serde_json::from_str::<Value>(&success_output).is_err(),
        "expected gpt-5.1 shell output to be plain text"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shell_timeout_includes_timeout_prefix_and_metadata_freeform_gpt_5_1() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let call_id = "shell-timeout-freeform-gpt-5.1";
    let timeout_ms = 50u64;
    let args = json!({
        "command": ["/bin/sh", "-c", "yes line | head -n 400; sleep 1"],
        "timeout_ms": timeout_ms,
    ...
    test.submit_turn_with_policies(
        "run a long command (freeform gpt-5.1)",
        AskForApproval::Never,
        SandboxPolicy::DangerFullAccess,
    )
    .await?;
    ...
    assert!(
        serde_json::from_str::<Value>(output_str).is_err(),
        "expected gpt-5.1 shell timeout output to be plain text"
    );
    assert!(
        output_str.contains("Exit code: 124"),
        "timeout output missing exit code 124: {output_str}"
    );
    assert!(
        output_str.contains("command timed out"),
        "timeout output missing `command timed out`: {output_str}"
    );
