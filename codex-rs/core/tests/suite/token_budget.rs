...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_context_remaining_returns_unbounded_threshold_without_window() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    assert_eq!(
        ...
        Some((
            Some("You have 9223372036854775807 tokens left in this context window.".to_string()),
            None,
        ))
    );
