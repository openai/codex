...
/// same sandbox with `codex sandbox -P "$CODEX_PERMISSION_PROFILE"`. The test
/// proves both halves of that contract: the wrapper writes outside the
...
/// named profile needed to reconstruct the original sandbox remotely.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
"#
    ...
    assert_eq!(
        ...
        "remote_bash.py should use the inherited profile to reconstruct the Codex sandbox"
    );
    let command_argv = shlex::split(&sandbox_argv[7]).context("parse remote bash command")?;
    assert_eq!(
        command_argv,
