...
async fn deferred_executor_replaces_a_colliding_host_skill_for_the_next_step() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_thread_extension_init(thread_extension_init)
    ...
    assert_eq!(skill_body_occurrences(&requests[0], HOST_BODY), 1);
    assert_eq!(skill_body_occurrences(&requests[0], EXECUTOR_BODY), 0);
    assert_eq!(skill_body_occurrences(&requests[1], HOST_BODY), 1);
    assert_eq!(skill_body_occurrences(&requests[1], EXECUTOR_BODY), 1);
    assert_eq!(environment_skill_occurrences(&requests[0], "deploy"), 0);
    assert_eq!(environment_skill_occurrences(&requests[1], "deploy"), 1);
    ...
    let replacement_catalog = second_developer_context
        .iter()
        .find(|text| text.contains("This complete skills list replaces"))
        .expect("second request should contain the replacement skill catalog");
    assert!(!replacement_catalog.contains("### How to use skills"));
    Ok(())
}

fn skill_body_occurrences(request: &ResponsesRequest, contents: &str) -> usize {
    request
        .message_input_texts("user")
        .iter()
        .filter(|text| text.starts_with("<skill>") && text.contains(contents))
        .count()
}

fn environment_skill_occurrences(request: &ResponsesRequest, name: &str) -> usize {
    request
