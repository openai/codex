...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shell_output_is_structured_for_gpt_5_1_freeform() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let call_id = "shell-json-freeform-gpt-5.1";
    let args = json!({
        "command": ["/bin/echo", "shell json"],
        "timeout_ms": 1_000,
    });
    ...
    test.submit_turn_with_policy(
        "run the json shell command with freeform",
        SandboxPolicy::DangerFullAccess,
    )
    .await?;
    ...
    assert!(
        ...
        "expected gpt-5.1 freeform shell output to be plain text",
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn shell_output_structures_fixture_json_for_gpt_5_1_freeform() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let call_id = "shell-json-fixture-freeform-gpt-5.1";
    let args = json!({
        "command": ["/usr/bin/sed", "-n", "p", fixture_path_str],
        "timeout_ms": 1_000,
    });
    ...
    test.submit_turn_with_policy(
        "read the fixture JSON with freeform structured output",
        SandboxPolicy::DangerFullAccess,
    )
    .await?;
