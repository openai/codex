...
#[cfg(test)]
mod test_helpers {
    use codex_core::context_analyzer::{ContextBreakdown, estimate_tokens};
    ...
    fn test_custom_tool_with_nested_json() {
        ...
        let history = vec![
            ...
            ResponseItem::CustomToolCallOutput {
                call_id: "tool_analyze_123".to_string(),
                output:
                    "Analysis complete: 3 security issues, 5 performance improvements suggested"
                        .to_string(),
            },
        ];
...
mod context_breakdown_scenarios {
    use super::test_helpers::*;
    use codex_core::context_analyzer::{ContextBreakdown, analyze_context};
    use codex_protocol::models::*;
    ...
    fn test_tool_definition_variations() {
        let test_cases = vec![
            ...
            (
                r#"{"tools": [{"name": "t1"}, {"name": "t2"}, {"name": "t3"}]}"#,
                10,
            ), // Multiple tools
        ];
...
mod stress_tests {
    ...
    fn test_memory_efficiency() {
        ...
        for _ in 0..1000 {
            ...
            let _ = analyze_context(Some("System prompt"), &history, Some(r#"{"tools": []}"#));
        }
