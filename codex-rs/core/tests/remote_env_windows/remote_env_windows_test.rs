...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn explicit_powershell_runs_in_windows_cwd() -> Result<()> {
    const CALL_ID: &str = "wine-explicit-powershell";
    const COMMAND: &str = r#"$cwd = (Get-Location).Path; if ($cwd -ne 'C:\windows') { Write-Error \"unexpected cwd: $cwd\"; exit 1 }"#;
    ...
    WineExecServer
        .scope(|exec_server_url| async move {
            ...
            test.codex
                .submit(Op::UserInput {
                    items: vec![UserInput::Text {
                        text: "run PowerShell in the Windows cwd".to_string(),
                        text_elements: Vec::new(),
                    }],
                    final_output_json_schema: None,
                    responsesapi_client_metadata: None,
            ...
            let end = end.context("exec_command should emit an end event")?;
            assert_eq!(
                (end.cwd, end.exit_code, end.status),
                (
                    PathUri::parse("file:///C:/windows")?,
                    0,
            ...
            let (_output, success) = request
                .function_call_output_content_and_success(CALL_ID)
                .context("PowerShell tool result should be present")?;
            assert_ne!(success, Some(false));
            Ok(())
        })
        .await
