...
use async_command::AsyncCommandRuntime;
pub use async_command::AsyncHookDelivery;
pub use async_command::AsyncHookDeliveryCutoff;
#[derive(Debug, Clone)]
...
impl ClaudeHooksEngine {
    ...
    pub(crate) fn async_delivery_cutoff(&self) -> AsyncHookDeliveryCutoff {
        self.async_runtime.delivery_cutoff()
    }

    pub(crate) fn commit_accepted_turn_and_drain_async_output(
        &self,
        cutoff: AsyncHookDeliveryCutoff,
    ) -> AsyncHookDelivery {
        self.async_runtime.commit_accepted_turn_and_drain(cutoff)
    }
    ...
    async fn maybe_spill_text(&self, session_id: ThreadId, text: Option<String>) -> Option<String> {
        match text {
            Some(text) => Some(
                self.async_runtime
                    .output_spiller()
                    .maybe_spill_text(session_id, text)
                    .await,
            ),
            None => None,
        }
