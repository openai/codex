...
fn otel_export_routing_policy_routes_startup_and_ttft_observability() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        let root_span = tracing::info_span!("root");
        let _root_guard = root_span.enter();
        manager.record_startup_phase(
            "startup_prewarm_build_tools",
            std::time::Duration::from_millis(87),
            Some("ready"),
        );
        manager.record_turn_ttft(std::time::Duration::from_millis(321));
    });
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let startup_log = find_log_by_event_name(&logs, "codex.startup_phase");
    let startup_log_attrs = log_attributes(&startup_log.record);
    assert_eq!(
        startup_log_attrs.get("startup.phase").map(String::as_str),
        Some("startup_prewarm_build_tools")
    );
    assert_eq!(
        startup_log_attrs.get("startup.status").map(String::as_str),
        Some("ready")
    );
    assert_eq!(
        startup_log_attrs.get("duration_ms").map(String::as_str),
        Some("87")
    );

    let ttft_log = find_log_by_event_name(&logs, "codex.turn_ttft");
    let ttft_log_attrs = log_attributes(&ttft_log.record);
    assert_eq!(
        ttft_log_attrs.get("duration_ms").map(String::as_str),
        Some("321")
    );

    let spans = span_exporter.get_finished_spans().expect("span export");
    let startup_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "codex.startup_phase");
    let startup_trace_attrs = span_event_attributes(startup_trace_event);
    assert_eq!(
        startup_trace_attrs.get("startup.phase").map(String::as_str),
        Some("startup_prewarm_build_tools")
    );

    let ttft_trace_event = find_span_event_by_name_attr(&spans[0].events.events, "codex.turn_ttft");
    let ttft_trace_attrs = span_event_attributes(ttft_trace_event);
    assert_eq!(
        ttft_trace_attrs.get("duration_ms").map(String::as_str),
        Some("321")
    );
}
