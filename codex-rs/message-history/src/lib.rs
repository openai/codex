...
fn history_filepath(config: &MessageHistoryConfig) -> AbsolutePathBuf {
...
///
/// The entry is silently skipped when `config.persistence` is
...
pub async fn history_metadata(config: &MessageHistoryConfig) -> (u64, usize) {
    let path = history_filepath(config);
...
/// `spawn_blocking`.
pub fn lookup(log_id: u64, offset: usize, config: &MessageHistoryConfig) -> Option<HistoryEntry> {
    let path = history_filepath(config);
