    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    match path_entry_guard {
        Some(path_entry_guard) => runtime.block_on(run_main_with_arg0_guard(
            path_entry_guard,
            current_exe,
            main_fn,
        )),
        None => {
            let paths = Arg0DispatchPaths {
                ...
                codex_linux_sandbox_exe: if cfg!(target_os = "linux") {
                    linux_sandbox_exe_path(/*path_entry_guard*/ None, current_exe)
                } else {
                    None
                },
                main_execve_wrapper_exe: None,
            };
            runtime.block_on(main_fn(paths))
        }
    }
}

async fn run_main_with_arg0_guard<F, Fut>(
    path_entry_guard: Arg0PathEntryGuard,
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        codex_self_exe: current_exe.clone(),
        codex_linux_sandbox_exe: if cfg!(target_os = "linux") {
            linux_sandbox_exe_path(Some(&path_entry_guard), current_exe)
        } else {
            None
        },
        main_execve_wrapper_exe: path_entry_guard.paths().main_execve_wrapper_exe.clone(),
    };
