...
"#,
    ...
    let updated_path_env_var = path_env_with_entry(path, std::env::var_os("PATH"));
...
fn prepend_package_path_dir() {
    let Some(updated_path_env_var) =
        path_env_with_package_path_dir(InstallContext::current(), std::env::var_os("PATH"))
    else {
        return;
    };
