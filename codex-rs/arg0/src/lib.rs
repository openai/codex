...
fn prepare_path_env_var() -> (Option<Arg0PathEntryGuard>, Option<OsString>) {
    prepare_path_env_var_with_aliases(
...
mod tests {
    ...
    fn install_context_with_package_path(
        package_dir: &Path,
        bin_dir: &Path,
    ...
    fn path_env_can_prepend_package_path_before_arg0_alias_dir() -> anyhow::Result<()> {
        ...
        let install_context =
            install_context_with_package_path(&package_dir, &bin_dir, path_dir.clone())?;
    ...
    fn package_path_survives_arg0_alias_setup_failure() -> anyhow::Result<()> {
        ...
        let (path_entry_guard, updated_path_env_var) = super::prepare_path_env_var_with_aliases(
            ...
            |path_for_aliases| {
                assert_eq!(
                    ...
                    .collect::<Vec<_>>(),
                    vec![path_dir.as_path().to_path_buf(), existing_dir.clone()],
                );
        ...
        assert_eq!(
            ...
            vec![path_dir.as_path().to_path_buf(), existing_dir],
        );
        Ok(())
    }
