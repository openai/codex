...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    match prepare_path_entry_for_codex_aliases(path_for_aliases) {
        Ok((path_entry, updated_path_env_var)) => {
            set_path_env_var(updated_path_env_var);
            Some(path_entry)
        }
        Err(err) => {
            if let Some(package_path) = package_path {
                set_path_env_var(package_path);
            }
...
/// that `apply_patch` can be on the PATH without requiring the user to install
/// a separate executable, simplifying the deployment of Codex CLI.
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    let (path_entry_guard, updated_path_env_var) =
        prepare_path_entry_for_codex_aliases(std::env::var_os("PATH"))?;
    set_path_env_var(updated_path_env_var);
    Ok(path_entry_guard)
}
