...
"#,
    ...
    let mut paths = Arg0DispatchPaths::default();
    #[cfg(target_os = "linux")]
    {
        paths.codex_linux_sandbox_exe = Some(path.join(LINUX_SANDBOX_ARG0));
    }
    #[cfg(unix)]
    {
        paths.main_execve_wrapper_exe = Some(path.join(EXECVE_WRAPPER_ARG0));
    }
