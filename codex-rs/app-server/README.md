...
- Local thread paths returned as `cwd`, including nested `thread.cwd` values from `thread/start`, `thread/resume`, `thread/fork`, `thread/list`, `thread/read`, and `thread/started`, are best-effort canonicalized so filesystem aliases resolve to one stable spelling. Paths that the app-server host cannot resolve retain their original spelling.
