...
- `thread/archive` — move a thread’s rollout file and any spawned descendant thread rollout files into the archived directory; returns `{}` on success and emits `thread/archived` for each archived thread.
...
Use `thread/archive` to move the persisted rollout (stored as a JSONL file on disk) and any spawned descendant thread rollouts into the archived sessions directory.
