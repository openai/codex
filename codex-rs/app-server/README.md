...
- `mcpServer/tool/call` — call a tool on a thread's configured MCP server by `threadId`, `server`, `tool`, optional `arguments`, and optional `_meta`, returning the MCP tool result. For direct MCP servers, app-server forwards caller-supplied `_meta` except private OpenAI/Codex identity, routing, workspace, turn, tracing, plugin, and connector fields, plus keys in the reserved `openai/`, `x-openai-`, `_openai_`, `codex/`, `x-codex-`, `_codex_`, and `codex_` namespaces. Codex does not inject `threadId` for direct calls. Host-owned servers retain their existing metadata behavior.
