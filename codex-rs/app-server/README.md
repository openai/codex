...
- `thread/start`, `thread/resume`, and `thread/fork` responses include the legacy `sandbox` compatibility projection. `instructionSources` lists loaded instruction files using each source environment's native absolute path syntax, including files loaded from remote environments. Experimental clients can read `runtimeWorkspaceRoots` for the thread-scoped runtime roots and `activePermissionProfile` for the named or implicit built-in profile identity/provenance when known. When a live thread owns a managed network proxy, experimental `thread.extra.networkProxy` reports its runtime `httpAddr`, `socksAddr`, and whether TLS `mitm` interception is active; these host-local endpoints are omitted from stored/listed thread metadata. Their deprecated experimental `multiAgentMode` field, and the corresponding thread setting, always report `explicitRequestOnly`; Ultra reasoning effort is the source of proactive multi-agent behavior.
...
- `thread/settings/updated` — experimental notification emitted to subscribed clients when a loaded thread’s effective next-turn settings change; includes `threadId`, the full `threadSettings`, and the latest live managed-proxy runtime in `networkProxy` (or `null` when inactive).
