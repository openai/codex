...
`executionMode` reports how a command hook runs. `sync` hooks participate in the current operation, while `async` hooks run in the background and can only contribute informational output to a later accepted fresh user turn.
