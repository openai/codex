...
- `plugin/list` — list discovered plugin marketplaces, including plugin id, installed/enabled state, and optional interface metadata. Pass `forceRemoteSync: true` to reconcile the official curated marketplace against the remote plugin state before listing (**under development; do not call from production clients yet**).
