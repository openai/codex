...
- `usage/read` — read forward-only local token usage for a rolling `day` or `week` range, grouped by skills, subagents, apps, MCP servers, and plugins when Codex has tracked those contributors in sqlite.
