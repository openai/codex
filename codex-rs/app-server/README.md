...
- `thread/rollback` — deprecated and will be removed soon. Drop the last N turns from the agent’s in-memory context and persist a rollback marker in the rollout so future resumes see the pruned history; returns the updated `thread` (with `turns` populated) on success. Use `thread/fork` instead; `thread/fork` will gain `turnId` so clients can retain history through a specific turn without mutating the original thread.
