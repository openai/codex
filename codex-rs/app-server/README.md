...
- `marketplace/checkUpdates` — check all configured Git plugin marketplaces, or one named marketplace when `marketplaceName` is provided, without changing config or installed marketplace snapshots. Returns an `upToDate`, `updateAvailable`, or `error` result for each selected marketplace.
