...
4. `item/commandExecution/approvalResolved` — `{ threadId, requestId }` confirms the pending approval request has been resolved or cleared, including lifecycle cleanup on turn start/complete/interrupt.
...
4. `item/fileChange/approvalResolved` — `{ threadId, requestId }` confirms the pending approval request has been resolved or cleared, including lifecycle cleanup on turn start/complete/interrupt.
...
When the client responds to `item/tool/requestUserInput`, the server emits `item/tool/requestUserInputResolved` with `{ threadId, requestId }`. If the pending request is cleared by turn start, turn completion, or turn interruption before the client answers, the server emits the same notification for that cleanup.
