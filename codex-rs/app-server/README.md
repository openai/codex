...
- `permissionProfile/list` — list available permission profile ids with optional display `description` text, using cursor pagination. Pass `cwd` when the caller needs project-local `[permissions.<id>]` entries to be included in the current catalog view. Managed `requirements.toml` profiles are included too, and `allowed_permissions` filters the returned catalog when present.
