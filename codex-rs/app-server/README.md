...
- `skills/list` — list skills for one or more `cwd` values (optional `forceReload`). Pass `threadId` to evaluate skills from a loaded thread's refreshed config and cwd. `threadId` cannot be combined with `cwds`.
...
Use `skills/list` to fetch the available skills (optionally scoped by either `cwds` or `threadId`, with `forceReload`).
`skills/list` might reuse a cached skills result per `cwd`; setting `forceReload` to `true` refreshes the result from disk.
When `threadId` is provided, the server lists skills for that loaded thread's cwd and refreshed config. Do not combine `threadId` with `cwds`.
