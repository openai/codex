...
- `marketplace/list` — list configured marketplaces from `config.toml`, returning paginated marketplace records with the derived installed `marketplace.json` path plus stored source metadata (`lastUpdated`, `sourceType`, `source`, `refName`, `sparsePaths`).
