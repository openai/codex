...
When the thread is already loaded and idle, `thread/resume` reconciles completed history written by another app-server process into the live session before responding. This supports sequential handoff between clients without restarting the first app-server. An active local turn is never overwritten; if persisted history is still incomplete or the live session changes during reconciliation, retry `thread/resume` after the writer finishes.
