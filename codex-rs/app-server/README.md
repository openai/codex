...
- `plugin/read` — read one plugin by `marketplacePath` plus `pluginName`, returning marketplace info, a list-style `summary`, manifest descriptions/interface metadata, and bundled skills/apps/MCP server names. For uninstalled remote Git plugin sources, `detailsUnavailableReason` is `installRequiredForRemoteSource` and bundled skills/apps/MCP servers are empty so browsing does not clone remote content. Returned plugin skills include their current `enabled` state after local config filtering. Plugin app summaries also include `needsAuth` when the server can determine connector accessibility (**under development; do not call from production clients yet**).
