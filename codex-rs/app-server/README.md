...
- `visualArtifact` — `{id, status, visuals, document, error}` for inline interactive visuals emitted by the `artifacts` tool; `status` is `inProgress`, `completed`, or `failed`, `visuals` entries are `{title, html, summary, heightPx}`, and `document` is either `null` or `{title, markdown}` for the live markdown workspace.
