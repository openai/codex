...
- `thread/start` and `thread/fork` accept optional `threadSourceContractVersion`. Set it to `1` only when the caller has implemented the version 1 `threadSource` classification contract; omit it for legacy or unaudited callers. App-server rejects unsupported versions, persists the value with the thread, restores it on resume, and returns it as nullable `thread.threadSourceContractVersion`. App-server never infers the version from `threadSource`.
