...
- `mcpServer/resource/read` — read a resource from a configured MCP server by optional `threadId`, `server`, `uri`, and `originCallId`, returning text/blob resource `contents`. When `originCallId` identifies a Codex Apps tool call backed by plugin-runtime, app-server validates the persisted app context and routes the read through that app's `fetch_resource` tool. Other reads continue to use MCP `resources/read`. `originCallId` requires `threadId`; if `threadId` is omitted, the server reads from the latest MCP config directly.
