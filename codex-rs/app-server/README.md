...
- Threads created internally by Codex are implicitly attached to every initialized connection so their events and server requests can be delivered. An implicit attachment does not count as a subscriber for the 30-minute idle unload. `thread/start`, `thread/resume`, and `thread/fork` establish retaining subscriptions; resuming an implicitly attached thread upgrades that connection.
