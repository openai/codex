...
- `httpState/set` — write the initialized client's recognized native-surface HTTP state file. Pass a string `expectedState` to write only when the file still contains that value; omit it or pass `null` to replace the file unconditionally. Returns `{ written }`.
- `httpState/clear` — delete the HTTP state file for the initialized client's recognized native surface. Returns `{}`.

These methods use the initialized `clientInfo.name` as routing metadata. App-server transport access remains the trust boundary.
