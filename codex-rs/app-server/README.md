...
- `mcpServerStatus/list` — enumerate configured MCP servers with their tools, resources, resource templates, and auth status; supports cursor+limit pagination and optional `cwd` to resolve config layers (e.g. `./.codex/config.toml`).
