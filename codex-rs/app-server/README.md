...
- `review/start` — start a regular turn that asks Codex to delegate the requested review to a sub-agent using the bundled `$review-agent` skill. Bundled skills must be enabled, and inline delivery requires an idle thread. The response shape is unchanged, while progress and the final review use the normal turn, collaboration, and `agentMessage` events.
...
active turn, or the active turn kind does not accept same-turn steering (manual compaction), the
request fails with an `invalid request` error.

### Example: Request a code review

Use `review/start` to ask Codex to delegate a review of the currently checked-out project to a sub-agent using the bundled `$review-agent` skill. The request takes the thread id plus a `target` describing what should be reviewed:
...
The delegation is an ordinary user turn. Clients receive the usual turn, collaboration, and final `agentMessage` events; new reviews do not emit `enteredReviewMode` or `exitedReviewMode`. Inline delivery requires an idle thread. If collaboration tools are unavailable, the turn completes with an explanation instead of reviewing in the parent agent.
...
- `enteredReviewMode` — legacy `{id, review}` item returned when reading or replaying reviews created by the removed core review flow; new `review/start` calls do not emit it.
- `exitedReviewMode` — legacy `{id, review}` item returned when reading or replaying reviews created by the removed core review flow; new `review/start` calls return the review as a normal `agentMessage`.
...
- `ActiveTurnNotSteerable { turnKind }`: `turn/start` or `turn/steer` was submitted while the
  current active turn was not steerable, currently manual `/compact`; `review` remains a possible
  legacy `turnKind` value for compatibility
- `BadRequest`
- `Unauthorized`
- `SandboxError`
- `InternalServerError`
