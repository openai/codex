...
- `environment/info` — experimental; connect to a configured environment by `environmentId` and return its detected `shell` plus environment-native `cwd`. Connection failures are returned as request errors.
