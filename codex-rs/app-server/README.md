...
- `config/inMemory/set` — replace the in-memory config layer used by subsequent config loads in this app-server process. The layer is shaped like `config.toml`, sits above user config and below project/session overrides, and `{ "config": {} }` clears it. Existing running threads retain their config snapshots, just as when `config.toml` changes.
