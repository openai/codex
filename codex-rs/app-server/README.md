...
- `imageGeneration` — `{id, status, revisedPrompt, result, savedPath?, error}` for standalone image generation; failed items use `status: "failed"`, an empty `result`, and a user-facing `error` message, while successful and in-progress items set `error` to `null`.
