...
- `environment/add` — experimental; register a named remote environment by `environmentId` for later selection by `thread/start` or `turn/start`; omit `execServerUrl` while it is still starting, then call again with the URL when available. Re-adding an `environmentId` updates existing selections, so use a new id for a different environment. Returns `{}` and does not change the default environment.
