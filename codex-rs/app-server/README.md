...
- `item/autoApprovalReview/completed` — [UNSTABLE] temporary auto-review notification carrying `{threadId, turnId, targetItemId, review, action}` when approval auto-review resolves. The review status distinguishes policy decisions (`approved`, `denied`) from reviews that timed out, failed before producing a decision, or were aborted. This shape is expected to change soon.
