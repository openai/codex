...
- `config/read` — fetch the effective config on disk after resolving config layering, including opaque `app_settings` values stored in `config.toml`.
...
- `config/value/write` — write a single config key/value to the user's config.toml on disk; dotted paths such as `app_settings.someKey` use the same generic write surface.
- `config/batchWrite` — apply multiple config edits atomically to the user's config.toml on disk, with optional `reloadUserConfig: true` to hot-reload loaded threads, including multiple `app_settings.*` edits.
