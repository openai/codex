...
- `skills/list` — list skills for one or more `cwd` values (optional `forceReload`). Pass `threadId` to evaluate skills from a loaded thread's refreshed config; when `cwds` is omitted, the server uses that thread's cwd.
...
Use `skills/list` to fetch the available skills (optionally scoped by `cwds` or `threadId`, with `forceReload`).
`skills/list` might reuse a cached skills result per `cwd`; setting `forceReload` to `true` refreshes the result from disk.
When `threadId` is provided and `cwds` is omitted, the server lists skills for that loaded thread's cwd and refreshed config.
