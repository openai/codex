...
- `externalAgentConfig/import` — apply selected external-agent migration items by passing explicit `migrationItems` with `cwd` (`null` for home) and any plugin/session `details` returned by detect. Returns an `importId` used to correlate follow-up notifications. When a request includes migration items, the server emits `externalAgentConfig/import/progress` after each import step with the same `importId`, item result, success/error counts, and raw errors. It emits `externalAgentConfig/import/completed` once after the full import finishes with type-level `itemResults` containing each migrated type's success count, error count, and raw errors (immediately after the response when everything completed synchronously, or after background imports finish).
