...
impl ThreadRequestProcessor {
    ...
    async fn thread_resume_inner(
        ...
        let permission_profile_selection = if let Some(permissions) = permissions {
            ...
            match resolve_legacy_sandbox_profile_selection(
                ...
                Ok(LegacySandboxResolution::Noop) => None,
        ...
        if let Some(selection) = permission_profile_selection {
            typesafe_overrides.permission_profile = Some(selection.permission_profile);
        } else if let Some(persisted_permission_state) = persisted_permission_state.as_ref() {
        ...
        if let Some((existing_thread_id, existing_thread, source_thread)) = running_thread {
            ...
            let permission_profile_selection = if let Some(permissions) = params.permissions.clone()
            {
                ...
                match resolve_legacy_sandbox_profile_selection(
                    ...
                    LegacySandboxResolution::Noop => None,
    ...
    async fn thread_fork_inner(
        ...
        typesafe_overrides.permission_profile =
            Some(if let Some(selection) = permission_profile_selection {
                selection.permission_profile
            } else {
                persisted_permission_state.permission_profile.clone()
            });
        if !workspace_roots_were_explicit {
            typesafe_overrides.workspace_roots = Some(
                persisted_permission_state
