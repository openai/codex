...
impl PluginRequestProcessor {
    ...
    fn remote_plugin_service_config(&self, config: &Config) -> RemotePluginServiceConfig {
        RemotePluginServiceConfig {
            chatgpt_base_url: config.chatgpt_base_url.clone(),
            auth_route_config: self.auth_manager.auth_route_config().cloned(),
        }
    }
    ...
    async fn plugin_list_response(
        ...
        let remote_plugin_service_config = self.remote_plugin_service_config(&config);
                ...
                let share_context = match share_context {
                    Some(context) => {
                        ...
                        let remote_plugin_service_config =
                            self.remote_plugin_service_config(&config);
    ...
    async fn remote_plugin_install_response(
        ...
        let result = codex_core_plugins::remote_bundle::download_and_install_remote_plugin_bundle(
            ...
            validated_bundle,
            remote_plugin_service_config.auth_route_config.as_ref(),
        )
        .await
        .map_err(remote_plugin_bundle_install_error_to_jsonrpc)?;
...
fn remote_plugin_catalog_error_to_jsonrpc(
    ...
    match &err {
        ...
        | RemotePluginCatalogError::HttpClient { .. }
