...
pub(super) struct LegacySandboxProfileSelection {
    ...
    expected_sandbox_policy: codex_protocol::protocol::SandboxPolicy,
}
...
pub(super) fn resolve_legacy_sandbox_profile_selection(
    ...
    let expected_sandbox_policy = sandbox_policy.to_core();
...
pub(super) fn validate_legacy_sandbox_profile_selection(
    ...
    cwd: &AbsolutePathBuf,
    workspace_roots: Option<&[AbsolutePathBuf]>,
    field_name: &str,
) -> Result<(), JSONRPCErrorError> {
    let workspace_roots = workspace_roots
        .or(legacy_selection.workspace_roots.as_deref())
        .unwrap_or(&[]);
    let materialized_profile = resolved_selection
        .permission_profile
    ...
    if active_sandbox != legacy_selection.expected_sandbox_policy {
...
fn legacy_sandbox_current_match(
    ...
    let implicit_legacy_workspace_roots = if explicit_workspace_roots.is_none()
        && matches!(
            sandbox_policy,
    ...
    let current_roots_satisfy_implicit_legacy_workspace =
        current.workspace_roots.iter().any(|root| root == cwd)
            && sandbox_policy.legacy_writable_roots().is_empty();
    let projection_workspace_roots =
        if let Some(explicit_workspace_roots) = explicit_workspace_roots {
            explicit_workspace_roots
        } else if let Some(implicit_legacy_workspace_roots) = implicit_legacy_workspace_roots {
            if current_roots_satisfy_implicit_legacy_workspace {
                current.workspace_roots
            } else {
                implicit_legacy_workspace_roots
            }
        } else {
            current.workspace_roots
        };
    ...
    let workspace_roots = legacy_workspace_roots
        .filter(|_| projection_workspace_roots != current.workspace_roots)
...
mod tests {
    ...
    #[test]
    fn legacy_workspace_sandbox_preserves_current_extra_roots_when_cwd_is_still_a_root() {
        let cwd = abs_test_path("codex-current-workspace-root-with-extra");
        let extra_root = abs_test_path("codex-extra-workspace-root");
    ...
    #[test]
    fn legacy_workspace_sandbox_honors_non_empty_legacy_writable_roots() {
        let cwd = abs_test_path("codex-current-workspace-root-with-legacy-extra");
        let current_extra_root = abs_test_path("codex-current-extra-workspace-root");
        let requested_extra_root = abs_test_path("codex-requested-extra-workspace-root");
        let workspace_roots = vec![cwd.clone(), current_extra_root];
        let policy = codex_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            network_access: false,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
            legacy_writable_roots: vec![requested_extra_root.clone()],
        };
        ...
        match resolution {
            LegacySandboxResolution::Noop {
                workspace_roots: Some(workspace_roots),
            } => assert_eq!(workspace_roots, vec![cwd, requested_extra_root]),
            _ => panic!("expected workspace-roots-only resolution, got unexpected selection"),
        }
    }

    #[test]
    fn legacy_workspace_selection_validation_requires_exact_projection() {
        let cwd = abs_test_path("codex-projection-mismatch-root");
        let policy = workspace_write_policy();
        let resolution = resolve_legacy_sandbox_profile_selection(
            &policy,
            /*current*/ None,
        ...
        .expect("legacy sandbox should resolve");
        let LegacySandboxResolution::Selection(selection) = resolution else {
            panic!("expected named profile selection");
        };
        let resolved_selection = ResolvedPermissionProfileSelection {
            ...
            active_permission_profile: codex_protocol::models::ActivePermissionProfile::new(
                ":workspace",
            ),
        };

        let err = validate_legacy_sandbox_profile_selection(
            &selection,
            &resolved_selection,
            &cwd,
            selection.workspace_roots.as_deref(),
            "sandboxPolicy",
        )
        .expect_err("mismatched workspace profile should be rejected");

        assert_eq!(
            err.message,
            "`sandboxPolicy` does not match permissions profile `:workspace`"
        );
    }
}
