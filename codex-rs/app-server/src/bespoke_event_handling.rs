...
async fn take_terminal_turn_snapshot(
    _conversation_id: ThreadId,
    thread_state: &Arc<Mutex<ThreadState>>,
) -> Option<Turn> {
    let mut state = thread_state.lock().await;
    state.take_terminal_turn_snapshot()
}
...
async fn handle_turn_complete(
    ...
    let turn_snapshot = take_terminal_turn_snapshot(conversation_id, thread_state).await;
    ...
    emit_turn_completed_with_status(
        ...
        TurnCompletionMetadata {
            status,
            error,
            started_at: turn_snapshot.and_then(|turn| turn.started_at),
