...
fn granted_request_permissions_within_requested_scope(
    ...
    intersect_permission_profiles(requested, granted, cwd).into()
}
...
mod tests {
    ...
    #[test]
    fn request_permissions_response_caps_broader_cwd_grant_to_requested_child_path() {
        let cwd = TempDir::new().expect("create temp dir");
        let child =
            AbsolutePathBuf::try_from(cwd.path().join("child.txt")).expect("absolute child path");
        let requested_permissions = CoreRequestPermissionProfile {
    ...
    #[test]
    fn request_permissions_response_preserves_requested_carveouts_when_client_grants_broad_root() {
        let cwd = TempDir::new().expect("create temp dir");
        let blocked = AbsolutePathBuf::try_from(cwd.path().join("blocked.txt"))
