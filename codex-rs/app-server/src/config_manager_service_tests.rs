...
#[tokio::test]
async fn read_allows_unrelated_legacy_profiles_with_selected_user_profile() -> Result<()> {
    let tmp = tempdir().expect("tempdir");
    std::fs::write(
        tmp.path().join(CONFIG_TOML_FILE),
        r#"model = "gpt-main"
...
"#,
    ...
    std::fs::write(&selected_path, "model = \"gpt-work\"\n")?;
    ...
    let read = service
        ...
        .await
        .expect("unrelated legacy profile table should not block config read");

    assert_eq!(read.config.model.as_deref(), Some("gpt-work"));
    assert!(!read.config.additional.contains_key("profiles"));
    assert!(
        read.origins
            .keys()
            .all(|path| !path.starts_with("profiles.")),
        "origins still expose legacy profiles: {:?}",
        read.origins
    );
    assert!(
        read.layers
            ...
            .iter()
            .all(|layer| layer.config.get("profiles").is_none()),
        "layers still expose legacy profiles"
    );
    Ok(())
}
