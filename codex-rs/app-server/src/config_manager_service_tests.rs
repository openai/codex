...
#[tokio::test]
async fn read_reports_writable_user_config_version_when_override_wins() {
    let tmp = tempdir().expect("tempdir");
    std::fs::write(tmp.path().join(CONFIG_TOML_FILE), "model = \"base\"").unwrap();
    std::fs::write(
        tmp.path().join(codex_config::CONFIG_OVERRIDE_TOML_FILE),
        "model = \"override\"",
    )
    .unwrap();
    ...
    assert_eq!(read.config.model.as_deref(), Some("override"));
    assert_eq!(read.user_config_version, Some(user_layer.version.clone()));
}

#[tokio::test]
async fn write_value_rejects_stale_user_config_version_when_override_wins() {
    let tmp = tempdir().expect("tempdir");
    std::fs::write(tmp.path().join(CONFIG_TOML_FILE), "model = \"base\"").unwrap();
    std::fs::write(
        tmp.path().join(codex_config::CONFIG_OVERRIDE_TOML_FILE),
    ...
    let read = service
        ...
        .await
        .expect("read succeeds");
    std::fs::write(tmp.path().join(CONFIG_TOML_FILE), "model = \"changed\"").unwrap();
    ...
    let err = service
        .write_value(ConfigValueWriteParams {
            ...
            merge_strategy: MergeStrategy::Replace,
            expected_version: read.user_config_version,
        })
        .await
        .expect_err("stale user config version should conflict");
