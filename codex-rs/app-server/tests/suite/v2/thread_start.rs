...
async fn thread_start_selects_permission_profile_by_id() -> Result<()> {
    ...
    assert_eq!(workspace_roots, vec![workspace_root.clone()]);
    let permission_profile: codex_protocol::models::PermissionProfile = permission_profile
        .expect("response should include effective profile")
        .into();
    assert!(
        permission_profile
            .file_system_sandbox_policy()
            .can_write_path_with_cwd(workspace_root.as_path(), workspace_root.as_path())
    );
    assert_eq!(
        sandbox,
