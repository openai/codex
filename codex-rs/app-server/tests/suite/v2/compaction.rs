...
async fn send_turn_and_wait(mcp: &mut AppServer, thread_id: &str, text: &str) -> Result<String> {
    let turn_id = mcp
...
async fn wait_for_turn_completed(mcp: &mut AppServer, turn_id: &str) -> Result<()> {
    loop {
