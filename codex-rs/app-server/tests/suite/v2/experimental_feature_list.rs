...
use app_test_support::AppServer;
...
async fn experimental_feature_list_returns_feature_metadata_with_stage() -> Result<()> {
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await?;
    let mut mcp = AppServer::new(codex_home.path()).await?;
...
"#,
    ...
    let mut mcp = AppServer::new_without_managed_config(codex_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn read_config(mcp: &mut AppServer, cwd: Option<String>) -> Result<ConfigReadResponse> {
    let request_id = mcp
...
async fn read_response<T: DeserializeOwned>(mcp: &mut AppServer, request_id: i64) -> Result<T> {
