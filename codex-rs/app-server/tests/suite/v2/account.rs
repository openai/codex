...
#[tokio::test]
async fn get_account_with_chatgpt_dedupes_workspace_role_refresh_after_success() -> Result<()> {
    let codex_home = TempDir::new()?;
    let mock_server = MockServer::start().await;
    create_config_toml(
        codex_home.path(),
    ...
    .await??;
    let first_account: GetAccountResponse = to_response(first_resp)?;
    assert_eq!(
        first_account,
        GetAccountResponse {
            account: Some(Account::Chatgpt {
    ...
    .await??;
    let second_account: GetAccountResponse = to_response(second_resp)?;
    assert_eq!(
        second_account,
        GetAccountResponse {
            account: Some(Account::Chatgpt {
    ...
    let duplicate_note = timeout(
        Duration::from_millis(250),
        mcp.read_stream_until_notification_message("account/updated"),
    )
    .await;
    assert!(
        duplicate_note.is_err(),
        "duplicate account/updated notification should not be emitted once workspace role is cached"
    );
