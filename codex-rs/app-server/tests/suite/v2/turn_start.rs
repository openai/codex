...
async fn turn_start_rejects_unknown_permission_selection_before_starting_turn() -> Result<()> {
    let codex_home = TempDir::new()?;
    let server = responses::start_mock_server().await;
    create_config_toml_with_extra_top_level_config(
        codex_home.path(),
        &server.uri(),
        "never",
        &BTreeMap::from([(Feature::Personality, true)]),
...
#[tokio::test]
async fn turn_start_legacy_workspace_sandbox_preserves_extra_roots() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let extra_root = tmp.path().join("extra-workspace-root");
    std::fs::create_dir(&cwd)?;
    std::fs::create_dir(&extra_root)?;
    ...
    assert_eq!(requests.len(), 2);
    let second_turn_developer_text = requests[1].message_input_texts("developer").join("\n");
    let extra_root_name = extra_root
        .file_name()
        .expect("extra root should have a final path component")
        .to_string_lossy();
    assert!(
        second_turn_developer_text.contains(extra_root_name.as_ref()),
        "legacy sandboxPolicy should preserve existing extra workspace roots; got {second_turn_developer_text:?}"
    );
...
fn create_config_toml_with_extra_top_level_config(
    codex_home: &Path,
    server_uri: &str,
    approval_policy: &str,
    feature_flags: &BTreeMap<Feature, bool>,
    sandbox_mode: &str,
    extra_top_level_config: &str,
) -> std::io::Result<()> {
