...
#[tokio::test]
async fn turn_start_goal_replaces_existing_goal_and_continues() -> Result<()> {
    let server = responses::start_mock_server().await;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
                responses::ev_assistant_message("msg-1", "Initial pass complete."),
            ...
            responses::sse(vec![
                responses::ev_assistant_message("msg-2", "Goal complete."),
                responses::ev_completed("resp-3"),
            ]),
            responses::sse(vec![
                responses::ev_assistant_message("msg-3", "Replacement initial pass complete."),
                responses::ev_completed("resp-4"),
            ]),
            responses::sse(vec![
                responses::ev_response_created("resp-5"),
                responses::ev_function_call(
                    "call-complete-replacement-goal",
                    "update_goal",
                    r#"{"status":"complete"}"#,
                ),
            ...
            responses::sse(vec![
                responses::ev_assistant_message("msg-4", "Replacement goal complete."),
    ...
    let goal_cwd = codex_home.path().join("goal-workspace");
    std::fs::create_dir_all(&goal_cwd)?;
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            input: vec![
                V2UserInput::Text {
                    text: "  Improve benchmark coverage  ".to_string(),
                    text_elements: Vec::new(),
                },
                V2UserInput::Text {
                    text: "Document the results".to_string(),
                    text_elements: Vec::new(),
                },
            ],
            goal: true,
            cwd: Some(goal_cwd.clone()),
    ...
    assert_eq!(
        (
            ...
            notification.goal.objective,
            notification.goal.status,
            notification.goal.token_budget,
            notification.goal.tokens_used,
    ...
    .await??;
    let settings_notification: ThreadSettingsUpdatedNotification = serde_json::from_value(
        settings_notification
            .params
            .expect("thread/settings/updated params"),
    )?;
    assert_eq!(
        (
            settings_notification.thread_id,
            settings_notification.thread_settings.cwd,
            settings_notification.thread_settings.approval_policy,
            settings_notification.thread_settings.sandbox_policy,
            settings_notification.thread_settings.model,
            settings_notification.thread_settings.effort,
            settings_notification.thread_settings.summary,
        ),
        (
            thread.id.clone(),
            goal_cwd.try_into()?,
    ...
    let continuation_started: TurnStartedNotification = serde_json::from_value(
        timeout(
            DEFAULT_READ_TIMEOUT,
            mcp.read_stream_until_notification_message("turn/started"),
        )
        .await??
        .params
        .expect("continuation turn/started params"),
    )?;
    let continuation_completed: TurnCompletedNotification = serde_json::from_value(
        timeout(
            DEFAULT_READ_TIMEOUT,
            mcp.read_stream_until_notification_message("turn/completed"),
        )
        .await??
        .params
        .expect("continuation turn/completed params"),
    )?;
    let continuation_turn_id = continuation_started.turn.id.clone();
    assert_eq!(
        (
            continuation_started.thread_id,
            continuation_completed.thread_id,
            continuation_completed.turn.id,
            continuation_turn_id != turn.id,
        ),
        (
            thread.id.clone(),
            thread.id.clone(),
    ...
    assert_eq!(
        (
            ...
            initial_body.pointer("/text/format/schema"),
            requests
                .get(1)
                .expect("continuation model request")
                .message_input_texts("user")
                .into_iter()
                .any(|text| text.contains("Continue working toward the active thread goal.")),
        ),
        (
            vec![
    ...
    let completed_goal_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("thread/goal/updated"),
    )
    .await??;
    let completed_goal_notification: ServerNotification = completed_goal_notification.try_into()?;
    let ServerNotification::ThreadGoalUpdated(completed_goal_notification) =
        completed_goal_notification
    else {
    ...
    let persisted_thread_id = ThreadId::from_string(&thread.id)?;
    let first_goal = state_db
        .thread_goals()
        .get_thread_goal(persisted_thread_id)
        .await?
        .expect("first goal should be persisted");
    assert_eq!(
        (
            completed_goal_notification.goal.status,
            first_goal.objective.clone(),
            first_goal.status,
        ),
        (
    ...
    let replacement_turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: thread.id.clone(),
            client_user_message_id: None,
            input: vec![V2UserInput::Text {
                text: "Publish the benchmark report".to_string(),
                text_elements: Vec::new(),
            }],
            goal: true,
            ..Default::default()
        ...
        .await?;
    let replacement_turn_resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(replacement_turn_req)),
    )
    .await??;
    let TurnStartResponse {
        turn: replacement_turn,
    } = to_response(replacement_turn_resp)?;
    let replacement_goal_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("thread/goal/updated"),
    )
    .await??;
    let replacement_goal_notification: ServerNotification =
        replacement_goal_notification.try_into()?;
    let ServerNotification::ThreadGoalUpdated(replacement_goal_notification) =
        replacement_goal_notification
    else {
        anyhow::bail!("expected replacement thread goal update notification");
    };
    let replacement_goal = state_db
        .thread_goals()
        .get_thread_goal(persisted_thread_id)
        .await?
        .expect("replacement goal should be persisted");
    assert_eq!(
        (
            replacement_goal_notification.thread_id,
            replacement_goal_notification.turn_id,
            replacement_goal_notification.goal.objective,
            replacement_goal_notification.goal.status,
            replacement_goal.goal_id != first_goal.goal_id,
            replacement_goal.tokens_used,
            replacement_goal.time_used_seconds,
        ),
        (
            thread.id.clone(),
            Some(replacement_turn.id.clone()),
            "Publish the benchmark report".to_string(),
            ThreadGoalStatus::Active,
            true,
            0,
            0,
    ...
    let mut replacement_lifecycles = Vec::new();
    for _ in 0..2 {
        let started: TurnStartedNotification = serde_json::from_value(
            timeout(
                DEFAULT_READ_TIMEOUT,
            ...
            .await??
            .params
            .expect("replacement turn/started params"),
        )?;
        let completed: TurnCompletedNotification = serde_json::from_value(
            timeout(
                DEFAULT_READ_TIMEOUT,
            ...
            .await??
            .params
            .expect("replacement turn/completed params"),
        )?;
        replacement_lifecycles.push((started.turn.id, completed.turn.id));
    }
    assert_eq!(
        (
            replacement_lifecycles[0].clone(),
            replacement_lifecycles[1].0 != replacement_turn.id,
            replacement_lifecycles[1].0 == replacement_lifecycles[1].1,
        ),
        (
            (replacement_turn.id.clone(), replacement_turn.id),
            true,
            true,
        )
    );
...
#[tokio::test]
async fn turn_start_goal_validates_input_and_thread_persistence() -> Result<()> {
    ...
    let persisted_thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
        })
        .await?;
    let persisted_thread_resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(persisted_thread_req)),
    )
    .await??;
    let ThreadStartResponse {
        thread: persisted_thread,
        ..
    } = to_response(persisted_thread_resp)?;
    let empty_goal_req = mcp
        .send_turn_start_request(TurnStartParams {
        ...
        .await?;
    let empty_goal_error: JSONRPCError = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_error_message(RequestId::Integer(empty_goal_req)),
    )
    .await??;

    let ephemeral_thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("mock-model".to_string()),
            ephemeral: Some(true),
            ..Default::default()
        ...
        .await?;
    let ephemeral_thread_resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(ephemeral_thread_req)),
    )
    .await??;
    let ThreadStartResponse {
        thread: ephemeral_thread,
        ..
    } = to_response(ephemeral_thread_resp)?;

    let ephemeral_goal_req = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: ephemeral_thread.id.clone(),
            client_user_message_id: None,
            input: vec![V2UserInput::Text {
                text: "Improve benchmark coverage".to_string(),
                text_elements: Vec::new(),
        ...
        .await?;
    let ephemeral_goal_error: JSONRPCError = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_error_message(RequestId::Integer(ephemeral_goal_req)),
    )
    .await??;
    assert_eq!(
        (
            empty_goal_error.error.message,
            ephemeral_goal_error.error.message,
        ),
        (
...
#[tokio::test]
async fn concurrent_goal_starts_reject_the_loser_without_starting_a_turn() -> Result<()> {
    let server = responses::start_mock_server().await;
    ...
    .await??;
    let TurnStartResponse { turn } = to_response(first_response)?;
