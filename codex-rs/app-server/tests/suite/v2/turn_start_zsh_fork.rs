...
//
// The suite resolves `tests/suite/zsh` via DotSlash on first use, so
// `dotslash` and network access are required the first time the artifact is
// fetched.
