...
"#,
        ...
        eprintln!("using zsh wrapper for zsh-fork test: {}", path.display());
        eprintln!(
            "zsh wrapper script:\n{}",
            std::fs::read_to_string(&path).unwrap_or_else(|err| format!("<read failed: {err}>"))
        );
        path
    };
    ...
    // This test forces `zsh -df -c`, but keep a small amount of tolerance for
    // unexpected non-target approvals so the assertions stay focused on the two
    // `/usr/bin/true` subcommands we care about.
    while target_decision_index < target_decisions.len() {
        ...
        eprintln!(
            "zsh-fork subcommand-decline approval request: command={:?} item_id={} approval_id={:?} target={} decision={:?}",
            params.command, params.item_id, params.approval_id, is_target_subcommand, decision
        );
        mcp.send_response(
            request_id,
