...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    while target_decision_index < target_decisions.len() {
        ...
        let decision = if is_target_subcommand {
            ...
            assert!(
                approval_command.contains(&zsh_path.display().to_string()),
                "expected parent zsh approval, got: {approval_command}"
            );
            assert!(
                approval_command.contains(&shell_command),
                "expected tool command in parent approval, got: {approval_command}"
            );
