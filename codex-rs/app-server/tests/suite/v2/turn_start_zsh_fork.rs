...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    // Vendored zsh may emit extra startup-related intercepted commands before
    // the tool-call subcommands we care about. Filter by `/usr/bin/true` so the
    // test stays strict about the two target approvals while tolerating shell
    // startup noise.
