...
-> Result<()> {
    ...
    assert_eq!(watch_response.path, absolute_path(git_dir.canonicalize()?));
    ...
    assert_eq!(
        ...
        absolute_path(fetch_head.canonicalize()?)
    );
    while timeout(
...
async fn fs_watch_file_reports_atomic_replace_events() -> Result<()> {
    ...
    assert_eq!(
        watch_response.path,
        absolute_path(head_path.canonicalize()?)
    );
    ...
    assert_eq!(
        ...
        FsChangedNotification {
            watch_id: watch_response.watch_id,
            changed_path: absolute_path(head_path.canonicalize()?),
        }
    );
...
async fn fs_watch_allows_missing_file_targets() -> Result<()> {
    ...
    assert_eq!(
        watch_response.path,
        absolute_path(git_dir.canonicalize()?.join("FETCH_HEAD"))
    );
    ...
    assert_eq!(
        ...
        FsChangedNotification {
            watch_id: watch_response.watch_id,
            changed_path: absolute_path(git_dir.canonicalize()?.join("FETCH_HEAD")),
        }
    );
