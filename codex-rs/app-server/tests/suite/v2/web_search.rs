...
async fn standalone_web_search_round_trips_encrypted_output() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(codex_home.path(), &[("OPENAI_API_KEY", None)]).await?;
...
async fn wait_for_web_search_completed(mcp: &mut TestAppServer) -> Result<ItemCompletedNotification> {
    loop {
        let notification = mcp
