...
enum LocalEnvironmentRegistration {
    Included,
    Omitted,
}

#[derive(serde::Serialize)]
struct GeneratedEnvironmentsToml<'a> {
    default: &'a str,
    include_local: bool,
    environments: [GeneratedEnvironmentToml<'a>; 1],
}

#[derive(serde::Serialize)]
struct GeneratedEnvironmentToml<'a> {
    id: &'a str,
    url: &'a str,
}

const GENERATED_ENVIRONMENTS_TOML_HEADER: &str =
    "# Generated by app_test_support for remote execution.\n";

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EnvironmentConfigFile {
    Missing,
    HarnessGenerated,
    TestOwned,
}

/// Adapts the shared integration-test execution environment for app-server tests.
pub struct TestAppServerEnvironment {
    state: TestAppServerEnvironmentState,
}

enum TestAppServerEnvironmentState {
    Local,
    Remote {
        execution: Box<TestExecutionEnvironment>,
        generated_thread_environment: Option<TurnEnvironmentParams>,
        local_environment_registration: LocalEnvironmentRegistration,
    },
}

impl TestAppServerEnvironment {
    pub async fn new() -> anyhow::Result<Self> {
        let local_environment_registration = match test_environment() {
            TestEnvironment::Local => {
                return Ok(Self {
                    state: TestAppServerEnvironmentState::Local,
                });
            }
            TestEnvironment::Docker { .. } => LocalEnvironmentRegistration::Included,
            TestEnvironment::WineExec => LocalEnvironmentRegistration::Omitted,
        };
        Ok(Self {
            state: TestAppServerEnvironmentState::Remote {
                execution: Box::new(TestExecutionEnvironment::new().await?),
                generated_thread_environment: None,
                local_environment_registration,
            },
        })
    }

    pub fn generated_environments_toml(
        &mut self,
        env_overrides: &[(&str, Option<&str>)],
        environment_config_file: EnvironmentConfigFile,
    ) -> anyhow::Result<Option<String>> {
        let TestAppServerEnvironmentState::Remote {
            execution,
        ...
        // A caller-provided `CODEX_EXEC_SERVER_URL` override, including removal, is intentional.
        // Do not generate a file because `environments.toml` takes precedence and would mask it.
        let exec_server_url_overridden = env_overrides
            .iter()
            .any(|(key, _value)| *key == CODEX_EXEC_SERVER_URL_ENV_VAR);
        if exec_server_url_overridden || environment_config_file == EnvironmentConfigFile::TestOwned
        {
            if matches!(test_environment(), TestEnvironment::WineExec) {
                panic!(
                    "Wine-exec app-server tests cannot use a caller-provided \
                     `CODEX_EXEC_SERVER_URL` or `environments.toml` because the harness cannot \
                     guarantee that the app server selects its Wine executor with a \
                     Windows-native cwd. Add a `skip_if_wine_exec!` guard with a TODO describing \
                     how to support this configuration across operating systems."
                );
            }
            *generated_thread_environment = None;
            return Ok(None);
        ...
        let exec_server_url = execution
            .exec_server_url()
            .context("remote test environment should have an exec-server URL")?;
        let environment_id = execution.environment_id();
        let include_local = match local_environment_registration {
            LocalEnvironmentRegistration::Included => true,
            LocalEnvironmentRegistration::Omitted => false,
        };
        *generated_thread_environment = Some(TurnEnvironmentParams {
            environment_id: environment_id.to_string(),
            cwd: execution.environment_cwd().clone(),
        });
        if environment_config_file == EnvironmentConfigFile::HarnessGenerated {
            return Ok(None);
        }
        let contents = toml::to_string(&GeneratedEnvironmentsToml {
            default: environment_id,
            include_local,
            environments: [GeneratedEnvironmentToml {
                id: environment_id,
                url: exec_server_url,
            }],
        })?;
        Ok(Some(format!(
            "{GENERATED_ENVIRONMENTS_TOML_HEADER}{contents}"
        )))
    }

    pub fn generated_thread_environment(&self) -> Option<TurnEnvironmentParams> {
        match &self.state {
            TestAppServerEnvironmentState::Local => None,
            TestAppServerEnvironmentState::Remote {
                generated_thread_environment,
                ..
            } => generated_thread_environment.clone(),
        }
    }
}
...
pub struct TestAppServer {
    ...
    /// Retains the environment resolved before app-server starts because it owns the remote
    /// workspace and records whether startup configuration was harness-generated. Recomputing it
    /// for `thread/start` would allocate a different cwd and lose that registration decision.
    test_environment: TestAppServerEnvironment,
    /// Remembers the initialize capability because later `thread/start` requests do not carry it.
...
impl TestAppServer {
    ...
    async fn new_with_program_env_and_args(
        ...
        let mut test_environment = TestAppServerEnvironment::new().await?;
        ...
        // Register the selected remote test executor at process startup, independently of the
        // client's experimental API capability. Wine-exec intentionally omits the local
        // environment so compatible operations cannot accidentally exercise the host executor.
        // Reuse harness-generated configuration across app-server restarts. Explicit overrides
        // and test-owned configuration remain untouched; Wine rejects those escape hatches so a
        // test cannot silently fall back to an executor with the wrong path convention.
        let environments_toml = codex_home.join("environments.toml");
        let environment_config_file = match std::fs::read_to_string(&environments_toml) {
            Ok(contents) if contents.starts_with(GENERATED_ENVIRONMENTS_TOML_HEADER) => {
                EnvironmentConfigFile::HarnessGenerated
            }
            Ok(_contents) => EnvironmentConfigFile::TestOwned,
        ...
        if let Some(contents) =
            test_environment.generated_environments_toml(env_overrides, environment_config_file)?
        {
            std::fs::write(&environments_toml, contents).with_context(|| {
                format!(
                    "write test environment config to {}",
                    environments_toml.display()
                )
            })?;
        }
    ...
    async fn initialize_with_params(
        ...
        let request_params = Some(serde_json::to_value(&params)?);
        let request_id = self.send_request("initialize", request_params).await?;
        ...
        match message {
            JSONRPCMessage::Response(response) => {
                ...
                // Client capabilities are negotiated during `initialize`; `thread/start` does
                // not repeat them. Retain this bit after a successful handshake so the harness
                // knows whether it may inject the experimental `environments` field later.
                self.experimental_api_enabled = params
                    .capabilities
                    .as_ref()
    ...
    pub async fn send_thread_start_request(
        ...
        // `environments.toml` registers and defaults the remote executor, but websocket
        // environments cannot configure a cwd. Core would therefore pair that executor with the
        // Linux app-server's `Config::cwd`. When the client negotiated experimental API support,
        // the harness automatically injects the generated environment with its target-native
        // Wine/Docker cwd on `thread/start`.
        let generated_environment =
            if self.experimental_api_enabled && params.environments.is_none() {
                self.test_environment.generated_thread_environment()
            } else {
                None
            };
        if let Some(environment) = generated_environment {
            params.environments = Some(vec![environment]);
        } else if matches!(test_environment(), TestEnvironment::WineExec) {
            panic!(
                "Wine-exec app-server tests must start threads with the harness-generated remote \
                 environment so the executor receives a Windows-native cwd. This test uses a \
                 non-experimental client, explicit thread environments, or test-owned environment \
                 configuration, so the harness cannot safely inject that selection. Add a \
                 `skip_if_wine_exec!` guard with a TODO describing how to make this setup cross-OS."
            );
        }
        let params = Some(serde_json::to_value(params)?);
