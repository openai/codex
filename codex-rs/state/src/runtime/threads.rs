...
mod tests {
    ...
    async fn apply_rollout_items_without_turn_context_persists_null_model_and_reasoning_effort() {
        ...
        let model: Option<String> = sqlx::query_scalar("SELECT model FROM threads WHERE id = ?")
            .bind(thread_id.to_string())
            .fetch_one(runtime.pool.as_ref())
            .await
            .expect("model column should load");
        let reasoning_effort: Option<String> =
            sqlx::query_scalar("SELECT reasoning_effort FROM threads WHERE id = ?")
                .bind(thread_id.to_string())
                .fetch_one(runtime.pool.as_ref())
                .await
                .expect("reasoning_effort column should load");
    ...
    #[tokio::test]
    async fn get_thread_ignores_unknown_reasoning_effort_values() {
        let codex_home = unique_temp_dir();
        let runtime = StateRuntime::init(codex_home.clone(), "test-provider".to_string())
            .await
            .expect("state db should initialize");
        ...
        runtime
            ...
            .await
            .expect("initial upsert should succeed");
        sqlx::query("UPDATE threads SET reasoning_effort = ? WHERE id = ?")
            .bind("future")
            .bind(thread_id.to_string())
            .execute(runtime.pool.as_ref())
            .await
            .expect("reasoning_effort update should succeed");
        ...
        let persisted = runtime
            ...
            .expect("thread should load")
            .expect("thread should exist");
        assert_eq!(persisted.model.as_deref(), Some("gpt-5"));
        assert_eq!(persisted.reasoning_effort, None);
    }

    #[tokio::test]
    async fn list_threads_ignores_unknown_reasoning_effort_values() {
        let codex_home = unique_temp_dir();
        let runtime = StateRuntime::init(codex_home.clone(), "test-provider".to_string())
            .await
            .expect("state db should initialize");
