...
RETURNING
    ...
    async fn list(&self, thread_id: ThreadId) -> anyhow::Result<Vec<crate::QueuedItemRecord>> {
        self.list_page(thread_id, /*offset*/ 0, i64::MAX as usize)
            .await
    }
...
ORDER BY queue_order ASC
        ...
        if expected_ids != requested_ids {
            anyhow::bail!("queue reorder must include every visible queued item exactly once");
        }
