...
#[tokio::test]
async fn session_configured_from_thread_response_prefers_permission_profile_field() {
    let codex_home = tempdir().expect("create temp codex home");
    let cwd = tempdir().expect("create temp cwd");
    let config = ConfigBuilder::default()
        .codex_home(codex_home.path().to_path_buf())
    ...
    let exact_profile = PermissionProfile::from_runtime_permissions(
        &file_system_policy,
        NetworkSandboxPolicy::Restricted,
    );
    let mut response = sample_thread_start_response();
    response.permission_profile = Some(exact_profile.clone().into());
    response.sandbox = codex_app_server_protocol::SandboxPolicy::DangerFullAccess;

    let event = session_configured_from_thread_start_response(&response, &config)
        .expect("build bootstrap session configured event");

    assert_eq!(event.permission_profile, exact_profile);
}
