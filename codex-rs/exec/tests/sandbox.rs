...
async fn allow_unix_socketpair_recvfrom() {
    // Re-exec the current test binary, setting a new ENV var to swap to the "in-sandbox" branch
    if std::env::var("IN_SANDBOX").is_err() {
        ...
        let mut child = spawn_command_under_sandbox(
            vec![
                ...
                "--exact".into(),
                // no __func__ in rust.
                "allow_unix_socketpair_recvfrom".into(),
            ],
            &policy,
            std::env::current_dir().expect("should be able to get current dir"),
            StdioPolicy::Inherit,
