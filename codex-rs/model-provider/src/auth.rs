...
pub(crate) async fn resolve_provider_auth(
    ...
    if let Some(aws) = provider.aws.as_ref() {
        return Ok(resolve_bedrock_auth(aws).await?.auth);
    }
...
pub(crate) async fn resolve_api_provider(
    ...
    if let Some(aws) = provider.aws.as_ref() {
        let region = resolve_bedrock_region(aws).await?;
        let mut api_provider_info = provider.clone();
        api_provider_info.base_url = Some(bedrock_mantle_base_url(&region)?);
        return api_provider_info.to_api_provider(/*auth_mode*/ None);
    }

    provider.to_api_provider(auth.map(CodexAuth::auth_mode))
}

struct ResolvedBedrockAuth {
    auth: SharedAuthProvider,
}

async fn resolve_bedrock_auth(
    aws: &ModelProviderAwsAuthInfo,
) -> codex_protocol::error::Result<ResolvedBedrockAuth> {
    ...
    Ok(ResolvedBedrockAuth {
        auth: Arc::new(AwsSigV4AuthProvider::with_context(config, context)),
    })
}
...
fn resolve_bedrock_bearer_auth(
    token: String,
) -> codex_protocol::error::Result<ResolvedBedrockAuth> {
    let _region =
        region_from_bedrock_bearer_token(&token).map_err(aws_auth_error_to_codex_error)?;
    Ok(ResolvedBedrockAuth {
        auth: Arc::new(AwsBedrockBearerAuthProvider::new(token)),
    })
}
...
mod tests {
    ...
    fn resolve_bedrock_bearer_auth_uses_token_region_and_header() {
        ...
        resolved.auth.add_auth_headers(&mut headers);
    ...
    #[test]
    fn resolve_api_provider_for_bedrock_bearer_token_uses_token_region_endpoint() {
        ...
        let mut api_provider_info = provider;
