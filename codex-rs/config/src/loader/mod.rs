...
pub async fn load_config_layers_state(
    ...
    let mut cloud_overlay_requirements_layers = Vec::new();
    ...
    let mut cloud_overlay_config_layers = Vec::new();
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            ...
            cloud_overlay_requirements_layers = system_overlay_requirements;
            cloud_baseline_config_layers = baseline_config;
            cloud_overlay_config_layers = system_overlay_config;
        }
    ...
    if !ignore_managed_requirements {
        ...
        requirements_layers.extend(cloud_overlay_requirements_layers);
    ...
    layers.extend(cloud_overlay_config_layers);
