...
#[tokio::test]
async fn replace_mcp_servers_serializes_oauth_callback_url() -> anyhow::Result<()> {
    let unique_suffix = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
    let codex_home = std::env::temp_dir().join(format!(
        "codex-config-mcp-oauth-callback-edit-test-{}-{unique_suffix}",
        std::process::id()
    ));
    let servers = BTreeMap::from([(
        "notion".to_string(),
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: None,
                callback_url: Some("http://127.0.0.1:55250/callback".to_string()),
            }),
            oauth_resource: None,
            tools: HashMap::new(),
        },
    ...
    assert_eq!(
        serialized,
        r#"[mcp_servers.notion]
url = "https://mcp.notion.com/mcp"

[mcp_servers.notion.oauth]
callback_url = "http://127.0.0.1:55250/callback"
"#
    );
