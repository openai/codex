...
fn call_tool_result_with_conversation_id(
...
pub async fn run_codex_tool_session(
    ...
    outgoing
        .send_event_as_notification(
            ...
            Some(OutgoingNotificationMeta::new(
                Some(id.clone()),
                Some(thread_id),
            )),
        )
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        let result = call_tool_result_with_conversation_id(
            thread_id,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    EventMsg::TurnComplete(TurnCompleteEvent { last_agent_message }) => {
                        ...
                        let result =
                            call_tool_result_with_conversation_id(conversation_id, text, None);
                        outgoing.send_response(request_id.clone(), result).await;
                        // unregister the id so we don't keep it in the map
                        running_requests_id_to_codex_uuid
                            .lock()
...
mod tests {
    ...
    #[test]
    fn call_tool_result_includes_conversation_id_in_structured_content() {
        let thread_id = ThreadId::new();
        let result = call_tool_result_with_conversation_id(thread_id, "done".to_string(), None);
        assert_eq!(
