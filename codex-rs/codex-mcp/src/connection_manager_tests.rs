...
#[tokio::test]
async fn list_all_tools_polls_required_clients_concurrently() {
...
#[tokio::test]
async fn list_all_tools_blocks_while_required_client_is_pending_without_cached_tools() {
    let pending_client = futures::future::pending::<Result<ManagedClient, StartupOutcomeError>>()
        .boxed()
        .shared();
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
...
#[tokio::test]
async fn list_all_tools_blocks_while_required_client_is_pending_with_cached_tools() {
    let codex_home = tempdir().expect("tempdir");
    let cache_context = create_codex_apps_tools_cache_context(
        codex_home.path().to_path_buf(),
        Some("account-one"),
