...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    let pending_client = futures::future::pending::<Result<ManagedClient, StartupOutcomeError>>()
        .boxed()
        .shared();
    let pool = McpClientPool::new_uninitialized();
    pool.clients.lock().expect("client lock").insert(
...
async fn list_all_tools_blocks_while_client_is_pending_without_startup_snapshot() {
    ...
    let timeout_result =
        tokio::time::timeout(Duration::from_millis(10), pool.list_all_tools()).await;
...
async fn list_all_tools_adds_server_metadata_to_cached_tools() {
    ...
    let pending_client = futures::future::pending::<Result<ManagedClient, StartupOutcomeError>>()
        .boxed()
        .shared();
    let mut pool = McpClientPool::new_uninitialized();
    pool.server_metadata.insert(
