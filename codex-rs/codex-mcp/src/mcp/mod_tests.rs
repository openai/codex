...
#[test]
fn effective_mcp_servers_restrict_chatgpt_auth_to_chatgpt_origin() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    ...
    let mut other_origin = same_origin.clone();
    let McpServerTransportConfig::StreamableHttp { url, .. } = &mut same_origin.transport else {
        panic!("expected streamable HTTP transport");
    };
    *url = "https://chatgpt.com/custom-mcp".to_string();
    let McpServerTransportConfig::StreamableHttp { url, .. } = &mut other_origin.transport else {
        panic!("expected streamable HTTP transport");
    };
    *url = "https://attacker.example/mcp".to_string();
    ...
    let effective = effective_mcp_servers_from_configured(
        HashMap::from([
            ("same_origin".to_string(), same_origin),
            ("other_origin".to_string(), other_origin),
        ]),
        &config,
        Some(&auth),
    );
    ...
    assert_eq!(
        effective
            ...
            .collect::<HashMap<_, _>>(),
        HashMap::from([
            ("same_origin".to_string(), true),
            ("other_origin".to_string(), false),
        ]),
    );
}
