...
/// Observer for live raw rollout appends.
///
/// Produces batches of mutations corresponding to a batch of raw RolloutItems. These batches are
/// handed to the ThreadStore to be saved with the (filtered) RolloutItems.
///
...
pub trait LiveHistoryRecorder: Send + Sync {
    ...
    ///
    /// Note that this history is the canonical persisted rollout item stream, not the raw live
    /// stream like prepare_append receives.
    fn initialize_from_history(&self, _items: &[RolloutItem]) -> ThreadStoreResult<()> {
        Ok(())
    }

    /// Generate a set of mutation events for the rollout item batch.
    fn prepare_append(
        &self,
        _items: &[RolloutItem],
    ...
    /// Mark a set of mutation events generated by prepare_append as durably stored.
    ///
    /// This is a signal to implementations that they can safely advance any internal state
    /// associated with the batch.
    fn commit_prepared_append(&self, _prepared: PreparedLiveHistoryAppend) {}

    /// Mark a set of mutation events as abandoned.
    ///
    /// This is a signal to implementations that they may want to roll back any internal state
    /// associated with the batch.
