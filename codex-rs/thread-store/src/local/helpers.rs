...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let name = item
        ...
        .filter(|title| item.first_user_message.as_deref().map(str::trim) != Some(*title))
        .filter(|title| preview.trim() != *title)
        .map(str::to_string);
...
pub(super) fn stored_thread_from_state_list_item(
    ...
    let name = if title_trimmed.is_empty()
        || first_user_message.as_deref().map(str::trim) == Some(title_trimmed)
        || preview.trim() == title_trimmed
    {
        None
    } else {
