...
pub(super) async fn list_threads(
    ...
    fill_legacy_thread_names(store, &mut items).await;
...
async fn fill_legacy_thread_names(store: &LocalThreadStore, items: &mut [StoredThread]) {
    ...
    if !thread_ids_missing_names.is_empty()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.codex_home.as_path(), &thread_ids_missing_names)
                .await
    {
        for thread in items {
