...
fn update_index_exact_paths_common(
    ...
    if (matches!(provenance, StagingProvenance::Standalone) && !skip_worktree.is_empty())
    ...
    if !exclusions.is_empty() && matches!(provenance, StagingProvenance::Standalone) {
        return Ok(StagePathsResult {
            exit_code: 1,
            stderr: exclusion_stderr,
        });
    }
    // Reverse pre-staging is intentionally best effort for ignored and sparse
    // paths. Treat those expected omissions as successful after every fallible
    // probe has completed, so a composed caller cannot report an error after
    // the eligible subset has already changed the index.
    let paths = paths
        .into_iter()
