...
- Final QA means checking every rendered slide for text superposition, clipped text, overflowing text, and inherited placeholder boxes. If one text box overlaps another, if stacked text becomes hard to read, if any line touches a box edge, or if PowerPoint shows `Click to add ...` placeholders, fix the layout or delete the inherited placeholder shapes and re-export before handoff.
