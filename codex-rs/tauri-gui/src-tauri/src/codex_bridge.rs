...
impl CodexBridge {
    ...
    fn find_codex_binary() -> Result<String> {
        ...
        Err(anyhow::anyhow!("Codex binary not found. Please install codex-cli."))
    }
    
    /// Create a new blueprint
    pub async fn create_blueprint(&self, description: String, mode: Option<String>) -> Result<Blueprint> {
...
pub async fn codex_create_blueprint(
    ...
    bridge.create_blueprint(description, mode).await
...
pub async fn codex_execute_blueprint(id: String) -> Result<ExecutionResult, String> {
    let bridge = CodexBridge::new().map_err(|e| e.to_string())?;
    bridge.execute_blueprint(id).await
        .map_err(|e| e.to_string())
}
...
pub async fn codex_research(query: String, depth: u8) -> Result<ResearchReport, String> {
    let bridge = CodexBridge::new().map_err(|e| e.to_string())?;
    bridge.research(query, depth).await
        .map_err(|e| e.to_string())
}
...
pub async fn codex_invoke_mcp_tool(
    ...
    let bridge = CodexBridge::new().map_err(|e| e.to_string())?;
    bridge.invoke_mcp_tool(name, args).await
        .map_err(|e| e.to_string())
}
