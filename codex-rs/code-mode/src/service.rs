...
impl CodeModeService {
            ...
            .await
        {
            Ok(pending_event) => Box::pin(async move {
                match pending_event.event().await {
                    ...
                    Err(runtime::Error::MissingCell(_)) => {
                        Ok(WaitOutcome::MissingCell(missing_cell_response(cell_id)))
                    }
                    Err(error) => Err(error.to_string()),
                }
            }),
            Err(runtime::Error::MissingCell(_)) => missing_wait(cell_id),
    ...
    pub async fn wait_to_pending(
            ...
            .await
        {
            ...
            Err(runtime::Error::MissingCell(_)) => Ok(WaitToPendingOutcome::MissingCell(
