...
pub struct ThreadItem {
    ...
    /// User-facing title from persisted metadata, when available.
    ...
    'outer: for (_year, year_path) in year_dirs.iter() {
        ...
        for (_month, month_path) in month_dirs.iter() {
            ...
            for (_day, day_path) in day_dirs.iter() {
                ...
                for (_ts, id, path, updated_at) in day_files.into_iter() {
                    *scanned_files += 1;
                    if *scanned_files > MAX_SCAN_FILES {
                        break 'outer;
                    }
...
fn collect_flat_files_by_updated_at_blocking(
    ...
    for entry in dir {
        ...
        let entry = entry?;
        let Some((_ts, id, path, updated_at)) = rollout_candidate_from_dir_entry(&entry) else {
            continue;
        };
        *scanned_files += 1;
        if *scanned_files > MAX_SCAN_FILES {
...
fn collect_rollout_day_file_candidates_blocking(
    ...
    let dir = std::fs::read_dir(day_path)?;
    for entry in dir {
        let entry = entry?;
    ...
    day_files.sort_by_key(|(ts, sid, _path, _updated_at)| (Reverse(*ts), Reverse(*sid)));
    Ok(day_files)
}
...
fn rollout_candidate_from_dir_entry(
    ...
    Some((ts, id, rollout_file.into_path(), updated_at))
}
