...
fn test_config(codex_home: &Path) -> RolloutConfig {
    RolloutConfig {
        ...
        workspace_roots: vec![codex_home.to_path_buf().try_into().expect("absolute path")],
...
async fn state_db_init_backfills_before_returning() -> anyhow::Result<()> {
    ...
    let session_meta_line = SessionMetaLine {
        meta: SessionMeta {
            ...
            agent_nickname: None,
            agent_role: None,
            workspace_roots: vec![home.path().to_path_buf().try_into().expect("absolute path")],
            model_provider: None,
...
async fn resume_candidate_matches_cwd_reads_latest_turn_context() -> std::io::Result<()> {
    ...
    let turn_context = RolloutLine {
        ...
        item: RolloutItem::TurnContext(TurnContextItem {
            ...
            workspace_roots: vec![latest_cwd.clone().try_into().expect("absolute path")],
            network: None,
