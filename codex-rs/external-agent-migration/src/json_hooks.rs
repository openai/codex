...
pub fn hook_migration_event_names_from_json_file(
    source_external_agent_dir: &Path,
    source_hooks: &Path,
    target_hooks: &Path,
    rewrite_profile: RewriteProfile,
) -> io::Result<Vec<String>> {
    let migration = hook_migration_from_json_file(
...
pub fn import_hooks_from_json_file(
    source_external_agent_dir: &Path,
    source_hooks: &Path,
    target_hooks: &Path,
    rewrite_profile: RewriteProfile,
...
fn hook_migration_from_json_file(
    source_external_agent_dir: &Path,
    source_hooks: &Path,
    ...
    for (source_event_name, handlers) in source_hooks {
        ...
        for handler in handlers {
            ...
            if handler.keys().any(|key| {
                !matches!(
                    key.as_str(),
                    "command" | "matcher" | "statusMessage" | "timeout" | "timeoutSec" | "type"
                )
            }) {
                continue;
            }
