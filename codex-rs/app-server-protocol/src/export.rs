...
mod tests {
    ...
    fn generated_ts_optional_nullable_fields_only_in_params() -> Result<()> {
        ...
        let approval_policy_ts =
            fs::read_to_string(output_dir.join("v2").join("AskForApproval.ts"))?;
        assert_eq!(approval_policy_ts.contains("\"guardian\""), false);
    ...
    fn generate_ts_with_experimental_api_retains_experimental_entries() -> Result<()> {
        ...
        let approval_policy_ts =
            fs::read_to_string(output_dir.join("v2").join("AskForApproval.ts"))?;
        assert_eq!(approval_policy_ts.contains("\"guardian\""), true);
        let login_account_params_ts =
            fs::read_to_string(output_dir.join("v2").join("LoginAccountParams.ts"))?;
        assert_eq!(
            login_account_params_ts.contains("\"chatgptAuthTokens\""),
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        assert_eq!(thread_start_json.contains("\"guardian\""), false);
