...
enum SandboxPolicyDeserialize {
    ...
    ReadOnly {
        ...
        network_access: bool,
        #[serde(default)]
        access: RemovedSandboxReadAccessField,
    },
    #[serde(rename_all = "camelCase")]
    ExternalSandbox {
        #[serde(default)]
    ...
    WorkspaceWrite {
        ...
        writable_roots: Vec<AbsolutePathBuf>,
        #[serde(default)]
        read_only_access: RemovedSandboxReadAccessField,
        #[serde(default)]
        network_access: bool,
        #[serde(default)]
        exclude_tmpdir_env_var: bool,
...
#[derive(Default)]
enum RemovedSandboxReadAccessField {
    #[default]
    Absent,
    Present,
}

impl RemovedSandboxReadAccessField {
    fn is_present(&self) -> bool {
        matches!(self, Self::Present)
    }
}

impl<'de> Deserialize<'de> for RemovedSandboxReadAccessField {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        D: serde::Deserializer<'de>,
    {
        match SandboxPolicyDeserialize::deserialize(deserializer)? {
            ...
            SandboxPolicyDeserialize::WorkspaceWrite {
                ...
                if read_only_access.is_present() {
                    return Err(serde::de::Error::custom(
...
mod tests {
    ...
    #[test]
    fn sandbox_policy_rejects_legacy_read_only_access_field() {
        ...
        .expect_err("read-only policy should reject removed access field");
        assert!(err.to_string().contains("readOnly.access"));
    }

    #[test]
    fn sandbox_policy_rejects_legacy_workspace_write_read_access_field() {
        let err = serde_json::from_value::<SandboxPolicy>(json!({
            "type": "workspaceWrite",
            "writableRoots": [],
            "readOnlyAccess": {
        ...
        .expect_err("workspace-write policy should reject removed readOnlyAccess field");
