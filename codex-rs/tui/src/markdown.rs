...
pub(crate) fn append_markdown_agent_with_cwd(
    ...
    let rendered = render_markdown_agent_with_source_map_with_cwd(markdown_source, width, cwd);
...
pub(crate) fn render_markdown_agent_with_source_map(
    ...
    render_markdown_agent_with_source_map_with_cwd(markdown_source, width, /*cwd*/ None)
}

pub(crate) fn render_markdown_agent_with_source_map_with_cwd(
    markdown_source: &str,
    width: Option<usize>,
    cwd: Option<&Path>,
) -> AgentRenderedWithSourceMap {
    let normalized = unwrap_markdown_fences_with_mapping(markdown_source);
    let rendered = crate::markdown_render::render_markdown_text_with_width_and_cwd_and_source_map(
        &normalized.text,
        width,
        cwd,
    );
