...
impl App {
    ...
    pub(super) async fn refresh_plugin_config_best_effort(
        &mut self,
        app_server: &AppServerSession,
        action: &str,
    ) {
        let cwd = self.chat_widget.config_ref().cwd.display().to_string();
        let result = crate::config_update::read_effective_config(app_server.request_handle(), cwd)
            .await
            .and_then(|response| self.sync_plugin_config_from_app_server_response(response));
        if let Err(err) = result {
            tracing::warn!(
                error = %err,
                action,
                "failed to refresh plugin config after app-server mutation"
            );
        }
    }

    fn sync_plugin_config_from_app_server_response(
        &mut self,
        response: ConfigReadResponse,
    ) -> Result<()> {
        // The plugin UI still reads these tables through ConfigLayerStack. Keep this bridge narrow:
        // the app server owns the mutation, and the TUI mirrors only the legacy plugin inputs.
        ...
        let user_config_file = match self.config.config_layer_stack.get_user_config_file() {
            Some(file) => file.clone(),
            None => AbsolutePathBuf::try_from(self.config.codex_home.as_path().join("config.toml"))
                .wrap_err("failed to resolve default user config path")?,
        };
...
"#,
    ...
    #[tokio::test]
    async fn sync_plugin_config_from_app_server_response_uses_server_tables() -> Result<()> {
        let mut app = make_test_app().await;
        let local_home = tempdir()?;
        let local_config_path = local_home.path().join("config.toml").abs();
        let local_config: TomlValue = toml::from_str(
            r#"
...
"#,
        ...
        app.config.config_layer_stack = app
            .config
            .config_layer_stack
            .with_user_config(&local_config_path, local_config);
        ...
        assert_eq!(
            ...
            expected_config.get("plugins")
        );
        assert_eq!(
            effective_config.get("marketplaces"),
            expected_config.get("marketplaces")
        );
        let widget_effective_config = app
            .chat_widget
            .config_ref()
            .config_layer_stack
            .effective_config();
        assert_eq!(
            widget_effective_config.get("plugins"),
            expected_config.get("plugins")
        );
        Ok(())
    }
