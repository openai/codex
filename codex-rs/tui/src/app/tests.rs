...
#[tokio::test]
async fn replayed_cancelled_review_restores_queued_input_and_stops_task() {
    let (mut app, _app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let thread_id = ThreadId::new();
    let session = test_thread_session(thread_id, test_path_buf("/tmp/project"));
    app.chat_widget.handle_thread_session(session.clone());
    ...
    app.chat_widget
        .apply_external_edit("queued after review".to_string());
    app.chat_widget
        .handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
    app.chat_widget
    ...
    assert_eq!(
        app.chat_widget.queued_user_message_texts(),
        vec!["queued after review".to_string()]
    );
    let input_state = app
        .chat_widget
        .capture_thread_input_state()
        .expect("expected queued review input state");
    ...
    assert!(
        new_op_rx.try_recv().is_err(),
        "replayed review cancellation should not submit queued input"
    );
}
