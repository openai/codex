...
impl MarkdownStreamCollector {
    pub fn new_with_cwd(width: Option<usize>, cwd: Option<PathBuf>) -> Self {
        Self {
            buffer: String::new(),
...
pub(crate) fn simulate_stream_markdown_for_tests(
    ...
    let mut collector = MarkdownStreamCollector::new_with_cwd(None, None);
    let mut out = Vec::new();
    for d in deltas {
...
mod tests {
    ...
    async fn no_commit_until_newline() {
        let mut c = super::MarkdownStreamCollector::new_with_cwd(None, None);
