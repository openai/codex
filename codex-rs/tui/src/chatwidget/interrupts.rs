...
impl InterruptManager {
    ...
    pub(crate) fn push_request_permission_preset(&mut self, ev: RequestPermissionPresetEvent) {
        self.queue
            .push_back(QueuedInterrupt::RequestPermissionPreset(ev));
    }
    ...
    pub(crate) fn flush_all(&mut self, chat: &mut ChatWidget) {
        while let Some(q) = self.queue.pop_front() {
            match q {
                ...
                QueuedInterrupt::RequestPermissionPreset(ev) => {
                    chat.handle_request_permission_preset_now(ev);
                }
