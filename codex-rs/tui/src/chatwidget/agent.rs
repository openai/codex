...
async fn initialize_notify_client(thread: &CodexThread) {
    if let Err(err) = thread
        .set_notify_client(Some(TUI_NOTIFY_CLIENT.to_string()))
        .await
    {
        tracing::error!("failed to set notify client: {err}");
    }
}
...
pub(crate) fn spawn_agent(
    ...
    tokio::spawn(async move {
        ...
        initialize_notify_client(thread.as_ref()).await;
