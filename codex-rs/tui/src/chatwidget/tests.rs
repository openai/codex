...
#[test]
fn warns_when_fallback_docs_loaded() {
    let tmp = tempfile::tempdir().expect("tempdir");
    std::fs::write(tmp.path().join("EXAMPLE.md"), "fallback contents").unwrap();
    ...
    cfg.project_doc_fallback_filenames = vec!["EXAMPLE.md".to_string()];
    ...
    widget.maybe_show_project_doc_fallback_warning();
    ...
    assert!(
        combined.contains("EXAMPLE.md"),
        "expected fallback warning to mention EXAMPLE.md, got: {combined:?}"
    );
    assert!(combined.contains("fallback"));
}

#[test]
fn does_not_warn_when_project_docs_disabled() {
    let tmp = tempfile::tempdir().expect("tempdir");
    std::fs::write(tmp.path().join("EXAMPLE.md"), "fallback contents").unwrap();
    ...
    assert!(
        messages.is_empty(),
        "expected no fallback warning when project docs are disabled, got: {:?}",
        messages
    );
}
