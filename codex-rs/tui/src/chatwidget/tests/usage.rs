...
async fn rate_limit_reset_popup_states_snapshot() {
    ...
    assert!(chat.finish_rate_limit_reset_credits_refresh(
        loading_request_id,
        Ok(GetAccountRateLimitResetCreditsResponse { available_count: 2 }),
    ));
    record_popup(&chat, &mut states);
    ...
    assert!(chat.finish_rate_limit_reset_credits_refresh(
        empty_request_id,
        Ok(GetAccountRateLimitResetCreditsResponse { available_count: 0 }),
    ));
    record_popup(&chat, &mut states);
    ...
    assert!(chat.finish_post_consume_reset_credits_refresh(
        success_request_id,
        Ok(GetAccountRateLimitResetCreditsResponse { available_count: 1 }),
    ));
    record_popup(&chat, &mut states);
