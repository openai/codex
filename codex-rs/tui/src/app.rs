...
impl App {
    ...
    async fn handle_app_server_action(&mut self, action: AppServerAction) -> Result<()> {
        match action {
            ...
            AppServerAction::ThreadSetName { name } => {
                let Some(thread_id) = self.chat_widget.thread_id() else {
                    return Ok(());
                };
                let params = codex_app_server_protocol::ThreadSetNameParams {
                    thread_id: thread_id.to_string(),
                    name,
                };
                let pending = self
                    .app_server
                    .request(
                        |request_id| codex_app_server_protocol::ClientRequest::ThreadSetName {
                            request_id,
                            params,
                        },
                    )
