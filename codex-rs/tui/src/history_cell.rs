...
pub(crate) enum HistoryCell {
    ...
    /// Link generated by the `/bug` command.
    BugReportLink { view: TextBlock },
...
impl HistoryCell {
    ...
    pub(crate) fn new_bug_report_link(message: String) -> Self {
        let mut lines: Vec<Line<'static>> = Vec::new();
        lines.push(Line::from("/bug".magenta()));
        lines.push(Line::from(message));
        lines.push(Line::from(""));
        HistoryCell::BugReportLink {
            view: TextBlock::new(lines),
        }
    }
...
impl CellWidget for HistoryCell {
    fn height(&self, width: u16) -> usize {
        match self {
            ...
            | HistoryCell::BugReportLink { view }
