...
/// Default OpenAI API base URL for tooltips and configuration views.
...
/// Summarizes unified exec activity for display in the transcript.
///
/// These summaries are aggregated across a turn so repeated unified exec calls can be collapsed
/// into a compact footer section without losing the user-facing command label.
struct UnifiedExecProcessSummary {
    /// Stable key for de-duplicating summary entries within a turn.
    key: String,

    /// Human-friendly command label for the transcript footer.
...
/// Tracks consecutive unified exec wait updates to collapse redundant output.
#[derive(Clone, Debug)]
struct UnifiedExecWaitStreak {
    /// Process id for the unified exec group currently in the wait streak.
    process_id: String,

    /// First non-empty display string observed for this wait streak.
    command_display: Option<String>,
}

impl UnifiedExecWaitStreak {
    /// Create a new wait streak for the given process and optional command label.
    ...
    /// Capture the first non-empty display string for this streak, if not yet set.
...
/// Return true if the exec source is part of a unified exec group.
...
/// Return true when every parsed command token is a known tool call.
...
/// Tracks which rate-limit warning thresholds have already been emitted.
///
...
enum RateLimitSwitchPromptState {
    ...
    /// Threshold reached, waiting to display the prompt.
    Pending,

    /// Prompt has been shown for the current snapshot.
    Shown,
}

/// Current lifecycle state of the external editor workflow.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub(crate) enum ExternalEditorState {
    /// No external editor flow is active.
    #[default]
    Closed,

    /// External editor launch has been requested but not yet spawned.
    Requested,

    /// External editor is active and edits may be applied.
    Active,
}
...
pub(crate) struct ChatWidget {
    ...
    /// Accumulates consecutive unified-exec wait messages.
    unified_exec_wait_streak: Option<UnifiedExecWaitStreak>,

    /// True when the UI is waiting to emit a task-complete summary.
    task_complete_pending: bool,

    /// Unified exec process summaries for the current turn.
    ...
    /// Current thread id once the session is configured.
    thread_id: Option<ThreadId>,

    /// Parent thread id when the session was forked.
...
impl ChatWidget {
    ...
    /// Restore the current reasoning-derived status header, falling back to "Working".
    ...
    /// Flush any pending unified-exec wait streak into history.
    ...
    /// Handle background terminal interaction updates for unified exec.
    ...
    /// Handle the end of an exec command, with unified-exec grouping logic.
    ...
    /// Record a unified exec process when its startup begins.
    ...
    /// Remove a unified exec process summary when it completes.
    ...
    /// Update the bottom pane with the current unified exec summaries.
    ...
    /// Clear all unified exec summaries and refresh the footer.
    ...
    /// Emit a `/ps` snapshot into the transcript.
    ...
    /// Build the shared header block for model selection popups.
    ...
    /// Return a warning line when model selection is routed via a custom base URL.
    ...
    /// Return the configured OpenAI base URL if it differs from the default.
    ...
    /// Open the experimental features toggle list.
    ...
    /// Return the current thread id, if configured.
