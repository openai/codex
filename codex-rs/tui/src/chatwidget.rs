...
use codex_app_server_client::legacy_core::DEFAULT_PROJECT_DOC_FILENAME;
use codex_app_server_client::legacy_core::config::Config;
use codex_app_server_client::legacy_core::config::Constrained;
use codex_app_server_client::legacy_core::config::ConstraintResult;
use codex_app_server_client::legacy_core::config_loader::ConfigLayerStackOrdering;
use codex_app_server_client::legacy_core::find_thread_name_by_id;
...
impl ChatWidget {
    ...
    fn new_with_op_target(common: ChatWidgetInit, codex_op_target: CodexOpTarget) -> Self {
        ...
        widget.bottom_pane.set_windows_degraded_sandbox_active(
            codex_app_server_client::legacy_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                && matches!(
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                let Some(name) = codex_app_server_client::legacy_core::util::normalize_thread_name(
                    &prepared_args,
                ) else {
    ...
    fn show_rename_prompt(&mut self) {
        ...
        let view = CustomPromptView::new(
            ...
            Box::new(move |name: String| {
                let Some(name) =
                    codex_app_server_client::legacy_core::util::normalize_thread_name(&name)
                else {
                    tx.send(AppEvent::InsertHistoryCell(Box::new(
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug =
                    codex_app_server_client::legacy_core::connectors::connector_mention_slug(&app);
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, from_replay: bool) {
        let hint = review.user_facing_hint.unwrap_or_else(|| {
            codex_app_server_client::legacy_core::review_prompts::user_facing_hint(&review.target)
        });
    ...
    fn on_exited_review_mode(&mut self, review: ExitedReviewModeEvent) {
        if let Some(output) = review.review_output {
            let review_markdown =
                codex_app_server_client::legacy_core::review_format::render_review_output_text(
                    &output,
                );
    ...
    fn open_theme_picker(&mut self) {
        let codex_home = codex_app_server_client::legacy_core::config::find_codex_home().ok();
        ...
        for preset in presets.into_iter() {
            ...
            let default_actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    {
                        let preset_clone = preset.clone();
                        if codex_app_server_client::legacy_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && codex_app_server_client::legacy_core::windows_sandbox::sandbox_setup_is_complete(
    ...
    fn plugins_for_mentions(
        &self,
    ) -> Option<&[codex_app_server_client::legacy_core::plugins::PluginCapabilitySummary]> {
    ...
    fn rename_confirmation_cell(name: &str, thread_id: Option<ThreadId>) -> PlainHistoryCell {
        let resume_cmd =
            codex_app_server_client::legacy_core::util::resume_command(Some(name), thread_id)
