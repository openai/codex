...
pub(crate) struct ParsedTimerSpec {
    pub(crate) trigger: TimerTrigger,
    pub(crate) prompt: String,
    pub(crate) delivery: TimerDelivery,
}

pub(crate) async fn parse_timer_spec(
    config: Config,
    target: AppServerTarget,
    spec: String,
) -> std::result::Result<ParsedTimerSpec, String> {
    ...
    let mut app_server = start_app_server_for_picker(&config, &target)
        .await
        .map_err(|err| format!("failed to start timer spec parser: {err}"))?;
    let started = app_server
        .start_ephemeral_thread_with_base_instructions(
            &config,
            PARSE_TIMER_SYSTEM_PROMPT.to_string(),
        )
        .await
        .map_err(|err| format!("failed to start timer spec parser thread: {err}"))?;
    ...
    let response = app_server
        ...
        .await
        .map_err(|err| format!("failed to parse timer spec with model: {err}"))?;
    let turn_id = response.turn.id;
    let result = wait_for_parser_response(&mut app_server, thread_id_string, turn_id).await?;
    let parsed: ParsedTimerSpec = serde_json::from_str(&result)
        .map_err(|err| format!("model returned invalid timer parse output: {err}"))?;
    validate_parsed_timer_spec(parsed)
}

pub(crate) fn format_timer_summary(
    ...
    format!(
        "{} ({mode}, {}) -> {prompt}",
        format_timer_trigger(trigger),
        delivery_str(delivery)
    )
}

pub(crate) fn format_timer_trigger(trigger: &TimerTrigger) -> String {
    match trigger {
        ...
        TimerTrigger::Schedule { dtstart, rrule } => match (dtstart, rrule) {
            (Some(dtstart), Some(rrule)) => format_timer_rrule(rrule, Some(dtstart))
                .unwrap_or_else(|| {
                    format!("schedule from {}; {rrule}", format_timer_dtstart(dtstart))
                }),
            (Some(dtstart), None) => format!("at {}", format_timer_dtstart(dtstart)),
            (None, Some(rrule)) => {
                format_timer_rrule(rrule, /*dtstart*/ None)
...
fn format_timer_dtstart(dtstart: &str) -> String {
    ...
    format!(
        ...
        dtstart.day(),
        dtstart.year(),
        format_timer_time(dtstart)
    )
}

fn format_timer_time(dtstart: NaiveDateTime) -> String {
...
fn format_timer_rrule(rrule: &str, dtstart: Option<&str>) -> Option<String> {
    ...
    let bysecond = rrule_part(rrule, "BYSECOND")
        .and_then(|value| value.parse::<u32>().ok())
        .unwrap_or(0);
    let time = format_timer_rrule_time(byhour, byminute, bysecond, dtstart);
    ...
    match freq {
        ...
        "WEEKLY" => {
            let cadence = byday.and_then(format_timer_rrule_days).unwrap_or_else(|| {
                if interval == 1 {
                    "weekly".to_string()
                } else {
...
fn format_timer_rrule_days(byday: &str) -> Option<String> {
    ...
    let days = byday
        .split(',')
        .filter_map(format_timer_rrule_day)
        .collect::<Vec<_>>()
        .join(", ");
    (!days.is_empty()).then_some(days)
}

fn format_timer_rrule_day(day: &str) -> Option<&'static str> {
    match day {
        "MO" => Some("Mondays"),
        "TU" => Some("Tuesdays"),
        "WE" => Some("Wednesdays"),
...
fn format_timer_rrule_time(
    ...
    .ok()?;
    Some(format_timer_time(dtstart))
}

pub(crate) fn trigger_is_recurring(trigger: &TimerTrigger) -> bool {
    match trigger {
        TimerTrigger::Delay { repeat, .. } => repeat.unwrap_or(false),
        TimerTrigger::Schedule { rrule, .. } => {
            rrule.as_ref().is_some_and(|rrule| !rrule.is_empty())
        }
    }
}

fn delivery_str(delivery: TimerDelivery) -> &'static str {
    match delivery {
        TimerDelivery::AfterTurn => "after-turn",
        TimerDelivery::SteerCurrentTurn => "steer-current-turn",
    }
}
    ...
    loop {
        let event =
            tokio::time::timeout(Duration::from_secs(/*secs*/ 120), app_server.next_event())
                .await
                .map_err(|_| "timed out while waiting for timer spec parser".to_string())?
                .ok_or_else(|| {
                    "timer spec parser disconnected before returning output".to_string()
                })?;
        match event {
                ...
                if notification.thread_id == thread_id && notification.turn.id == turn_id =>
            {
                ...
                {
                    return Err(format!("timer spec parser failed: {}", error.message));
                }
                return last_agent_message.ok_or_else(|| {
                    "timer spec parser did not return an agent message".to_string()
                });
            }
            AppServerEvent::ServerNotification(_) | AppServerEvent::Lagged { .. } => {}
            AppServerEvent::ServerRequest(_) => {
                return Err("timer spec parser unexpectedly requested user input".to_string());
            }
            AppServerEvent::Disconnected { message } => {
                return Err(format!("timer spec parser disconnected: {message}"));
            }
        }
    }
}
...
fn validate_parsed_timer_spec(
    parsed: ParsedTimerSpec,
) -> std::result::Result<ParsedTimerSpec, String> {
    if parsed.prompt.trim().is_empty() {
        return Err("model did not return an timer prompt".to_string());
    }
    Ok(ParsedTimerSpec {
...
const PARSE_TIMER_SYSTEM_PROMPT: &str = r#"Parse Codex `/loop` timer specs into a structured timer definition.

Return only the JSON object requested by the response schema.

Rules:
- Extract the timer prompt by removing the scheduling phrase but preserving the user's requested task.
...
- For "now", "immediately", or specs with no explicit timing, use { "kind": "delay", "seconds": 0, "repeat": true } unless the user clearly asked for one-shot behavior. This means the timer fires whenever the thread is idle.
...
mod tests {
    ...
    #[test]
    fn format_timer_summary_renders_delay() {
        assert_eq!(
    ...
    #[test]
    fn format_timer_trigger_renders_one_shot_schedule_as_local_time() {
        assert_eq!(
            format_timer_trigger(&TimerTrigger::Schedule {
    ...
    #[test]
    fn format_timer_trigger_preserves_invalid_schedule_dtstart() {
        assert_eq!(
            format_timer_trigger(&TimerTrigger::Schedule {
    ...
    #[test]
    fn format_timer_trigger_renders_daily_rrule_as_local_time() {
        assert_eq!(
            format_timer_trigger(&TimerTrigger::Schedule {
                dtstart: Some("2026-04-07T21:00:00".to_string()),
    ...
    #[test]
    fn format_timer_trigger_renders_weekday_rrule_as_local_time() {
        assert_eq!(
            format_timer_trigger(&TimerTrigger::Schedule {
    ...
    #[test]
    fn format_timer_trigger_renders_hourly_rrule_as_text() {
        assert_eq!(
            format_timer_trigger(&TimerTrigger::Schedule {
                dtstart: None,
    ...
    #[test]
    fn format_timer_trigger_preserves_unrecognized_rrule() {
        assert_eq!(
            format_timer_trigger(&TimerTrigger::Schedule {
                dtstart: Some("2026-04-07T21:00:00".to_string()),
    ...
    fn parser_prompt_defaults_ambiguous_loop_to_idle_recurring() {
        assert!(
            PARSE_TIMER_SYSTEM_PROMPT
                .contains(r#"{ "kind": "delay", "seconds": 0, "repeat": true }"#)
        );
        assert!(PARSE_TIMER_SYSTEM_PROMPT.contains("A bare absolute date/time is a single run"));
        assert!(PARSE_TIMER_SYSTEM_PROMPT.contains("For absolute wall-clock timing like"));
        assert!(PARSE_TIMER_SYSTEM_PROMPT.contains("choose the soonest future local occurrence"));
        assert!(PARSE_TIMER_SYSTEM_PROMPT.contains("2026-04-06T23:30:00"));
    }

    #[test]
    fn parsed_timer_spec_accepts_permissive_delay_trigger_shape() {
        let parsed: ParsedTimerSpec = serde_json::from_value(json!({
            "trigger": {
                "kind": "delay",
                "seconds": 10,
                "repeat": false,
