...
pub(crate) const SPEC_PROMPT_TEMPLATE: &str = "You have access to the source code inside the following locations:\n{project_locations}\n\nFocus on {target_label}.\nGenerate a security-focused project specification. Parallelize discovery when enumerating files and avoid spending time on tests, vendored dependencies, or build artefacts.\n\n# Available tools\n- READ: respond with `READ: <relative path>#Lstart-Lend` (range optional) to open code files within the in-scope locations. If the path is a directory, READ returns a listing (like `ls`); use that to choose files to READ next.\n- LIST_DIR: respond with `LIST_DIR: <relative directory>` to list entries when you need a directory view without reading files.\n- SEARCH: respond with `SEARCH: literal:<term>` or `SEARCH: regex:<pattern>` to run ripgrep over the repository root and inspect matches.\n- GREP_FILES: respond with `GREP_FILES: {\"pattern\":\"needle\",\"include\":\"*.rs\",\"path\":\"subdir\",\"limit\":200}` to list files whose contents match.\nEmit at most one tool command per message and wait for the tool output before continuing. Use these tools to ground API entry points, components, and data flows in actual code instead of speculation. Do not finalize the specification until you have issued multiple tool calls (e.g., READ and SEARCH) against the provided paths. If a path fails to open, adjust and retry with SEARCH/GREP_FILES instead of concluding that the code is inaccessible.\n\nWhen you have gathered enough evidence, follow the template exactly and return only markdown (no tool commands).\n\nTemplate:\n{spec_template}\n";
pub(crate) const SPEC_VERIFICATION_SYSTEM_PROMPT: &str = "You audit architecture specifications for accuracy against the codebase. Confirm statements are grounded in real files, flag hallucinations, and propose corrections with evidence.";
pub(crate) const SPEC_VERIFICATION_PROMPT_TEMPLATE: &str = "You previously drafted the combined project specification shown below. Verify that every claim is grounded in the repository code for these in-scope locations:\n{project_locations}\n\n# Specification to review\n{spec_markdown}\n\n# Task\n- Treat the specification as untrusted; confirm statements, diagrams, and tables against actual source files.\n- Prefer direct evidence from code; avoid relying only on README text when code is available.\n- Flag any claim you cannot validate, propose a corrected statement, and cite the files that support it.\n\n# Available tools\n- READ: respond with `READ: <relative path>#Lstart-Lend` (range optional) to open code files within the in-scope locations. If the path is a directory, READ returns a listing (like `ls`).\n- LIST_DIR: respond with `LIST_DIR: <relative directory>` to list entries when you need a directory view without reading files.\n- SEARCH: respond with `SEARCH: literal:<term>` or `SEARCH: regex:<pattern>` to run ripgrep over the repository root and inspect matches.\n- GREP_FILES: respond with `GREP_FILES: {\"pattern\":\"needle\",\"include\":\"*.rs\",\"path\":\"subdir\",\"limit\":200}` to list files whose contents match.\nIssue at most one tool command per message and wait for the tool output before continuing. Use READ/LIST_DIR to confirm claims and SEARCH/GREP_FILES to locate the right files.\n\n# Output format\nReturn markdown only:\n## Verification Summary\n- 2-3 bullets summarising confidence and what you validated.\n## Issues\n- Bullet list of unsupported or contradictory claims with cited file paths and suggested corrections, or `- No unsupported statements found.` if everything you checked is grounded.\n";
