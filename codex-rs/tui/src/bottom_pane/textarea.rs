...
mod tests {
    ...
    /// Deep fuzzing variant of `fuzz_textarea_randomized` that exercises many
    /// more randomized operations. Ignored by default; run explicitly when
    /// needed, e.g.:
    ///   cargo test -p codex-tui --lib fuzz_textarea_randomized_deep -- --ignored
    #[test]
    #[ignore]
    fn fuzz_textarea_randomized_deep() {
        // Deterministic seed for reproducibility (same logic as the fast test)
        let pst_today_seed: u64 = (chrono::Utc::now() - chrono::Duration::hours(8))
            .date_naive()
            .and_hms_opt(0, 0, 0)
            .unwrap()
