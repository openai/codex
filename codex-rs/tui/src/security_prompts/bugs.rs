...
- For findings you rate **high severity** that involve third-party libraries/protocols/crypto formats, explicitly ground impact and exploitability in realistic usage: name 2–3 public usage examples (with links) when web search is available; otherwise state `Real-world usage: unknown` rather than guessing.
...
- Additionally, for findings you rate `Severity: high` in third-party libraries/protocols/crypto formats: include a short `Real-world usage` note in `Context` (2–3 public examples with links when `web_search` is available; otherwise `Real-world usage: unknown`) and make the `Impact` explanation + PoC scenario match those common usage flows.
...
- **Context:** Optional. If the component/flow is obscure, add 2-4 sentences explaining (based on the specification context, and if needed corroborated by public docs/GitHub usage via `web_search`) what the relevant components are and how this code path is typically used or reached. For `Severity: high` findings in third-party libraries/protocols/crypto formats, include a `Real-world usage` note here (2–3 public examples with links when `web_search` is available; otherwise `Real-world usage: unknown`).
