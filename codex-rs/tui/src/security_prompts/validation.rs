...
- Build an ASan-instrumented binary (or library + harness) locally.
- Run a CONTROL execution that should not crash.
- Run a TRIGGER execution expected to crash under ASan.
- Print the exact trigger command/inputs (PoC) and the ASan stack trace with clear section headers.
