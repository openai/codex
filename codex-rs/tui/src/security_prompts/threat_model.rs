...
- Include the following sections (keep them short and derived from the spec; 4–8 bullets each where applicable):
  ...
  - `### Trust Boundaries`:
    - First: bullet list using arrow notation such as `Internet → API Gateway`, `Gateway → Native library`, etc. For each boundary, note the key data types crossing it, the message channel/protocol, the transport/security guarantees (origin checks, auth, encryption, rate limits), and any validation or schema enforcement applied.
    - Then: a compact diagram under `#### Diagram`, containing exactly one `mermaid flowchart TD` (or `flowchart LR`) that shows the primary components and highlights trust boundaries (for example, with `subgraph` zones or annotations). Keep it compact (no more than ~12 nodes), label edges with the action/payload (plain words only; avoid `{}`, `[]`, `()`, quotes, or other Mermaid syntax characters inside edge labels), and include a `title <descriptive label>` line inside the mermaid block.
  - `### Assets` as a 2-column table: `Asset` | `Why it matters`
  - `### Attacker model` with two sublists: `Capabilities` and `Non-capabilities`. Prefer realistic remote attacker assumptions; unless the spec indicates a shared-host/multi-tenant or insider threat context, treat direct host filesystem tampering outside the application's own write surfaces (e.g., writing into user home/config/key directories via shared/world-writable mounts) as a non-capability.
