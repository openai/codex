...
pub(crate) const EXTERNAL_AGENT_CONFIG_IMPORT_IN_PROGRESS_MESSAGE: &str =
    "A previous import is still running. Wait for it to finish before importing again.";
...
impl AppServerSession {
    ...
    pub(crate) async fn external_agent_config_detect(
        ...
        self.client
            ...
            .await
            .wrap_err("externalAgentConfig/detect failed during import")
    }
    ...
    pub(crate) async fn external_agent_config_import(
        ...
        let response: Result<ExternalAgentConfigImportResponse> = self
            ...
            .await
            .wrap_err("externalAgentConfig/import failed during import");
        match response {
            Ok(_) => Ok(()),
            Err(err) => {
