...
mod tests {
    ...
    #[tokio::test]
    async fn remote_thread_response_prefers_exact_permission_profile() {
        let temp_dir = tempfile::tempdir().expect("tempdir");
        let config = build_config(&temp_dir).await;
        let cwd = test_path_buf("/tmp/project").abs();
        let extra_root = test_path_buf("/workspace/cache").abs();
        ...
        .into();
        let response_permission_profile: AppServerPermissionProfile =
            permission_profile.clone().into();

        assert_eq!(
            permission_profile_from_thread_response(
                Some(&response_permission_profile),
    ...
    async fn embedded_thread_response_uses_local_config_permission_profile() {
        ...
        assert_eq!(
            permission_profile_from_thread_response(
                Some(&AppServerPermissionProfile::Disabled),
