...
fn source_bytes_for_rendered_count(
    ...
    for (i, _) in raw_source.match_indices('\n') {
        ...
        append_markdown_agent_with_cwd(prefix, width, Some(cwd), &mut lines);
...
mod tests {
  ...
  pipes\n\nCol A | Col B | Col C\n--- | --- | ---\nx | y | z\n10 | 20 | 30\n\n### 3) Another table\n\n| Key | Value |\n|---|---|\n| a | b |\n";
    ...
    fn source_bytes_plain_paragraphs() {
        ...
        assert_eq!(source_bytes_for_rendered_count(src, width, &cwd, 0), 0);
        assert_eq!(
            source_bytes_for_rendered_count(src, width, &cwd, total),
            src.len()
        );

        let off = source_bytes_for_rendered_count(src, width, &cwd, 1);
    ...
    fn source_bytes_list_items() {
        ...
        let off = source_bytes_for_rendered_count(src, width, &cwd, 2);
        assert!(off > 0);
        let mut partial = Vec::new();
        crate::markdown::append_markdown_agent(&src[..off], width, &mut partial);
        assert!(
