...
use crate::markdown::render_markdown_agent_with_source_map_with_cwd;
...
impl StreamCore {
    ...
    fn finalize_remaining(&mut self) -> Vec<Line<'static>> {
        ...
        let rendered = render_markdown_agent_with_source_map_with_cwd(
            &self.raw_source,
            self.width,
...
mod tests {
    ...
    fn remap_emitted_len_preserves_source_boundary_tie_breaks() {
        let cwd = test_cwd();
        let mut core = StreamCore::new(Some(80), &cwd);
