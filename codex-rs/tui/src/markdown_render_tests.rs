...
fn blockquote_with_multiline_code_block() {
    ...
    let lines: Vec<String> = text
        ...
        .collect();
    assert_eq!(lines, vec!["> first".to_string(), "> first".to_string(), "> second".to_string(), "> second".to_string()]);
}
...
fn code_block_unhighlighted() {
    ...
    let expected = Text::from_iter([Line::from_iter(["", "fn main() {}"]), Line::from_iter(["", "fn main() {}"])]);
    assert_eq!(text, expected);
}
...
fn code_block_inside_unordered_list_item_is_indented() {
    ...
    let lines: Vec<String> = text
        ...
        .collect();
    assert_eq!(lines, vec!["- Item", "", "  code line", "  code line"]);
}
...
fn code_block_multiple_lines_inside_unordered_list() {
    ...
    let lines: Vec<String> = text
        ...
        .collect();
    assert_eq!(lines, vec!["- Item", "", "  first", "  first", "  second", "  second"]);
}
