...
mod progress_bar_edge_cases_tests {
    ...
    fn test_unicode_character_width() {
        ...
        assert_eq!(
            char_count, 10,
            "Bar should contain exactly 10 Unicode characters"
        );

        // Verify each character is the expected width
        for ch in bar_content.chars() {
            assert!(ch == '█' || ch == '░', "Unexpected character: {:?}", ch);
        }
    }
    ...
    fn test_string_buffer_overflow_protection() {
        ...
        assert!(
            bar.len() < 200,
            "Bar string suspiciously long: {} chars",
            bar.len()
        );
    }
    ...
    fn test_special_percentage_values() {
        ...
        for (percentage, expected_filled) in special_cases {
            ...
            let filled = bar.chars().filter(|&c| c == '█').count();
            assert_eq!(filled, expected_filled, "Failed for {}%", percentage);
        }
    }
    ...
    fn test_empty_blocks_never_negative() {
        ...
        for percentage in 0..=100 {
            ...
            assert!(
                empty <= 10,
                "Empty blocks exceed maximum at {}%",
                percentage
            );

            let filled = bar.chars().filter(|&c| c == '█').count();
            assert_eq!(filled + empty, 10, "Total blocks not 10 at {}%", percentage);
        }
    }
