...
fn test_context_command_active_conversation() {
    ...
    assert!(
        text.contains("17%"),
        "Should show usage percentage (23000/128000 ≈ 17%)"
    );
    ...
    assert!(
        text.contains("Reasoning: 1,500"),
        "Should show reasoning tokens"
    );
...
fn test_context_command_different_models() {
    ...
    for (model_name, expected_context) in models {
        ...
        assert!(
            text.contains(&format!("{},000", expected_context / 1000)),
            "Should show context window for {}",
            model_name
        );
    }
}
...
fn test_context_command_after_compaction() {
    ...
    assert!(
        text_before.contains("High Context Usage Warning"),
        "Should show warning before compaction"
    );
    assert!(
        text_before.contains("93%"),
        "Should show high usage before compaction"
    );
    ...
    assert!(
        !text_after.contains("High Context Usage Warning"),
        "Should not show warning after compaction"
    );
    assert!(
        text_after.contains("5%"),
        "Should show low usage after compaction"
    );
}
...
fn test_context_command_cached_tokens() {
    ...
    assert!(
        text.contains("35%"),
        "Should show cached percentage (45000/128000)"
    );
}
...
fn test_context_command_reasoning_tokens() {
    ...
    assert!(
        text.contains("Reasoning: 15,000"),
        "Should show reasoning tokens"
    );
    assert!(
        text.contains("11%"),
        "Should show reasoning percentage (15000/128000)"
    );
}
...
fn test_context_command_responsive_display() {
    ...
    for width in widths {
        ...
        assert!(
            max_line_width <= width as usize,
            "Lines should not exceed width {} (got {})",
            width,
            max_line_width
        );
    }
...
fn test_context_command_rapid_updates() {
    ...
    for tokens in token_increments {
        ...
        if percentage > 70 {
            ...
            assert!(
                !text.contains("High Context Usage Warning"),
                "Should not show warning at {} tokens ({}%)",
                tokens,
                percentage
            );
        }
...
fn test_context_command_mixed_tokens() {
    ...
    for (input, output, cached, reasoning) in test_cases {
        ...
        assert!(
            text.contains(&format!("{},", input).replace(",000", ",000")),
            "Should show input tokens: {}",
            input
        );
        assert!(
            text.contains(&format!("{},", output).replace(",000", ",000"))
        ...
        if cached > 0 {
            assert!(
                ...
                "Should show cached section when > 0"
            );
        }
        if reasoning > 0 {
            assert!(
                text.contains("Reasoning:"),
                "Should show reasoning section when > 0"
            );
        }
    }
}
...
fn test_context_command_long_session() {
    ...
    assert!(
        text.contains("93%"),
        "Should show high usage for long session"
    );
    assert!(
        text.contains("High Context Usage Warning"),
    ...
    assert!(
        text.contains("Cached: 60,000"),
        "Should show high cache utilization"
    );
    assert!(text.contains("Session"), "Should show session info");
}
