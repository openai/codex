...
mod render_progress_bar_tests {
    ...
    fn test_render_progress_bar_unicode_consistency() {
        ...
        assert!(
            ...
            "Should contain light shade character (░)"
        );

        // Ensure no other block characters are used
        assert!(
            !result.contains('\u{2592}'),
            "Should not contain medium shade (▒)"
        );
        assert!(
            !result.contains('\u{2593}'),
            "Should not contain dark shade (▓)"
        );
        assert!(
            !result.contains('\u{258C}'),
            "Should not contain left half block (▌)"
        );
    }
