...
"#,
    ...
    let CodexCliOutput { output, .. } = run_codex_cli(codex_home, cwd).await?;
    let lower = output.to_ascii_lowercase();
    assert!(
        !lower.contains("panicked at") && !lower.contains("thread 'main' panicked"),
        "expected codex CLI to avoid panicking on malformed rules, got: {output}"
    );
    Ok(())
}
