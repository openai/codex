...
fn test_empty_session_no_panic() {
    ...
    assert!(
        result.is_ok(),
        "new_context_output should not panic with empty session"
    );
}
...
fn test_empty_session_displays_all_sections() {
    ...
    assert!(
        output.contains("Context Window Usage"),
        "Missing Context Window Usage section"
    );
    assert!(
        output.contains("Component Breakdown"),
        "Missing Component Breakdown section"
    );
...
fn test_empty_session_zero_values() {
    ...
    // Verify zero values
    assert!(
        output.contains("0 / 128,000"),
        "Should show 0 out of 128,000 tokens"
    );
    ...
    assert!(
        ...
        "Should not show Cached line when zero"
    );
    assert!(
        !output.contains("Reasoning:"),
        "Should not show Reasoning line when zero"
    );
}
...
fn test_empty_session_various_widths() {
    ...
    for width in widths {
        ...
        assert!(
            ...
            "Missing header at width {}",
            width
        );
        assert!(
            output.contains("0%"),
            "Missing percentage at width {}",
            width
        );
    }
}
...
fn test_empty_session_with_without_session_id() {
    ...
    assert!(
        !output_no_id.contains("Session"),
        "Should not show Session section without ID"
    );
    ...
    assert!(
        output_with_id.contains("Session"),
        "Should show Session section with ID"
    );
}
...
fn test_empty_session_no_warning() {
    ...
    assert!(
        !output.contains("High Context Usage Warning"),
        "Should not show warning for empty session"
    );
    assert!(
        !output.contains("/compact"),
        "Should not suggest /compact for empty session"
    );
}
...
fn test_transition_from_empty_to_populated() {
    ...
    let empty_cell =
        codex_tui::history_cell::new_context_output(&config, &empty_usage, &session_id);
...
fn test_empty_session_different_models() {
    ...
    let models = vec!["gpt-4", "gpt-3.5-turbo", "claude-3-opus", "test-model"];
...
fn test_empty_session_progress_bar() {
    ...
    assert!(
        output.contains('['),
        "Should have progress bar opening bracket"
    );
    assert!(
        output.contains(']'),
        "Should have progress bar closing bracket"
    );
    assert!(
        output.contains('░'),
        "Should have empty progress bar characters"
    );
    assert!(
        !output.contains('█'),
        "Should not have filled progress bar characters"
    );
...
fn test_empty_session_partial_data() {
    ...
    let cell =
        codex_tui::history_cell::new_context_output(&config, &usage_output_only, &session_id);
    let lines = cell.display_lines(80);
    let output = extract_text_from_lines(&lines);
...
fn test_empty_session_history_cell_trait() {
    ...
    assert!(
        ...
        "display_lines should not be empty"
    );
    ...
    assert!(
        ...
        "transcript_lines should not be empty"
    );
    ...
    assert!(
        height < 100,
        "desired_height should be reasonable for empty session"
    );

    // Test is_stream_continuation
    assert!(
        !cell.is_stream_continuation(),
        "Should not be a stream continuation"
    );
}
...
fn test_empty_session_formatting() {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        assert!(
            line_width <= 80,
            "Line {} width ({}) exceeds display width",
            i,
            line_width
        );
    }
...
fn test_empty_session_narrow_width() {
    ...
    for width in narrow_widths {
        ...
        assert!(
            output.contains("context"),
            "Should contain context at width {}",
            width
        );
    }
}
...
fn test_empty_session_with_config_overrides() {
    ...
    assert!(
        output.contains("custom-model"),
        "Should show custom model name"
    );
    assert!(output.contains("0%"), "Should show 0% usage");
}
...
fn test_multiple_empty_sessions() {
    ...
    for i in 0..5 {
        ...
        if session_id.is_some() {
            ...
            assert!(
                !output.contains("Session"),
                "Session {} should not show Session info",
                i
            );
        }
    }
...
fn test_empty_session_hides_optional_components() {
    ...
    assert!(
        !output.contains("Cached:"),
        "Should not show Cached section when zero"
    );
    assert!(
        !output.contains("Reasoning:"),
        "Should not show Reasoning section when zero"
    );
    assert!(
        !output.contains("High Context Usage Warning"),
        "Should not show warning when empty"
    );

    // Should show these always
    assert!(
        output.contains("Input:"),
        "Should always show Input section"
    );
    assert!(
        output.contains("Output:"),
        "Should always show Output section"
    );
    assert!(
        output.contains("Total:"),
        "Should always show Total section"
    );
}
...
fn test_empty_session_maximum_width() {
    ...
    assert!(
        output.contains("0%"),
        "Should show percentage at maximum width"
    );
}
...
fn test_empty_session_deterministic() {
    ...
    let filtered1: Vec<_> = output1
        .lines()
        .filter(|l| !l.contains("Session ID:"))
        .collect();
    let filtered2: Vec<_> = output2
        .lines()
        .filter(|l| !l.contains("Session ID:"))
        .collect();
