...
impl ReadDenyMatcher {
    ...
    fn build(
        ...
        // canonical target.
        let resolved_entries = file_system_sandbox_policy.resolved_entries_with_cwd(cwd);
        let canonical_deny_candidates: Vec<ResolvedFileSystemEntry> = resolved_entries
            .iter()
            ...
            .collect();
        let exact_candidates = if canonical_deny_candidates.is_empty() {
            Vec::new()
        } else {
            resolved_entries
        };
    ...
    pub fn is_read_denied(&self, path: &Path) -> bool {
        ...
        // Exact entries preserve most-specific precedence for the lexical
        // spelling the caller asked about. Canonical spellings are considered
        // only for deny entries: an explicitly narrower lexical allow may
        // reopen a descendant, but a broad allow cannot hide a more-specific
        // canonical deny reached through a symlink alias.
        ...
        let canonical_deny_entry = self
            ...
            .max_by_key(|entry| resolved_entry_precedence(entry));
        if canonical_deny_entry.is_some_and(|deny_entry| {
            !lexical_entry.is_some_and(|entry| {
                entry.access.can_read()
                    && normalized_and_canonical_candidates(entry.path.as_path())
                        .iter()
                        .any(|candidate| {
                            candidate.starts_with(deny_entry.path.as_path())
                                && candidate.components().count()
                                    > deny_entry.path.as_path().components().count()
                        })
            })
        }) {
            return true;
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn broad_read_allow_does_not_reopen_canonical_deny_via_symlink() {
        let temp = TempDir::new().expect("tempdir");
        let real_dir = temp.path().join("real");
        let alias_dir = temp.path().join("alias");
        let real_file = real_dir.join("secret.txt");
        let alias_file = alias_dir.join("secret.txt");
        std::fs::create_dir_all(&real_dir).expect("create real dir");
        std::fs::write(&real_file, "secret").expect("write secret");
        symlink_dir(&real_dir, &alias_dir).expect("symlink alias");
