...
pub(crate) async fn run_http_proxy_with_attribution(
    ...
    run_http_proxy_with_listener(state, listener, policy_decider, attribution).await
}
...
pub async fn run_http_proxy_with_std_listener(
    ...
    run_http_proxy_with_std_listener_and_attribution(
        state,
        listener,
        policy_decider,
...
pub(crate) async fn run_http_proxy_with_std_listener_and_attribution(
    state: Arc<NetworkProxyState>,
    listener: StdTcpListener,
    policy_decider: Option<Arc<dyn NetworkPolicyDecider>>,
    attribution: ConnectionAttribution,
        ...
        (
            UpgradeLayer::new(
                ...
                service_fn({
                    ...
                    move |req: Request| {
                        let request_context =
                            req.extensions().get::<NetworkRequestContext>().cloned();
                        let policy_decider = policy_decider.clone();
                        async move {
                            match request_context {
                                Some(request_context) => {
                                    http_connect_accept(policy_decider, request_context, req).await
                                }
            ...
            .into_layer(service_fn({
                ...
                move |req: Request| {
                    ...
                    async move {
                        match request_context {
                            Some(request_context) => {
                                http_plain_proxy(policy_decider, request_context, req).await
                            }
