...
async fn set_extra_roots_replaces_runtime_roots_and_clears_cache() {
    ...
    let empty_outcome = skills_manager
        .skills_for_cwd(&skills_input, /*force_reload*/ false, local_file_system())
        .await;
    assert!(
        empty_outcome
            .skills
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    ...
    let outcome = skills_manager
        .skills_for_cwd(&skills_input, /*force_reload*/ true, local_file_system())
        .await;
...
async fn skills_for_cwd_without_local_fs_skips_local_roots() {
    ...
    .expect("valid config layer stack");
    let skills_input =
        local_skills_input(cwd.path().abs(), Vec::new(), config_layer_stack.clone())
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    ...
    let parent_outcome = skills_manager
        .skills_for_cwd(&parent_input, /*force_reload*/ true, local_file_system())
        .await;
