...
mod tests {
    ...
    #[test]
    fn catalog_rendering_prefers_short_descriptions() {
        let entry = SkillCatalogEntry::new(
            SkillPackageId("package".to_string()),
            SkillAuthority::new(SkillSourceKind::Executor, "executor"),
            "deploy",
            "The full deployment instructions.",
            SkillResourceId::new("deploy/SKILL.md"),
        )
        .with_short_description(Some("Deploy the service.".to_string()));
        ...
        assert_eq!(
            ...
            vec![
                "- deploy: Deploy the service. (environment resource: deploy/SKILL.md)".to_string()
            ]
        );
    }
