...
const MAX_INJECTIONS_PER_STEP: usize = 8;
const MAX_INJECTION_BYTES_PER_STEP: usize = 32 * 1024;
...
impl SkillsSnapshot {
    ...
    pub async fn injections(
        ...
        already_injected: &HashSet<SkillInjectionIdentity>,
        already_injected_names: &HashSet<String>,
        ...
        let mut injected_names = already_injected_names.clone();
        let mut remaining_bytes = MAX_INJECTION_BYTES_PER_STEP;
        let mut injection_limit_reached = false;
        for entry in &selected {
            ...
            if already_injected.contains(&identity) {
                continue;
            }
            let replaces_previous = injected_names.contains(&entry.name);
            if result.items.len() == MAX_INJECTIONS_PER_STEP {
                injection_limit_reached = true;
                break;
            }
            ...
            match source.read(request).await {
                Ok(read) => {
                    let Some((instructions, truncated)) =
                        SkillInstructions::from_runtime_with_total_limit(
                            entry,
                            &read.contents,
                            remaining_bytes,
                            replaces_previous,
                        ...
                        result.warnings.push(
                            "Additional selected skills were not loaded because this step reached the 32 KB skill instruction limit."
                                .to_string(),
                        );
                        break;
                    };
                    let instruction_bytes = instructions.render().len();
                    remaining_bytes = remaining_bytes.saturating_sub(instruction_bytes);
                    if truncated {
                        result.warnings.push(format!(
                            "Skill `{}` exceeded the step's instruction limit and was truncated.",
                            entry.name
                        ));
                    }
                    result.items.push(RuntimeSkillInjection {
                        identity,
                        entry: entry.clone(),
                        instructions,
                    });
                    injected_names.insert(entry.name.clone());
                }
                Err(err) => result
                    .warnings
        ...
        if injection_limit_reached {
            result.warnings.push(format!(
                "Only the first {MAX_INJECTIONS_PER_STEP} selected skills were loaded for this step."
            ));
        }
        result
    }
