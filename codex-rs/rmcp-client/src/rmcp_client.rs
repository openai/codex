...
struct StreamableHttpResponseClient {
    backend: Arc<dyn StreamableHttpResponseBackend>,
}

#[derive(Clone)]
struct LocalStreamableHttpResponseClient {
    inner: reqwest::Client,
}

#[derive(Clone)]
struct RemoteStreamableHttpResponseClient {
...
impl StreamableHttpResponseClient {
    fn new(placement: StreamableHttpClientPlacement, default_headers: HeaderMap) -> Result<Self> {
        let backend: Arc<dyn StreamableHttpResponseBackend> = match placement {
            StreamableHttpClientPlacement::Local => Arc::new(LocalStreamableHttpResponseClient {
                inner: build_http_client(&default_headers)?,
            }),
            StreamableHttpClientPlacement::Remote { exec_client } => {
                Arc::new(RemoteStreamableHttpResponseClient {
...
/// `StreamableHttpResponseClient` remains the shared transport-facing handle
/// used by RMCP setup and OAuth wiring.
trait StreamableHttpResponseBackend: Send + Sync {
    fn post_message(
        &self,
        uri: Arc<str>,
    ...
    fn delete_session(
        ...
        '_,
        std::result::Result<(), StreamableHttpError<StreamableHttpResponseClientError>>,
    >;
...
impl StreamableHttpResponseBackend for LocalStreamableHttpResponseClient {
    fn post_message(
        &self,
        uri: Arc<str>,
        message: rmcp::model::ClientJsonRpcMessage,
        ...
        async move {
            ...
            match content_type.as_deref() {
                ...
                _ => {
                    ...
                    let content_type =
                        content_type.unwrap_or_else(|| "missing-content-type".into());
...
impl StreamableHttpResponseBackend for RemoteStreamableHttpResponseClient {
    fn post_message(
        &self,
        uri: Arc<str>,
        message: rmcp::model::ClientJsonRpcMessage,
