...
For OAuth MCP servers that disable dynamic client registration but support Client ID Metadata Documents (SEP-991/CIMD), Codex automatically retries login with its built-in client metadata document at [`client-metadata.json`](./client-metadata.json). When callback settings are not explicitly configured, Codex binds the local OAuth callback listener to the first available port in `33418..=33428` so the redirect URI matches the built-in metadata document.
