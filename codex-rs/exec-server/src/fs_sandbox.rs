...
impl FileSystemSandboxRunner {
    ...
    pub(crate) async fn run(
        ...
        let command = self.sandbox_exec_request(&permission_profile, &cwd, sandbox, &helper)?;
...
async fn run_command(
    ...
    let output = run_helper_command(command, request_json).await?;
    if !output.success {
...
async fn run_helper_command(
    ...
    if command.sandbox == codex_sandboxing::SandboxType::WindowsRestrictedToken {
        return tokio::task::spawn_blocking(move || {
            run_windows_sandbox_command(command, request_json)
        })
        .await
        .map_err(|err| internal_error(format!("windows fs sandbox helper task failed: {err}")))?;
    }
...
#[cfg(target_os = "windows")]
fn run_windows_sandbox_command(
    SandboxExecRequest {
        mut command,
        cwd,
        env,
    ...
    let capture = codex_windows_sandbox::run_windows_sandbox_capture_for_level(
        windows_sandbox_level,
        &permission_profile,
    ...
    let capture = capture
        .map_err(|err| internal_error(format!("windows fs sandbox helper failed to run: {err}")))?;
