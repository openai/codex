...
async fn install_runtime_from_archive(
    ...
    if let Some(bundle_version) = manifest.bundle_version.as_ref()
        ...
        .await
    {
        let paths = configure_runtime_paths(paths).await?;
...
async fn configure_runtime_paths(
    mut paths: RuntimeInstallPaths,
) -> Result<RuntimeInstallPaths, JSONRPCErrorError> {
    if paths.bundled_plugin_marketplace_paths.is_empty()
        && paths.bundled_skill_paths.is_empty()
        && paths.skills_to_remove.is_empty()
    {
        return Ok(paths);
    }

    let codex_home = default_codex_home()?;
    let marketplace_roots = materialize_bundled_plugin_marketplaces(
        &codex_home,
        &paths.bundled_plugin_marketplace_paths,
    )
    .await?;
    let skill_paths = sync_primary_runtime_skills(
        &codex_home,
    ...
    paths.bundled_plugin_marketplace_paths = marketplace_roots;
    paths.bundled_skill_paths = skill_paths;
    Ok(paths)
}

fn default_codex_home() -> Result<PathBuf, JSONRPCErrorError> {
...
async fn materialize_bundled_plugin_marketplaces(
    ...
    let destination_root = codex_home
        .join("plugins")
    ...
    for marketplace_root in marketplace_roots {
        ...
        let destination = destination_root.join(safe_path_segment(marketplace_name));
        copy_dir(marketplace_root.as_path(), &destination).await?;
        materialized.push(absolute_path(destination)?);
    }
    Ok(materialized)
}
...
async fn sync_primary_runtime_skills(
    ...
    move_legacy_primary_runtime_skills(codex_home, skills_to_remove).await?;

    if bundled_skill_paths.is_empty() {
        return Ok(Vec::new());
    }

    let destination_root = codex_home.join("skills").join(PUBLISHED_ARTIFACT_NAME);
    remove_dir_if_exists(&destination_root).await?;
    fs::create_dir_all(&destination_root).await.map_err(|err| {
        internal_error(format!(
    ...
    for bundled_skill_path in bundled_skill_paths {
        let skill_root = bundled_skill_path.as_path().parent().ok_or_else(|| {
            invalid_params(format!(
                "bundled skill path {} has no parent directory",
                bundled_skill_path.display()
            ))
        })?;
        let skill_name = skill_root.file_name().ok_or_else(|| {
            invalid_params(format!(
        ...
        copy_dir(skill_root, &destination).await?;
        materialized.push(absolute_path(destination.join("SKILL.md"))?);
    }
...
async fn move_legacy_primary_runtime_skills(
    ...
    for skill_dir in skills_to_remove {
        ...
        let metadata = match fs::metadata(&skill_root).await {
        ...
        if let Some(parent) = backup_path.parent() {
            fs::create_dir_all(parent).await.map_err(|err| {
                internal_error(format!(
                    "failed to create legacy skill backup directory {}: {err}",
                    parent.display()
                ))
            })?;
        }
        match fs::rename(&skill_root, &backup_path).await {
            Ok(()) => {
                tracing::info!(
                    skill_dir = %skill_dir,
            ...
            Err(err) => {
                return Err(internal_error(format!(
                    "failed to move legacy skill directory {} to {}: {err}",
                    skill_root.display(),
                    backup_path.display()
                )));
            }
...
fn resolve_legacy_skill_directory(skills_root: &Path, skill_dir: &str) -> PathBuf {
    let relative = Path::new(skill_dir);
    if relative
        .components()
        .all(|component| matches!(component, Component::Normal(_)))
    {
        return skills_root.join(relative);
    }
    skills_root.join(
        relative
            .file_name()
...
async fn copy_dir(source: &Path, destination: &Path) -> Result<(), JSONRPCErrorError> {
    remove_dir_if_exists(destination).await?;
    fs::create_dir_all(destination).await.map_err(|err| {
        internal_error(format!(
    ...
    while let Some((source_dir, destination_dir)) = pending.pop() {
        let mut entries = fs::read_dir(&source_dir).await.map_err(|err| {
            internal_error(format!(
        ...
        while let Some(entry) = entries.next_entry().await.map_err(|err| {
            ...
            if file_type.is_dir() {
                fs::create_dir_all(&destination_path).await.map_err(|err| {
                    internal_error(format!(
                        "failed to create destination directory {}: {err}",
...
mod tests {
    ...
    #[tokio::test]
    async fn materialize_bundled_plugin_marketplaces_copies_to_codex_home() {
        ...
        let materialized = materialize_bundled_plugin_marketplaces(
            codex_home.path(),
            &[AbsolutePathBuf::try_from(marketplace_root).expect("absolute path")],
        )
        .await
        .expect("materialize marketplaces");
        ...
        assert_eq!(
            materialized,
            vec![AbsolutePathBuf::try_from(expected_root.clone()).expect("absolute path")]
        );
        assert!(
            expected_root
    ...
    #[tokio::test]
    async fn sync_primary_runtime_skills_copies_bundled_and_moves_legacy() {
        let codex_home = tempfile::tempdir().expect("codex home");
        let runtime = tempfile::tempdir().expect("runtime");
        ...
        let materialized = sync_primary_runtime_skills(
            codex_home.path(),
            &[
                AbsolutePathBuf::try_from(bundled_skill_root.join("SKILL.md"))
        ...
        assert_eq!(
            materialized,
            vec![AbsolutePathBuf::try_from(expected_skill_path.clone()).expect("absolute path")]
        );
        assert_eq!(
