...
/// Discovers the remote environments that should be available to Codex.
///
/// Implementations are source-specific: they may read process environment,
/// `config.toml`, or a remote service. They should return plain environment
/// specs and leave validation plus runtime object construction to
/// [`EnvironmentManager`].
#[async_trait]
pub trait EnvironmentProvider: Send + Sync {
    /// Returns the default environment id and remote environment specs.
    ///
    /// `EnvironmentManager` always adds `local` itself. A missing or empty
    /// default means `local`; `"none"` disables the default environment.
    async fn get_environments(
        &self,
    ) -> Result<(Option<String>, Vec<EnvironmentSpec>), ExecServerError>;
}

#[derive(Clone, Debug)]
pub struct EnvVarEnvironmentProvider {
    pub exec_server_url: Option<String>,
}

#[async_trait]
impl EnvironmentProvider for EnvVarEnvironmentProvider {
    async fn get_environments(
        &self,
    ) -> Result<(Option<String>, Vec<EnvironmentSpec>), ExecServerError> {
        ...
        if environment_disabled {
            return Ok((Some("none".to_string()), Vec::new()));
        }
        ...
        match exec_server_url {
            Some(url) => Ok((
                ...
                vec![EnvironmentSpec {
                    ...
                    transport: RemoteExecServerTransport::WebSocket { url },
                }],
            )),
            None => Ok((Some(LOCAL_ENVIRONMENT_ID.to_string()), Vec::new())),
        }
    }
}
...
impl EnvironmentProvider for StaticEnvironmentProvider {
    async fn get_environments(
        ...
        Ok((self.default_environment.clone(), self.environments.clone()))
    }
}
...
impl EnvironmentManager {
    ...
    pub fn new(args: EnvironmentManagerArgs) -> Self {
        Self::from_exec_server_url_args(args)
    }
    ...
    pub fn try_new(args: EnvironmentManagerArgs) -> Result<Self, ExecServerError> {
        Ok(Self::from_exec_server_url_args(args))
    }

    pub async fn try_from_provider(
        provider: &dyn EnvironmentProvider,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
        let (default_environment, environments) = provider.get_environments().await?;
        Self::from_environment_specs(default_environment, environments, local_runtime_paths)
    }

    fn from_environment_specs(
        default_environment: Option<String>,
        environment_specs: Vec<EnvironmentSpec>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
        let mut environments = HashMap::from([(
            LOCAL_ENVIRONMENT_ID.to_string(),
