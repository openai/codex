...
pub(crate) fn build_router() -> RpcRouter<ExecServerHandler> {
    ...
    router.request(
        FS_READ_TEXT_PREFIXES_BATCH_METHOD,
        |handler: Arc<ExecServerHandler>, params: FsReadTextPrefixesBatchParams| async move {
            handler.fs_read_text_prefixes_batch(params).await
        },
    );
    router.request(
        FS_OPEN_METHOD,
    ...
    router.request(
        FS_FIND_UP_BATCH_METHOD,
        |handler: Arc<ExecServerHandler>, params: FsFindUpBatchParams| async move {
            handler.fs_find_up_batch(params).await
        },
    );
    router.request(
