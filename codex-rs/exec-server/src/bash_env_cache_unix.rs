...
impl BashEnvSnapshotRequest {
    fn new(params: &ExecParams, environment: &HashMap<String, String>) -> Option<Self> {
        let workspace_root = params.env_policy.as_ref()?.bash_env_cache_scope.as_ref()?;
        if params.tty
            || params.pipe_stdin
            || params.arg0.is_some()
            || params.argv.len() < 3
            || params.argv.get(1).map(String::as_str) != Some("-c")
            || !params.cwd.starts_with(workspace_root)
        ...
        let mut preserve_env_keys = params
            .env_policy
            .as_ref()?
            .r#set
            .keys()
...
if [[ $- == *x* || $- == *v* ]] || [[ -n "$(builtin trap -p)" ]]; then builtin exit 125; fi
