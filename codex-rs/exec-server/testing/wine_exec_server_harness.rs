...
/// Configures a Wine-backed Windows exec-server fixture.
pub struct WineExecServerHarnessBuilder {
    powershell_runtime_dir: Option<PathBuf>,
...
impl WineExecServerHarness {
    /// Creates a builder for an isolated fixture.
    pub fn builder() -> WineExecServerHarnessBuilder {
        WineExecServerHarnessBuilder {
            powershell_runtime_dir: None,
            workspace: None,
        }
    }
}

impl WineExecServerHarnessBuilder {
    /// Overrides the pinned PowerShell runtime directory supplied by Bazel.
    #[must_use]
    pub fn powershell_runtime_dir(mut self, runtime_dir: impl Into<PathBuf>) -> Self {
        self.powershell_runtime_dir = Some(runtime_dir.into());
        self
    }

    /// Creates `workspace` inside the isolated Wine prefix before startup.
    #[must_use]
    ...
    /// Starts the configured fixture.
    pub async fn start(self) -> Result<(WineExecServerHarness, String)> {
        let prefix = TempDir::new().context("create Wine prefix")?;
        let powershell_runtime = match self.powershell_runtime_dir {
            Some(runtime_dir) => runtime_dir,
            None => codex_utils_cargo_bin::cargo_bin("pwsh-runtime-marker")?
                .parent()
        ...
        let server = WineExecServerHarness {
