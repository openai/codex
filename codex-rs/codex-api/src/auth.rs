...
pub trait AuthProvider: Send + Sync {
    ...
    /// Returns auth headers for a concrete HTTP request URL.
    fn to_auth_headers_for_url(&self, request_url: &str) -> HeaderMap {
        let mut headers = HeaderMap::new();
        self.add_auth_headers_for_url(request_url, &mut headers);
        headers
    }

    /// Observes response headers for auth state that may need to rotate after a request.
    ///
    /// Most providers do not need this. Providers with server-minted, request-scoped state may
    /// validate the URL and selectively persist response headers here.
    fn observe_response_headers(&self, _request_url: &str, _headers: &HeaderMap) {}
