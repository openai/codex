...
pub struct RealtimeWebsocketEvents {
    ...
    active_transcript: Arc<Mutex<Vec<RealtimeTranscriptEntry>>>,
    event_parser: RealtimeEventParser,
    is_closed: Arc<AtomicBool>,
}
    ...
    fn new(
        ...
        Self {
            ...
            events: RealtimeWebsocketEvents {
                ...
                active_transcript: Arc::new(Mutex::new(Vec::new())),
                event_parser,
                is_closed,
            },
        }
    ...
    async fn update_active_transcript(&self, event: &mut RealtimeEvent) {
        ...
        match event {
            ...
            RealtimeEvent::InputTranscriptDone(update) => {
                append_transcript_done(&mut active_transcript, "user", update);
            }
            RealtimeEvent::OutputTranscriptDelta(_) => {}
            RealtimeEvent::OutputTranscriptDone(update) => {
                append_transcript_done(&mut active_transcript, "assistant", update);
            }
            RealtimeEvent::HandoffRequested(handoff) => {
                if self.event_parser == RealtimeEventParser::V1 {
                    handoff.active_transcript = std::mem::take(&mut *active_transcript);
                }
            }
