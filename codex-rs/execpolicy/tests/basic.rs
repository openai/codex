...
fn host_executable_rejects_name_with_path_separator() {
    ...
    let policy_src = format!(r#"host_executable(name = "{git_path}", paths = ["{git_path}"])"#);
    let mut parser = PolicyParser::new();
    let err = parser
        .parse("test.rules", &policy_src)
        .expect_err("expected parse error");
...
fn host_executable_rejects_path_with_wrong_basename() {
    ...
    let policy_src = format!(r#"host_executable(name = "git", paths = ["{rg_path}"])"#);
    let mut parser = PolicyParser::new();
    let err = parser
        .parse("test.rules", &policy_src)
        .expect_err("expected parse error");
...
fn host_executable_last_definition_wins() -> Result<()> {
    ...
    parser.parse(
        "shared.rules",
        &format!(r#"host_executable(name = "git", paths = ["{usr_git}"])"#),
    )?;
    parser.parse(
        "user.rules",
        &format!(r#"host_executable(name = "git", paths = ["{homebrew_git}"])"#),
    )?;
    let policy = parser.build();
...
"#
    ...
    let evaluation = policy.check_with_options(
        &vec![git_path.clone(), "status".to_string()],
        &allow_all,
        &MatchOptions {
...
fn host_executable_resolution_ignores_path_not_in_allowlist() -> Result<()> {
    ...
    let policy_src = format!(
        r#"
prefix_rule(pattern = ["git"], decision = "prompt")
host_executable(name = "git", paths = ["{allowed_git}"])
"#
    );
    let mut parser = PolicyParser::new();
    parser.parse("test.rules", &policy_src)?;
    ...
    let evaluation = policy.check_with_options(
        &vec![other_git.clone(), "status".to_string()],
        &allow_all,
        &MatchOptions {
            resolve_host_executables: true,
        },
...
fn host_executable_resolution_does_not_override_exact_match() -> Result<()> {
    ...
    let policy_src = format!(
        r#"
prefix_rule(pattern = ["{git_path}"], decision = "allow")
prefix_rule(pattern = ["git"], decision = "prompt")
host_executable(name = "git", paths = ["{git_path}"])
"#
    );
