...
impl ExternalAuth for BearerTokenRefresher {
    ...
    /// Returns a fresh cached provider token when possible.
    ///
    /// On a cache miss, refresh work is serialized by `refresh_lock`. The cache
    /// is checked again after acquiring the permit because another task may have
    /// refreshed the token while this task was waiting.
...
impl BearerTokenRefresher {
    /// Clones the cached token if it is still fresh.
    ///
    /// This only holds `cached_token` long enough to inspect freshness and clone
    /// the token string. Provider command execution happens outside this mutex.
    ...
    /// Runs the provider auth command and replaces the cached token.
    ///
    /// The permit argument documents the required synchronization: callers must
    /// hold `refresh_lock` while this runs so only one provider-token refresh is
    /// in flight at a time.
    ...
    /// Acquires the one-permit refresh gate.
    ///
    /// This is not the cache data lock. It intentionally covers the slow
    /// provider command so concurrent cache misses share one refresh result
    /// instead of spawning duplicate provider auth commands.
...
struct ExternalBearerAuthState {
    ...
    /// Serializes provider token refreshes; `cached_token` protects the data.
