...
mod tests {
    ...
    #[test]
    fn ensure_workspace_allowed_rejects_empty_workspace_list() {
        let workspace_ids = ForcedChatgptWorkspaceIds::Multiple(Vec::new());

        let err = ensure_workspace_allowed(Some(&workspace_ids), &workspace_jwt("org_actual"))
            .expect_err("empty workspace list should reject ChatGPT auth");

        assert!(err.contains("Login is restricted to a configured workspace"));
    }
