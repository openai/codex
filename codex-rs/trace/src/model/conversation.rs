...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub(crate) enum ConversationChannel {
    Analysis,
    Commentary,
    Final,
...
pub(crate) enum ConversationPart {
    Text {
        text: String,
    },
...
pub(crate) struct InferenceCall {
    pub(crate) inference_call_id: InferenceCallId,
    pub(crate) thread_id: AgentThreadId,
    ...
    pub(crate) request_item_ids: Vec<ConversationItemId>,
    /// Ordered output items produced by this response.
    pub(crate) response_item_ids: Vec<ConversationItemId>,
    /// Runtime tool calls whose model-visible call item came from this response.
    pub(crate) tool_call_ids_started_by_response: Vec<ToolCallId>,
