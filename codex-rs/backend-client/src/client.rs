...
impl Client {
    pub fn new(
        ...
        let http = RouteAwareClientPool::with_chatgpt_cloudflare_cookies(
            http_client_factory,
    ...
    pub fn from_auth(
        ...
        Ok(Self::new(base_url, http_client_factory)?
    ...
    async fn request(&self, method: Method, url: &str) -> Result<RequestBuilder> {
        Ok(self.http.client_for_url(url).await?.request(method, url))
    }
