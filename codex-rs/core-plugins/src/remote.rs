...
impl RemotePluginServiceConfig {
    ...
    pub fn new(chatgpt_base_url: String, http_client_factory: HttpClientFactory) -> Self {
        let http_clients = RouteAwareClientPool::with_chatgpt_cloudflare_cookies(
    ...
    pub(crate) async fn http_client_for_url(
        &self,
        request_url: &str,
    ) -> Result<HttpClient, RouteAwareClientPoolError> {
        self.http_clients.client_for_url(request_url).await
    }

    async fn catalog_client_for_url(
        &self,
        request_url: &str,
    ) -> Result<HttpClient, RemotePluginCatalogError> {
        self.http_client_for_url(request_url)
            .await
            .map_err(|source| RemotePluginCatalogError::HttpClient {
...
impl PartialEq for RemotePluginServiceConfig {
    fn eq(&self, other: &Self) -> bool {
        self.chatgpt_base_url == other.chatgpt_base_url
            && self.http_client_factory == other.http_client_factory
    }
}
...
pub enum RemotePluginCatalogError {
    ...
    #[error("failed to build HTTP client for remote plugin request to {url}: {source}")]
    HttpClient {
        url: String,
        #[source]
...
pub async fn fetch_recommended_plugins(
    ...
    let request =
        authenticated_request(client.get(&url), auth)?.timeout(RECOMMENDED_PLUGINS_TIMEOUT);
