...
async fn fetch_curated_repo_remote_sha(
    http_clients: &dyn HttpClientSelector,
    api_base_url: &str,
) -> Result<String, String> {
...
async fn fetch_github_text(
    ...
    let client = client_for_url(http_clients, url, context).await?;
...
async fn client_for_url(
    ...
    http_clients
        .client_for_url(url)
        .await
        .map_err(|err| format!("failed to build HTTP client to {context} from {url}: {err}"))
}
