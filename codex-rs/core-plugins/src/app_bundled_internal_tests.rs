...
#[test]
fn matching_declarations_become_internal_and_persist_a_receipt() {
    ...
    assert!(
        fs::read_to_string(installed_root.join(APP_BUNDLED_INTERNAL_HOOK_RECEIPT_PATH))
            .expect("read receipt")
            .contains(r#""status": "verified""#)
    );
}
...
fn mismatched_relative_source_path_fails_closed() {
    ...
    let plugin_data_root =
        AbsolutePathBuf::from_absolute_path(plugin_data_root).expect("plugin data root");
    let (standalone_sources, standalone_warnings) = apply_app_bundled_internal_hook_authority(
        &plugin_id,
        &installed_root,
        &plugin_data_root,
        hook_sources,
    ...
    assert_eq!(standalone_sources, Vec::<PluginHookSource>::new());
    assert_eq!(standalone_warnings, Vec::<String>::new());
}

#[test]
fn exact_app_authority_supports_other_bundled_plugin_ids() {
    let temp = TempDir::new().expect("tempdir");
    let installed_root = plugin_root(&temp, "installed");
    let packaged_root = plugin_root(&temp, "packaged");
...
#[test]
fn receipt_preserves_internal_classification_without_the_app_running() {
    let temp = TempDir::new().expect("tempdir");
    let installed_root = plugin_root(&temp, "installed");
    let packaged_root = plugin_root(&temp, "packaged");
    write_manifest(&installed_root, r#"{ "name": "computer-use" }"#);
    ...
    assert_eq!(standalone_warnings, Vec::<String>::new());
    assert_eq!(
        standalone_sources[0].kind,
        PluginHookSourceKind::AppBundledInternal
    );
}

#[test]
fn modified_declarations_fail_closed_when_only_a_receipt_is_available() {
    let temp = TempDir::new().expect("tempdir");
    let installed_root = plugin_root(&temp, "installed");
    let packaged_root = plugin_root(&temp, "packaged");
    write_manifest(&installed_root, r#"{ "name": "computer-use" }"#);
    write_manifest(&packaged_root, r#"{ "name": "computer-use" }"#);
    write_default_hooks(&installed_root, "echo original", None);
    write_default_hooks(&packaged_root, "echo original", None);
    let plugin_id = computer_use_plugin_id();
    let (hook_sources, hook_load_warnings) = load_sources(&installed_root, &plugin_id);
    let _ = apply_authority(
        &plugin_id,
        &installed_root,
        &packaged_root,
        hook_sources,
    ...
    write_default_hooks(&installed_root, "echo modified", None);
    let (hook_sources, hook_load_warnings) = load_sources(&installed_root, &plugin_id);
    let plugin_data_root = installed_root
        .as_path()
        .parent()
...
#[test]
fn modified_hook_payload_fails_closed_when_only_a_receipt_is_available() {
    let temp = TempDir::new().expect("tempdir");
    let installed_root = plugin_root(&temp, "installed");
    let packaged_root = plugin_root(&temp, "packaged");
    write_manifest(&installed_root, r#"{ "name": "computer-use" }"#);
    write_manifest(&packaged_root, r#"{ "name": "computer-use" }"#);
    write_default_hooks(&installed_root, "python3 hooks/check.py", None);
    write_default_hooks(&packaged_root, "python3 hooks/check.py", None);
    fs::write(installed_root.join("hooks/check.py"), "print('original')\n")
        .expect("write installed payload");
    fs::write(packaged_root.join("hooks/check.py"), "print('original')\n")
        .expect("write packaged payload");
    let plugin_id = computer_use_plugin_id();
    let (hook_sources, hook_load_warnings) = load_sources(&installed_root, &plugin_id);
    let (verified_sources, warnings) = apply_authority(
        &plugin_id,
    ...
    fs::write(installed_root.join("hooks/check.py"), "print('modified')\n")
        .expect("modify installed payload");
    let (hook_sources, hook_load_warnings) = load_sources(&installed_root, &plugin_id);
    let plugin_data_root = installed_root
        .as_path()
        .parent()
