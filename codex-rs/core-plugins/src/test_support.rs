...
use crate::http_client_selector::HttpClientSelectionFuture;
...
impl HttpClientSelector for RecordingHttpClientSelector {
    fn client_for_url<'a>(&'a self, request_url: &'a str) -> HttpClientSelectionFuture<'a> {
        match self.selected_urls.lock() {
            Ok(mut selected_urls) => selected_urls.push(request_url.to_string()),
            Err(error) => panic!("selected URL recorder lock should not be poisoned: {error}"),
        }
        Box::pin(self.delegate.client_for_url(request_url))
    }
}
