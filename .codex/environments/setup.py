...
def copy_from_root_to_worktree(repo_relative_path: str) -> None:
    relative_path = Path(repo_relative_path)
    if relative_path.is_absolute() or ".." in relative_path.parts:
        raise ValueError(f"path must be repository-relative: {repo_relative_path}")

    worktree_root, root_checkout = repo_roots()
    source_path = root_checkout / relative_path
    ...
    if source_path == destination_path:
        print("  result: running in the root checkout; nothing to copy")
...
def main() -> None:
    ...
    copy_from_root_to_worktree("user.bazelrc")
